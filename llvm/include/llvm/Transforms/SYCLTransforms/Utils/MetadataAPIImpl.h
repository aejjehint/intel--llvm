...
template <> struct MDValueTraits<uint64_t, void> {
  ...
  static value_type load(llvm::Metadata *pNode) {
    ...
    auto pval = llvm::mdconst::dyn_extract<llvm::ConstantInt>(pNode);
    assert(pval && "Can't locate pval, wrong node type!");
    return (uint64_t)pval->getValue().getSExtValue();
  }
...
template <> struct MDValueTraits<llvm::KernelArgument, void> {
  typedef llvm::KernelArgument value_type;

  static value_type load(llvm::Metadata *Node) {
    if (nullptr == Node) {
      return value_type();
    }

    auto *Tuple = llvm::cast<llvm::MDTuple>(Node);
    assert(Tuple->getNumOperands() == 4);
    value_type Val;
    Val.Ty = (llvm::KernelArgumentType)MDValueTraits<int32_t>::load(
        Tuple->getOperand(0));
    Val.SizeInBytes = MDValueTraits<uint64_t>::load(Tuple->getOperand(1));
    Val.OffsetInBytes = MDValueTraits<uint64_t>::load(Tuple->getOperand(2));
    Val.IsMemoryObject = MDValueTraits<bool>::load(Tuple->getOperand(3));
    return Val;
  }

  static llvm::Metadata *generateValue(llvm::LLVMContext &Context,
                                       const value_type &Val) {
    auto *MDTy = llvm::ConstantAsMetadata::get(
        llvm::ConstantInt::get(llvm::Type::getInt32Ty(Context), (int)Val.Ty));
    auto *MDSize = llvm::ConstantAsMetadata::get(llvm::ConstantInt::get(
        llvm::Type::getInt64Ty(Context), Val.SizeInBytes));
    auto *MDOffset = llvm::ConstantAsMetadata::get(llvm::ConstantInt::get(
        llvm::Type::getInt64Ty(Context), Val.OffsetInBytes));
    auto *MDMem = llvm::ConstantAsMetadata::get(
        llvm::ConstantInt::getBool(Context, Val.IsMemoryObject));
    llvm::Metadata *Ops[] = {MDTy, MDSize, MDOffset, MDMem};
    return llvm::MDTuple::get(Context, Ops);
  }
};
