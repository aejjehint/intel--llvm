...
struct KernelInternalMetadataAPI {
  ...
  typedef NamedMDValue<uint64_t, MDValueGlobalObjectStrategy>
      BarrierBufferSizeTy;
  ...
  typedef NamedMDList<std::string, MDValueGlobalObjectStrategy>
      ArgTargetExtTypeListTy;
  typedef NamedMDList<llvm::KernelArgument, MDValueGlobalObjectStrategy>
      ArgInfoTy;
  typedef NamedMDValue<bool, MDValueGlobalObjectStrategy> HasRootGroupBarrierTy;
  ...
  KernelInternalMetadataAPI(llvm::Function *Func)
      : LocalBufferSize(Func, "local_buffer_size"),
        ...
        ArgTargetExtTypeList(Func, "kernel_arg_target_ext_type"),
        ArgProperties(Func, "kernel_arg_properties"),
        HasRootGroupBarrier(Func, "has_root_group_barrier") {}
  ...
  ArgTargetExtTypeListTy ArgTargetExtTypeList;
  ArgInfoTy ArgProperties;
  NamedMDValueAccessor<HasRootGroupBarrierTy> HasRootGroupBarrier;
  ...
  const llvm::SmallVectorImpl<llvm::StringRef> &getMDNames() const {
    ...
    if (MDNames.empty()) {
      MDNames = {LocalBufferSize.getID(),
                 ...
                 ArgTargetExtTypeList.getID(),
                 ArgProperties.getID(),
                 HasRootGroupBarrier.getID()};
    }
    return MDNames;
  }
