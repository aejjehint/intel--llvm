...
#ifndef LLVM_TRANSFORMS_SYCLTRANSFORMS_UTILS_COMPILATION_UTILS_H
#define LLVM_TRANSFORMS_SYCLTRANSFORMS_UTILS_COMPILATION_UTILS_H
...
/// Replace use of old function with new function in given users.
...
inline bool hasByrefArgs(const Function *F) {
  if (!F)
    return false;
  return llvm::any_of(F->args(), [](auto &Arg) {
...
/// Find all dbg intrinsic users of a value.
...
/// Create call instruction to a work-item builtin function.
/// \param M module.
/// \param Builder IRBuilder instance.
/// \param FuncName name of the builtin function to call.
/// \param Args builtin arguments.
/// \param NameStr name of the call.
...
// Parse "!spirv.Decorations" metadata that contains the
// "clang::annotate(<str>)" info. The <str> is returned if the annotation string
// exists.
std::optional<StringRef> getUserAnnotationStr(Instruction *I);

// Given the following function @foo and value %x:
// define void @foo(ptr %x) {
//   call void @baz(ptr %x)
// }
//
// This will find all memory def instructions that are used as actual arguments
// on @foo's call sites (directly and indirectly): {%a.x, %b.x}
//
// define void @a() {
//   %a.x = alloca ...
//   call void @foo(ptr %a.x)
// }
// define void @b() {
//   %b.x.ptr = alloca ...
//   store ptr %b.x, ptr %b.x.ptr
//   %b.x.load = load ptr, ptr %b.x.ptr
//   call void @bar(ptr %b.x.load)
// }
// define void @bar(ptr %x) {
//   call void @foo(ptr %x)
// }
void findMemoryDefsOverCallGraph(
    Value *V, SmallPtrSetImpl<Instruction *> &InstDefs,
    function_ref<MemorySSA &(Function &F)> GetMemorySSA);

bool isModuleUsingAsan(const Module &M);

// Max size of variable-length array.
constexpr uint64_t VLAMaxSize = 4 * 1024;

} // namespace CompilationUtils
} // namespace llvm

#endif // LLVM_TRANSFORMS_SYCLTRANSFORMS_UTILS_COMPILATION_UTILS_H
