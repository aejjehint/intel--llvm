...
entry:
  ...
  %call = call i64 @oneWord(ptr byval(%struct.OneWord) align 8 %OW) #4, !spirv.Decorations !2

; CHECK: call void @llvm.lifetime.start.p0(i64 8, ptr %{{.*}})
; CHECK-NEXT: [[LOAD:%[a-zA-Z0-9]+]] = load i64, ptr %OW
; CHECK-NEXT: call i64 @oneWord(i64 [[LOAD]]) #{{[0-9]+}}, !spirv.Decorations

  call void @llvm.lifetime.start.p0(i64 16, ptr %TW) #3
  %call1 = tail call i64 @twoWords(ptr byval(%struct.TwoWords) align 8 %TW) #4, !spirv.Decorations !2
...
; CHECK-NEXT: tail call i64 @twoWords(i64 [[LOAD0]], i64 [[LOAD1]]) #{{[0-9]+}}, !spirv.Decorations
