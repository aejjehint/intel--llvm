...
; RUN: opt -passes=sycl-kernel-analysis %s -S -debug 2>&1 | FileCheck %s
...
; CHECK-NOT: define {{.*}}void @func_call_intel_device_barrier({{.*}} !has_root_group_barrier {{![0-9]+}}
...
; CHECK: define {{.*}}void @kernel_contains_intel_device_barrier({{.*}} !has_root_group_barrier {{![0-9]+}}
