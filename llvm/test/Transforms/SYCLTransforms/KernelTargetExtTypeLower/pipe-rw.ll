...
entry:
; CHECK: define dso_local spir_kernel void @test_write(ptr addrspace(1) %p, i32 noundef %data) {{.*}} !kernel_arg_target_ext_type [[MD_ARG_TARGET_EXT_TY_W:![0-9]+]]
...
entry:
; CHECK: define dso_local spir_kernel void @test_read(ptr addrspace(1) %p, i32 noundef %data) {{.*}} !kernel_arg_target_ext_type [[MD_ARG_TARGET_EXT_TY_R:![0-9]+]]
; CHECK: %p.addr = alloca ptr addrspace(1), align 8
; CHECK: store ptr addrspace(1) %p, ptr %p.addr, align 8
; CHECK: load ptr addrspace(1), ptr %p.addr, align 8
...
; CHECK: [[MD_ARG_TARGET_EXT_TY_W]] = !{!"spirv.Pipe_1", !""}
; CHECK: [[MD_ARG_TARGET_EXT_TY_R]] = !{!"spirv.Pipe_0", !""}
