...
; CHECK-SAME: %t3
; CHECK: Dim=0, Contains=F, IsGID=T, IsSigned=T, IsUpperBound=F
; CHECK-SAME: %t4
...
entry:
  ...
  %b1 = icmp sge i32 %id, %t1
  br i1 %b1, label %c2, label %ret

c2:
  %b2 = icmp sge i32 %t2, %id
  br i1 %b2, label %c3, label %ret

c3:
  %b3 = icmp sge i32 %id, %t3
  br i1 %b3, label %ret, label %c4

c4:
  %b4 = icmp sge i32 %t4, %id
...
; DEBUGIFY-COUNT-33: Instruction with empty DebugLoc in function WG.boundaries.program
