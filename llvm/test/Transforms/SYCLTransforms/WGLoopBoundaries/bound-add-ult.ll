...
entry:
  %gid  = tail call i64 @_Z13get_global_idj(i32 0)
  %conv = trunc i64 %gid to i32
  %new_lb = add i32 4, %conv
  %cmp = icmp ult i32 %new_lb, 9
  br i1 %cmp, label %if.end, label %if.then
...
; CHECK: define [7 x i64] @WG.boundaries.constant_kernel1(ptr addrspace(1) noalias %{{.*}})
; CHECK-NOT: define [7 x i64] @WG.boundaries.constant_kernel2(ptr addrspace(1) noalias %{{.*}})
; CHECK: define [7 x i64] @WG.boundaries.constant_kernel3(ptr addrspace(1) noalias %{{.*}})
; CHECK-NOT: define [7 x i64] @WG.boundaries.constant_kernel4(ptr addrspace(1) noalias %{{.*}})
...
; DEBUGIFY-COUNT-16: WARNING: Instruction with empty DebugLoc in function constant_kernel
; DEBUGIFY-COUNT-56: WARNING: Instruction with empty DebugLoc in function WG.boundaries.constant_kernel
