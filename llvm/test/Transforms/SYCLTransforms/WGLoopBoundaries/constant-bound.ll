; RUN: opt -passes="sycl-kernel-analysis,sycl-kernel-wg-loop-bound" %s -S -enable-debugify -disable-output 2>&1 | FileCheck %s -check-prefix=DEBUGIFY
; RUN: opt -passes="sycl-kernel-analysis,sycl-kernel-wg-loop-bound" %s -S | FileCheck %s

; CHECK: @test
; CHECK: ret
; CHECK: @WG.boundaries.test
; CHECK: ret

target datalayout = "e-p:32:32:32-i1:8:8-i8:8:8-i16:16:16-i32:32:32-i64:64:64-f32:32:32-f64:64:64-f80:128:128-v64:64:64-v128:128:128-a0:0:64-f80:32:32-n8:16:32"
target triple = "i686-pc-win32"

define void @test(ptr addrspace(1) nocapture %paths) nounwind {
  %1 = call i32 @_Z13get_global_idj(i32 0) nounwind
  %2 = icmp sgt i32 %1, 131071
 br i1 %2, label %5, label %3

; <label>:3                                       ; preds = %0
  %4 = getelementptr inbounds float, ptr addrspace(1) %paths, i32 %1
  store float 1.000000e+00, ptr addrspace(1) %4, align 4
  br label %5

; <label>:5                                      ; preds = %0, %3
  ret void
}

declare i32 @_Z13get_global_idj(i32)

!sycl.kernels = !{!0}
!opencl.compiler.options = !{!2}

!0 = !{ptr @test}
!2 = !{}

; DEBUGIFY-COUNT-24: Instruction with empty DebugLoc in function WG.boundaries.
; DEBUGIFY-COUNT-1: Missing line
; DEBUGIFY-NOT: WARNING
