...
; RUN: opt -passes=sycl-kernel-wgloop-creator %s -S -pass-remarks=sycl-kernel-wgloop-creator -disable-output 2>&1 | FileCheck -check-prefix=REMARK %s
...
; CHECK-NEXT:    store <16 x i32> splat (i32 12345), ptr [[SCALAR_GEPVECTOR_FUNC]], align 4
...
; CHECK-NEXT:    br i1 [[DIM_0_CMP_TO_MAX]], label [[DIM_0_EXIT]], label [[SCALAR_KERNEL_ENTRY]], !llvm.loop [[LOOP7:![0-9]+]]
; CHECK:       dim_0_exit:
...
; CHECK-NEXT:    br i1 [[DIM_1_CMP_TO_MAX]], label [[DIM_1_EXIT]], label [[DIM_0_PRE_HEAD]], !llvm.loop [[LOOP9:![0-9]+]]
; CHECK:       dim_1_exit:
...
; CHECK-NEXT:    br i1 [[DIM_2_CMP_TO_MAX]], label [[DIM_2_EXIT:%.*]], label [[DIM_1_PRE_HEAD]], !llvm.loop [[LOOP10:![0-9]+]]
; CHECK:       dim_2_exit:
...
; REMARK: remark: {{.*}}work-group loop of kernel _Z30ParallelForNDRangeImplKernel1DPiS_S_mmm is created from vector loop with scalar remainder loop
