; RUN: opt -passes=sycl-kernel-add-implicit-args %s -S -o - | FileCheck %s

; Check that function's use in metadata is replaced with new function.

; CHECK: define i16 @_Z19ConvolutionFunctionssssPtSt5arrayIfLm9EE(i16 %row, ptr byval(%"struct.std::array") align 4 %coefficients, ptr addrspace(3) noalias %pLocalMemBase
