; RUN: opt -passes='sycl-kernel-add-implicit-args,sycl-kernel-private-to-heap,sycl-kernel-prepare-args' -sycl-kernel-max-private-mem-size=1024 -S %s -enable-debugify -disable-output 2>&1 | FileCheck  -check-prefix=DEBUGIFY %s
; RUN: opt -passes='sycl-kernel-add-implicit-args,sycl-kernel-private-to-heap,sycl-kernel-prepare-args' -sycl-kernel-max-private-mem-size=1024 -S %s | FileCheck %s --check-prefixes CHECK,CHECK-IMP
; RUN: opt -passes='sycl-kernel-add-tls-globals,sycl-kernel-private-to-heap,sycl-kernel-prepare-args' -sycl-kernel-max-private-mem-size=1024 -S %s -enable-debugify -disable-output 2>&1 | FileCheck  -check-prefix=DEBUGIFY-TLS %s
; RUN: opt -passes='sycl-kernel-add-tls-globals,sycl-kernel-private-to-heap,sycl-kernel-prepare-args' -sycl-kernel-max-private-mem-size=1024 -S %s | FileCheck %s --check-prefixes CHECK,CHECK-TLS
...
; CHECK-TLS-LABEL: void @foo2(
; CHECK-TLS-SAME: ptr addrspace(1) noundef [[DATA:%.*]], ptr noundef [[DATA2:%.*]])
...
;;;;; load heap memory address in [[PWGID]]
; CHECK:        [[TMP0:%.*]] = getelementptr ptr, ptr [[PWGID]], i32 3
; CHECK:        [[PHEAPMEM:%.*]] = load ptr, ptr [[TMP0]], align 1
;;;;; compare heap memory pointer with null
; CHECK:        [[TMP1:%.*]] = icmp eq ptr [[PHEAPMEM]], null

;;;;; initialize kernel implicit argument %pPrivateHeapMem
; CHECK:        [[TMP20:%.*]] = getelementptr i8, ptr [[PHEAPMEM]], i64 0
; CHECK-IMP:    [[PPRIVATEHEAPMEM:%.*]] = select i1 [[TMP1]], ptr null, ptr [[TMP20]]

;;;;; initialize @__pPrivateHeapMem
; CHECK-TLS:    [[TMP23:%.*]] = select i1 [[TMP1]], ptr null, ptr [[TMP20]]
; CHECK-TLS:    store ptr [[TMP23]], ptr @__pPrivateHeapMem, align 8
; CHECK-TLS:    [[PPRIVATEHEAPMEM:%.*]] = load ptr, ptr @__pPrivateHeapMem, align 8
...
; DEBUGIFY-COUNT-35: WARNING: Instruction with empty DebugLoc in function test
; DEBUGIFY-NOT: WARNING

; DEBUGIFY-TLS-NOT: WARNING
; DEBUGIFY-TLS-COUNT-40: WARNING: Instruction with empty DebugLoc in function test
; DEBUGIFY-TLS-NOT: WARNING
