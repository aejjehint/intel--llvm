...
define void @test_canonicalized_gep1(ptr addrspace(3) %sSharedStorage, ptr addrspace(1) nocapture readonly %srcValues, ptr addrspace(1) nocapture readnone %offsets, ptr addrspace(1) nocapture writeonly %destBuffer, i32 %alignmentOffset, ptr addrspace(3) noalias %pLocalMemBase, ptr noalias %pWorkDim, ptr noalias %pWGId, [4 x i64] %BaseGlbId, ptr noalias %pSpecialBuf, ptr noalias %RuntimeHandle) #0 !kernel_arg_base_type !4 {
entry:
; CHECK-LABEL: void @test_canonicalized_gep1
; CHECK: [[GEP0:%[0-9]+]] = getelementptr ptr, ptr %pWGId, i32 3
; CHECK-NEXT: %pHeapMem = load ptr, ptr [[GEP0]], align 1
; CHECK-NEXT: [[HEAP_MEM_NULL:%[0-9]+]] = icmp eq ptr %pHeapMem, null
; CHECK-NEXT: [[GEP1:%[0-9]+]] = getelementptr i8, ptr %UniformArgs, i32 0
; CHECK-NEXT: [[LOAD:%[0-9]+]] = load i64, ptr [[GEP1]], align 8
; CHECK-NEXT: [[ALLOCA_SIZE:%[0-9]+]] = select i1 [[HEAP_MEM_NULL]], i64 [[LOAD]], i64 0
; CHECK-NEXT: [[ALLOCA:%[0-9]+]] = alloca i8, i64 [[ALLOCA_SIZE]], align 64
; CHECK-NEXT: [[HEAP_MEM_LOC:%[0-9]+]] = getelementptr i8, ptr %pHeapMem, i64 0
; CHECK-NEXT: [[LOCAL_ARG_BUFFER:%[0-9]+]] = select i1 [[HEAP_MEM_NULL]], ptr [[ALLOCA]], ptr [[HEAP_MEM_LOC]]
; CHECK-NEXT: %explicit_0 = addrspacecast ptr [[LOCAL_ARG_BUFFER]] to ptr addrspace(3)
; CHECK-NEXT: {{[0-9]+}} = add i64 0, [[LOAD]]
  %call1 = call i64 @_Z12get_local_idj(i32 0)
  %sext = shl i64 %call1, 32
  %0 = ashr exact i64 %sext, 26
  %arrayidx = getelementptr inbounds i8, ptr addrspace(3) %sSharedStorage, i64 %0
  store <8 x double> zeroinitializer, ptr addrspace(3) %arrayidx, align 64
  br label %exit
...
define void @test_canonicalized_gep2(ptr addrspace(3) %sSharedStorage, ptr addrspace(1) nocapture readonly %srcValues, ptr addrspace(1) nocapture readnone %offsets, ptr addrspace(1) nocapture writeonly %destBuffer, i32 %alignmentOffset, ptr addrspace(3) noalias %pLocalMemBase, ptr noalias %pWorkDim, ptr noalias %pWGId, [4 x i64] %BaseGlbId, ptr noalias %pSpecialBuf, ptr noalias %RuntimeHandle) #0 !kernel_arg_base_type !5 {
entry:
; CHECK-LABEL: void @test_canonicalized_gep2
; CHECK: [[GEP0:%[0-9]+]] = getelementptr ptr, ptr %pWGId, i32 3
; CHECK-NEXT: %pHeapMem = load ptr, ptr [[GEP0]], align 1
; CHECK-NEXT: [[HEAP_MEM_NULL:%[0-9]+]] = icmp eq ptr %pHeapMem, null
; CHECK-NEXT: [[GEP1:%[0-9]+]] = getelementptr i8, ptr %UniformArgs, i32 0
; CHECK-NEXT: [[LOAD:%[0-9]+]] = load i64, ptr [[GEP1]], align 8
; CHECK-NEXT: [[ALLOCA_SIZE:%[0-9]+]] = select i1 [[HEAP_MEM_NULL]], i64 [[LOAD]], i64 0
; CHECK-NEXT: [[ALLOCA:%[0-9]+]] = alloca i8, i64 [[ALLOCA_SIZE]], align 32
; CHECK-NEXT: [[HEAP_MEM_LOC:%[0-9]+]] = getelementptr i8, ptr %pHeapMem, i64 0
; CHECK-NEXT: [[LOCAL_ARG_BUFFER:%[0-9]+]] = select i1 [[HEAP_MEM_NULL]], ptr [[ALLOCA]], ptr [[HEAP_MEM_LOC]]
; CHECK-NEXT: %explicit_0 = addrspacecast ptr [[LOCAL_ARG_BUFFER]] to ptr addrspace(3)
; CHECK-NEXT: {{[0-9]+}} = add i64 0, [[LOAD]]
  ...
  %0 = ashr exact i64 %sext, 26
  %arrayidx = getelementptr inbounds i8, ptr addrspace(3) %sSharedStorage, i64 %0
  %1 = load <8 x float>, ptr addrspace(3) %arrayidx, align 32
  br label %exit
...
!0 = !{i32 3, i32 300000}
!1 = !{i16 6, i16 14}
!2 = !{ptr @test, ptr @test_canonicalized_gep1, ptr @test_canonicalized_gep2}
!3 = !{!"long*", !"char*", !"float2*", !"char*", !"short*", !"half16*"}
!4 = !{!"double8*", !"double8*", !"uint*", !"double8*", !"uint"}
!5 = !{!"float8*", !"float8*", !"uint*", !"float8*", !"uint"}

; DEBUGIFY-COUNT-81: WARNING: Instruction with empty DebugLoc in function test
; DEBUGIFY-COUNT-47: WARNING: Instruction with empty DebugLoc in function test_canonicalized_gep1
; DEBUGIFY-COUNT-47: WARNING: Instruction with empty DebugLoc in function test_canonicalized_gep2
