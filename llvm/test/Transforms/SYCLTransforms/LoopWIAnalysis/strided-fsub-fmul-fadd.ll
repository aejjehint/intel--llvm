...
; CHECK-NEXT:   STR %dim_0_vector_ind_var = phi i64 [ 0, %dim_0_vector_pre_head ], [ %dim_0_vector_inc_ind_var, %entryvector_func ]
; CHECK-NEXT:   STR %dim0__tid = phi i64 [ %0, %dim_0_vector_pre_head ], [ %dim0_inc_tid, %entryvector_func ]
; CHECK-NEXT:   STR    %3 = trunc i64 %dim0__tid to i32
; CHECK-NEXT:   RND    %broadcast.splatinsertvector_func = insertelement <8 x i32> poison, i32 %3, i32 0
...
; CHECK-NEXT:   STR    %4 = add nuw <8 x i32> %broadcast.splatvector_func, <i32 0, i32 1, i32 2, i32 3, i32 4, i32 5, i32 6, i32 7>
; CHECK-NEXT:   STR    %sextvector_func = shl i64 %dim0__tid, 32
; CHECK-NEXT:   RND    %.extract.0.vector_func = ashr exact i64 %sextvector_func, 32
; CHECK-NEXT:   STR    %5 = sitofp <8 x i32> %4 to <8 x float>
; CHECK-NEXT:   STR    %6 = fsub <8 x float> %5, splat (float -2.000000e+00)
; CHECK-NEXT:   STR    %7 = fmul <8 x float> %6, splat (float 0x3F35D867C0000000)
; CHECK-NEXT:   STR    %8 = fadd <8 x float> %7, splat (float -1.000000e+00)
...
; CHECK-NEXT:   RND    br i1 %dim_0_vector_cmp.to.max, label %dim_0_vector_exit, label %entryvector_func
; CHECK-DAG:    UNI  <8 x float> splat (float -2.000000e+00)
; CHECK-DAG:    UNI  i64 1
; CHECK-DAG:    UNI  <8 x float> splat (float 0x3F35D867C0000000)
; CHECK-DAG:    UNI  i64 32
; CHECK-DAG:    UNI  <8 x float> splat (float -1.000000e+00)
; CHECK-DAG:    UNI  i64 8
; CHECK:      StrideIntermediate:
; CHECK-NEXT:   %broadcast.splatinsertvector_func = insertelement <8 x i32> poison, i32 %3, i32 0
; CHECK-NEXT: ConstStrides:
; CHECK-NEXT:   i64 1    %dim_0_vector_ind_var = phi i64 [ 0, %dim_0_vector_pre_head ], [ %dim_0_vector_inc_ind_var, %entryvector_func ]
; CHECK-NEXT:   i64 8    %dim0__tid = phi i64 [ %0, %dim_0_vector_pre_head ], [ %dim0_inc_tid, %entryvector_func ]
; CHECK-NEXT:   i32 8    %3 = trunc i64 %dim0__tid to i32
...
; CHECK-NEXT:   i32 8    %broadcast.splatinsertvector_func = insertelement <8 x i32> poison, i32 %3, i32 0
; CHECK-NEXT:   i32 8    %4 = add nuw <8 x i32> %broadcast.splatvector_func, <i32 0, i32 1, i32 2, i32 3, i32 4, i32 5, i32 6, i32 7>
; CHECK-NEXT:   float 8.000000e+00    %5 = sitofp <8 x i32> %4 to <8 x float>
