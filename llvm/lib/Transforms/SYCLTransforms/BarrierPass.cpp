...
public:
  KernelBarrierImpl(DataPerBarrier *DPB, DataPerValue *DPV,
                    ...
                    function_ref<DominatorTree &(Function &F)> GetDT,
                    function_ref<MemorySSA &(Function &F)> GetMemorySSA)
      : DPB(DPB), DPV(DPV), ICI(ICI), GetDT(GetDT), GetMemorySSA(GetMemorySSA) {
  }
...
private:
  ...
  void fixCallInstruction(CallInst *CallToFix, Function *CalledFunc);
  void fixDirectCallInstruction(CallInst *CallToFix, Function *CalledFunc);
  void fixIndirectCallInstruction(CallInst *CallToFix,
                                  SmallVectorImpl<Function *> &CalledFuncs);
...
bool KernelBarrierImpl::run(Module &M) {
  ...
  LocalIdAllocTy = PointerType::getUnqual(M.getContext());
  ...
  for (Function *FuncToFix : FunctionsWithSync) {
    ...
    for (auto *U : FuncToFix->users())
      if (auto *CI = dyn_cast<CallInst>(U);
          CI && CI->getCalledFunction() == FuncToFix)
        fixDirectCallInstruction(CI, FuncToFix);
  }
  // Fix indirect calls.
  DenseMap<CallBase *, SmallVector<Function *, 4>> IndirectCallsMap;
  for (Function *FuncToFix : FunctionsWithSync) {
    llvm::for_each(ICI->getIndirectCalls(FuncToFix), [&](CallBase *CB) {
      IndirectCallsMap[CB].push_back(FuncToFix);
    });
  }
  for (auto &[CB, FuncsToFix] : IndirectCallsMap)
    fixIndirectCallInstruction(cast<CallInst>(CB), FuncsToFix);
...
bool KernelBarrierImpl::fixSynclessTIDUsers(Module &M,
  ...
  CompilationUtils::patchNotInlinedTIDUserFunc(
      M, ICI, Builder, FuncsWithSync, TIDCallsToFix, PatchedFToLocalIds,
...
void KernelBarrierImpl::fixAllocaAndDbg(Function &F) {
  ...
  if (SP) {
    if (BasicBlock *FirstBB = OldToNewSyncBBMap[&F][&F.getEntryBlock()]) {
      // Find llvm.dbg.value intrinsics referencing arguments that don't cross
      // barrier. Hoist them to the newly created entry block.
      // Cross-barrier arguments will be handled later in this function.
      TinyPtrVector<DbgVariableRecord *> HoistedDVIs;
      auto IsArgument = [&](Value *V) -> bool {
        return isa<Argument>(V) && !DPV->hasOffset(V);
      };
      for (Instruction &I : *FirstBB) {
        if (I.hasDbgRecords()) {
          for (DbgVariableRecord &DVR : filterDbgVars(I.getDbgRecordRange())) {
            if (llvm::all_of(DVR.location_ops(), IsArgument))
              HoistedDVIs.push_back(&DVR);
          }
        }
      }
      BasicBlock *BB = &F.getEntryBlock();
      for (DbgVariableRecord *DVI : HoistedDVIs) {
        DVI->removeFromParent();
        BB->insertDbgRecordBefore(DVI, BB->getFirstInsertionPt());
      }
    }
  }
  ...
  // If BarrierRegionInfo fails to construct regions, we need to load from
  // special buffer for every use of a cross-barrier value.
  DominatorTree DT(F);
  DominanceFrontier DF;
  DF.analyze(DT);
  auto BRI = std::make_unique<BarrierRegionInfo>(&F, &DF, &DT);
  ...
  for (Value *V : WorkList) {
    ...
    SmallVector<DbgVariableRecord *, 1> DIRec;
    SmallVector<DbgVariableIntrinsic *, 4> DIs;
    if (SP)
      DIs = CompilationUtils::findDbgUses(V, &DIRec);

    // Only use the first DbgVariableIntrinsic.
    DbgVariableRecord *DI = DIRec.empty() ? nullptr : DIRec.front();
    ...
    PointerType *AllocatedTy =
        llvm::PointerType::get(F.getContext(), SPECIAL_BUFFER_ADDR_SPACE);
    AllocaInst *AddrAI =
        new AllocaInst(AllocatedTy, DL.getAllocaAddrSpace(),
                       AllocaName + ".addr", AddrInsertBefore->getIterator());
    if (AI)
    ...
    if (DI) {
      ...
      Instruction *InsertBefore =
          AI ? AI->getNextNode() : AddrAI->getNextNode();
      ...
      if (DI->isDbgDeclare())
        DIB.insertDeclare(AddrAI, DI->getVariable(), Expr,
                          DI->getDebugLoc().get(), InsertBefore);
      else
        DIB.insertDbgValueIntrinsic(AddrAI, DI->getVariable(), Expr,
                                    DI->getDebugLoc().get(), InsertBefore);
    }
    ...
    for (auto &Pair : BBUsers) {
      ...
      if (!InsertBefore) {
        bool PHIAndBarrierInBranch = llvm::any_of(Pair.second, [&](auto *I) {
          if (auto *PHI = dyn_cast<PHINode>(I)) {
            auto *Header = BRI->getRegionHeaderFor(PHI->getIncomingBlock(0));
            for (unsigned Idx = 1, E = PHI->getNumIncomingValues(); Idx < E;
                 ++Idx)
              if (Header != BRI->getRegionHeaderFor(PHI->getIncomingBlock(Idx)))
                return true;
          }
          return false;
        });
        if (PHIAndBarrierInBranch) {
          // Special handling of user in PHINode in the case of barrier in a
          // branch. Insert point should be in incoming block or a common
          // dominator block.
                  ...
                  auto *DominatorBlock =
                      InsertBefore
                          ? DT.findNearestCommonDominator(
                                InsertBefore->getParent(), IncomingBlock)
                          : IncomingBlock;
                  InsertBefore = DominatorBlock->getTerminator();
                }
              }
            }
          }
          ...
          if (auto It = SyncPerBB.find(BB); It != SyncPerBB.end()) {
            ...
            assert(InsertBefore->getParent() == BB &&
                   "sync instruction must not be the last instruction in the "
                   "block");
          } else {
...
void KernelBarrierImpl::fixSpecialValues() {
  for (Value *V : *SpecialValues) {
    ...
    PointerType *Ty =
        llvm::PointerType::get(Inst->getContext(), SPECIAL_BUFFER_ADDR_SPACE);
    ...
    if (!(CI && (DPV->hasOffset(CI->getCalledFunction()) ||
      ...
      Instruction *InstToStore =
          !OneBitBaseType
              ? Inst
              : CastInst::CreateZExtOrBitCast(Inst, TyInSP, "ZEXT-i1Toi32",
      ...
      StoreInst *SI = new StoreInst(InstToStore, AddrInSpecialBuffer,
                                    NextInst->getIterator());
      SI->setDebugLoc(DB);
    }
    ...
    auto InsertLoadedValue = [&](Instruction *InsertBefore,
      ...
      Instruction *LoadedValue =
          new LoadInst(TyInSP, AddrInSpecialBuffer, "loadedValue",
...
static bool hoistUniformCrossBarrierInstToEntryBlock(Instruction *I,
                                                     Instruction *InsertBefore,
                                                     MemorySSA &MSSA) {
  ...
  auto NotSafeToHoist = [&](Instruction *Inst) {
    MemoryUseOrDef *UD = MSSA.getMemoryAccess(Inst);
    if (!UD)
      return false;
    MemoryAccess *MA = UD->getDefiningAccess();
    if (auto *MD = dyn_cast<MemoryDef>(MA)) {
      if (MSSA.isLiveOnEntryDef(MA))
        return false;
      auto *MDInst = MD->getMemoryInst();
      if (auto *CI = dyn_cast<CallInst>(MDInst))
        if (auto *Callee = CI->getCalledFunction();
            Callee && Callee->getName() == DUMMY_BARRIER_FUNC_NAME)
          return false;
    }
    return true;
  };
...
void KernelBarrierImpl::fixCrossBarrierValues(Function &F) {
  if (CrossBarrierValues->empty())
    return;

  ValueVec WorkList;
  if (!F.hasOptNone()) {
    // Update DT for use in MemorySSA.
    DominatorTree &DT = GetDT(F);
    DT.recalculate(F);
    MemorySSA &MSSA = GetMemorySSA(F);
  ...
  for (Value *V : WorkList) {
    ...
    AllocaInst *AI =
        new AllocaInst(Inst->getType(), DL->getAllocaAddrSpace(),
                       Inst->getName(), InsertBefore->getIterator());
    // Add Store instruction after the value instruction.
    StoreInst *SI = new StoreInst(Inst, AI, NextInst->getIterator());
    ...
    for (Instruction *UserInst : UserInsts) {
      ...
      if (auto *PhiNode = dyn_cast<PHINode>(UserInst)) {
        ...
        for (auto *InsertBefore : InsertBefores) {
          // Calculate the pointer of the current special in the special buffer.
          Instruction *LoadedValue =
              new LoadInst(AI->getAllocatedType(), AI, "loadedValue",
                           InsertBefore->getIterator());
          LoadedValue->setDebugLoc(DB);
          PhiNode->setIncomingValueForBlock(InsertBefore->getParent(),
                                            LoadedValue);
        ...
        // Calculate the pointer of the current special in the special buffer.
        Instruction *LoadedValue = new LoadInst(
            AI->getAllocatedType(), AI, "loadedValue", UserInst->getIterator());
        LoadedValue->setDebugLoc(DB);
...
void KernelBarrierImpl::createBarrierKeyValues(Function *Func,
  ...
  KeyValues->CurrBarrierId =
      new AllocaInst(Type::getInt32Ty(*Context), AllocaAddrSpace,
                     "pCurrBarrier", InsertBefore->getIterator());

  // Will hold the index in special buffer and will be increased by stride size.
  KeyValues->CurrSBIndex = new AllocaInst(
      SizeTTy, AllocaAddrSpace, "pCurrSBIndex", InsertBefore->getIterator());

  if (!HasTLSGlobals) {
    // get_local_id()
    KeyValues->LocalIdValues =
        new AllocaInst(LocalIdArrayTy, AllocaAddrSpace, "pLocalIds",
                       InsertBefore->getIterator());
  }
  ...
  uint64_t StructureSize = DPV->getStrideSize(Func);
...
Value *KernelBarrierImpl::resolveGetLocalIDCall(CallInst *Call) {
  ...
  ConstantInt *C = cast<ConstantInt>(Dimension);
  uint64_t Dim = C->getZExtValue();
  assert(Dim < MAX_WORK_DIM && "out-of-bound dim in local id call");
  ...
  for (auto *I : GetGIDInstructions) {
    ...
    if (KIMD.NoBarrierPath.hasValue() && KIMD.NoBarrierPath.get()) {
      ...
      Instruction *GlobalID =
          BinaryOperator::CreateAdd(LID, BaseGID, Name, OldCall->getIterator());
...
void KernelBarrierImpl::fixArgumentUsage(Value *OriginalArg) {
  ...
  SmallVector<DbgVariableRecord *, 1> DbgVars;
  SmallVector<DbgVariableIntrinsic *, 4> DIs;
  DIs = CompilationUtils::findDbgUses(OriginalArg, &DbgVars);
  if (HasTLSGlobals && (!DIs.empty() || !DbgVars.empty()))
    return;
  ...
  auto InsertLoadedValue = [&](Instruction *InsertBefore, const DebugLoc &DB) {
    ...
    PointerType *Ty = llvm::PointerType::get(InsertBefore->getContext(),
...
void KernelBarrierImpl::fixReturnValue(Value *RetVal, unsigned int OffsetRet,
  ...
  // Add Store instruction after the value instruction.
  auto *SI =
      new StoreInst(RetVal, AddrInSpecialBuffer, InsertBefore->getIterator());
...
void KernelBarrierImpl::fixCallInstruction(CallInst *CallToFix,
  ...
  for (CallInst::const_op_iterator Opi = CallToFix->arg_begin(),
    ...
    // Calculate the pointer of the current special in the special buffer.
    PointerType *Ty =
        llvm::PointerType::get(Func->getContext(), SPECIAL_BUFFER_ADDR_SPACE);
    Value *AddrInSpecialBuffer =
        getAddressInSpecialBuffer(Offset, Ty, InsertBefore, &DB);
    // Add Store instruction before the synchronize instruction (in the pre
    // basic block)
    StoreInst *SI =
        new StoreInst(OpVal, AddrInSpecialBuffer, InsertBefore->getIterator());
    SI->setDebugLoc(DB);
  }
}

void KernelBarrierImpl::fixDirectCallInstruction(CallInst *CallToFix,
                                                 Function *CalledFunc) {
  fixCallInstruction(CallToFix, CalledFunc);
  ...
  assert(DPB->getSyncInstructions(CallToFix->getFunction())
             .contains(&*FirstInst) &&
         "assume first instruction to be sync instruction");
  // Find next instruction so we can create new instruction before it.
  ...
  // Calculate the pointer of the current special in the special buffer.
  PointerType *Ty = llvm::PointerType::get(CallToFix->getContext(),
                                           SPECIAL_BUFFER_ADDR_SPACE);
  const DebugLoc &DB = CallToFix->getDebugLoc();
  Value *AddrInSpecialBuffer =
      getAddressInSpecialBuffer(Offset, Ty, NextInst, &DB);
  ...
  LoadInst *LoadedValue =
      new LoadInst(CallToFix->getType(), AddrInSpecialBuffer, "loadedValue",
                   NextInst->getIterator());
  LoadedValue->setDebugLoc(DB);
  ...
  if (DPV->hasOffset(CallToFix)) {
    ...
    StoreInst *SI = new StoreInst(LoadedValue, AddrInSpecialBuffer,
                                  NextInst->getIterator());
    SI->setDebugLoc(DB);
  } else {
...
void KernelBarrierImpl::fixIndirectCallInstruction(
    CallInst *CallToFix, SmallVectorImpl<Function *> &CalledFuncs) {
  // TODO only need to fix one called function since each time only a single
  // function is called.
  ...
  if (llvm::none_of(CalledFuncs, [&](auto *F) { return DPV->hasOffset(F); }))
    return;
  // Need to handle return value.
  ...
  DebugLoc DB = CallToFix->getDebugLoc();
  Builder.SetCurrentDebugLocation(DB);
  auto *ASC = Builder.CreateAddrSpaceCast(CallToFix->getCalledOperand(),
                                          CalledFuncs[0]->getType());
  PHINode *PHI = nullptr;
  PointerType *Ty = llvm::PointerType::get(CallToFix->getContext(),
                                           SPECIAL_BUFFER_ADDR_SPACE);

  // If CallToFix is indirect call, iterate over all potential CalledFunc as we
  // don't have accurate IndirectCallInfo yet. Compare called operand with each
  // CalledFunc. If compare result is true, load from CalledFunc's return value.
  for (auto *CalledFunc : CalledFuncs) {
    auto *Cond = Builder.CreateICmpEQ(ASC, CalledFunc);
    auto *ThenTerm = SplitBlockAndInsertIfThen(Cond, NextInst, false);
    // Calculate the pointer of the current special in the special buffer.
    unsigned Offset = DPV->getOffset(CalledFunc);
    Value *AddrInSpecialBuffer =
        getAddressInSpecialBuffer(Offset, Ty, ThenTerm, &DB);
    // Add Load instruction from special buffer at function offset.
    Builder.SetInsertPoint(ThenTerm);
    auto *LoadedValue = Builder.CreateLoad(CallToFix->getType(),
                                           AddrInSpecialBuffer, "loadedValue");

    Builder.SetInsertPoint(NextInst);
    auto *OldPHI = PHI;
    PHI = Builder.CreatePHI(CallToFix->getType(), 2, "loadedValue");
    PHI->addIncoming(LoadedValue, LoadedValue->getParent());
    if (OldPHI)
      PHI->addIncoming(OldPHI, OldPHI->getParent());
    else
      PHI->addIncoming(UndefValue::get(CallToFix->getType()),
                       cast<Instruction>(Cond)->getParent());
  }

  // Now PHI is the actually loaded value.
  if (DPV->hasOffset(CallToFix)) {
    // CallInst return value has an offset in the special buffer
    // Store the value to this offset.
    unsigned int OffsetRet = DPV->getOffset(CallToFix);
    ...
    // Add Store instruction to special buffer at return value offset
    StoreInst *SI =
        new StoreInst(PHI, AddrInSpecialBuffer, NextInst->getIterator());
    SI->setDebugLoc(DB);
  } else {
    CallToFix->replaceAllUsesWith(PHI);
  }
}
...
void KernelBarrierImpl::updateStructureStride(Module &M,
  ...
  for (auto *Func : TodoList) {
    auto KIMD = SYCLKernelMetadataAPI::KernelInternalMetadataAPI(Func);
    uint64_t StrideSize = DPV->getStrideSize(Func);
    // Indirect call, e.g. of virtual function, isn't vectorized at the moment.
    // If there is sync instruction in indirectly called function, we can't
    // divide StrideSize by VecWidth because indirectly called function is in
    // the same equivalence set as scaler kernel and vector kernel.
    // StrideSize must be unchanged for the indirectly called function.
    // This would over-estimate barrier special buffer size, but won't cause
    // incorrect issue.
    bool IndirectlyCalledFuncHasSync =
        llvm::any_of(ICI->getIndirectlyCalledFuncs(),
                     [&](auto *F) { return FunctionsWithSync.contains(F); });
    if (!ICI->hasIndirectCallInFunc(Func) || !IndirectlyCalledFuncHasSync) {
...
PreservedAnalyses KernelBarrier::run(Module &M, ModuleAnalysisManager &MAM) {
  ...
  IndirectCallInfo *ICI = &MAM.getResult<KernelIndirectCallAnalysis>(M);
  auto &FAM = MAM.getResult<FunctionAnalysisManagerModuleProxy>(M).getManager();
  auto GetDT = [&](Function &F) -> DominatorTree & {
    return FAM.getResult<DominatorTreeAnalysis>(F);
  };
  auto GetMemorySSA = [&](Function &F) -> MemorySSA & {
    return FAM.getResult<MemorySSAAnalysis>(F).getMSSA();
  };
  KernelBarrierImpl Impl(DPB, DPV, ICI, GetDT, GetMemorySSA);
  if (!Impl.run(M))
