...
InstCountResultImpl::InstCountResultImpl(Function &F, TargetTransformInfo &TTI,
  ...
  if (!F.getName().starts_with(VolcanoVectorKernelPrefix) &&
...
int InstCountResultImpl::estimateCall(CallInst *CI) {
  ...
  // 11 IMO.
  if (Name.starts_with("vload") || Name.starts_with("_Z6vload") ||
      Name.starts_with("vstore") || Name.starts_with("_Z7vstore")) {
    return MemOpWeight;
  }
  ...
  if (Name.starts_with("fake.insert") || Name.starts_with("fake.extract"))
