...
collectEnqueueKernelAndQueryFuncs(Module &M,
  ...
  for (Function &EEF : M) {
    if (EEF.isDeclaration()) {
      ...
      if (EEFName.starts_with("__ocl20_enqueue_kernel_") ||
          EEFName == "__ocl20_get_kernel_wg_size" ||
          EEFName == "__ocl20_get_kernel_preferred_wg_size_multiple")
        Funcs.push_back(&EEF);
    }
  }
}
...
std::vector<Value *> PrepareKernelArgsPass::createArgumentLoads(
    IRBuilder<> &Builder, Function *Wrapper, Function *WrappedKernel,
    Argument *UniformArgsBuffer, Argument *NonUniformArgsBuffer,
  ...
  // Save arguments to wrapper's metadata.
  auto KIMDWrapper = SYCLKernelMetadataAPI::KernelInternalMetadataAPI(Wrapper);
  KIMDWrapper.ArgProperties.set(Arguments);
  ...
  bool HasPrivateHeapImpArg =
      ImplicitArgEnums.contains(ImplicitArgsUtils::IA_PRIVATE_MEM_HEAP);
  if (hasLocalOrSpecialBuffer(Arguments, &KIMD) || HasPrivateHeapImpArg)
  ...
  for (unsigned ArgNo = 0, E = Arguments.size(); ArgNo < E; ++ArgNo) {
    ...
    if (KArg.Ty == KRNL_ARG_COMPOSITE || KArg.Ty == KRNL_ARG_VECTOR_BY_REF) {
      ...
      if (auto A = CallIt->getParamAlign()) {
        ...
        while (!WorkList.empty()) {
          ...
          if (isa<AddrSpaceCastInst, SelectInst, PHINode>(U)) {
            Def.insert(U);
          ...
          if (auto *GEP = dyn_cast<GetElementPtrInst>(U)) {
            for (auto *GEPU : GEP->users()) {
              // Add GEP's load and store users into alignment estimation in
              // case GEP might be canonicalized to *i8*.
              if (isa<LoadInst, StoreInst>(GEPU)) {
                Def.insert(GEP);
                WorkList.emplace_back(GEPU);
              }
            }
          }
          // TODO: Refine the alignment estimation for GEP, since it might still
          // result in underestimation of alignment when canonicalized GEP has
          // users but none of the users are load/store.
      ...
      if (HasLocalArg)
        ...
        // using heap memory
        HeapCurrentOffset = CompilationUtils::updateValueWithAlignment(
            HeapCurrentOffset, Alignment, Builder);
      Value *LocalArgBuffer =
          CompilationUtils::allocaArrayForLocalPrivateBuffer(
              Builder, HeapMem, DL, BufferSize, Alignment, HeapCurrentOffset);
      LLVM_DEBUG(CompilationUtils::insertPrintf(
          "LOCAL ARG BUFFER HEAP_MEMORY_POINTER == NULL : ", Builder,
          {HeapMem.second}, {"CMP"}));
  ...
  for (unsigned int I : ImplicitArgEnums) {
    ...
    assert((I == ImplicitArgsUtils::IA_SLM_BUFFER ||
            ...
            I == ImplicitArgsUtils::IA_PRIVATE_MEM_HEAP) &&
    ...
    case ImplicitArgsUtils::IA_SLM_BUFFER: {
      if (LocalBufferSizeInBytes == 0) {
        ...
        Value *SLMBuffer = CompilationUtils::allocaArrayForLocalPrivateBuffer(
            Builder, HeapMem, DL,
            ConstantInt::get(SizetTy, LocalBufferSizePadding),
            TypeAlignment::MAX_ALIGNMENT, HeapOffset);
        LLVM_DEBUG(CompilationUtils::insertPrintf(
            "SPECIAL BUFFER HEAP_MEMORY_POINTER == NULL : ", Builder,
            {HeapMem.second}, {"CMP"}));
        Value *CastBuf =
    ...
    case ImplicitArgsUtils::IA_BARRIER_BUFFER: {
      ...
      Value *BarrierBuffer = CompilationUtils::allocaArrayForLocalPrivateBuffer(
          Builder, HeapMem, DL, BarrierBufferSize, TypeAlignment::MAX_ALIGNMENT,
          HeapCurrentOffset);
      LLVM_DEBUG(CompilationUtils::insertPrintf(
          "LOCAL BUFFER HEAP_MEMORY_POINTER == NULL : ", Builder,
          {HeapMem.second}, {"CMP"}));
      if (HasPrivateHeapImpArg)
        HeapCurrentOffset =
            Builder.CreateAdd(HeapCurrentOffset, BarrierBufferSize);
      Arg = BarrierBuffer;
      LLVM_DEBUG(CompilationUtils::insertPrintf(
    ...
    case ImplicitArgsUtils::IA_PRIVATE_MEM_HEAP: {
      HeapCurrentOffset = CompilationUtils::updateValueWithAlignment(
          HeapCurrentOffset, TypeAlignment::MAX_ALIGNMENT, Builder);
      Value *HeapMemAddr =
          Builder.CreateGEP(I8Ty, HeapMem.first, HeapCurrentOffset);
      Value *HeapPtr = Builder.CreateSelect(
          HeapMem.second, Constant::getNullValue(HeapMem.first->getType()),
          HeapMemAddr);
      Arg = HeapPtr;
    } break;
    }
...
CallInst *PrepareKernelArgsPass::createWrapperBody(Function *Wrapper,
  ...
  std::vector<Value *> Params = createArgumentLoads(
      Builder, Wrapper, WrappedKernel, ArgsBuffer, WGId, RuntimeContext);
...
void PrepareKernelArgsPass::replaceFunctionPointers(Function *Wrapper,
  ...
  for (Function *EEF : EnqueueKernelAndQueryFuncs) {
    StringRef EEFName = EEF->getName();
    unsigned BlockInvokeIdx = (EEFName.starts_with("__ocl20_enqueue_kernel_"))
