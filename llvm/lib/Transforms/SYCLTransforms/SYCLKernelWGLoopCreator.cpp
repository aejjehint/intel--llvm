...
public:
  WGLoopCreatorImpl(
      Module &M, MapFunctionToReturnInst &FuncReturn, FuncSet &AllKernels,
      ...
      : M(M), Ctx(M.getContext()), Builder(Ctx), FuncReturn(FuncReturn),
        AllKernels(AllKernels), ICI(ICI), GetORE(GetORE) {}
...
bool WGLoopCreatorImpl::run() {
  ...
  for (auto *F : Kernels) {
    ...
    if (VectOrMaskedKernel) {
      ...
      if (VKIMD.CanUniteWorkgroups.hasValue())
...
void WGLoopCreatorImpl::patchNotInlinedFuncs(FuncSet &KernelAndSyncFuncs,
  ...
  patchNotInlinedTIDUserFunc(M, ICI, Builder, KernelAndSyncFuncs, TIDCallsToFix,
...
void WGLoopCreatorImpl::processFunction(Function *F, Function *VectorF,
  ...
  GetORE(*F).emit([&]() {
    StringRef Msg = (VectorF && MaskedF)
                        ? "vector loop with masked remainder loop"
                    : VectorF ? "vector loop with scalar remainder loop"
                    : MaskedF ? "masked loop"
                              : "scalar loop";
    return OptimizationRemark(DEBUG_TYPE, "KernelWGLoopCreator", F)
           << "work-group loop of kernel " << F->getName()
           << " is created from " << Msg;
  });
...
void WGLoopCreatorImpl::handleUniformEE(BasicBlock *RetBB) {
  ...
  Instruction *UniEECond = ExtractValueInst::Create(
      EECall, UniInd, "uniform.early.exit", InsertPt->getIterator());
  auto *TruncCond = new TruncInst(UniEECond, Type::getInt1Ty(Ctx), "",
...
std::pair<LoopRegion, Value *> WGLoopCreatorImpl::addWGLoops(
  ...
  // Move alloca and local id GEP in the entry kernel entry
  ...
  moveDbgDeclareIf(KernelEntry, NewEntry, [](Instruction &I) {
    if (isa<AllocaInst>(&I))
      return true;
    for (DbgVariableRecord &DI : filterDbgVars(I.getDbgRecordRange())) {
      if (DI.isDbgDeclare()) {
        for (unsigned Idx = 0; Idx < DI.getNumVariableLocationOps(); ++Idx) {
          auto *Val = DI.getVariableLocationOp(Idx);
          auto *AI = dyn_cast<AllocaInst>(Val);
          // If any variable location operand is an implicit GID, return false.
          if (AI && isImplicitGID(AI)) {
            return false;
          }
        }
  ...
  for (unsigned Dim = 0; Dim < NumDim; ++Dim) {
    ...
    if (UseTLSGlobals) {
      Builder.SetInsertPoint(Head->getFirstNonPHIIt());
...
PHINode *WGLoopCreatorImpl::createLIDPHI(Value *InitVal, Value *IncBy,
                                         ...
                                         BasicBlock *Latch) {
  PHINode *DimTID =
      PHINode::Create(IndTy, 2, DimStr + "tid", Head->getFirstNonPHIIt());

  BinaryOperator *IncTID = BinaryOperator::Create(
      Instruction::Add, DimTID, IncBy, DimStr + "inc_tid",
...
PreservedAnalyses SYCLKernelWGLoopCreatorPass::run(Module &M,
  ...
  auto GetORE = [&](Function &F) -> OptimizationRemarkEmitter & {
  ...
  WGLoopCreatorImpl Impl(M, FuncReturn, AllKernels, ICI, GetORE);
