//==--- PhiCanonicalization.cpp - Canonicalization of Phi nodes -- C++ -*---==//
//
...
bool PhiCanonicalization::runImpl(Function &F, DominatorTree *DT,
  ...
  for (Function::iterator bbit = F.begin(); bbit != F.end(); ++bbit) {
    assert(std::distance(pred_begin(&*bbit), pred_end(&*bbit)) < 3 &&
           "Phi canon failed");
  }
...
void PhiCanonicalization::fixBlock(BasicBlock *toFix, DominatorTree *DT,
  ...
  while (std::distance(pred_begin(toFix), pred_end(toFix)) > 2) {
    ...
    assert(current_bb != scan_bb && "Unable to get previous blocks ");
...
BasicBlock* PhiCanonicalization::makeNewPhiBB(BasicBlock* toFix,
  ...
  BasicBlock* new_bb = BasicBlock::Create(
    ...
    assert(std::find(pred_begin(toFix), pred_end(toFix), new_bb) !=
           pred_end(toFix));
  ...
  assert(std::find(pred_begin(new_bb), pred_end(new_bb), prev0) !=
         pred_end(new_bb));
  assert(std::find(pred_begin(new_bb), pred_end(new_bb), prev1) !=
         pred_end(new_bb));
  ...
  for (BasicBlock::iterator it = toFix->begin(),
    ...
    if (v0 != v1) {
      ...
      PHINode *phi_new =
          PHINode::Create(phi->getType(), 2, "new_phi", new_bb->begin());
...
void PhiCanonicalization::fixBasicBlockSucessor(BasicBlock* to_fix,
                                   BasicBlock* old_target,
                                   BasicBlock* new_target) {

  Instruction* term = to_fix->getTerminator();
  assert(term && "Basic Block must have a terminator");
