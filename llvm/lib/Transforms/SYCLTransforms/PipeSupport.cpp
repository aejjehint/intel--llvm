...
static bool isPipeTy(StringRef TyName) {
  return TyName.starts_with("spirv.Pipe") ||
         TyName.starts_with("spirv.Channel");
}
...
static int getNumUsedPipes(Function &F, const PipeTypesHelper &PipeTypes) {
  ...
  if (KIMD.ArgTargetExtTypeList.hasValue()) {
    if (F.arg_size() == KIMD.ArgTargetExtTypeList.size()) {
      ...
      // pipe arrays.
      auto MaxNumPipeArgs =
          (unsigned)llvm::count_if(KIMD.ArgTargetExtTypeList, [](auto TyName) {
            return isPipeTy(TyName);
          });
      for (auto &A : F.args()) {
...
static Value *getPipeCallRetcode(const PipeCallInfo &PC) {
  ...
  if (PC.Kind.Access == PipeKind::AccessKind::Read &&
    ...
    return new LoadInst(Ty, RetcodePtr, "", std::next(PC.Call->getIterator()));
  }
