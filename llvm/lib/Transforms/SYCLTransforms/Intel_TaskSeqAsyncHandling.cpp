...
constexpr const char *BuiltinCreateTaskSeqPattern =
    "_Z66__spirv_TaskSequenceCreateINTEL";
...
private:
  ...
  DenseMap<Function *, SmallSetVector<Function *, 8>> AsyncBuiltinToTaskFuncMap;
...
void Impl::findAllAsyncBuiltins() {
  for (Function &F : M) {
    ...
    if (FName.starts_with(BuiltinGetPattern))
      Gets.push_back(&F);
    else if (FName.starts_with(BuiltinAsyncPattern))
      Asyncs.push_back(&F);
    else if (FName.starts_with(BuiltinCreateTaskSeqPattern))
      Creates.push_back(&F);
    else if (FName.starts_with(BuiltinReleaseTaskSeqPattern))
...
void Impl::fixupTaskFuncUsersAfterLoweringSRetArg(Function *TaskF,
  ...
  for (auto *U : make_early_inc_range(TaskF->users())) {
    ...
    if (Builtin->getName().starts_with(BuiltinAsyncPattern))
...
void Impl::updateTaskFuncInAsyncMap(Function *Builtin, Function *TaskF,
                                    Function *NewTaskF) {
  if (AsyncBuiltinToTaskFuncMap[Builtin].remove(TaskF)) {
    LLVM_DEBUG(dbgs() << "Erase mapping builtin " << Builtin->getName()
                      << " to " << TaskF->getName() << " in task func map!\n");
    AsyncBuiltinToTaskFuncMap[Builtin].insert(NewTaskF);
...
void Impl::lowerBuiltinsSRetArgToReturnTypeAndFixupUsers(
  ...
  for (Function *Builtin : make_early_inc_range(BuiltinsWithSRet)) {
    ...
    StringRef FName = Builtin->getName();
    if (FName.starts_with(BuiltinAsyncPattern))
...
// new task_sequence header does not pass task_func_pointer to
// _Z30__spirv_TaskSequenceAsyncINTEL function but it's required for it.
// We extend argument list of _Z30__spirv_TaskSequenceAsyncINTEL and add
// task_func_pointer as the second argument. It's the same position for
// task_func_pointer as previous design.
// We get the task_func_pointer from task sequence create function and
// pass it to _Z30__spirv_TaskSequenceAsyncINTEL.
void Impl::extendAsyncWithFuncArg() {
  auto *PtrTy = PointerType::getUnqual(Ctx);
  FunctionVector newAsyncs;
  for (Function *F : Asyncs) {
    // add a new ptr parameter to Async func and create a new function
    SmallVector<Type *> ArgTys;
    auto I = F->arg_begin();
    ArgTys.push_back(&*I->getType()); // First arg is task sequence. keep it.
    ArgTys.push_back(PtrTy);          // Add function pointer as second arg.
    for (auto I = F->arg_begin() + 1, E = F->arg_end(); I != E; I++)
      ArgTys.push_back(&*I->getType());

    FunctionType *AsyncFuncType = F->getFunctionType();
    assert(AsyncFuncType && "task func type not found");
    auto RetType = AsyncFuncType->getReturnType();

    FunctionType *NewFTy = FunctionType::get(RetType, ArgTys, false);
    std::string Name = F->getName().str();
    F->setName(F->getName() + "_before.addingArg");
    Function *NewF =
        Function::Create(NewFTy, F->getLinkage(), Name, F->getParent());
    assert(NewF && "Failed to create new Async Function");
    newAsyncs.push_back(NewF);
    // For every invoke of Async function, pass the parameters to invoke of new
    // func.
    for (auto *U : make_early_inc_range(F->users())) {
      auto *CI = cast<CallInst>(U);
      assert(CI->getCalledFunction() == F &&
             "__spirv_TaskSequenceAsync isn't directly called?");
      auto *TaskSequenceInstance = cast<CallInst>(CI->getArgOperand(0));
      auto *TaskFunc = cast<Function>(
          TaskSequenceInstance->getArgOperand(0)->stripPointerCasts());

      assert(NewF && "function is NULL");
      assert(CI->arg_size() + 1 == NewF->arg_size() &&
             "Function argument number mismatch");

      SmallVector<Value *, 16> Args;
      Args.push_back(CI->getArgOperand(
          0));                  // pass first argument of task_sequence instance
      Args.push_back(TaskFunc); // pass second argment of async function pointer
      for (unsigned I = 1, E = CI->arg_size(); I != E; ++I)
        Args.push_back(CI->getArgOperand(I));

      // Replace the original function with a call
      CallInst *NewC = CallInst::Create(NewF, Args, "", CI->getIterator());
      NewC->setCallingConv(CI->getCallingConv());
      NewC->setAttributes(NewF->getAttributes());

      // Copy debug metadata to new function if available
      if (CI->hasMetadata()) {
        NewC->setDebugLoc(CI->getDebugLoc());
      }

      CI->replaceAllUsesWith(NewC);
  ...
  // Remove all old Async functions from Asyncs
  Asyncs.clear();
  // copy new Async functions to Asyncs
  Asyncs = std::move(newAsyncs);
}
...
void Impl::collectTaskFuncs() {
  for (Function *F : Asyncs) {
    ...
    AsyncBuiltinToTaskFuncMap[F] = std::move(TaskFuncs);
  }
}
...
static FunctionType *findTaskFuncType(Function *F) {
  for (User *U : F->users())
    if (auto *CI = dyn_cast<CallInst>(U)) {
      ...
      auto *TaskFunc = cast<Function>(
          TaskSequenceInstance->getArgOperand(0)->stripPointerCasts());
      return TaskFunc->getFunctionType();
    }
  return nullptr;
}
...
FunctionCallee Impl::getBackendCreateTaskSeq() {
  // void *__create_task_sequence(size_t result_size, int32 pipelined, int32
  // fpga_cluster,
  // uint32 response_capacity,
  // uint32 invocation_capacity)
  auto *SizeTTy = Type::getIntNTy(Ctx, sizeof(size_t) * 8);
  auto *I32Ty = Type::getInt32Ty(Ctx);
  auto *RetTy = PointerType::getUnqual(Ctx);
  auto *FTy =
      FunctionType::get(RetTy, {SizeTTy, I32Ty, I32Ty, I32Ty, I32Ty}, false);
...
void Impl::generateCreateTaskSeqBodies() {
  ...
  //   void *impl = __create_task_sequence(return_type_size, pipelined,
  //   fpga_cluster, response_capacity, invocation_capacity); return
  //   (size_t)impl;
  // }
  ...
  //       %11 = call ptr @__create_task_sequence(i64 %10, i64 %0,i64 %1,i64
  //       %2,i64 %3) %12 = ptrtoint ptr %11 to i64 ret i64 %12
  //     }
  ...
  for (Function *F : Creates) {
    ...
    for (auto *U : F->users()) {
      ...
      auto *TaskFunc =
          cast<Function>(CI->getArgOperand(0)->stripPointerCasts());
      TaskFuncs.insert(TaskFunc);
    }
    ...
    if (TaskFuncs.size() > 1) {
      auto *IntPtrTy = IntegerType::getIntNTy(Ctx, sizeof(intptr_t) * 8);
      auto *TaskFuncVar = IRB.CreatePtrToInt(F->getArg(0), IntPtrTy);
    ...
    //    &f, pipelined, fpga_cluster, response_capacity, invocation_capacity);
    assert(F->arg_size() == 5 &&
           "Invalid number of arguments for create function");
    // The last 4 functions should be passed to backend create function
    // directly.
    auto *pipelined = F->getArg(1);
    auto *fpga_cluster = F->getArg(2);
    auto *response_capacity = F->getArg(3);
    auto *invocation_capacity = F->getArg(4);

    auto *CI = IRB.CreateCall(BackendCreateTaskSeq,
                              {LastVal, pipelined, fpga_cluster,
                               response_capacity, invocation_capacity});
...
void Impl::generateGetBodies() {
  ...
  // ReturnT __spirv_TaskSequenceGetINTEL(task_sequence *obj) {
  ...
  for (Function *F : Gets) {
    ...
    if (RetTy->isVoidTy())
      ...
      auto *RetValPtr = IRB.CreatePointerCast(
          CI, llvm::PointerType::getUnqual(CI->getContext()));
...
void Impl::generateAsyncBodies() {
  ...
  // we added one additional Function pointer as the second parameter of this
  // function void __spirv_TaskSequenceAsyncINTEL(
  //     task_sequence *obj, &f, ...) {
  ...
  for (Function *F : Asyncs) {
    ...
    // Exclude the first 2 args of F: task_sequnce and function pointer.
    constexpr unsigned FArgStart = 2;
    ...
    IRB.CreateCall(BackendAsync, {TaskSeqObjBC, TaskInvBC, LiteralBC});
    IRB.CreateRetVoid();
    F->setLinkage(GlobalValue::InternalLinkage);
  }
  Changed = true;
  ...
  for (Function *F : Asyncs) {
    ...
    for (Function *TaskFunc : AsyncBuiltinToTaskFuncMap[F]) {
      ...
      auto *Literal = IRB.CreatePointerCast(
          FuncInvoke->getArg(0), llvm::PointerType::getUnqual(F->getContext()),
          "literal");
      ...
      if (!TaskFuncRetTy->isVoidTy()) {
        ...
        auto *ResPtrBC = IRB.CreatePointerCast(
            ResPtr, llvm::PointerType::getUnqual(F->getContext()),
            "res.ptr.bcast");
