...
CallInst *GroupBuiltinPass::getWICall(Instruction *Before, StringRef FuncName,
  ...
  // Function call
  CallInst *Call = CallInst::Create(Func, ArrayRef<Value *>(Params), "WIcall",
                                    Before->getIterator());
...
static Instruction *calculate2DimLinearID(Instruction *Before, Value *LocalID_0,
  ...
  BinaryOperator *Mul = BinaryOperator::CreateMul(LocalSize_0, LocalID_1, "",
                                                  Before->getIterator());
  return BinaryOperator::CreateAdd(Mul, LocalID_0, "getLinearId2D",
                                   Before->getIterator());
}
...
static Instruction *calculate3DimLinearID(Instruction *Before,
  ...
  BinaryOperator *Mul1 = BinaryOperator::CreateMul(LocalSize_1, LocalID_2, "",
                                                   Before->getIterator());
  BinaryOperator *Mul2 =
      BinaryOperator::CreateMul(LocalSize_0, Mul1, "", Before->getIterator());
...
bool GroupBuiltinPass::runImpl(Module &M, RuntimeService &RTS) {
  ...
  for (auto *I : CallWgFunc) {
    ...
    AllocaInst *Result =
        new AllocaInst(RetType, DL.getAllocaAddrSpace(), "AllocaWGResult",
                       FirstInstr->getIterator());

    // 2. Initialize the result according to function name and argument
    // type.
    Value *InitValue = getInitializationValue(Callee);
    (void)new StoreInst(InitValue, Result, FirstInstr->getIterator());
    ...
    CallInst *NewCall =
        CallInst::Create(NewFunc, ArrayRef<Value *>(Params), "CallWGForItem",
                         WGCallInst->getIterator());
    ...
    NewCall->copyMetadata(*WGCallInst);
    ...
    if (isWorkGroupUniform(FuncName)) {
      ...
      LoadInst *LoadResult =
          new LoadInst(Result->getAllocatedType(), Result, "LoadWGFinalResult",
                       WGCallInst->getIterator());
      ...
      CallInst *FinalizeCall = CallInst::Create(
          FinalizeFunc, Params, "CallFinalizeWG", WGCallInst->getIterator());
    ...
    (void)new StoreInst(InitValue, Result, WGCallInst->getIterator());
