...
bool ResolveWICallPass::runImpl(Module &M, ImplicitArgsInfo *IAInfo,
  ...
  for (Function &F : M) {
    if (!ImplicitArgsUtils::needImplicitArgs(&F))
      continue;
...
Function *ResolveWICallPass::runOnFunction(Function *F) {
  ...
  if (HasTLSGlobals) {
    ...
    CompilationUtils::getImplicitArgs(F, nullptr, &WorkInfo, &WGId, &BaseGlbId,
                                      &SpecialBuf, &RuntimeHandle, nullptr);
  }
...
Value *ResolveWICallPass::updateGetFunction(CallInst *CI,
  ...
  PHINode *AttrResult =
      PHINode::Create(CI->getType(), 2, "", splitContinue->getFirstNonPHIIt());
...
FunctionType *ResolveWICallPass::getOrCreatePrintfFuncType(unsigned AddrSpace) {
  // The prototype of __opencl_printf is:
  // int __opencl_printf(AS char *format, char *args, void *Callback,
  // void *RuntimeHandle)
  // where AS is either __constant or __generic.
  std::vector<Type *> Params;
  assert((AddrSpace == 2 || AddrSpace == 4) &&
         "format string has invalid address space");
  Params.push_back(PointerType::get(IntegerType::get(*Ctx, 8), AddrSpace));
