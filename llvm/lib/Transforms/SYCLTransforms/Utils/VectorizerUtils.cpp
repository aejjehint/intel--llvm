...
static Instruction *convertUsingShuffle(Value *V, const Type *RealTy,
  ...
  Instruction *Shuffle =
      new ShuffleVectorInst(V, UndefVect, Mask, "", Loc->getIterator());
...
static Value *isInsertEltExtend(Instruction *I, Type *RealTy) {
  ...
  for (unsigned Idx = 0; Idx < DestNElts; ++Idx) {
    ...
    GatherdVals = InsertElementInst::Create(GatherdVals, Val, Index, "",
                                            Loc->getIterator());
  }
  return GatherdVals;
}
...
Instruction *createBroadcast(Value *V, unsigned VectorWidth,
  ...
  auto *IEI = InsertElementInst::Create(UndefVec, V, Zero, "insert",
                                        InsertBefore->getIterator());
  auto *Shuffle = new ShuffleVectorInst(IEI, UndefVec, ZeroVector, "vector",
                                        InsertBefore->getIterator());
