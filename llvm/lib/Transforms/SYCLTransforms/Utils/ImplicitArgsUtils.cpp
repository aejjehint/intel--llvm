...
// Maximum size of private memory allowed for a workitem.
static size_t MaxPrivateMemorySize = 256 * 1024;

static cl::opt<size_t, true>
    OptMaxPrivateMemSize("sycl-kernel-max-private-mem-size", cl::Hidden,
                         cl::desc("Set max private memory size allowed for a "
                                  "workitem"),
                         cl::location(MaxPrivateMemorySize));
...
unsigned NDInfo::internalCall2NDInfo(unsigned InternalCall) {
  ...
  case ICT_GET_NUM_GROUPS:
    return NDInfo::INTERNAL_WG_NUMBER;
...
static const ArgData ImpArgs[ImplicitArgsUtils::IA_NUMBER] = {
    ...
    {"pPrivateHeapMem", true}}; // IA_PRIVATE_MEM_HEAP
...
void ImplicitArgsUtils::getImplicitArgEnums(
    SmallSetVector<unsigned, 8> &ImplicitArgEnums, Module *M) {
  bool NeedPrivateHeapMem = false;
  for (auto *Kernel : SYCLKernelMetadataAPI::KernelList(M)) {
    if (Kernel) {
      ...
      if (KIMD.NoBarrierPath.hasValue() && KIMD.NoBarrierPath.get() == true &&
          ...
          KIMD.PrivateMemorySize.get() > MaxPrivateMemorySize) {
        NeedPrivateHeapMem = true;
        break;
      }
    }
  }

  for (unsigned I = 0; I < NUM_IMPLICIT_ARGS; ++I) {
    if (!NeedPrivateHeapMem && I == IA_PRIVATE_MEM_HEAP)
      continue;
    ImplicitArgEnums.insert(I);
  }
}

size_t ImplicitArgsUtils::getMaxPrivateMemorySize() {
  return MaxPrivateMemorySize;
}

bool ImplicitArgsUtils::needImplicitArgs(Function *Func) {
