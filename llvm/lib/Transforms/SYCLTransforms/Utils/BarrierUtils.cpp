...
Instruction *BarrierUtils::createDummyBarrier(Instruction *InsertBefore) {
  ...
  return InsertBefore ? CallInst::Create(DummyBarrierFunc, "",
                                         InsertBefore->getIterator())
                      : CallInst::Create(DummyBarrierFunc, "");
}
...
Instruction *BarrierUtils::createGetSpecialBuffer(Instruction *InsertBefore) {
  if (!GetSpecialBufferFunc) {
    ...
    GetSpecialBufferFunc->addRetAttr(Attribute::NoAlias);
  }
  auto *CI = CallInst::Create(GetSpecialBufferFunc, "pSB",
                              InsertBefore->getIterator());
