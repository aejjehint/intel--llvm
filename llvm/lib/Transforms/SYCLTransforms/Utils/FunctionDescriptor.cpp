...
size_t FunctionDescriptor::getNumOfParams() {
  size_t Count = Parameters.size();
  if (Count != 1)
    return Count;

  // Check whether the only parameter is void
  const auto *Type = (const ParamType *)Parameters[0].get();
  if (Type->getTypeId() == TYPE_ID_PRIMITIVE) {
    const auto *PrimType = static_cast<const PrimitiveType *>(Type);
    // 'void' type shouldn't be counted.
    if (PrimType->getPrimitive() == PRIMITIVE_VOID)
      return 0;
  }
  return Count;
}
