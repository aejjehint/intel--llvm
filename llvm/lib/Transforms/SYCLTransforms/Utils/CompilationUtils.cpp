...
#define DEBUG_TYPE "sycl-kernel-compilation-utils"

std::string VectInfoFile;
static cl::opt<std::string, true>
    OptVectInfoFile("sycl-vect-info", cl::Hidden,
                    cl::desc("Builtin VectInfo list"),
                    cl::value_desc("filename"), cl::location(VectInfoFile));

static cl::opt<std::string>
    OptBuiltinAliasFile("sycl-builtin-alias", cl::Hidden,
                        cl::desc("Builtin Alias map"),
...
// OpenCL ExtendedInstructionSet
const StringRef NAME_PRINTF_EIS = "__spirv_ocl_printf";
...
bool isAtomicBuiltin(StringRef S) {
  ...
  if (!isMangledName(S))
    return false;
  return stripName(S).starts_with("atom");
}
...
bool isPrintf(StringRef S) {
  ...
  return FD.Name == NAME_PRINTF_EIS;
}
...
PipeKind getPipeKind(StringRef Name) {
  ...
  if (S.consume_front("_") && S.starts_with("v"))
...
StructType *getStructByName(StringRef Name, const Module *M) {
  ...
  for (auto *STy : StructTys) {
    ...
    if (stripStructNameTrailingDigits(STy->getName()) ==
        stripStructNameTrailingDigits(Name))
      return STy;
  }
...
bool hasWorkGroupFinalizePrefix(StringRef S) {
  if (!isMangledName(S))
    return false;
  return stripName(S).starts_with(NAME_FINALIZE_WG_FUNCTION_PREFIX);
}
...
bool isSubGroupNonUniformFlow(StringRef S) {
  ...
  if (S.consume_front("reduce_") || S.consume_front("scan_inclusive_") ||
    ...
    return S == "add" || S == "min" || S == "max" || S == "mul" || S == "and" ||
           S == "or" || S == "xor" || S == "logical_and" || S == "logical_or" ||
           S == "logical_xor";

  // From extension cl_khr_subgroup_non_uniform_vote and cl_khr_subgroup_ballot.
  return S == "all" || S == "any" || S == "all_equal" || S == "broadcast";
}

bool isSubGroupClusteredReduce(StringRef S) {
  if (!isMangledName(S))
    return false;
  S = stripName(S);
  if (!S.consume_front("sub_group_clustered_reduce_"))
    return false;
  // From extension cl_khr_subgroup_clustered_reduce.
  return S == "add" || S == "min" || S == "max" || S == "mul" || S == "and" ||
         S == "or" || S == "xor" || S == "logical_and" || S == "logical_or" ||
         S == "logical_xor";
}
...
bool isSubGroupUniformFlowUniformRet(StringRef S) {
  return isGetSubGroupSize(S) || isGetSubGroupId(S) ||
         ...
         isSubGroupSort(S) || isSubGroupClusteredReduce(S);
}
...
static void collectDbgToMove(Instruction *I,
                             SmallVectorImpl<DbgVariableRecord *> &ToMoveDVIs) {
  for (DbgVariableRecord &DVR : filterDbgVars(I->getDbgRecordRange())) {
    if (DVR.isDbgDeclare()) {
      for (unsigned Idx = 0; Idx < DVR.getNumVariableLocationOps(); ++Idx) {
        ToMoveDVIs.push_back(&DVR);
      }
    }
  }
}
...
void moveDbgDeclareIf(BasicBlock *FromBB, BasicBlock *ToBB,
                      function_ref<bool(Instruction &)> Predicate) {
  SmallVector<DbgVariableRecord *> ToMoveDVIs;
  for (auto &I : *FromBB) {
    if (Predicate(I)) {
      collectDbgToMove(&I, ToMoveDVIs);
    }
  }
  auto MovePos = ToBB->getFirstInsertionPt();
  for (auto *DVI : ToMoveDVIs) {
    DVI->removeFromParent();
    ToBB->insertDbgRecordBefore(DVI, MovePos);
  }
}
...
CallInst *AddMoreArgsToCall(CallInst *OldC, ArrayRef<Value *> NewArgs,
  ...
  CallInst *NewC = CallInst::Create(NewF, Args, "", OldC->getIterator());
  NewC->setCallingConv(OldC->getCallingConv());
  NewC->copyMetadata(*OldC);
  // Copy attributes
  AttributeList OldAL = OldC->getAttributes();
  AttributeList NewAL = NewF->getAttributes();
  SmallVector<AttributeSet, 16> ArgAttrs;
  unsigned I = 0;
  for (; I < OldC->arg_size(); ++I)
    ArgAttrs.push_back(OldAL.getParamAttrs(I));
  for (; I < NewF->arg_size(); ++I)
    ArgAttrs.push_back(NewAL.getParamAttrs(I));
  NewC->setAttributes(AttributeList::get(OldC->getContext(), OldAL.getFnAttrs(),
                                         OldAL.getRetAttrs(), ArgAttrs));
...
CallInst *addMoreArgsToIndirectCall(CallInst *OldC, ArrayRef<Value *> NewArgs) {
  assert(OldC && "CallInst is NULL");
  [[maybe_unused]] auto *Callee = OldC->getCalledFunction();
  assert((!Callee || Callee->getName() == "__intel_indirect_call") &&
  ...
  auto *Cast = CastInst::CreatePointerCast(
      ...
      PointerType::get(NewFType, FPtrType->getAddressSpace()), "",
      OldC->getIterator());
  assert(Cast && "Failed to create CastInst");

  // Replace the original function with a call
  auto *NewC = CallInst::Create(NewFType, Cast, Args, "", OldC->getIterator());
  assert(NewC && "Failed to create CallInst");
  NewC->setCallingConv(OldC->getCallingConv());
  NewC->setAttributes(OldC->getAttributes());
  NewC->copyMetadata(*OldC);
...
void getImplicitArgs(Function *pFunc, Value **LocalMem, Value **WorkDim,
                     ...
                     Value **RunTimeHandle, Value **PrivateHeapMemPtr) {
  ...
  if (ImplicitArgEnums.contains(ImplicitArgsUtils::IA_PRIVATE_MEM_HEAP)) {
    if (nullptr != PrivateHeapMemPtr)
      *PrivateHeapMemPtr = &*DestI;
    ++DestI;
  }
...
// Construct TargetExtType from name, e.g. spirv.Image_1_0_0_0_0_0_0, which is
// mangled in KernelTargetExtTypeLowerPass.
static TargetExtType *parseTargetExtTyFromName(LLVMContext &Ctx,
                                               StringRef TyName) {
  SmallVector<StringRef, 8> SubStrings;
  llvm::SplitString(TyName, SubStrings, "_");
  assert(!SubStrings.empty() && "invalid TargetExtType name");
  SmallVector<unsigned, 8> IntParams;
  for (unsigned I = 1; I < SubStrings.size(); ++I) {
    unsigned Param;
    [[maybe_unused]] bool Res = llvm::to_integer(SubStrings[I], Param);
    assert(Res && "expect int param");
    IntParams.push_back(Param);
  }
  SmallVector<Type *, 8> TypeParams(IntParams.size(), Type::getInt32Ty(Ctx));
  return TargetExtType::get(Ctx, SubStrings[0], TypeParams, IntParams);
}

void parseKernelArguments(Module *M, Function *F, bool HasTLSGlobals,
                          SmallVector<KernelArgument, 8> &Arguments) {
  ...
  // TODO fix LIT which don't have ArgTargetExtTypeList yet.
  if (KIMD.ArgTargetExtTypeList.hasValue()) {
    assert((ArgsCount - KIMD.ArgTargetExtTypeList.size() <= 1) &&
           "arg count mismatch");
    auto &Ctx = M->getContext();
    for (const auto &[Idx, TyName] :
         llvm::enumerate(KIMD.ArgTargetExtTypeList)) {
      if (TyName.empty())
        ArgTypes.push_back(F->getArg(Idx)->getType());
      else
        ArgTypes.push_back(parseTargetExtTyFromName(Ctx, TyName));
    }
    // Masked kernel has an additional mask argument.
    if (ArgsCount > KIMD.ArgTargetExtTypeList.size())
  ...
  for (unsigned i = 0; i < ArgsCount; ++i) {
    ...
    case Type::PointerTyID: {
      ...
      if (!ArgTyStr.empty()) {
        ...
        // It is safe to use starts_with while there are no names which aren't
        // prefix of another name.
        if (ArgTyStrRef.starts_with("image1d_array_"))
          CurArg.Ty = KRNL_ARG_PTR_IMG_1D_ARR;
        else if (ArgTyStrRef.starts_with("image1d_buffer_"))
          CurArg.Ty = KRNL_ARG_PTR_IMG_1D_BUF;
        else if (ArgTyStrRef.starts_with("image1d_"))
          CurArg.Ty = KRNL_ARG_PTR_IMG_1D;
        else if (ArgTyStrRef.starts_with("image2d_depth_"))
          CurArg.Ty = KRNL_ARG_PTR_IMG_2D_DEPTH;
        else if (ArgTyStrRef.starts_with("image2d_array_depth_"))
          CurArg.Ty = KRNL_ARG_PTR_IMG_2D_ARR_DEPTH;
        else if (ArgTyStrRef.starts_with("image2d_array_"))
          CurArg.Ty = KRNL_ARG_PTR_IMG_2D_ARR;
        else if (ArgTyStrRef.starts_with("image2d_"))
          CurArg.Ty = KRNL_ARG_PTR_IMG_2D;
        else if (ArgTyStrRef.starts_with("image3d_"))
          CurArg.Ty = KRNL_ARG_PTR_IMG_3D;
        else if (ArgTyStrRef.starts_with("pipe_ro_t"))
          CurArg.Ty = KRNL_ARG_PTR_PIPE_T;
        else if (ArgTyStrRef.starts_with("pipe_wo_t"))
          CurArg.Ty = KRNL_ARG_PTR_PIPE_T;
        else if (ArgTyStrRef.starts_with("queue_t"))
          CurArg.Ty = KRNL_ARG_PTR_QUEUE_T;
        else if (ArgTyStrRef.starts_with("clk_event_t"))
          CurArg.Ty = KRNL_ARG_PTR_CLK_EVENT_T;
        else if (ArgTyStrRef.starts_with("sampler_t"))
    ...
    case Type::HalfTyID: {
      CurArg.Ty = KRNL_ARG_HALF;
      Type *HalfType = Type::getHalfTy(M->getContext());
      CurArg.SizeInBytes = HalfType->getScalarSizeInBits() / 8;
    } break;
...
void replaceFunctionUsesInMetadata(
    DenseMap<Function *, Function *> &FunctionMap) {
  // Replace use of F in metadata, e.g. debug metadata.
  for (auto &[F, NewF] : FunctionMap)
    if (F->isUsedByMetadata())
      ValueAsMetadata::handleRAUW(F, NewF);
}

void replaceFunctionUses(Function *F, Function *NewF,
                         SmallVectorImpl<User *> &Users,
                         ValueMapTypeRemapper *TypeMapper) {
  ValueToValueMapTy VMap;
  VMap[F] = NewF;
  ValueMapper VMapper(VMap, RF_NoModuleLevelChanges | RF_IgnoreMissingLocals,
                      TypeMapper);
  for (User *U : make_early_inc_range(Users)) {
    if (auto *I = dyn_cast<Instruction>(U))
      ...
      llvm_unreachable("unhandled function user");
  }

  DenseMap<Function *, Function *> Map;
  Map[F] = NewF;
  replaceFunctionUsesInMetadata(Map);
}
...
static void initializeVectInfoOnce(ArrayRef<VectItem> VectInfos) {
  ...
  if (!VectInfos.empty()) {
    ...
    static ErrorOr<std::unique_ptr<MemoryBuffer>> BufOrErr =
        MemoryBuffer::getFile(VectInfoFile, /* IsText */ true);
    if (BufOrErr) {
...
static std::string getFormatStr(Type *T) {
  std::string Res = "";
  if (auto *VT = dyn_cast<FixedVectorType>(T)) {
    auto *ScalarType = VT->getScalarType();
    Res = '{';
    for (unsigned I = 0; I < VT->getNumElements(); ++I)
      Res += getFormatStr(ScalarType) + ", ";
    Res += '}';
    return Res;
  }

  if (T->isIntegerTy()) // print any int type as i32
    return "%d";
  if (T->is16bitFPTy()) // print bf16/fp16 in hex
    return "0x%X";
  if (T->isFloatTy())
    return "%f";
  if (T->isDoubleTy())
...
static void addValueToPrintfArgs(Value *V, SmallVectorImpl<Value *> &PrintfArgs,
                                 IRBuilder<> &Builder) {
  if (auto *VT = dyn_cast<FixedVectorType>(V->getType())) {
    for (unsigned I = 0; I < VT->getNumElements(); ++I) {
      auto *Ele = Builder.CreateExtractElement(V, I);
      addValueToPrintfArgs(Ele, PrintfArgs, Builder);
    }
    return;
  }

  auto &C = V->getContext();
  // print bf16/fp16 in hex
  if (V->getType()->is16bitFPTy())
    V = Builder.CreateBitCast(V, IntegerType::getInt16Ty(C));

  // print any int type as i32
  // FIXME: may print incomplete data for int types with larger bit width
  if (V->getType()->isIntegerTy())
    V = Builder.CreateIntCast(V, IntegerType::getInt32Ty(C), false);
...
void insertPrintf(const Twine &Prefix, IRBuilder<> &Builder,
  ...
  for (const auto &[I, V] : enumerate(Inputs)) {
    // Build format str for each value:
    // "[InputPrefix if any][ValueType]ValueName: Value "
    std::string ValuePrefix =
        I < InputPrefixes.size() ? ('[' + InputPrefixes[I].str() + ']') : "";
    bool HasUnnamedType = false;
    ValuePrefix += '[' + getMangledTypeStr(V->getType(), HasUnnamedType) + ']';
    FormatStr += ValuePrefix;
    FormatStr += V->getName().str() + ": ";
    auto *VType = V->getType();
    FormatStr += getFormatStr(VType) + ' ';

    // Collect actual values to be print
    addValueToPrintfArgs(V, PrintfArgs, Builder);
  }
  FormatStr += '\n';
  ...
  StringRef GVName = "format.str.";
  GlobalVariable *FormatStrGV = nullptr;
  // If global variable already exists, reuse it.
  for (GlobalVariable &GV : BB->getModule()->globals()) {
    if (GV.getName().starts_with(GVName) && GV.getValueType() == ArrayType &&
        GV.isConstant() && GV.getInitializer() == StrVal &&
        GV.getAddressSpace() == StrAddrSpace) {
      FormatStrGV = &GV;
      break;
    }
  }
  if (!FormatStrGV) {
    FormatStrGV = new GlobalVariable(
        *BB->getModule(), ArrayType, true, GlobalValue::InternalLinkage, StrVal,
        GVName, 0, GlobalVariable::NotThreadLocal, StrAddrSpace);
  }

  auto *StrPtr = Builder.CreateConstGEP2_32(ArrayType, FormatStrGV, 0, 0);

  // Insert call.
  PrintfArgs.insert(PrintfArgs.begin(), StrPtr);
  Builder.CreateCall(PrintFunc, PrintfArgs, "PRINT.");
}
...
bool isJointMatrixStructType(Type *Ty) {
  auto *STy = dyn_cast<StructType>(Ty);
  if (!STy || STy->getNumElements() != 1 || !STy->hasName() ||
      !STy->getName().contains("joint_matrix"))
    return false;
  auto *VTy = dyn_cast<FixedVectorType>(STy->getElementType(0));
  return VTy && isValidMatrixType(VTy);
}
...
void patchNotInlinedTIDUserFunc(
    Module &M, IndirectCallInfo *ICI, IRBuilderBase &Builder,
    const FuncSet &KernelAndSyncFuncs, const InstVec &TIDCallsToFix,
  ...
  for (CallInst *CI : CIsToPatch) {
    ...
    if (PatchedF) {
      std::ignore = AddMoreArgsToCall(CI, NewArgs, PatchedF);
    } else {
      std::ignore = addMoreArgsToIndirectCall(CI, NewArgs);
    }
    CI->eraseFromParent();
  }

  // Replace with the ones patched and then erase.
  for (Function *F : FuncsToPatch) {
    SmallVector<User *, 16> Users{F->users()};
    replaceFunctionUses(F, OldF2PatchedF[F], Users);
    F->eraseFromParent();
  }
}
...
bool isBlockInvocationKernel(Function *F) {
  ...
  return Name.contains("_block_invoke_") && Name.ends_with("_kernel");
}

SmallVector<DbgVariableIntrinsic *, 4>
findDbgUses(Value *V, SmallVectorImpl<DbgVariableRecord *> *DbgVar) {
  SmallVector<DbgVariableIntrinsic *, 4> DbgInsts;
  findDbgUsers(DbgInsts, V, DbgVar);
  if (!DbgInsts.empty())
    return DbgInsts;
  ...
  for (auto *U : V->users()) {
    if (auto *ASC = dyn_cast<AddrSpaceCastInst>(U)) {
      findDbgUsers(DbgInsts, ASC, DbgVar);
      if (!DbgInsts.empty())
        break;
    }
  }
...
CallInst *createWIFunctionCall(Module *M, IRBuilderBase &Builder,
  ...
  if (!F) {
    ...
    Type *RetTy = LoopUtils::getIndTy(M);
  ...
  NewInst->setDoesNotAccessMemory();
  return NewInst;
}
...
Value *allocaArrayForLocalPrivateBuffer(IRBuilder<> &Builder,
                                        ...
                                        const DataLayout &DL, Value *BufferSize,
                                        unsigned Alignment,
  ...
  LLVMContext &C = HeapMemPtr->getContext();
...
Value *updateValueWithAlignment(Value *OriginalAddr, size_t Alignment,
                                IRBuilder<> &Builder) {
  ...
  return Builder.CreateAnd(
      AddInst, ConstantInt::get(OriginalAddr->getType(), ~(Alignment - 1)));
}

std::optional<StringRef> getUserAnnotationStr(Instruction *I) {
  // https://github.com/KhronosGroup/SPIRV-Headers/blob/8b246ff75c6615ba4532fe4fde20f1be090c3764/include/spirv/unified1/spirv.hpp#L585
  const unsigned SpirvDecorationUserSemantic = 5635;
  if (auto *DecoMD = I->getMetadata("spirv.Decorations")) {
    for (const auto &MDOp : DecoMD->operands()) {
      if (auto *Node = dyn_cast<MDNode>(MDOp); Node->getNumOperands() == 2) {
        unsigned DecorationType =
            mdconst::extract<ConstantInt>(Node->getOperand(0))->getZExtValue();
        if (DecorationType != SpirvDecorationUserSemantic)
          continue;
        if (auto *MDStr = dyn_cast<MDString>(Node->getOperand(1)))
...
static Value *
findMemoryDefOfValue(Value *V,
                     function_ref<MemorySSA &(Function &F)> GetMemorySSA) {
  // TODO: currently we only handle inst definition and argument. We need to
  // handle constant and other definition.
  if (!isa<Instruction>(V) && !isa<Argument>(V))
    return nullptr;

  if (auto *A = dyn_cast<Argument>(V)) {
    LLVM_DEBUG(dbgs().indent(2) << "is a formal argument of "
                                << A->getParent()->getName() << '\n');
    return A;
  }
  ...
  auto &MSSA = GetMemorySSA(*F);
  MSSA.ensureOptimizedUses();
  auto *MA = MSSA.getMemoryAccess(I);
  if (!MA)
    return I;
  Value *MemDefiningInst = nullptr;
  if (isa<MemoryPhi>(MA))
    return nullptr;
  if (auto *MDef = dyn_cast<MemoryDef>(MA)) {
    LLVM_DEBUG(dbgs().indent(2) << "is a memory defining inst: "
                                << *MDef->getMemoryInst() << '\n');
    MemDefiningInst = MDef->getMemoryInst();
  } else {
    auto *DefiningMA = cast<MemoryUse>(MA)->getDefiningAccess();
    if (isa<MemoryPhi>(DefiningMA))
      return nullptr;
    MemDefiningInst = cast<MemoryDef>(DefiningMA)->getMemoryInst();
    LLVM_DEBUG(dbgs().indent(2)
               << "uses memory defined by inst: " << *MemDefiningInst << '\n');
  }

  // If the memory is clobbered by a store, recursively find the memory def of
  // the store value.
  if (auto *Store = dyn_cast<StoreInst>(MemDefiningInst))
    MemDefiningInst =
        findMemoryDefOfValue(Store->getValueOperand(), GetMemorySSA);

  return MemDefiningInst;
}

void findMemoryDefsOverCallGraph(
    Value *V, SmallPtrSetImpl<Instruction *> &MemDefInsts,
    function_ref<MemorySSA &(Function &F)> GetMemorySSA) {
  LLVM_DEBUG(dbgs() << "Finding memory defs for " << *V
                    << " over call graph\n");

  SmallMapVector<Function *, /*ArgIdx*/ unsigned, 8> WorkList;
  SmallPtrSet<Function *, 4> Visited;

  auto *MemDefiningV = findMemoryDefOfValue(V, GetMemorySSA);
  if (!MemDefiningV)
    return;
  if (auto *I = dyn_cast<Instruction>(MemDefiningV)) {
    // Already found the memory defining inst within the current function.
    MemDefInsts.insert(I);
    return;
  }

  auto *A = cast<Argument>(MemDefiningV);
  WorkList.insert({A->getParent(), A->getArgNo()});
  Visited.insert(A->getParent());

  // For each function formal argument in the worklist, visit all the call sites
  // and analyze the actual argument as well.
  while (!WorkList.empty()) {
    auto &[F, ArgIdx] = WorkList.back();
    WorkList.pop_back();
    for (auto *U : F->users()) {
      if (auto *CI = dyn_cast<CallInst>(U)) {
        auto *ActualArg = CI->getArgOperand(ArgIdx);
        auto *MemDefiningV = findMemoryDefOfValue(ActualArg, GetMemorySSA);
        if (!MemDefiningV)
          continue;
        if (isa<Instruction>(MemDefiningV)) {
          MemDefInsts.insert(cast<Instruction>(MemDefiningV));
          continue;
        }
        ...
        // The actual arg is a formal arg of the caller function, insert caller
        // func into work list.
        auto *FormalArg = cast<Argument>(MemDefiningV);
        WorkList.insert({Caller, FormalArg->getArgNo()});
