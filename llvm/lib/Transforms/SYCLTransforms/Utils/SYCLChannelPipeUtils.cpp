...
std::string tryParseSYCLPipeReadableName(StringRef SYCLPipeStorageGVName) {
  std::string GVName = SYCLPipeStorageGVName.str();
  std::string Parsed = std::string("");
  ItaniumPartialDemangler Demangler;
  if (!Demangler.partialDemangle(GVName.c_str())) {
    size_t BufferSize = 400;
    char *Buf = static_cast<char *>(std::malloc(BufferSize));
    // Demangler may reallocate Buf if BufferSize is not enough.
    Buf = Demangler.finishDemangle(Buf, &BufferSize);
    StringRef Demangled(Buf);
    const std::string PipeClassPattern(
        "sycl::_V1::ext::intel::experimental::pipe<");
    size_t Idx = Demangled.find(PipeClassPattern);
    // First template argument is user-defined ClassID name.
    if (Idx != StringRef::npos) {
      size_t StartIdx = Idx + PipeClassPattern.length();
      size_t EndIdx = Demangled.find_first_of(',', StartIdx);
      assert(EndIdx > StartIdx);
      Parsed = Demangled.substr(StartIdx, EndIdx - StartIdx).str();
    }
    std::free(Buf);
  }
  return Parsed;
...
void initializeGlobalPipeScalar(GlobalVariable *PipeGV, const ChannelPipeMD &MD,
  ...
  if (const char *depth_emulation_mode =
          std::getenv("CL_CONFIG_CHANNEL_DEPTH_EMULATION_MODE"))
    if (StringRef(depth_emulation_mode) == "ignore-depth" ||
        (StringRef(depth_emulation_mode) == "default" && depth == 0))
      ignoreDepth = true;

  // Hostpipe will be specially handled if it's protocal is not AVALON_MM.
  // AVALON_MM is 2 and defined in fpga pipes.hpp file.
  if (PipeGV->getName().starts_with(
          "_ZN4sycl3_V13ext5intel12experimental4pipe") &&
      ignoreDepth && MD.Protocol != 2) // Hostpipe
    depth = -1;
  ...
  for (Function *F : getKernels(M)) {
    ...
    if (KIMD.ArgTargetExtTypeList.hasValue()) {
      for (const auto &[Idx, TyName] :
        ...
        StringRef Name(TyName);
        if (Name.consume_front("spirv.Pipe")) {
          [[maybe_unused]] bool Res = Name.consume_front("_");
          assert(Res && "expect int param");
          Type *Ty = F->getArg(Idx)->getType();
          unsigned AccessQualifer;
          Res = llvm::to_integer(Name, AccessQualifer);
          assert(Res && "param should be integer");
          switch (AccessQualifer) {
          case 0:
            OpaquePipeROTy = Ty;
            break;
          case 1:
