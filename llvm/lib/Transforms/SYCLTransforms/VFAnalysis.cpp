...
bool VFAnalysisInfo::hasUnsupportedPatterns(Function *Kernel) {
  ...
  if (hasFunctionCallInCGNodeIf(Node, [](const Function *CalledFunc) {
    ...
    Kernel->getContext().diagnose(
        OptimizationWarningDiagInfo("kernel \"" + Kernel->getName() +
                                    "\": Kernel can't be vectorized due to "
                                    "unsupported struct type return in "
                                    "callee"));
    return true;
  }
  ...
  if (!KMD.VecLenHint.hasValue() && KMD.VecTypeHint.hasValue()) {
    ...
    if (!Vectorizable) {
      ...
      Kernel->getContext().diagnose(OptimizationWarningDiagInfo(
...
bool VFAnalysisInfo::isSubgroupBroken(Function *Kernel) {
  ...
  if (getVF(Kernel) == 1) {
    ...
    if (KMD.ReqdIntelSGSize.hasValue() && KMD.ReqdIntelSGSize.get() == 1) {
      Kernel->getContext().diagnose(OptimizationErrorDiagInfo(
          "kernel \"" + Kernel->getName() +
          "\": Required subgroup size can't be 1 for subgroup calls"));
    }
  }
...
void VFAnalysisInfo::analyzeModule(
  ...
  for (auto *Kernel : Kernels) {
    ...
    if (hasConflictVFConstraints(Kernel)) {
      M.getContext().diagnose(OptimizationErrorDiagInfo(
          "kernel \"" + Kernel->getName() +
    ...
    if (tryFallbackUnimplementedBuiltins(Kernel, HeuristicVF)) {
      ...
      M.getContext().diagnose(OptimizationErrorDiagInfo(
          "kernel \"" + Kernel->getName() +
          "\": Unimplemented function(s): " + join(R, ", ")));
    }
    ...
    if (!(VF && (VF & (VF - 1)) == 0)) {
      ...
      M.getContext().diagnose(OptimizationErrorDiagInfo(
          "kernel \"" + Kernel->getName() + "\": Vectorization width " +
          Twine(VF) + " is not a power of 2"));
    }
  }
}
