...
/// Save function parameter's target extension type and integer params as string
/// to metadata.
static void generateArgTargetExtTypeListMD(SmallVectorImpl<Function *> &Funcs,
                                           ValueToValueMapTy &VMap) {
  for (Function *F : Funcs) {
    SmallVector<std::string, 8> ArgTypesMD;
    llvm::transform(F->args(), std::back_inserter(ArgTypesMD),
                    [&](Argument &A) {
                      if (auto *TETy = dyn_cast<TargetExtType>(A.getType())) {
                        // Mangle TargetExtType into a name.
                        std::string TyName = TETy->getName().str();
                        for (unsigned IntParam : TETy->int_params())
                          TyName += "_" + std::to_string(IntParam);
                        return TyName;
                      }
                      return std::string("");
                    });
    ...
    KIMD.ArgTargetExtTypeList.set(std::move(ArgTypesMD));
  }
}
...
static void findInstUsers(Value *Val, SmallPtrSetImpl<Instruction *> &Users,
  ...
  if (isa<AllocaInst>(Val)) {
    auto DbgUses = CompilationUtils::findDbgUses(Val);
...
static bool materializeTargetExtType(Module &M,
  ...
  if (TETypeMap.empty()) {
    generateArgTargetExtTypeListMD(Kernels, VMap);
  ...
  for (Function &F : M) {
    ...
    if (F.getName().starts_with("__") && F.getName().ends_with("_block_invoke"))
      for (auto &Arg : F.args())
  ...
  generateArgTargetExtTypeListMD(FuncsToAddMD, VMap);
