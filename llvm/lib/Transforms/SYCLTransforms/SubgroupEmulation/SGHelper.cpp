...
Value *SGHelper::createGetMaxSubGroupSize(Instruction *IP) {
  ...
  return CallInst::Create(GetMaxSGSizeF, "sg.max.size.", IP->getIterator());
}
...
Value *SGHelper::createGetSubGroupSize(Instruction *IP) {
  ...
  CallInst *CI = CallInst::Create(GetSGSizeF, "sg.size.", IP->getIterator());
  CI->setDebugLoc(IP->getDebugLoc());
  return CI;
}
...
Value *SGHelper::createGetSubGroupLId(Instruction *IP) {
  ...
  CallInst *CI = CallInst::Create(GetSGLIdF, "sg.lid.", IP->getIterator());
  CI->setDebugLoc(IP->getDebugLoc());
  return CI;
}
