...
void SGLoopConstructPass::updateTIDCalls(Module &M) {
  ...
  for (auto &TIDFuncName : TIDFuncNames) {
    ...
    for (auto *U : Fn->users()) {
      ...
      if (!SizeInfo->hasEmuSize(PF))
        continue;
      // Variable TID Calls have been resolved in ResolveVarTIDCall pass.
  ...
  if (auto *GV = M.getNamedGlobal(getTLSLocalIdsName())) {
    for (auto *U : GV->users())
      if (auto *LD = dyn_cast<LoadInst>(U))
        TIDValues.push_back(LD);
  }

  for (auto *Id : TIDValues) {
