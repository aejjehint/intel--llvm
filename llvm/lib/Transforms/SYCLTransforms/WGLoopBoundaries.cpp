...
private:
  ...
  /// Map from bound generation instruction to real bound value.
  std::unordered_map<Value *, ConstantInt *> ConstantBoundMap;
  ...
  void collectTIDCalls(SmallVectorImpl<Function *> &Kernels);
  ...
  ///          early exit.
  bool createRightBound(Instruction *Loc, Value **Bound, Value *LeftBound,
...
bool WGLoopBoundariesImpl::run() {
  ...
  for (auto *Kernel : Kernels) {
    ...
    if (KIMD.NoBarrierPath.get()) {
      ...
      KernelWorkList.push_back(Kernel);
    }
  }

  collectTIDCalls(KernelWorkList);

  for (auto *Kernel : KernelWorkList)
...
void WGLoopBoundariesImpl::collectTIDData() {
  ...
  auto ProcessTIDCalls = [this](auto &TIDCalls, bool IsGID) {
    for (unsigned Dim = 0; Dim < MAX_WORK_DIM; ++Dim) {
      for (auto *I : TIDCalls[Dim]) {
        TIDs[I] = {Dim, IsGID};
        TIDByDim[Dim].push_back(I);
      }
    }
  };
  // Go over all get_global_id
  ProcessTIDCalls(FuncToGIDCalls[F], true);
  // Go over all get_local_id
  ProcessTIDCalls(FuncToLIDCalls[F], false);
}

void WGLoopBoundariesImpl::collectTIDCalls(
    SmallVectorImpl<Function *> &Kernels) {
  FuncSet Funcs{Kernels.begin(), Kernels.end()};
  FuncToGIDCalls = std::move(getTIDCallsInFuncs(M, mangledGetGID(), Funcs));
  FuncToLIDCalls = std::move(getTIDCallsInFuncs(M, mangledGetLID(), Funcs));
}
...
bool WGLoopBoundariesImpl::findAndHandleTIDMinMaxBound() {
  ...
  for (unsigned Dim = 0; Dim < NumDim; ++Dim) {
    ...
    if (TIDByDim[Dim].size() != 1)
      continue;
    Instruction *I = TIDByDim[Dim][0];

    // Allow truncation for 64 bit systems.
    Instruction *TidInst = I;
    if (I->hasOneUse())
      if (auto *TI = dyn_cast<TruncInst>(*(I->user_begin())))
        TidInst = TI;
...
bool WGLoopBoundariesImpl::createRightBound(Instruction *Loc, Value **Bound,
                                            Value *LeftBound, Type *OriginalTy,
                                            Type *NewTy,
                                            Instruction::BinaryOps Inst) {
  ...
  if (ReverseLowerUpperBound && Inst == Instruction::Add)
    Bound[0] = BinaryOperator::CreateNeg(Bound[0], "bound.0.reverse",
                                         Loc->getIterator());
  ...
  if (OriginalTy) {
    auto *CastedLeftBound = new TruncInst(
        LeftBound, OriginalTy, "casted_left_bound", Loc->getIterator());
    LeftBound = new SExtInst(CastedLeftBound, NewTy, "left_sext_bound",
                             Loc->getIterator());
  }
  ...
  if (Inst == Instruction::Sub) {
    CreateEmptyRange = new ICmpInst(Loc->getIterator(), CondPred, Bound[0],
                                    LeftBound, "right_lt_left");
  } else {
    auto *NegLeft = BinaryOperator::CreateNeg(LeftBound, "left_boundary",
                                              Loc->getIterator());
    CreateEmptyRange = new ICmpInst(Loc->getIterator(), CondPred, Bound[0],
                                    NegLeft, "right_lt_left");
  }

  // Detect right bound overflow.
  Value *NonNegativeRightBound = BinaryOperator::CreateNot(
      CreateEmptyRange, "non_negative_right_bound", Loc->getIterator());
  Bound[0] = BinaryOperator::Create(Inst, Bound[0], LeftBound,
                                    "right_boundary_align", Loc->getIterator());
  ...
  Bound[0] = SelectInst::Create(CreateEmptyRange, MinusOne, Bound[0],
                                "right_bound", Loc->getIterator());

  // Detect right bound overflow.
  // Left and right parts are positive but the total is negative.
  Value *Zero = ConstantInt::get(Bound[0]->getType(), 0);
  Value *RightBoundNeg = new ICmpInst(Loc->getIterator(), CmpInst::ICMP_SLT,
                                      Bound[0], Zero, "negative_right");
  Value *RightOverflow = BinaryOperator::Create(
      Instruction::And, RightBoundNeg, NonNegativeRightBound, "right_overflow",
      Loc->getIterator());
  Bound[0] = SelectInst::Create(RightOverflow, Max, Bound[0],
                                "final_right_bound", Loc->getIterator());
...
bool WGLoopBoundariesImpl::traceBackBound(Value *V1, Value *V2,
  ...
  while (auto *TidInst = dyn_cast<Instruction>(Tid)) {
    ...
    case Instruction::Trunc: {
      ...
      Value *Org = Bound[0];
      Bound[0] =
          CastInst::CreateIntegerCast(Bound[0], Tid->getType(), IsCmpSigned,
                                      "to_tid_type", Loc->getIterator());
      if (ConstantBoundMap.count(Org)) {
        ConstantBoundMap[Bound[0]] = ConstantBoundMap[Org];
        ConstantBoundMap.erase(Org);
      }

      if (Bound[1]) {
        Org = Bound[1];
        Bound[1] =
            CastInst::CreateIntegerCast(Bound[1], Tid->getType(), IsCmpSigned,
                                        "to_tid_type1", Loc->getIterator());
        if (ConstantBoundMap.count(Org)) {
          ConstantBoundMap[Bound[1]] = ConstantBoundMap[Org];
          ConstantBoundMap.erase(Org);
        }
      }
      break;
    ...
    case Instruction::Add: {
      ...
      // computations:
      // 1. a and b are not all constants.
      // 2. a and b are constants, but there are two intervals. Filter it out on
      //    obtainBoundaryEE function.
      ...
      if (IsCmpSigned && !createRightBound(Loc, Bound, LeftBound, OriginalTy,
                                           Tid->getType(), Instruction::Sub))
        return false;
      if (!IsCmpSigned) {
        auto *UniformValue = IsV1Uniform ? V1 : V2;
        // a and b are not all constants, return early.
        if (!isa<ConstantInt>(UniformValue) || !isa<ConstantInt>(LeftBound)) {
          Bound[0] = Bound[1] = nullptr;
          return false;
        }
        auto A = cast<ConstantInt>(LeftBound)->getZExtValue();
        auto B = cast<ConstantInt>(UniformValue)->getZExtValue();
        auto *IntType = cast<ConstantInt>(UniformValue)->getIntegerType();
        // Right bound: id < b - a
        Bound[0] =
            BinaryOperator::Create(Instruction::Sub, UniformValue, LeftBound,
                                   "final_right_bound", Loc->getIterator());
        // Left bound: -a <= id
        Bound[1] = BinaryOperator::CreateNeg(
            LeftBound, "final_left_bound",
            cast<BinaryOperator>(Bound[0])->getIterator());
        ConstantBoundMap.insert({Bound[0], ConstantInt::get(IntType, B - A)});
        ConstantBoundMap.insert({Bound[1], ConstantInt::get(IntType, -A)});
      }
      break;
    }
    case Instruction::Sub: {
      ...
      //   id < (b + a)
      //
      // Unsigned "id - a < b" pattern is simliar to unsigned "id + a < b".
      Tid = IsFirstOperandUniform ? TidInst->getOperand(1)
                                  : TidInst->getOperand(0);
      Value *LeftBound = IsFirstOperandUniform ? TidInst->getOperand(0)
                                               : TidInst->getOperand(1);
      // If the pattern is (a - id) < b, we need to reverse lower/upper bound.
      ...
      if (!IsCmpSigned) {
        ...
        if (ReverseLowerUpperBound) {
          ...
          Bound[0] = LeftBound;
          Bound[1] =
              BinaryOperator::Create(Instruction::Sub, LeftBound, UniformValue,
                                     "final_left_bound", Loc->getIterator());
          ConstantBoundMap.insert({Bound[0], ConstantInt::get(IntType, A)});
          ConstantBoundMap.insert({Bound[1], ConstantInt::get(IntType, A - B)});
        } else {
          // id - a < b  ---> [a, a + b)
          Bound[1] = LeftBound;
          Bound[0] =
              BinaryOperator::Create(Instruction::Add, LeftBound, UniformValue,
                                     "final_right_bound", Loc->getIterator());
          ConstantBoundMap.insert({Bound[0], ConstantInt::get(IntType, A + B)});
          ConstantBoundMap.insert({Bound[1], ConstantInt::get(IntType, A)});
        }
      }
      break;
...
void WGLoopBoundariesImpl::replaceTidWithBound(bool IsGID, unsigned Dim,
  ...
  auto &TidCalls = IsGID ? FuncToGIDCalls[F] : FuncToLIDCalls[F];
  for (Instruction *TidCall : TidCalls[Dim]) {
...
bool WGLoopBoundariesImpl::obtainBoundaryEE(ICmpInst *Cmp, Value **Bound,
  ...
  // Don't handle inverse bound of unsigned "a - id < b" pattern as it
  // complicates following unsigned "id +/- a < b".
  if (!IsSigned && Bound[1] && ReverseLowerUpperBound) {
    LLVM_DEBUG(dbgs() << "Don't generate loop boundary for unsigned \"a - id < "
                         "b\" pattern\n");
    return false;
  }
  if (Bound[1] && ConstantBoundMap.count(Bound[1]) &&
      ConstantBoundMap.count(Bound[0]) && !IsSigned) {
    auto Low = ConstantBoundMap[Bound[1]]->getZExtValue();
    auto High = ConstantBoundMap[Bound[0]]->getZExtValue();
    if (IsUpper) {
      // If Low is less than High, there is only one interval, we don't need to
      // care if the interval has memory access issue, since function
      // obtainEEBoundaries adusts it to safe value:
      //   %upper.bound0.i = llvm.umin.i64(%init.upper.bound0.i, High)
      //   %lower.bound0.i = llvm.umax.i64(%base.gid0, Low)
      // Other cases may have two intervals, we skip them.
      if (Low > High)
        return false;
      EEVec.push_back({Bound[0], Dim, IsUpper, ContainsVal, IsSigned, IsGID});
      EEVec.push_back({Bound[1], Dim, !IsUpper, true, IsSigned, IsGID});
    } else {
      // Reverse Low and High and filter out two-interval cases.
      if (High > Low)
        return false;
      EEVec.push_back({Bound[1], Dim, true, false, IsSigned, IsGID});
      EEVec.push_back({Bound[0], Dim, false, ContainsVal, IsSigned, IsGID});
    }
    return true;
  }
