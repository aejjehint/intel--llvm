...
/// When cloning a root function, we need to clone local variables used in the
...
cloneFunctions(Module &M, CallGraph &CG,
  ...
  for (Function *F : FuncsToClone) {
    // When a function that is called inside F was cloned in a previous
    // iteration, CloneFunctionInto may map F's Subprogram to itself.
    // Erase F's Subprogram from VMap so that it won't be reused for NewF.
    // A new Subprogram should be created for NewF.
    if (auto &MDMap = VMap.getMDMap())
      if (auto It = MDMap->find(F->getSubprogram()); It != MDMap->end())
        MDMap->erase(It);

    SmallVector<ReturnInst *, 8> Returns;
    Function *NewF = cast<Function>(VMap[F]);
    CloneFunctionInto(NewF, F, VMap, CloneFunctionChangeType::GlobalChanges,
                      Returns);
