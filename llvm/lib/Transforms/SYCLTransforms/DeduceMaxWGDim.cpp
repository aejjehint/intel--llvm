...
static void runOnFunction(Function &F, CallGraph &CG, IndirectCallInfo &ICI,
  ...
  while (!WorkList.empty()) {
    ...
    for (auto It = df_begin(N), E = df_end(N); It != E; ++It) {
      for (const auto &Pair : **It) {
        ...
        if (!Callee) {
          // It is fine to over-estimate MaxWGDimensions when ICI analysis is
          // not accurate. But we can't under-estimate.
          for (auto *IndirectF : ICI.getIndirectlyCalledFuncs())
            if (IndirectF->getFunctionType() ==
                    cast<CallInst>(*Pair.first)->getFunctionType() &&
                Visited.insert(IndirectF).second)
              WorkList.push_back(IndirectF);
          continue;
        }
