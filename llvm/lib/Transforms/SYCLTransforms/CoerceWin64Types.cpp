...
static Type *getPointerTypeInStructType(Type *Ty) {
  ...
  if (isa<PointerType>(EltTy))
    return EltTy;
  return getPointerTypeInStructType(EltTy);
}

static Type *getBitCastType(Type *Ty, uint64_t Bytes, LLVMContext &C) {
  Type *NewType;
  switch (Bytes) {
  case 1:
  ...
  case 4:
    ...
    break;
  case 8: {
    auto *PTy = getPointerTypeInStructType(Ty);
    NewType = PTy ? PTy : Type::getInt64Ty(C);
    break;
  }
  default:
  ...
  for (unsigned I = 0; I < CI->arg_size(); ++I) {
    ...
    if (CI->paramHasAttr(I, Attribute::ByVal)) {
      ...
      if (shouldPassByval(MemSize)) {
        ...
        Value *Alloca = CoerceTypesImpl::createAllocaInst(
            ElementTy, CI->getFunction(), Alignment, PT->getAddressSpace());
        Value *DstPtr =
      ...
      // Drop all the attributes of 'byval', including alignment and byval attr.
      AL = AL.removeParamAttributes(C, I);
    } else {
      Args.push_back(ArgI);
    }
  }
  CallInst *NewCI = Builder.CreateCall(NewFun, Args);
  CoerceTypesImpl::copyAttributeMetadata(CI, NewCI, AL);
...
moveFunctionBody(Function *OldF, Function *NewF,
  ...
  for (Argument &OldArg : OldF->args()) {
    ...
    if (shouldPassByval(MemSize)) {
      ...
      Value *Alloca = CoerceTypesImpl::createAllocaInst(
          OldArgByvalType, NewF, Alignment, OldArgPT->getAddressSpace());
...
bool CoerceTypesWin64Impl::runOnFunction(Function *F) {
  bool Change = false;
  ...
  for (Argument &Arg : F->args()) {
    if (Arg.hasByValAttr()) {
      ...
      uint64_t MemSize = 0;
      if (auto OldStructT = dyn_cast<StructType>(ArgMemTy)) {
      ...
      if (shouldPassByval(MemSize))
        NewArgTypes.push_back(getBitCastType(ArgMemTy, MemSize, C));
      else
