...
static bool replaceWithBuiltInCall(BinaryOperator *DivInst,
  ...
  CallInst *NewCall =
      CallInst::Create(DivFuncInModule, Args, FuncName, DivInst->getIterator());
...
PreservedAnalyses OptimizeIDivAndIRemPass::run(Function &F,
  ...
  if (NumReplacedInsts) {
    ...
    ORE.emit([&]() {
      return OptimizationRemark(DEBUG_TYPE, "OptimizeIDivAndIRem", &F)
             << Twine(NumReplacedInsts).str()
             << " udiv/urem/sdiv/srem instructions are replaced with builtins "
                "in "
                "function "
             << F.getName();
    });
  ...
  return NumReplacedInsts ? PreservedAnalyses::none()
