...
void LocalBufferInfoImpl::computeLocalsSizeOffset() {
  ...
  // If the device code is built with device santizer, the local variable
  // '__AsanLaunchInfo' may be used by more than one kernel. So we need to
  // handle it in a special way and fix its offset to 0. And this scenario may
  // not be legal, the solution here is just a WA.
  const bool DeviceSanitizerEnabled = CompilationUtils::isModuleUsingAsan(M);
  const size_t LocalSizeInit = DeviceSanitizerEnabled ? sizeof(void *) : 0;

  for (Function *F : CompilationUtils::getKernels(M)) {
    // Calculate total local buffer size.
    size_t LocalSize = LocalSizeInit;
    if (auto MapIt = LocalUseMap.find(F); MapIt != LocalUseMap.end()) {
      for (auto *GV : MapIt->second) {
        if (DeviceSanitizerEnabled &&
            GV->getName().starts_with("__AsanLaunchInfo")) {
          LocalGVToOffsetMap[GV] = 0;
          continue;
        }
