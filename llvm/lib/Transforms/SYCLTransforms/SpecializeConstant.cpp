...
static void collectExternalSpecializations(LLVMContext &C,
  ...
  for (StringRef SpecConstOpt : SYCLSpecConstantOpt) {
    ...
    if (Params.size() != 3) {
      C.diagnose(OptimizationWarningDiagInfo(
          "Option --sycl-spec-constant=" + SpecConstOpt +
          " is ignored because the format is invalid. The expected format is "
          "id:type:value"));
      continue;
    }
    ...
    if (Params[0].getAsInteger(10, SpecId)) {
      C.diagnose(OptimizationWarningDiagInfo(
          "Option --sycl-spec-constant=" + SpecConstOpt +
          " is ignored because the id must be an unsigned int"));
      continue;
    }
    ...
    if (!Ty) {
      C.diagnose(OptimizationWarningDiagInfo(
          "Option --sycl-spec-constant=" + SpecConstOpt +
          " is ignored because the type is invalid. The expected type is one "
          "of i1, "
    ...
    if (It != ExtSpecConstants.end())
      C.diagnose(OptimizationWarningDiagInfo(
          "Option --sycl-spec-constant=" + std::get<2>(It->getSecond()) +
          " is ignored because it is overrided by the later option with same "
          "id (" +
          SpecConstOpt + ")"));
...
static void resolveSpecConstantCalls(Function &SpecConstantFunc,
  ...
  for (auto *U : make_early_inc_range(SpecConstantFunc.users())) {
    if (auto *CI = dyn_cast<CallInst>(U)) {
      ...
      if (It != ExtSpecConstants.end()) {
        ...
        if (Ty != CI->getType())
          SpecConstantFunc.getContext().diagnose(OptimizationWarningDiagInfo(
              "Option --sycl-spec-constant=" + OptValue +
              " is ignored because the specified type doesn't match "
  ...
  for (auto &Entry : ExtSpecConstants)
    if (!UsedExtIds.count(Entry.getFirst()))
      SpecConstantFunc.getContext().diagnose(OptimizationWarningDiagInfo(
          "Option --sycl-spec-constant=" + std::get<2>(Entry.getSecond()) +
          " is ignored because there's no matched spec constant in the module "
          "of the given id"));
