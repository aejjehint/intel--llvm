...
// New spec changes actually broke the binary backward compatibility, SPIRV team
// hopes device runtime side to support both two version IDs (legacy and new)
// firstly to make the switch process smoothly.
constexpr uint32_t SPIRV_HOST_ACCESS_DECOR_LEGACY = 6147;
        ...
        if (Node && Node->getNumOperands() == 3) {
          uint32_t Capability =
              mdconst::extract<ConstantInt>(Node->getOperand(0))
                  ->getZExtValue();
          if (Capability == SPIRV_HOST_ACCESS_DECOR ||
              Capability == SPIRV_HOST_ACCESS_DECOR_LEGACY) {
            ...
            // Update legacy host access mode to new one.
            if (Capability == SPIRV_HOST_ACCESS_DECOR_LEGACY) {
              uint32_t HostAccessMode =
                  mdconst::extract<ConstantInt>(Node->getOperand(1))
                      ->getZExtValue() +
                  1;
              if (HostAccessMode > 3)
                HostAccessMode = 0;
              auto NewMD = ConstantAsMetadata::get(ConstantInt::get(
                  Type::getInt32Ty(Node->getContext()), HostAccessMode));
              Node->replaceOperandWith(1, NewMD);
            }
            Changed = true;
            break;
          }
...
bool ExternalizeGlobalVariablesPass::runImpl(Module &M) {
  ...
  for (auto &GVar : M.globals()) {
    bool SkipConvertLinkage =
        ...
        (GVar.hasName() && GVar.getName().starts_with("llvm.")) ||
