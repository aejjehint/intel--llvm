...
; RUN: SATest -BUILD --config=%S/math-relaxed-masked-f32.tst.cfg -tsize=32 -cpuarch=skx -llvm-option=-print-after=sycl-kernel-relaxed-math -dump-llvm-file %t.ll 2>&1 | FileCheck %s -check-prefixes=CHECK,CHECK-RM
; RUN: FileCheck %s --input-file=%t.ll -check-prefix=CHECK-SVML

; RUN: SATest -BUILD --config=%S/math-relaxed-masked-f32.tst.cfg -tsize=32 -cpuarch=core-avx2 -llvm-option=-print-after=sycl-kernel-relaxed-math 2>&1 | FileCheck %s -check-prefixes=CHECK,CHECK-RM
...
; CHECK-RM: call{{.*}} <32 x float> @_Z6pow_rmDv32_fS_(<32 x float> {{.*}}, <32 x float> {{.*}})
...
; CHECK-RM: call{{.*}} <32 x float> @_Z6cos_rmDv32_fS_(<32 x float> {{.*}}, <32 x float> {{.*}})
; CHECK-RM: call{{.*}} <32 x float> @_Z6exp_rmDv32_fS_(<32 x float> {{.*}}, <32 x float> {{.*}})
; CHECK-RM: call{{.*}} <32 x float> @_Z7exp2_rmDv32_fS_(<32 x float> {{.*}}, <32 x float> {{.*}})
; CHECK-RM: call{{.*}} <32 x float> @_Z8exp10_rmDv32_fS_(<32 x float> {{.*}}, <32 x float> {{.*}})
; CHECK-RM: call{{.*}} <32 x float> @_Z6log_rmDv32_fS_(<32 x float> {{.*}}, <32 x float> {{.*}})
; CHECK-RM: call{{.*}} <32 x float> @_Z7log2_rmDv32_fS_(<32 x float> {{.*}}, <32 x float> {{.*}})
; CHECK-RM: call{{.*}} <32 x float> @_Z6pow_rmDv32_fS_S_(<32 x float> {{.*}}, <32 x float> {{.*}}, <32 x float> {{.*}})
; CHECK-RM: call{{.*}} <32 x float> @_Z6sin_rmDv32_fS_(<32 x float> {{.*}}, <32 x float> {{.*}})
; CHECK-RM: call{{.*}} <32 x float> @_Z6tan_rmDv32_fS_(<32 x float> {{.*}}, <32 x float> {{.*}})
; CHECK-RM: call{{.*}} <32 x float> @_Z9sincos_rmDv32_fPfS_(<32 x float> {{.*}}, {{.*}}, <32 x float> {{.*}})
