...
; RUN: SATest -BUILD --config=%s.cfg -tsize=32 -cpuarch="skx" -llvm-option=-print-after=vplan-vec,sycl-kernel-builtin-import 2>&1 | FileCheck %s -check-prefix=CHECK32
; RUN: SATest -BUILD --config=%s.cfg -tsize=64 -cpuarch="skx" -llvm-option=-print-after=vplan-vec,sycl-kernel-builtin-import 2>&1 | FileCheck %s -check-prefix=CHECK64
...
; CHECK32-NEXT: lshr <32 x i64> [[BITCAST]], splat (i64 63)
...
; IR Dump After sycl-kernel-builtin-import
; CHECK32: call <32 x i32> @_Z16isfinite_v1widenDv32_d(<32 x double> {{.*}})
; CHECK32: call <32 x i32> @_Z13isinf_v1widenDv32_d(<32 x double> {{.*}})
; CHECK32: call <32 x i32> @_Z16isnormal_v1widenDv32_d(<32 x double> {{.*}})
; CHECK32: call <32 x double> @_Z9bitselectDv32_dS_S_(<32 x double> {{.*}}, <32 x double> {{.*}}, <32 x double> {{.*}})
...
; CHECK64-NEXT: lshr <64 x i64> [[BITCAST]], splat (i64 63)
