...
VectInfo::VectInfo(const Record *record) : m_builtins(4) {
  ...
  // Get OclType.
  std::vector<std::vector<const Record *>> types(numOfVariants);
  std::transform(
  ...
  std::transform(types.begin(), types.end(), oclTypes.begin(),
                 [=](const std::vector<const Record *> &rs) {
...
VectInfoGenerator::VectInfoGenerator(const RecordKeeper &keeper)
...
void VectInfoGenerator::decodeParam(StringRef scalarName,
  ...
  size_t v1ParamsNum = v1Func.getNumOfParams();
  VectEntry::isMasked = (v1ParamsNum + 1) == v4Func.getNumOfParams();
...
void VectInfoGenerator::run(raw_ostream &os) {
  ...
  std::vector<const Record *> vectInfos =
  ...
  for (const Record *record : vectInfos) {
