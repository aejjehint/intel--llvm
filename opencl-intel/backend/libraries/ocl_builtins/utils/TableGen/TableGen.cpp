...
cl::opt<ActionType> Action(
    ...
    cl::values(
        ...
        clEnumValN(GenOCLBuiltinAliases, "gen-ocl-builtin-aliases",
                   "Generates the aliases for builtins")),
...
bool OCLTableGenMain(raw_ostream &OS, const RecordKeeper &Records) {
  switch (Action) {
  case PrintRecords:
    OS << Records;
    return false;
  case PrintEnums: {
    std::vector<const Record *> Recs = Records.getAllDerivedDefinitions(Class);
  ...
  case GenVectorizationInfo:
    ...
    return false;
  case GenOCLBuiltinAliases:
    OclBuiltinAliasesGenerator(Records).run(OS);
    return false;
  }
