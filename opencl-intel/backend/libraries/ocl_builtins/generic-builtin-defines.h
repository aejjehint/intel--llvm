...
#define double_QNAN_MASK                                                       \
  0x7FF8000000000000 // 0b0111_1111_1111_1000_0000_0000_...
...
// Functions copied from the conformance tests
void INTERNAL_INLINE_ATTRIBUTE multiply_unsigned_64_by_64(ulong sourceA,
                                                          ulong sourceB,
                                                          ulong *destLow,
                                                          ulong *destHi) {
  ulong lowA, lowB;
  ...
  // = ( ( aHI << 32 * bHI << 32 ) + ( aHI << 32 * bLO ) + ( aLO * bHI << 32 ) +
  // ( aLO * bLO ) ) >> 64 = ( ( aHI * bHI << 64 ) + ( aHI * bLO << 32 ) + ( aLO
  // * bHI << 32 ) + ( aLO * bLO ) ) >> 64 = ( aHI * bHI ) + ( aHI * bLO >> 32 )
  // + ( aLO * bHI >> 32 ) + ( aLO * bLO >> 64 )

  // Now, since each value is 32 bits, the max size of any multiplication is:
  // ( 2 ^ 32 - 1 ) * ( 2 ^ 32 - 1 ) = 2^64 - 4^32 + 1 = 2^64 - 2^33 + 1, which
  // fits within 64 bits Which means we can do each component within a 64-bit
  // integer as necessary (each component above marked as AB1 - AB4)
  ...
  //
  //          (2**N-1)^2 + 2*(2**N-1) = 2**2N - 2**(N+1) + 1 + 2**(N+1) - 2 =
  //          2**2N - 1
  //
  ...
  //===============================================================================
  //
  // The (b.lo*a.lo).lo term cannot cause a carry, so we can ignore them for
  // now.  We also know from above, that we can add (b.lo*a.lo).hi and
  // (b.hi*a.lo).lo to the 2N bit term [(b.lo*a.hi).hi + (b.lo*a.hi).lo] without
  // overflow.  That takes care of all of the terms on the right half that might
  // carry.  Do that now.
  //
  ...
  // into the [ (b.lo*a.hi).hi + (b.lo*a.hi).lo ] 2N bit term. We can safely
  // separate into high and low parts. Per our rule above, we know we can
  // accumulate the high part of that and (b.hi*a.lo).hi into the 2N bit term
  // (b.lo*a.hi) without carry.  The low part can be pieced together with
  // (b.lo*a.lo).lo, to give the final low result

  (*destHi) = aHibHi + (aHibLo >> 32) + (aLobHi >> 32); // Cant overflow
  (*destLow) = (aHibLo << 32) | (aLobLo & 0xFFFFFFFFUL);
}

void INTERNAL_INLINE_ATTRIBUTE multiply_signed_64_by_64(long sourceA,
                                                        long sourceB,
                                                        ulong *destLow,
                                                        ulong *destHi) {
  // Find sign of result
...
void __opencl_unreachable(__constant char *);
