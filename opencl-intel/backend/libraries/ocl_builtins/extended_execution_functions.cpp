...
int __attribute__((always_inline)) __enqueue_kernel_basic_events(
    queue_t queue, kernel_enqueue_flags_t flags, const ndrange_t ndrange,
...
queue_t __attribute__((const)) __attribute__((always_inline))
__attribute__((overloadable)) get_default_queue(void) {
  void *DCM = __get_device_command_manager();
...
////////// - retain_event, release_event, create_user_event,
/// set_user_event_status, capture_event_profiling_info, is_valid_event
...
bool __attribute__((overloadable)) __attribute__((always_inline))
is_valid_event(clk_event_t event) {
  void *DCM = __get_device_command_manager();
...
__get_kernel_work_group_size_impl(void *block_invoke, void *block_literal) {
  void *DCM = __get_device_command_manager();
  void *B2K = __get_block_to_kernel_mapper();
...
__get_kernel_preferred_work_group_size_multiple_impl(void *block_invoke,
                                                     void *block_literal) {
  void *DCM = __get_device_command_manager();
  void *B2K = __get_block_to_kernel_mapper();
...
const __global clk_event_t *__attribute__((always_inline))
__attribute__((overloadable))
cast_to_global_const(const __generic clk_event_t *ptr) {
  return (const __global clk_event_t *)ptr;
}

__global clk_event_t *__attribute__((always_inline))
__attribute__((overloadable)) cast_to_global(__generic clk_event_t *ptr) {
...
#define ADDR_SPACE_OVERLOADING(ADDR_SPACE_1ST, ADDR_SPACE_2ND)                 \
  int __attribute__((always_inline)) __attribute__((overloadable))             \
  enqueue_marker(queue_t queue, uint num_events_in_wait_list,                  \
                 const ADDR_SPACE_1ST clk_event_t *event_wait_list,            \
                 ADDR_SPACE_2ND clk_event_t *event_ret) {                      \
...
extern void *__ocl_task_sequence_create(size_t result_size, int pipelined,
                                        int fpga_cluster, int response_capacity,
                                        int invocation_capacity);

void *__attribute__((always_inline))
__create_task_sequence(size_t result_size, int pipelined, int fpga_cluster,
                       int response_capacity, int invocation_capacity) {
  return __ocl_task_sequence_create(result_size, pipelined, fpga_cluster,
                                    response_capacity, invocation_capacity);
}

extern void __ocl_task_sequence_async(void *obj, void *block_invoke,
                                      void *block_literal, void *DCM, void *B2K,
                                      void *RuntimeHandle);
void __attribute__((always_inline)) __async(void *obj, void *block_invoke,
                                            void *block_literal) {
  void *DCM = __get_device_command_manager();
  void *B2K = __get_block_to_kernel_mapper();
  void *RuntimeHandle = __get_runtime_handle();
  __ocl_task_sequence_async(obj, block_invoke, block_literal, DCM, B2K,
                            RuntimeHandle);
}
