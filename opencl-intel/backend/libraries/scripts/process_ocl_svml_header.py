...
def process_file(inputfile, outputfile, generate_shared_header):
    lines = get_input_lines(inputfile)
...
def get_input_lines(inputfile):
    '''
    Get all the lines from input file with include directives expanded, assuming the
    included non-system header files are in the same directory as the input file.
    '''
    lines = []
    ...
    with open(inputfile, 'r') as f:
        for input_line in f.readlines():
            ...
            if not line.startswith('#include'):
                lines.append(line)
                continue
            included_file = line[len('#include'):].lstrip()
            # Expand non-system include directives
            if included_file.startswith(r'"'):
                included_file = os.path.join(
                    input_dir, included_file.strip(r'"').strip())
                lines.extend(get_input_lines(included_file))
    return lines
