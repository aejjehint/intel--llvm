...
public:
  ...
  bool handleDiagnostics(const DiagnosticInfo &DI) override {
    if (isa<OptimizationErrorDiagInfo>(&DI) ||
        (Opt->isFpgaEmulator() && isa<FPGAMemoryScopeErrorDiagInfo>(&DI)))
      Opt->setHasErrors();
    // Save diagnostic message to build log.
...
Optimizer::Optimizer(Module &M, SmallVectorImpl<Module *> &RtlModules,
    ...
    : m_M(M), m_RtlModules(RtlModules.begin(), RtlModules.end()),
      ...
      m_IsFpgaEmulator(Config.isFpgaEmulator()), HasErrors(false) {
