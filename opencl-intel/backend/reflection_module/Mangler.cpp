...
const std::string Mangler::prefix_scatter_prefetch =
    "internal.prefetch.scatter";
...
llvm::StringRef imageFunctions[] = {"read_imagei", "read_imageui",
...
template <class T> inline std::string toString(const T &elem) {
  std::stringstream ss;
  ss << elem;
  return ss.str();
}
...
bool is2DImage(reflection::TypePrimitiveEnum image) {
  return (image == reflection::PRIMITIVE_IMAGE_2D_T ||
...
std::string Mangler::getGatherScatterName(bool isMasked,
                                          GatherScatterType gatherType,
                                          FixedVectorType *DataTy,
                                          FixedVectorType *IndexTy) {
...
std::string Mangler::getGatherScatterInternalName(GatherScatterType gatherType,
                                                  Type *maskType,
  ...
  result << prefix;
  result << ".v" << numElements << elemRetTyName << "[" << elemIndexTyName
         << "].m";
  result << (isa<FixedVectorType>(maskType)
                 ? cast<FixedVectorType>(maskType)->getNumElements()
                 : 1);
  return result.str();
}
...
std::string Mangler::getVectorizedPrefetchName(const std::string &name,
                                               int packetWidth) {
...
std::string Mangler::demangle(const std::string &name, bool masked) {
  if (::isMangledName(name.c_str())) {
    llvm::StringRef stripped = stripName(name.c_str());
    if (stripped.starts_with(IMG_MASK_PREFIX)) {
      for (unsigned i = 0; i < IMG_SIZE; ++i) {
        // in the case of image functions, the name should be unchanged, since
        // masked functions are 'real functions' to be looked in the reflection
        // module
        if (stripped.ends_with(imageFunctions[i]))
          return name;
      }
    }
    if (!masked)
  ...
  V_ASSERT(name.find(mask_prefix_func) != name.npos &&
           "not a mangled function");
  // Format:
  // masked_83_function
  size_t start =
      name.find(mask_prefix_func) + std::string(mask_prefix_func).length() + 1;
...
unsigned Mangler::getMangledStoreAlignment(const std::string &name) {
  ...
  unsigned alignStart =
      name.find(mask_prefix_store) + mask_prefix_store.length();
...
std::string Mangler::get_original_scalar_name_from_retbyvector_builtin(
    const std::string &name) {
  V_ASSERT(isRetByVectorBuiltin(name) &&
           "not a mangled ret-by-vector builtin function");
  reflection::FunctionDescriptor desc = ::demangle(name.c_str());
...
std::string Mangler::getTransposeBuiltinName(bool isLoad, bool isScatterGather,
                                             bool isMasked,
  ...
  if (origVecType->getScalarSizeInBits() == 8) {
    typeName = "char";
  } else if ((origVecType->getScalarSizeInBits() == 16) &&
             origVecType->getElementType()->isIntegerTy()) {
    typeName = "short";
  } else if ((origVecType->getScalarSizeInBits() == 32) &&
             origVecType->getElementType()->isIntegerTy()) {
    typeName = "int";
  } else if ((origVecType->getScalarSizeInBits() == 32) &&
             origVecType->getElementType()->isFloatTy()) {
    typeName = "float";
  }

  std::stringstream funcName;
  funcName << "__ocl_" << maskedName << baseFuncName << typeName << "_"
           << origVecType->getNumElements() << "x" << packetWidth;

  return funcName.str();
}

std::string Mangler::getMaskedLoadStoreBuiltinName(bool isLoad,
  ...
  if (vecType->getScalarSizeInBits() == 8) {
    typeName = "char";
  } else if ((vecType->getScalarSizeInBits() == 16) &&
             vecType->getElementType()->isIntegerTy()) {
    typeName = "short";
  } else if ((vecType->getScalarSizeInBits() == 32) &&
             vecType->getElementType()->isIntegerTy()) {
    typeName = "int";
  } else if ((vecType->getScalarSizeInBits() == 64) &&
             vecType->getElementType()->isIntegerTy()) {
    typeName = "long";
