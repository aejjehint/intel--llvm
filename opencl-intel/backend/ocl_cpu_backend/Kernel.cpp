...
Kernel::Kernel(const std::string &name,
               const SmallVector<KernelArgument, 8> &args,
  ...
  if (!m_explicitArgs.empty()) {
    ...
    m_explicitArgsSizeInBytes = ImplicitArgsUtils::getAdjustedAlignment(
        ...
        sizeof(size_t));
    for (size_t I = 0, E = m_explicitArgs.size(); I < E; ++I)
      if (m_explicitArgs[I].IsMemoryObject)
        m_memArgs.push_back(I);
  } else {
...
void Kernel::CreateWorkDescription(UniformKernelArgs *UniformImplicitArgs,
  ...
  if (canUniteWG && !UseAutoGroupSize) {
    ...
    size_t localSizeUpperLimit = std::min(
    ...
    size_t loopUpperLimit =
        std::min(minMultiplyFactor * localWorkSizeX, localSizeUpperLimit);
    ...
    const size_t globalWorkSize = globalWorkSizeX * globalWorkSizeYZ;
    // These variables hold the max utility of SIMD and work threads
    const size_t workThreadUtils = UniformImplicitArgs->MinWorkGroupNum;
    ...
    size_t localSizeMaxLimit = localSizeUpperLimit;
    ...
    if (isLargeGlobalWGsize) {
      if (m_pProps->HasGlobalSyncOperation()) {
        localSizeMaxLimit =
            std::min(localSizeMaxLimit,
                     globalWorkSize / (workThreadUtils << simdUtilsLog));
      } else {
        localSizeMaxLimit =
            std::min(localSizeMaxLimit,
                     // Calculating lower bound for
                     // [sqrt(global/(WT*SIMD))*SIMD] Starting the search from
                     // this number improves the chances to find a local size
                     // that satisfies the "balance" factor. Optimal balanced
                     // local size applies the following: Let,
                     //   X - local size
                     //   Y - number of work groups = (global size / local size)
                     // Then: (X * workThreadUtils) == (Y << simdUtilsLog)
                     static_cast<size_t>(sqrtf(static_cast<float>(
                         globalWorkSize / (workThreadUtils << simdUtilsLog))))
      ...
      // work threads.
      const size_t workGroupNumMinLimit =
          (workThreadUtils + (globalWorkSizeYZ - 1)) / globalWorkSizeYZ;
      localSizeMaxLimit =
          std::max((size_t)1, localSizeMaxLimit / workGroupNumMinLimit);
    }
...
#define BALANCE_FACTOR (2)
        ...
        if (balanceFactor > 0) {
          localSizeUpperLimit = std::min(
              localSizeUpperLimit,
              static_cast<size_t>(sqrtf(static_cast<float>(globalWorkSize))));
          ...
          for (; newHeuristicUp <= localSizeUpperLimit; newHeuristicUp++) {
            if (globalWorkSizeX % newHeuristicUp == 0) {
              ...
              const size_t workGroupsUp = globalWorkSize / newHeuristicUp;
...
cl_dev_err_code Kernel::PrepareThreadState(ICLDevExecutionState &state) const {
  unsigned int uiCSR = _mm_getcsr();
  if (GetKernelProporties()->HasRootGroupBarrier()) {
    llvm::call_once(m_CSRControlBitsFlag,
                    [&]() { m_CSRControlBits = uiCSR & ~_MM_EXCEPT_MASK; });
    // The preserved control bits hold the denormals-as-zero mode (Bit 6),
    // exception mask (Bits 7-12), rounding mode (Bits 13-14) and flush-to-zero
    // mode (Bit 15) in MXCSR register. Bits 0-5 (_MM_EXCEPT_MASK) are status
    // flags for SIMD floating-point exceptions that occurred in previous
    // execution. Assume the control bits be the same for master and TBB worker
    // threads and be not changed by kernel execution.
    assert(m_CSRControlBits == (uiCSR & ~_MM_EXCEPT_MASK));
  } else {
    state.MXCSRstate = uiCSR;
  }
  unsigned int uiNewCSR = (uiCSR & ~m_CSRMask) | m_CSRFlags;
  _mm_setcsr(uiNewCSR);
  return CL_DEV_SUCCESS;
}
...
cl_dev_err_code Kernel::PrepareKernelArguments(
  ...
  size_t localBufferSize =
      m_pProps->GetImplicitLocalMemoryBufferSize() + CPU_DEV_MAXIMUM_ALIGN * 2;
...
static void ErrorExit(LPCTSTR lpszFunction) {
  ...
  llvm::call_once(PrintErrorMessageOnce, [&]() {
    ...
    if (BufferLength) {
      assert(BufferLength >= 2 && "unsigned overflow");
...
extern std::map<std::thread::id, std::pair<void *, size_t>> ThreadHeapMem;
extern std::mutex HeapMutex;
...
cl_dev_err_code Kernel::RunGroup(const void *pKernelUniformArgs,
  ...
  } catch (...) { // ignore any exception, and treat it as a normal exit
  }
  SSC_MARK_2;
  ...
  try {
    if (m_stackActualSize < stackSize)
      kernel(pKernelUniformArgs, PKenrelNonUniformArgs, pRuntimeHandle);
    else {
      void *HeapAddr = AllocaHeapForPrivateLocalMem(m_stackActualSize);
      if (!HeapAddr)
        return CL_DEV_OUT_OF_MEMORY;
      KenrelNonUniformArgs.HeapForPrivateLocal = reinterpret_cast<void *>(
          alignAddr(HeapAddr, Align{CPU_DEV_MAXIMUM_ALIGN}));
      kernel(pKernelUniformArgs, PKenrelNonUniformArgs, pRuntimeHandle);
    }
  } catch (...) { // ignore any exception, and treat it as a normal exit
  }
...
cl_dev_err_code Kernel::RestoreThreadState(ICLDevExecutionState &state) const {
  // When root group barrier is used:
  //   * The preserved MXCSR control bits and current MXCSR status bits are used
  //   to restore thread state. The status bits restoration might be inaccurate,
  //   but this generally won't lead to crash later, for example, an unexpected
  //   floating-point exception.
  //   * Note the thread local storage of MXCSR state is unreliable to restore
  //   thread state for kernels that use root group barrier, because the
  //   restoration might be performed across threads. Using thread local storage
  //   can lead to incorrect thread state restoration and even crash later.
  //
  // When root group barrier is not used, the thread local storage of MXCSR
  // state is reliable for thread state restoration.
  if (GetKernelProporties()->HasRootGroupBarrier())
    _mm_setcsr(m_CSRControlBits | (_mm_getcsr() & _MM_EXCEPT_MASK));
  else
...
void *Kernel::AllocaHeapForPrivateLocalMem(size_t HeapSize) const {
  ...
  HeapMutex.lock();
  Align Alignment{static_cast<uint64_t>(CPU_DEV_MAXIMUM_ALIGN)};
  HeapSize = alignTo(HeapSize, Alignment);
  void *HeapBase = nullptr;
  std::thread::id TId = std::this_thread::get_id();
  if (auto It = ThreadHeapMem.find(TId); It != ThreadHeapMem.end()) {
    auto *Item = &(It->second);
    if (Item->second >= HeapSize) {
      HeapBase = Item->first;
    } else {
      HeapBase = realloc(Item->first, HeapSize);
      Item->first = HeapBase;
      Item->second = HeapSize;
    }
  } else {
    HeapBase = malloc(HeapSize);
    ThreadHeapMem.insert(
        std::make_pair(TId, std::make_pair(HeapBase, HeapSize)));
  }
  HeapMutex.unlock();
  assert(HeapBase && "Error: System memory is out of resource");
  return reinterpret_cast<void *>(alignAddr(HeapBase, Alignment));
}
...
void Kernel::SetRuntimeConfig(const ICompilerConfig *Config) {
  ...
  m_pProps->SetMaxPrivateMemorySize(Config->TargetDevice() == FPGA_EMU_DEVICE
