...
KernelProperties *ProgramBuilder::CreateKernelProperties(
  ...
  const size_t scalarExecutionLength = skimd.KernelExecutionLength.hasValue()
                                           ...
                                           : 0;
  const uint64_t scalarBufferStride =
  ...
  if (skimd.VectorizedKernel.hasValue() &&
    ...
    privateMemorySize =
        std::max<uint64_t>(privateMemorySize, vkimd.PrivateMemorySize.get());
  ...
  uint64_t barrierBufferSize =
      std::max<uint64_t>(scalarBufferStride, vectorBufferStride);
  ...
  if (skimd.HasRootGroupBarrier.hasValue())
    pProps->SetHasRootGroupBarrier(skimd.HasRootGroupBarrier.get());
...
void ProgramBuilder::BuildLibraryProgram(Program *Prog,
  ...
  try {
    ...
    std::string ModuleDir = getModuleDirOrOverrideFromEnvOrFallback(
