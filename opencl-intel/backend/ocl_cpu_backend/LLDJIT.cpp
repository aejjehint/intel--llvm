...
LLDJIT::createJIT(std::unique_ptr<Module> M, std::string *ErrorStr,
  ...
  return std::unique_ptr<LLDJIT>(
      new LLDJIT(std::move(M), std::move(TM), programHash));
}
...
LLDJIT::LLDJIT(std::unique_ptr<Module> M, std::unique_ptr<TargetMachine> TM,
    ...
    : ExecutionEngine(TM->createDataLayout(), std::move(M)), TM(std::move(TM)),
      ...
      ProgramFilePrefix("CPUDeviceProgram-" + programHash) {
...
void LLDJIT::DeleteTempFiles(
  ...
  auto B = FilesToDelete.begin();
  auto E = FilesToDelete.end();
  for (size_t i = 0; i < OwnedTempFiles.size();) {
...
std::string LLDJIT::emitObject(Module *M) {
  ...
  TmpFile ObjFile = TmpFile(ProgramFilePrefix, "obj");
  ...
  if (TM->addPassesToEmitFile(PM, ObjStream,
                              /*raw_pwrite_stream*/ nullptr,
                              CodeGenFileType::ObjectFile,
                              /*DisableVerify*/ true))
    throw Exceptions::CompilerException(
        "failed to add passes to emit LLDJIT object file");
...
void LLDJIT::LoadDLL() {
  TmpFile DLLFile = TmpFile(ProgramFilePrefix, "dll");
  std::string DLLPath = DLLFile.FileName();
  SmallString<256> PDBPath(DLLPath);
  sys::path::replace_extension(PDBPath, "pdb");
  TmpFile PDBFile = TmpFile(PDBPath);
  ...
  // keep *.dll & *.pdb for use by vtune and advisor.
  DLLFile.keep();
  PDBFile.keep();
...
void LLDJIT::buildDllFromObjs(
  ...
  TmpFile ImpLibFile(ProgramFilePrefix + "-implib", "lib");
  ...
  TmpFile JumpTableObjectFile =
      TmpFile(ProgramFilePrefix + "-jumptable", "obj");
...
// TmpFile name with suffix of random chars.
LLDJIT::TmpFile::TmpFile(const llvm::Twine &Prefix,
                         llvm::StringRef FileExtension) {
  SmallString<256> ResultPath;
  sys::path::system_temp_directory(/*erasedOnReboot*/ true, ResultPath);
  sys::path::append(ResultPath, Prefix);
  std::string EnvStr;
  if (!Intel::OpenCL::Utils::getEnvVar(EnvStr,
                                       "CL_CONFIG_LLDJIT_STABLE_HASH") ||
      !Intel::OpenCL::Utils::ConfigFile::ConvertStringToType<bool>(EnvStr)) {
    raw_svector_ostream OS(ResultPath);
    OS << "-";
  ...
  int FD;
  if (auto EC = sys::fs::openFileForWrite(ResultPath, FD))
    report_fatal_error("Failed to create a temporary file " + ResultPath.str() +
                       " on the system for writing: " + EC.message());
  File = std::make_unique<llvm::ToolOutputFile>(ResultPath, FD);
  Name = std::string(ResultPath);
}

// Tmp file with given name.
LLDJIT::TmpFile::TmpFile(const llvm::SmallString<256> &Filename) {
  int FD;
  if (auto EC = sys::fs::openFileForWrite(Filename, FD))
    report_fatal_error("Failed to create a temporary file " + Filename.str() +
                       " on the system for writing: " + EC.message());
  File = std::make_unique<llvm::ToolOutputFile>(Filename, FD);
