 ...
 ******************************************************************************/
LLVM_BACKEND_API void __cpu_dbg_print(const char *fmt, ...) {
  va_list va;
  va_start(va, fmt);
...
LLVM_BACKEND_API void __cpu_lprefetch(const char *ptr, size_t numElements,
                                      size_t elmSize) {
...
LLVM_BACKEND_API unsigned long long __cpu_get_time_counter() {
...
__devicelib_assert_fail_opencl(const char *expr, const char *file, int32_t line,
                               ...
                               uint64_t lid2);
LLVM_BACKEND_API void __devicelib_assert_read_opencl(void *);

LLVM_BACKEND_API void __devicelib_exit() { throw std::exception(); }

LLVM_BACKEND_API void __opencl_unreachable(const char *msg);

LLVM_BACKEND_API void __opencl_dbg_declare_local(void *addr,
                                                 uint64_t var_metadata_addr,
                                                 uint64_t expr_metadata_addr,
                                                 uint64_t gid0, uint64_t gid1,
                                                 uint64_t gid2);
LLVM_BACKEND_API void __opencl_dbg_declare_global(void *addr,
                                                  uint64_t var_metadata_addr,
                                                  uint64_t gid0, uint64_t gid1,
                                                  uint64_t gid2);
LLVM_BACKEND_API void __opencl_dbg_stoppoint(uint64_t metadata_addr,
                                             uint64_t gid0, uint64_t gid1,
                                             uint64_t gid2);
LLVM_BACKEND_API void __opencl_dbg_enter_function(uint64_t metadata_addr,
                                                  uint64_t gid0, uint64_t gid1,
                                                  uint64_t gid2);
LLVM_BACKEND_API void __opencl_dbg_exit_function(uint64_t metadata_addr,
                                                 uint64_t gid0, uint64_t gid1,
                                                 uint64_t gid2);
...
LLVM_BACKEND_API unsigned __opencl_get_cpu_node_id() { return getCpuNodeId(); }
LLVM_BACKEND_API unsigned __opencl_get_hw_thread_id() {
  return getHWThreadId();
}

LLVM_BACKEND_API int __opencl_atexit(void (*function)(void)) { return 0; }
...
#define SORT_KEY_ONLY_BI_DECLARATION(ARGTYPE)                                  \
  LLVM_BACKEND_API void __ocl_sort_##ARGTYPE(ARGTYPE *, uint32_t, bool);

#define SORT_KEY_VALUE_BI_DECLARATION(KeyType, ValueType)                      \
  LLVM_BACKEND_API void __ocl_sort_##KeyType##_##ValueType(                    \
...
LLVM_BACKEND_API void intel_device_barrier(unsigned int);
LLVM_BACKEND_API bool intel_is_device_barrier_valid();
...
#define REGISTER_BI_FUNCTION_FLAG(name, ptr, flag)                             \
  if (LLJIT) {                                                                 \
...
#define REGISTER_BI_FUNCTION(name, ptr)                                        \
  REGISTER_BI_FUNCTION_FLAG(name, ptr, llvm::JITSymbolFlags::None);
...
llvm::Error RegisterCPUBIFunctions(bool isFPGAEmuDev, llvm::orc::LLJIT *LLJIT) {
  ...
  REGISTER_BI_FUNCTION("__opencl_unreachable", __opencl_unreachable)
  REGISTER_BI_FUNCTION("__devicelib_exit", __devicelib_exit)
...
#ifdef _WIN32
  REGISTER_BI_FUNCTION_FLAG("_Znwy", _Znwy, llvm::JITSymbolFlags::Weak)
  REGISTER_BI_FUNCTION_FLAG("_ZdlPvy", _ZdlPvy, llvm::JITSymbolFlags::Weak)
  REGISTER_BI_FUNCTION_FLAG("_ZSt14_Xlength_errorPKc", _ZSt14_Xlength_errorPKc,
                            llvm::JITSymbolFlags::Weak)
  REGISTER_BI_FUNCTION_FLAG("_ZdlPv", _ZdlPv, llvm::JITSymbolFlags::Weak)
#ifdef _WIN64
