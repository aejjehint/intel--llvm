...
Kernel *CPUProgramBuilder::CreateKernel(Function *pFunc,
  ...
  assert(KIMD.ArgProperties.hasValue() &&
         "kernel_arg_properties metadata is expected");
  SmallVector<KernelArgument, 8> Arguments = KIMD.ArgProperties.getList();
...
CPUProgramBuilder::CreateKernels(Program *pProgram, const char *pBuildOpts,
  ...
  for (auto *pFunc : Kernels) {
    ...
    std::unique_ptr<Kernel> spKernel(
        CreateKernel(pFunc, pFunc->getName().str(), spKernelProps.get(),
                     CompilationUtils::hasTLSGlobals(*pModule)));

    // We want the JIT of the wrapper function to be called
    AddKernelJIT(static_cast<CPUProgram *>(pProgram), spKernel.get(), pFunc,
    ...
    if (pVecFunc) {
      ...
      AddKernelJIT(static_cast<CPUProgram *>(pProgram), spKernel.get(),
                   pVecFunc, spVKernelJITProps.release());
    }
...
static void dumpAssembly(Module *M, TargetMachine *TM,
  ...
  TM->Options.MCOptions.AsmVerbose = Verbose;
...
void CPUProgramBuilder::JitProcessing(
  ...
  auto EE = useLLDJIT ? m_compiler.GetOwningExecutionEngine() : nullptr;

  assert((useLLDJIT == (nullptr != EE)) &&
         "JIT mode and execution engine mismatch");
  ...
  if (Intel::OpenCL::Utils::getEnvVar(envStr, "CL_CONFIG_DUMP_ASM") &&
    ...
    TargetMachine *TM =
        useLLDJIT ? EE->getTargetMachine() : targetMachine.get();
    bool Verbose = false;
#ifndef INTEL_PRODUCT_RELEASE
    // CL_CONFIG_DUMP_ASM=2 for verbose asm output. Helpful for debugging.
    if (Intel::OpenCL::Utils::ConfigFile::ConvertStringToType<int>(envStr) == 2)
      Verbose = true;
#endif // INTEL_PRODUCT_RELEASE
    dumpAssembly(module, TM, filename, Verbose);
  }

  if (useLLDJIT) {
    EE->setObjectCache(objCache);
    program->SetExecutionEngine(std::move(EE));
  } else {
    auto LLJIT =
  ...
  for (auto *pFunc : Kernels)
    kernelNames.push_back(pFunc->getName().str());
