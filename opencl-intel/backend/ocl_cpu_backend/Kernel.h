...
public:
  ...
  Kernel(const std::string &name,
         const llvm::SmallVector<llvm::KernelArgument, 8> &args,
...
protected:
  ...
  mutable unsigned int m_CSRControlBits =
      0; // Control bits to restore MXCSR state ONLY for kernels that use root
         // group barrier
  mutable llvm::once_flag m_CSRControlBitsFlag;
  llvm::SmallVector<llvm::KernelArgument, 8> m_explicitArgs;
