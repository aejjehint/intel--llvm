...
__ocl_task_sequence_create(size_t ret_type_size, int pipelined,
  ...
  // The following two field is only used on fpga hardware. Ignore them here.
  data->pipelined = pipelined;
  data->fpga_cluster = fpga_cluster;

  data->response_capacity = response_capacity;
  data->invocation_capacity = invocation_capacity;
  // align the same value in SYCL task sequence instance.
  data->outstanding = 0;
...
extern "C" LLVM_BACKEND_API void
__ocl_task_sequence_async(task_sequence *obj, void *block_invoke,
                          void *block_literal, IDeviceCommandManager *DCM,
                          IBlockToKernelMapper *B2K, void *RuntimeHandle) {
  task_sequence_data *data = reinterpret_cast<task_sequence_data *>(obj);
  assert(data && "data of task_sequence lossed!");
  unsigned invocation_capacity = data->invocation_capacity;
  ...
  // execution.
  data->outstanding++;
  bool is_first_async = (data->outstanding == 1);
...
__ocl_task_sequence_get(task_sequence *obj, IDeviceCommandManager *DCM) {
  task_sequence_data *data = reinterpret_cast<task_sequence_data *>(obj);
  assert(data && "data of task_sequence lossed!");
  assert(data->delivered < data->outstanding &&
         "No enough async is inovked before get result.");
