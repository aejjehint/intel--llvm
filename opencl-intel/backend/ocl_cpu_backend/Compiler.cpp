...
CompilerBuildOptions::CompilerBuildOptions(const char *pBuildOpts) {
  ...
  for (const auto &opt : splittedOptions) {
    ...
    if (opt == "-profiling")
      ...
      m_coverage = true;
    else if (opt == "-cl-fast-relaxed-math")
      m_relaxedMath = true;
    else if (opt == "-cl-opt-disable")
      m_disableOpt = true;
...
void Compiler::InitGlobalState(const IGlobalCompilerConfig &config) {
  ...
  Args.push_back("-inline-threshold=2048");
...
Compiler::BuildProgram(llvm::Module *pModule, const char *pBuildOptions,
  ...
  if (optimizer->hasErrors())
    throw Exceptions::CompilerException("Optimization error",
  ...
  if (useLLDJITForExecution(pModule)) {
    ...
    // latest
    CreateExecutionEngine(pModule, programHash);
  }
  return pModule;
}
