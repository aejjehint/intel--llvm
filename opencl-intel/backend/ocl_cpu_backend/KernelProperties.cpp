...
KernelProperties::KernelProperties()
    : m_hasNoBarrierPath(false), m_hasMatrixCall(false), m_hasGlobalSync(false),
      ...
      m_subGroupConstructionMode(0), m_hasRootGroupBarrier(false) {
...
void KernelProperties::Serialize(IOutputStream &ost,
  ...
  if (OCL_CACHED_BINARY_VERSION >= 21)
    Serializer::SerialPrimitive<bool>(&m_hasRootGroupBarrier, ost);
}
...
void KernelProperties::Deserialize(IInputStream &ist,
  ...
  if (stats->m_binaryVersion >= 21)
    Serializer::DeserialPrimitive<bool>(&m_hasRootGroupBarrier, ist);
}
...
size_t KernelProperties::getMaterializedSubGroupSize() const {
  ...
  // If a kernel has valid 'intel_reqd_sub_group_size' attribute but is not
  // vectorized, the required subgroup size will be returned. This is to align
  // kernel max subgroup size querying with GPU handling.
  const static std::unordered_set<size_t> SupportedSGSizes =
      CPU_DEV_SUB_GROUP_SIZES;
  size_t MaterializedSGSize =
      m_vectorizationWidth == 1
          ? (SupportedSGSizes.count(m_reqdSubGroupSize) ? m_reqdSubGroupSize
                                                        : CPU_DEV_MAX_SG_SIZE)
          : m_vectorizationWidth;
#ifndef NDEBUG
...
bool KernelProperties::HasRootGroupBarrier() const {
  return m_hasRootGroupBarrier;
}
...
size_t KernelProperties::GetMaxWorkGroupSize(
  ...
  // Return 1 if there is not enough private memory. But take into account what
  // in the vectorized JIT each lane has it's own private memory (not only
  // barrier buffer).
  unsigned minGroupSizeFactorial = GetMinGroupSizeFactorial();
  if (wgPrivateMemSizeUpperBound <
      GetPrivateMemorySize() * minGroupSizeFactorial) {
    // Private memory requirements exceed available resources.
    // Return 1 to minimize size of allocated heap for replacing private memory.
    maxWorkGroupSize = 1;
  ...
  if (maxWorkGroupSize > minGroupSizeFactorial) {
    assert(minGroupSizeFactorial >= 1 && "unsigned overflow");
    maxWorkGroupSize &= ~(minGroupSizeFactorial - 1);
  }
...
void KernelProperties::Print() const {
  ...
  outs().indent(NS) << "hasRootGroupBarrier: " << m_hasRootGroupBarrier << "\n";
}
} // namespace DeviceBackend
} // namespace OpenCL
} // namespace Intel
