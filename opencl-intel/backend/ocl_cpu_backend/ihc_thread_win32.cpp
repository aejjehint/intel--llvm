...
LLVM_BACKEND_API int _ihc_mutex_delete(void *handle) {
  SRWLOCK *_m = (SRWLOCK *)handle;
  delete _m;
  return 0;
}

LLVM_BACKEND_API int _ihc_mutex_lock(void *handle) {
  SRWLOCK *_m = (SRWLOCK *)handle;
  AcquireSRWLockExclusive(_m);
  return 0;
}

LLVM_BACKEND_API int _ihc_mutex_unlock(void *handle) {
  SRWLOCK *_m = (SRWLOCK *)handle;
...
LLVM_BACKEND_API int _ihc_cond_notify_one(void *cv) {
  CONDITION_VARIABLE *_cond = (CONDITION_VARIABLE *)cv;
  WakeConditionVariable(_cond);
  return 0;
}

LLVM_BACKEND_API int _ihc_cond_wait(void *m, void *cv) {
  CONDITION_VARIABLE *_cond = (CONDITION_VARIABLE *)cv;
...
LLVM_BACKEND_API int _ihc_pthread_join(void *_thread) {
...
LLVM_BACKEND_API int _ihc_pthread_detach(void *_thread) {
...
LLVM_BACKEND_API void *_Znwy(unsigned long long size) {
  return ::operator new(size);
}

LLVM_BACKEND_API void _ZdlPvy(void *ptr, unsigned long long size) {
  return ::operator delete(ptr, size);
}

LLVM_BACKEND_API void _ZSt14_Xlength_errorPKc(char const *str) {
  return std::_Xlength_error(str);
}

LLVM_BACKEND_API void _ZdlPv(void *ptr) { return ::operator delete(ptr); }
