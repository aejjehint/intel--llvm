...
OclBuiltin::OclBuiltin(const OclBuiltinDB &DB, const Record *R)
  ...
  // templates
  {
    ...
    std::vector<const Record *> Templ =
        TemplClass->getValueAsListOfDefs("tAttrs");

    for (std::vector<const Record *>::const_iterator I = Templ.begin(),
                                                     E = Templ.end();
         I != E; ++I) {
      OclBuiltinAttr *Attr = new OclBuiltinAttr(*I);
...
/// OclBuiltinDB
OclBuiltinDB::OclBuiltinDB(const RecordKeeper &R) : m_Records(R) {
  // OclType
  {
  ...
  // REF_Conversions
  {
    ...
    for (std::vector<const Record *>::const_iterator I = Rs.begin(), E = Rs.end();
      ...
      const Record *Rec = *I;
      OclBuiltin *OB;
      std::vector<const Record *> RModes = Rec->getValueAsListOfDefs("RModes");

      for (std::vector<const Record *>::const_iterator I = RModes.begin(),
                                                       E = RModes.end();
           I != E; ++I) {
  ...
  // get prolog and epilog
  {
    ...
    // Prolog & Epilog
    {
      ...
      m_EpilogInit = std::string(Rec->getValueAsString("EpilogInit"));
