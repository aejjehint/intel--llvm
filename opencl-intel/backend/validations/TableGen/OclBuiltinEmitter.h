...
typedef enum {
  TYPE = 0 /*generic builtins*/,
  VECTOR /*vector size*/,
  UNMAPPED_VECTOR /*size of not mapped vector type*/,
  UDEF /*user definde*/,
  RETURN_TYPE,
  RETURN_VSIZE,
  SATURATE,
...
public:
  ...
  typedef std::vector<const OclBuiltinAttr *>::const_iterator
      const_trule_iterator;
...
public:
  explicit OclConversions(const OclBuiltinDB &, const Record *,
                          const OclRoundingMode *);
  ...
  virtual std::string getTemplate(unsigned, const std::string &) const override;
...
class OclBuiltinDB {
public:
  explicit OclBuiltinDB(const RecordKeeper &);
