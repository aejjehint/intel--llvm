...
OclBuiltinsHeaderGen::OclBuiltinsHeaderGen(const RecordKeeper &rk)
    : m_recordKeeper(rk) {}

// generates the prototype of a fucntion as string
static std::string getPrototype(const llvm::OclBuiltin *bi,
...
static std::string processFunctionName(const std::string &oldName) {
  ...
  if ((oldName.find("vload", 0, 5) == 0) ||
      (oldName.find("vstore", 0, 6) == 0)) {
    char numbers[] = "123468";
...
static std::string getRefFunction(const llvm::OclBuiltin *bi,
                                  const std::string &type) {
  std::string ret;
  ret.append("lle_X_");
  ...
  // we have two functions async_work_group_strided_copy, distinguishied by
  // memory access qualifiers only so, we add suffixes _g2l and _l2g to the
  // basic function name, but only here, after .ll code generated, because if we
  // add suffixes before generation of .ll code, we will get wrong mangled names
  if ((funcName.find(asyncFuncName)) != std::string::npos) {
    ret.append(processFunctionName(funcName));
  } else {
    // we need to get different names for reference functions for 2d and 3d
    // images, but we need to have the same name before generation of .ll code,
    // so we add "2" or "3" here
    ...
    if ((tempFuncName.find("vstore_half_", 0, 12) == 0)) {
      ...
      typeSuffix.erase(1,
                       typeSuffix.size() - 1); // delete all exept first letter
      if (typeSuffix == "f" || typeSuffix == "d") // float or double
        tempFuncName.replace(0, 12,
                             std::string("vstore") + typeSuffix +
                                 "_half_"); // rebuild name
    }
...
protected:
  ...
  std::string generateBuiltinOverload(const OclBuiltin *pBuiltin,
    ...
    // hack for functions async_work_group_coopy and
    ...
    ret += generateDummyBody(pBuiltin->getReturnBaseCType(typeName),
