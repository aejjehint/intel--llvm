...
static void _CreateNEATBufferContainerByPtr(
  ...
  if (argTypeName.contains("*")) {
  ...
  } else if (argTypeName.ends_with("char") || argTypeName.ends_with("short") ||
             argTypeName.ends_with("int") || argTypeName.ends_with("long")) {
    return;
    ...
    if (argTypeName == "float")
      ...
      return;
  } else if (argTypeName.starts_with("struct ") ||
             argTypeName.starts_with("image")) {
    neatVal.PointerVal = buffer->GetDataPtr();
    out_NEATArgValues[&*arg_it] = neatVal;
  } else {
...
bool NEATDataLayout ::IsNEATSupported(const Type *Ty, Value *V) {
  ...
  case Type::PointerTyID: {
    ...
    // Ty is handled but element type isn't deduced. Optimistically return true.
    if (!EltTy)
      return true;
