...
OCLKernelArgumentsList OpenCLKernelArgumentsParser::KernelArgumentsParser(
  ...
  for (const auto &[Idx, Arg] : llvm::enumerate(m_pKernel->args())) {
    ...
    case Type::PointerTyID: {
      ...
      // ArgTypeName.contains(' ') is to determine if there is additional
      // attribute.
      if (ArgTypeName.contains(' ') || !SetSubType(ElemDesc, ArgTypeName)) {
      ...
      if ((ST && !ST->isLiteral()) || ArgTypeName.starts_with("opencl")) {
        const StringRef ImgArg = ST ? ST->getName() : ArgTypeName;
        if (ImgArg.starts_with("opencl.image")) {
          // Image identifier was found.
          // Get dimension image type
          if (ImgArg.starts_with("opencl.image1d_t") ||
              ImgArg.starts_with("opencl.image1d_array_t") ||
              ImgArg.starts_with("opencl.image1d_buffer_t") ||
              ImgArg.starts_with("opencl.image2d_t") ||
              ImgArg.starts_with("opencl.image2d_depth_t") ||
              ImgArg.starts_with("opencl.image2d_array_t") ||
              ImgArg.starts_with("opencl.image2d_array_depth_t") ||
              ImgArg.starts_with("opencl.image3d_t")) {
            ImageDesc ImgDesc;
