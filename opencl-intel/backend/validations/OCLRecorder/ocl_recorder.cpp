...
namespace Utils {
/// \brief Converts type description from LLVM data structures to Data Manager
/// structures.
TypeDesc GetTypeDesc(llvm::Type *type, const llvm::DataLayout *td) {
  ...
  case llvm::Type::ArrayTyID: {
    ret.SetType(TARRAY);
    ret.SetNumberOfElements(
        llvm::cast<llvm::ArrayType>(type)->getNumElements());
    TypeDesc subElemType =
        GetTypeDesc(llvm::cast<llvm::ArrayType>(type)->getElementType(), td);
  ...
  case llvm::Type::ScalableVectorTyID: {
    ...
    TypeDesc subElemType =
        GetTypeDesc(llvm::cast<llvm::VectorType>(type)->getElementType(), td);
    ret.SetSubTypeDesc(0, subElemType);
  ...
  case llvm::Type::StructTyID: {
    ...
    if (SLO->getSizeInBytes() != td->getTypeAllocSize(type)) {
      throw Exception::ValidationExceptionBase(
          "Target data struct size is not equal to StructLayout size!");
    }
    break;
  }
  default:
...
ImageChannelDataTypeVal GetImageChannelDataTypeVal(cl_channel_type t) {
  ...
  switch (t) {
  case CLK_SNORM_INT8:
    res = OpenCL_SNORM_INT8;
    break;
  case CLK_SNORM_INT16:
    res = OpenCL_SNORM_INT16;
    break;
  case CLK_UNORM_INT8:
    res = OpenCL_UNORM_INT8;
    break;
  case CLK_UNORM_INT16:
    res = OpenCL_UNORM_INT16;
    break;
  case CLK_UNORM_SHORT_565:
    res = OpenCL_UNORM_SHORT_565;
    break;
  case CLK_UNORM_SHORT_555:
    res = OpenCL_UNORM_SHORT_555;
    break;
  case CLK_UNORM_INT_101010:
    res = OpenCL_UNORM_INT_101010;
    break;
  case CLK_SIGNED_INT8:
    res = OpenCL_SIGNED_INT8;
    break;
  case CLK_SIGNED_INT16:
    res = OpenCL_SIGNED_INT16;
    break;
  case CLK_SIGNED_INT32:
    res = OpenCL_SIGNED_INT32;
    break;
  case CLK_UNSIGNED_INT8:
    res = OpenCL_UNSIGNED_INT8;
    break;
  case CLK_UNSIGNED_INT16:
    res = OpenCL_UNSIGNED_INT16;
    break;
  case CLK_UNSIGNED_INT32:
    res = OpenCL_UNSIGNED_INT32;
    break;
  case CLK_HALF_FLOAT:
    res = OpenCL_HALF_FLOAT;
    break;
  case CLK_FLOAT:
    res = OpenCL_FLOAT;
    break;
  default:
    res = INVALID_IMAGE_DATA_TYPE;
    break;
  }
  return res;
}
...
ImageChannelOrderVal GetImageChannelOrderVal(cl_channel_order t) {
  ...
  case CLK_A:
    ...
    break;
  case CLK_INTENSITY:
    res = OpenCL_INTENSITY;
    break;
  case CLK_LUMINANCE:
    res = OpenCL_LUMINANCE;
    break;
  case CLK_RG:
    res = OpenCL_RG;
    break;
  ...
  case CLK_BGRA:
    ...
    break;
  default:
    res = INVALID_CHANNEL_ORDER;
    break;
  }
  return res;
}

const ImageDesc GetImageDesc(cl_mem_obj_descriptor *pmem_obj,
                             const llvm::Argument &func_arg) {
  assert(NULL != pmem_obj);
  ...
  ImageTypeVal imageType =
      GetImageTypeFromDimCount(pmem_obj->dim_count, isArray);
...
BinaryContext *KernelContext::getOrCreateBinaryContext(
    const char *name, const _cl_work_description_type *workDesc,
    bool &created) {
  BinaryContext context(name, workDesc, m_binaries.size());

  BinaryContextList::iterator it =
      std::find(m_binaries.begin(), m_binaries.end(), context);
...
RecorderContext::RecorderContext(const std::string &logsPath,
                                 const std::string &prefix)
    : m_DL(NULL), m_logsPath(logsPath) {
...
RecorderContext::~RecorderContext() { delete m_DL; }
...
const std::string RecorderContext::getPath(const std::string &kernelName,
...
const std::string
RecorderContext::getFileName(const std::string &suffix) const {
  if (m_baseName.back() != '.') {
...
const std::string
RecorderContext::getFileName(const std::string &kernelName,
                             const std::string &suffix) const {
...
const std::string RecorderContext::getBaseName() const { return m_baseName; }
...
const std::string
RecorderContext::getInputFilePath(const std::string &kernelName) const {
  return getPath(kernelName, DATAFILE_SUFFIX);
}

const std::string
RecorderContext::getInputFileName(const std::string &kernelName) const {
  return getFileName(kernelName, DATAFILE_SUFFIX);
}

const std::string
RecorderContext::getReferenceFilePath(const std::string &kernelName) const {
  return getPath(kernelName, REFERENCE_SUFFIX);
}

const std::string
RecorderContext::getReferenceFileName(const std::string &kernelName) const {
  return getFileName(kernelName, REFERENCE_SUFFIX);
}

const std::string
RecorderContext::getNeatFilePath(const std::string &kernelName) const {
  return getPath(kernelName, NEAT_SUFFIX);
}

const std::string
RecorderContext::getNeatFileName(const std::string &kernelName) const {
...
KernelContext *
RecorderContext::getKernelContext(const ICLDevBackendKernel_ *pKernel) {
  std::lock_guard<llvm::sys::Mutex> lock(m_kernelsLock);
...
void RecorderContext::createKernelContext(const ICLDevBackendKernel_ *pKernel,
  ...
  m_kernels.insert(std::make_pair(
      pKernel, KernelContext(pKernel->GetKernelName(), pFunction)));
}

OCLRecorder::OCLRecorder(const std::string &logsDir, const std::string &prefix)
    : m_logsDir(logsDir), m_prefix(prefix), m_pSourceRecorder(NULL) {}

OCLRecorder::~OCLRecorder() {}

RecorderContext *
OCLRecorder::GetProgramContext(const ICLDevBackendProgram_ *pProgram) {
...
RecorderContext *
OCLRecorder::GetProgramContextForKernel(const ICLDevBackendKernel_ *pKernel) {
  std::lock_guard<llvm::sys::Mutex> lock(m_contextsLock);

  for (RecorderContextMap::iterator i = m_contexts.begin(),
                                    e = m_contexts.end();
       i != e; ++i) {
  ...
  throw Exception::ValidationExceptionBase(
      "Can't find the given kernel in context");
}

void OCLRecorder::AddNewProgramContext(const ICLDevBackendProgram_ *pProgram,
...
void OCLRecorder::OnCreateBinary(const ICLDevBackendKernel_ *pKernel,
  ...
  BinaryContext *pBinaryContext = pKernelContext->getOrCreateBinaryContext(
      pKernel->GetKernelName(), pWorkDesc, created);

  if (created) {
    std::string pathToDataInputFile;
    RecordKernelInputs(*pProgramContext, *pKernelContext, *pBinaryContext,
                       pKernel, bufSize, pArgsBuffer, pathToDataInputFile);
...
void OCLRecorder::OnCreateKernel(const ICLDevBackendProgram_ *pProgram,
  ...
  if (!pProgramContext->containsKernel(pKernel)) {
    pProgramContext->createKernelContext(pKernel,
                                         (const llvm::Function *)pFunction);
  }
}

void OCLRecorder::OnCreateProgram(const void *pBinary, size_t uiBinarySize,
                                  const ICLDevBackendProgram_ *pProgram) {
  assert(m_contexts.end() == m_contexts.find(pProgram));
  llvm::LLVMContext context;
  std::unique_ptr<RecorderContext> spContext(
      new RecorderContext(m_logsDir, m_prefix));
  // Create target data object.
  llvm::StringRef bitCodeStr((const char *)pBinary, uiBinarySize);
  std::unique_ptr<llvm::MemoryBuffer> pMemBuff =
      llvm::MemoryBuffer::getMemBufferCopy(bitCodeStr);
  if (NULL == pMemBuff) {
    throw Exception::ValidationExceptionBase(
        "Can't create memory buffer from IR.");
  }
  ...
  spContext->m_DL = new llvm::DataLayout(pModule->getDataLayout());
...
void OCLRecorder::RecordProgramConfig(RecorderContext &context) {
  AddChildTextNode(context.m_pRunConfig, "ByteCodeFile",
                   context.getByteCodeFileName());
  context.Flush();
}
...
void OCLRecorder::RecordSourceCode(RecorderContext &context,
  ...
  clStream.close();
  TiXmlElement *pSourceNode =
      AddChildTextNode(context.m_pRunConfig, // configuration file
  ...
  pSourceNode->SetAttribute(std::string("compilation_flags"), compilationFlags);
  AddChildTextNode(context.m_pRunConfig, "ProgramFileType", "CL");
  TiXmlElement *includeDirsNode =
      AddChildTextNode(context.m_pRunConfig, "IncludeDirs", "");
  std::string headers(CLANG_HEADERS);
  AddChildTextNode(includeDirsNode, "IncludeDir", headers);
}

TiXmlElement *OCLRecorder::AddChildTextNode(TiXmlElement *pParentNode,
...
void OCLRecorder::RecordKernelConfig(RecorderContext &programContext,
  ...
  {
    AddChildTextNode(
        pNodeKernelConfig, "ReferenceDataFile",
        programContext.getReferenceFileName(binaryContext.getBaseName()));
  }
  ...
  {
    AddChildTextNode(
        pNodeKernelConfig, "NeatDataFile",
        programContext.getNeatFileName(binaryContext.getBaseName()));
  }
...
void OCLRecorder::RecordKernelInputs(const RecorderContext &programContext,
  ...
  for (unsigned int i = 0; i < argsCount; ++i) {
    ...
    if ((KRNL_ARG_PTR_IMG_2D == pKernelArgs[i].Ty) ||
      ...
      size = desc.GetSizeInBytes();
      memset(pData, 0, size);
    } else if (KRNL_ARG_VECTOR == pKernelArgs[i].Ty ||
               KRNL_ARG_VECTOR_BY_REF == pKernelArgs[i].Ty) {
  ...
  if (path != 0) {
    ...
    pathToInputFile =
        programContext.getInputFileName(binaryContext.getBaseName());
    m_hashToPath[hash] = pathToInputFile;
  }

  BinaryContainerListWriter bufferWriter(
      programContext.getInputFilePath(binaryContext.getBaseName()));
...
void OCLRecorder::RecordByteCode(const void *pBinary, size_t uiBinarySize,
                                 const RecorderContext &context) {
  std::error_code error;
  llvm::raw_fd_ostream binStream(context.getByteCodeFilePath().c_str(), error,
...
public:
  ...
  static OclRecorderPlugin *Instance() { return &instance; }
  ...
  DeviceBackend::ICLDevBackendPlugin *getBackendPlugin() override {
    {
      ...
      fileName =
          llvm::sys::path::stem(llvm::sys::fs::getMainExecutable(argv0, &addr));
      ...
      pOclRecorder =
          new OCLRecorder(std::string(logpath.c_str()), std::string(fileName));
    }
