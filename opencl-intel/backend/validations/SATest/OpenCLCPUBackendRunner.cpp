...
private:
  ...
  unique_ptr_aligned m_pArgumentBuffer;
  Sample m_sample;
...
void OpenCLCPUBackendRunner::Run(IRunResult *runResult, const IProgram *program,
  ...
  cl_dev_err_code ret =
      m_pServiceFactory->GetExecutionService(options.get(), &pExecutionService);
  ...
  ret =
      m_pServiceFactory->GetCompilationService(options.get(), &pCompileService);
  ...
  ret = m_pServiceFactory->GetImageService(options.get(), &pImageService);
  ICLDevBackendImageServicePtr spImageService(pImageService);
  if (CL_DEV_FAILED(ret)) {
