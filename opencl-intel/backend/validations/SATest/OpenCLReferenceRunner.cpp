...
OpenCLReferenceRunner::~OpenCLReferenceRunner(void) {
  for (std::list<cl_mem_obj_descriptor *>::iterator it =
           m_ClMemObjScratchMemList.begin();
       it != m_ClMemObjScratchMemList.end(); ++it) {
    align_free(*it);
  }
...
static void SplitFmuladdIntrinsic(Module &M) {
  for (Function &F : M) {
    ...
    for (auto &I : instructions(F)) {
      ...
      Instruction *MulResult = BinaryOperator::CreateFMul(
          Mul0, Mul1, "splitfma", Intrin->getIterator());
      Instruction *AddResult = BinaryOperator::CreateFAdd(
          MulResult, Add0, "splitfma", Intrin->getIterator());
...
void OpenCLReferenceRunner::Run(IRunResult *runResult, const IProgram *program,
  ...
  const ReferenceRunOptions *pRunConfig =
      static_cast<const ReferenceRunOptions *>(runConfig);
...
void OpenCLReferenceRunner::LoadOutput(IRunResult *pRunResult,
                                       const IProgramConfiguration *pConfig) {
  const OpenCLProgramConfiguration *pProgramConfig =
      static_cast<const OpenCLProgramConfiguration *>(pConfig);

  for (OpenCLProgramConfiguration::KernelConfigList::const_iterator it =
           pProgramConfig->beginKernels();
       it != pProgramConfig->endKernels(); ++it) {
    std::string kernelName = (*it)->GetKernelName();
    std::cout << " trying to load reference "
              << (*it)->GetStampedPathReference() << std::endl;
    ReadReferenceBuffer(*it, &pRunResult->GetOutput(kernelName.c_str()));

    if (m_bUseNEAT) {
      std::cout << " trying to load NEAT " << (*it)->GetStampedPathNeat()
                << std::endl;
...
void OpenCLReferenceRunner::StoreOutput(
    const IRunResult *pRunResult, const IProgramConfiguration *pConfig) const {
  typedef std::map<std::string, uint32_t> BcIdxMap;

  const OpenCLProgramConfiguration *pProgramConfig =
      static_cast<const OpenCLProgramConfiguration *>(pConfig);

  /// Map to track how many buffer containers were saved to files for different
  /// kernels
  BcIdxMap bcIndex;
  ...
  for (OpenCLProgramConfiguration::KernelConfigList::const_iterator it =
    ...
    if (bcIdIter == bcIndex.end()) {
      ...
      /// This kernel was run multiple times. We saved data for previous
      /// executions Now we should save the next buffer container
      (*bcIdIter).second++;
    ...
    /// Execution index
    ContainerCopier cc;
    const IBufferContainerList &pBcl =
        pRunResult->GetOutput(kernelName.c_str());
    std::unique_ptr<IBufferContainerList> pNewBcl(
        cc.CloneBufferContainer(&pBcl, bcIdx));
    /// Save it to file
    std::cout << " generating reference " << (*it)->GetStampedPathReference()
              << std::endl;
    WriteReferenceBuffer(*it, pNewBcl.get());

    if (m_bUseNEAT) {
      const IBufferContainerList &neatBCL =
          pRunResult->GetNEATOutput(kernelName.c_str());
      std::unique_ptr<IBufferContainerList> pNewBclNeat(
          cc.CloneBufferContainer(&neatBCL, bcIdx));
      std::cout << " generating NEAT " << (*it)->GetStampedPathNeat()
                << std::endl;
...
void OpenCLReferenceRunner::ReadInputBuffer(
    OpenCLKernelConfiguration *pKernelConfig, IContainer *pContainer,
    const uint64_t seed) {
  ...
  case Random: {
    ...
    if (pKernelConfig->GetGeneratorConfig() != 0) {
      throw Exception::InvalidArgument(
          "[OpenCLReferenceRunner]Unused OCLKernelDataGeneratorConfig found. \
                                                 Switch InputDataFileType to \'config\' or remove OCLKernelDataGeneratorConfig block");
    }
    OCLKernelArgumentsList args =
        parser.KernelArgumentsParser(pKernelConfig->GetKernelName(), m_pModule);
    args = OpenCLKernelArgumentsParser::KernelArgHeuristics(
        args, pKernelConfig->GetGlobalWorkSize(),
        pKernelConfig->GetWorkDimension());
    OCLKernelDataGeneratorConfig *cfg =
        OCLKernelDataGeneratorConfig::defaultConfig(args);
    cfg->setSeed(seed);
  ...
  case Config: {
    const OCLKernelDataGeneratorConfig *cfg =
        pKernelConfig->GetGeneratorConfig();
    // if there is no OCLKernelDataGeneratorConfig in configuration file or it
    // is incorrect GetGeneratorConfig will return zero value
    if (cfg == 0) {
      throw Exception::InvalidArgument(
          "[OpenCLReferenceRunner]No config is provided. \
  ...
  default: {
    ReadBufferContainer(pKernelConfig->GetInputFilePath(),
                        pKernelConfig->GetInputFileType(), pContainer);
  }
  }
}

void OpenCLReferenceRunner::ReadReferenceBuffer(
    OpenCLKernelConfiguration *pKernelConfig, IContainer *pContainer) {
  assert(NULL != pKernelConfig);
  assert(NULL != pContainer);
  ...
  if (filename.empty()) {
    throw Exception::ParserBadTypeException(
        "ReadReferenceBuffer: reference file name is empty.");
  }

  ReadBufferContainer(pKernelConfig->GetStampedPathReference(),
                      pKernelConfig->GetReferenceFileType(), pContainer);
}

void OpenCLReferenceRunner::ReadNEATBuffer(
    OpenCLKernelConfiguration *pKernelConfig, IContainer *pContainer) {
  assert(NULL != pKernelConfig);
  assert(NULL != pContainer);
  ...
  if (filename.empty()) {
    throw Exception::ParserBadTypeException(
        "ReadNEATBuffer: NEAT file name is empty.");
  }

  ReadBufferContainer(pKernelConfig->GetStampedPathNeat(),
                      pKernelConfig->GetNeatFileType(), pContainer);
}

void OpenCLReferenceRunner::WriteReferenceBuffer(
    OpenCLKernelConfiguration *pKernelConfig, IContainer *pContainer) const {
  assert(NULL != pKernelConfig);
  assert(NULL != pContainer);
...
void OpenCLReferenceRunner::WriteNEATBuffer(
    OpenCLKernelConfiguration *pKernelConfig, IContainer *pContainer) const {
  assert(NULL != pKernelConfig);
  assert(NULL != pContainer);
...
void OpenCLReferenceRunner::ReadKernelArgs(
    std::vector<llvm::GenericValue> &ArgVals, const IBufferContainerList *input,
  ...
  for (std::size_t i = 0; arg_it != m_pKernel->arg_end() && i < numOfArguments;
       ++arg_it, ++i) {
    IMemoryObject *currBuffer = currContainer->GetMemoryObject(i);
    GenericValue currArg;

    if (Image::GetImageName() == currBuffer->GetName()) { // image

      // TODO: This code is almost identical to the next branch. Rewrite it
      // using common function. Kernel argument is an image - need to pass a
      // pointer in the arguments buffer
      ImageDesc imageDesc =
          GetImageDescription(currBuffer->GetMemoryObjectDesc());
      ...
      std::unique_ptr<cl_mem_obj_descriptor> spMemDesc(
          (cl_mem_obj_descriptor *)align_malloc(sizeof(cl_mem_obj_descriptor),
                                                CPU_DEV_MAXIMUM_ALIGN));
      FillMemObjDescriptor(*spMemDesc.get(), imageDesc,
                           outputImage->GetDataPtr(), NULL);
      currArg.PointerVal = spMemDesc.get();
      // push pointer to scratch memory
      m_ClMemObjScratchMemList.push_back(spMemDesc.release());
    } else if (Buffer::GetBufferName() == currBuffer->GetName()) { // buffer
      BufferDesc buffDsc =
          GetBufferDescription(currBuffer->GetMemoryObjectDesc());
      ...
      case Type::FloatTyID: {
        if (buffDsc.GetElementDescription().GetType() != TFLOAT) {
          throw TestReferenceRunnerException(
              "Input data type doesn't match kernel argument type. \
                                                          Input data type: " +
        ...
        {
          throw TestReferenceRunnerException(
              "Input data type doesn't match kernel signature type.. \
                                                          Expected buffer length: 1");
        }
        ((float *)outputBuffer->GetDataPtr())[0] =
            ((float *)currBuffer->GetDataPtr())[0];
      ...
      case Type::DoubleTyID: {
        ...
        ((double *)outputBuffer->GetDataPtr())[0] =
            ((double *)currBuffer->GetDataPtr())[0];
...
#define COPY_INT(TYPE)                                                         \
  ...
  ((TYPE *)outputBuffer->GetDataPtr())[0] =                                    \
      ((TYPE *)currBuffer->GetDataPtr())[0];                                   \
  sign = true
...
#define COPY_UINT(TYPE)                                                        \
  ...
  sign = false
        ...
        currArg.IntVal =
            sign ? APInt(arg_it->getType()->getPrimitiveSizeInBits(),
                         *(uint64_t *)&iVal, true)
...
#undef COPY_UINT
      ...
      throw TestReferenceRunnerException("Not a buffer or image");
  ...
  if (m_bUseNEAT) {
    ...
    // create NEAT "mirror" of OCL program inputs. it is for zero
    // BufferContainer
    IBufferContainer *pBCNeat =
        CreateNEATBufferContainer(*outputContainer, neatArgs);
...
void OpenCLReferenceRunner::ReadIntegerFromBuffer(
    const IMemoryObject *inBuffer, IMemoryObject *outBuffer,
    const std::size_t vectorId, const std::size_t elementInVectorId,
    const unsigned int numOfBits, GenericValue &val) {
  ...
  case TUSHORT: {
    BufferAccessor<uint16_t> intBufferAcc(*inBuffer), outBufferAcc(*outBuffer);
    outBufferAcc.GetElem(vectorId, elementInVectorId) =
        intBufferAcc.GetElem(vectorId, elementInVectorId);
    uVal = intBufferAcc.GetElem(vectorId, elementInVectorId);
  } break;
  ...
  case TUINT: {
    BufferAccessor<uint32_t> intBufferAcc(*inBuffer), outBufferAcc(*outBuffer);
    outBufferAcc.GetElem(vectorId, elementInVectorId) =
        intBufferAcc.GetElem(vectorId, elementInVectorId);
    uVal = intBufferAcc.GetElem(vectorId, elementInVectorId);
  } break;
  ...
  case TULONG: {
    BufferAccessor<uint64_t> intBufferAcc(*inBuffer), outBufferAcc(*outBuffer);
    outBufferAcc.GetElem(vectorId, elementInVectorId) =
        intBufferAcc.GetElem(vectorId, elementInVectorId);
    uVal = intBufferAcc.GetElem(vectorId, elementInVectorId);
  } break;
...
getLocalVariables(llvm::Module *pModule) {
  std::vector<const GlobalVariable *> locList;
  // pass on global variables in module
  for (Module::const_global_iterator I = pModule->global_begin(),
                                     E = pModule->global_end();
       I != E; ++I) {
...
static void ConvertSizeTtoUint64T(const size_t *pI, std::vector<uint64_t> &O,
                                  uint32_t num) {
...
void OpenCLReferenceRunner::RunKernel(IRunResult *runResult,
  ...
  ReadKernelArgs(ArgVals, &input, kernelName,
  ...
  ConvertSizeTtoUint64T(pKernelConfig->GetGlobalWorkSize(), globalWGSizes,
                        workDim);
  ...
  ConvertSizeTtoUint64T(pKernelConfig->GetLocalWorkSize(), localWGSizes,
                        workDim);

  // init work offset workgroup sizes with zeros
  std::vector<uint64_t> GlobalWorkOffset(MAX_WORK_DIM, 0);
  // convert size_t to uint64_t
  ConvertSizeTtoUint64T(pKernelConfig->GetGlobalWorkOffset(), GlobalWorkOffset,
                        workDim);

  // For OpenCL 1.2 and lower work groups are always uniform
  unsigned OCLVersion = CompilationFlags::getCLVersionFromMetadata(m_pModule);
  ...
  wgSizeMustUniform |=
      OCLVersion > 120 && CompilationFlags::hasUniformWGSizeFlag(cFlags);

  // check usage of default local work size ( == 0 )
  for (uint32_t i = 0; i < workDim; ++i) {
    if (localWGSizes[i] == 0) { // set local Work size
      localWGSizes[i] = std::min<uint64_t>(
          globalWGSizes[i], uint64_t(runConfig->GetValue<uint32_t>(
                                RC_COMMON_DEFAULT_LOCAL_WG_SIZE, 0)));
      ...
      if (globalWGSizes[i] % localWGSizes[i] != 0 && wgSizeMustUniform) {
        // SATest could get the value of localSize from .cfg only, so if
        // globalWorkSize is not evenly divisible by localWorkSize, set
        // localWGSizes to 1, then print warning and continue working check
  ...
  // work item storage
  Validation::WorkItemStorage wiStorage(workDim, globalWGSizes, localWGSizes,
                                        GlobalWorkOffset);
  Validation::WorkGroupStorage wgStorage;
  // set wiStorage as interface for OCLBuiltins workitem functions
  OCLBuiltins::WorkItemInterfaceSetter::inst()->SetWorkItemInterface(
      &wiStorage);
  OCLBuiltins::WorkItemInterfaceSetter::inst()->SetWorkGroupInterface(
      &wgStorage);
  ...
  const uint32_t totalLocalWIs =
      localWGSizes[0] * localWGSizes[1] * localWGSizes[2];
  ...
  for (uint64_t idx = 0; idx < totalLocalWIs; ++idx) {
    // GCC 4.7.3 denies to compile temporary (i.e. rvalue)
    // std::unique_ptr<Module) passed to the EngineBuilder ctor. So, create an
    // lvalue std::unique_ptr and pass it to the std::move instead [LLVM 3.6
    // UPGRADE} TODO: calling ExecutionEngine::create moves ownership of
    // llvm::Module from EngineBuilder to ExecutionEngine and we lose control
    // over Module lifetime. Need to figure out a way of creating
    // ExecutionEngine instances without llvm::Module cloning.
    ...
    if (m_bUseNEAT) {
      ...
      InterpreterPluggable *pInterp =
          static_cast<InterpreterPluggable *>(pExecEngine);
  ...
  for (uint64_t tid_z = 0; tid_z < loopGlobalWGSizes[2];
       tid_z += localWGSizes[2]) {
    for (uint64_t tid_y = 0; tid_y < loopGlobalWGSizes[1];
         tid_y += localWGSizes[1]) {
      for (uint64_t tid_x = 0; tid_x < loopGlobalWGSizes[0];
           tid_x += localWGSizes[0]) {

#define FOR3_LOCALWG                                                           \
  for (uint64_t idx = 0, lid_z = 0; lid_z < wiStorage.GetLocalSize(tid_z, 2);  \
       ++lid_z)                                                                \
    for (uint64_t lid_y = 0; lid_y < wiStorage.GetLocalSize(tid_y, 1);         \
         ++lid_y)                                                              \
      for (uint64_t lid_x = 0; lid_x < wiStorage.GetLocalSize(tid_x, 0);       \
           ++lid_x, ++idx)
        ...
        FOR3_LOCALWG {
          ...
          for (uint32_t i = 0; i < localList.size(); ++i) {
            if (idx == 0) { // zero WI allocate space
              void *globalMem = EE->getOrEmitGlobalVariable(localList[i]);
              localMemList.push_back(globalMem);
            } else { // next WIs add mapping
              EE->updateGlobalMapping(localList[i], localMemList[i]);
            }
          }
        }
          ...
          FOR3_LOCALWG {
            ...
            if (Result.IntVal.getBitWidth() == 8 &&
                (Result.IntVal == APInt(8, InterpreterPluggable::BARRIER) ||
                 Result.IntVal ==
                     APInt(8, InterpreterPluggable::BLOCKING_WG_FUNCTION))) {
...
#undef FOR3_LOCALWG
  ...
  for (NEATPlugIn::GlobalAddressMapTy::iterator I = NEATlocalMap.begin(),
                                                E = NEATlocalMap.end();
       I != E; ++I) {
    delete[] (int8_t *)(*I).second;
  }
  if (m_bUseNEAT)
    for (auto *I : localNEATs)
      delete I;
