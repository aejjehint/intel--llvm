...
template <class T> struct ReleaseDeleter {
  void operator()(T *pT) {
    if (pT)
      pT->Release();
  }
...
typedef std::unique_ptr<ICLDevBackendCompilationService,
                        ReleaseDeleter<ICLDevBackendCompilationService>>
    ICLDevBackendCompileServicePtr;
typedef std::unique_ptr<ICLDevBackendExecutionService,
                        ReleaseDeleter<ICLDevBackendExecutionService>>
    ICLDevBackendExecutionServicePtr;
typedef std::unique_ptr<ICLDevBackendImageService,
                        ReleaseDeleter<ICLDevBackendImageService>>
    ICLDevBackendImageServicePtr;
typedef std::unique_ptr<ICLDevBackendProgram_,
                        ReleaseDeleter<ICLDevBackendProgram_>>
    ICLDevBackendProgramPtr;
typedef std::unique_ptr<ICLDevBackendKernel_,
                        ReleaseDeleter<ICLDevBackendKernel_>>
    ICLDevBackendKernelPtr;
...
private:
  ...
  /// @brief Executes OpenCL test kernel (actually executes all the kernels in
  /// the given program)
  ...
  /// @param [IN] pExecutionService back-end execution service to use for
  /// execution
  /// @param [IN] oclConfig OpenCL configuration of the test run.
  /// @param [IN] runConfig OpenCL back-end configuration of the test run.
  void ExecuteKernel(IBufferContainerList &input, IRunResult *runResult,
