...
OpenCLBackendRunner::OpenCLBackendRunner(const BERunOptions &runConfig) {
  ...
  m_pServiceFactory =
      OpenCLBackendWrapper::GetInstance().GetBackendServiceFactory();
  m_RandomDataGeneratorSeed =
...
ICLDevBackendProgram_ *OpenCLBackendRunner::CreateProgram(
    const OpenCLProgram *oclProgram,
    /*const*/ ICLDevBackendCompilationService *pCompileService) {
  assert(pCompileService);
  assert(oclProgram);
  assert(oclProgram->GetProgramContainerSize() > 0 &&
         "Invalid binary buffer.\n");
  ...
  cl_dev_err_code ret =
      pCompileService->CreateProgram(pBinary, uiBinarySize, &pProgram);
...
void OpenCLBackendRunner::BuildProgram(
  ...
  std::string injectedObjectPath =
      runConfig->GetValue<std::string>(RC_BR_OBJECT_FILE, "");
  ...
  if (injectedObjectPath != "") {
    ...
    if (!errorOrBuffer) {
      throw Exception::TestRunnerException(
          "Loading the specified object file \"" + injectedObjectPath +
          "\" has failed.\n");
    }
    injectedObject = std::move(*errorOrBuffer);
    buildProgramOptions.SetInjectedObject(injectedObject->getBufferStart(),
...
void OpenCLBackendRunner::LoadInputBuffer(
  ...
  case Random: {
    ...
    if (pKernelConfig->GetGeneratorConfig() != 0) {
      throw Exception::InvalidArgument(
          "[OpenCLBackendRunner]Unused OCLKernelDataGeneratorConfig found. \
    ...
    argDescriptions = OpenCLKernelArgumentsParser::KernelArgHeuristics(
        argDescriptions, pKernelConfig->GetGlobalWorkSize(),
  ...
  case Config: {
    ...
    if (cfg == 0) {
      throw Exception::InvalidArgument(
          "[OpenCLBackendRunner]No config is provided. \
...
void OpenCLBackendRunner::FillIgnoreList(std::vector<bool> &ignoreList,
                                         const KernelArgument *pKernelArgs,
                                         int kernelNumArgs) {
  ...
  for (int i = 0; i < kernelNumArgs; ++i) {
    ...
    case KRNL_ARG_HALF:   // Argument is a half.
    case KRNL_ARG_VECTOR: // Argument is a vector of basic types, like int8,
                          // float4, etc.
    case KRNL_ARG_VECTOR_BY_REF: //!< Argument is a byval pointer to a vector of
    ...
    case KRNL_ARG_PTR_GLOBAL:
      ...
      ignoreList[i] = false;
      break;
    case KRNL_ARG_PTR_CONST: // Argument is a pointer to buffer declared in
