...
RunResultComparison::~RunResultComparison() {
  for (CompResultsMap::iterator it = m_comparisons.begin();
       it != m_comparisons.end(); ++it) {
    delete it->second;
  }
}
...
void RunResultComparison::AddComparison(const char *name,
                                        IComparisonResults *comparison) {
...
RunResult::~RunResult(void) {
  for (auto &[Key, Value] : m_refOutputs) {
    if (Value) {
      delete Value;
      Value = nullptr;
    }
  ...
  for (auto &[Key, Value] : m_neatOutputs) {
    if (Value) {
      delete Value;
      Value = nullptr;
    }
...
const IBufferContainerList &RunResult::GetOutput(const char *kernelName) const {
  if (m_refOutputs.find(kernelName) == m_refOutputs.end()) {
    throw Exception::InvalidArgument(
        std::string("There are no run results for the kernel: ") + kernelName);
  }
...
const IBufferContainerList &
RunResult::GetNEATOutput(const char *kernelName) const {
  if (m_neatOutputs.find(kernelName) == m_neatOutputs.end()) {
    throw Exception::InvalidArgument(
        std::string("There are no run NEAT results for the kernel: ") +
        kernelName);
  }
...
RunResult::GetComparatorIgnoreList(const char *kernelName) {
...
size_t RunResult::GetOutputsCount() const { return m_refOutputs.size(); }

IPerformance &RunResult::GetPerformance() { return m_performance; }

void RunResult::SetComparatorIgnoreList(const char *kernelName,
                                        const std::vector<bool> &ignoreList) {
  LLVM_DEBUG({
    if (m_comparatorIgnoreList.find(kernelName) !=
        m_comparatorIgnoreList.end()) {
