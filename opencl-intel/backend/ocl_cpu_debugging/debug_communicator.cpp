...
#include <windows.h>
void sleep_ms(int ms) { SleepEx(ms, TRUE); }
...
RETURN_TYPE_ENTRY_POINT DebugCommunicator::Run() {
  ...
  try {
    // Bind the server socket to localhost:port and wait for a client
    // connection. Once a client has connected, we can proceed to processing
    // commands in the communication queue.
    //
    ...
    while (true) {
      ...
      assert((cmd == EXIT || cmd == SEND_MESSAGE || cmd == RECEIVE_MESSAGE) &&
             "Invalid command");
      switch (cmd) {
      case EXIT:
...
ClientToServerMessage DebugCommunicator::receiveMessage() {
  ...
  //
  m_recv_event.Wait();
  assert(!m_msg_recv_queue.IsEmpty());
  return m_msg_recv_queue.PopFront();
}

void DebugCommunicator::waitForConnection() { m_connect_event.Wait(); }

void DebugCommunicator::waitForListen() { m_listen_event.Wait(); }
