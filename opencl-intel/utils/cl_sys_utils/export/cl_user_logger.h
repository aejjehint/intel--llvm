 ...
 * and printing it to the user log at its destruction
 */
class ApiLogger {
public:
  /**
  ...
  ApiLogger(ApiLogger &) = delete;
  ApiLogger &operator=(ApiLogger &) = delete;
  ...
  /**
   * @return the return value from the last API function (if the return value of
   * the function is a pointer, then this method returns CL_SUCCESS if it is not
   * NULL; if there is no return value, then this method always returns
   * CL_SUCCESS)
   */
   ...
   */
  template <typename T> ApiLogger &operator<<(const T &val) {
    if (m_bLogApis) {
      m_strStream << val;
    }
    return *this;
  ...
  /**
   * Print a C-string value in an API call (operator<< cannot be used in this
   * case, because it assumes that the string is a parameter type and name)
   ...
   */
  template <typename T> ApiLogger &PrintArray(cl_uint size, const T *values) {
    if (m_bLogApis) {
      m_strStream << values;
      if (size >= 1 && values) {
        m_strStream << " [";
   ...
   */
  template <typename T> ApiLogger &PrintProperties(const T *properties) {
    if (!properties) {
      m_strStream << "nullptr";
      return *this;
    }
  ...
  /**
   * End logging of an API function with a cl_int return value, which will be
   * printed with the macro's name
...
private:
  ...
  template <typename T> void PrintIntegerOutputParam(const void *ptr) {
    if (nullptr != ptr) {
  ...
  // we use this to collect all data about the API function call, so that when
  // it ends, we'll be able to put its duration before the log of the call
  // itself
...
public:
   ...
   */
  LogMessageWrapper(cl_dev_cmd_id id, const std::string &msg)
      : m_id(id), m_msg(msg) {
    Serialize();
  }
