...
// If the environment variable is set, set LibDirPath to the value of the
// environment variable. If the env variable isn't set, try to search library
// from fallback directory as well.
// Also checks whether LibFileToCheck exists in the directory.
std::string getModuleDirOrOverrideFromEnvOrFallback(
    const std::string &LibFileToCheck, const char *FallbackDir,
    const char *EnvName = "OVERRIDE_OCL_LIBRARY_DIR");
...
/// @brief OpenCL property parsing helper function. TODO: refine OpenCL API
/// implementations with this helper for property parsing.
/// @tparam T property type, e.g. cl_context_properties
/// @param Properties a pointer to the property list
/// @param ParsedMap a map to store the parsed property (name, value) pairs
/// @param ValidProperties an array of valid property names
/// @param ErrcodeRet an optional pointer to return error code
/// @return true on succuess
template <typename T>
bool parseProperties(const T *Properties, std::unordered_map<T, T> &ParsedMap,
                     llvm::ArrayRef<T> ValidProperties,
                     llvm::ArrayRef<T> HighestBitAllowed,
                     cl_int *ErrcodeRet = nullptr,
                     cl_int DefaultErrcode = CL_INVALID_PROPERTY) {
  // It's legal to leave properties as NULL, meaning all properties are default.
  if (!Properties)
    return true;
  // Property list is terminated with 0.
  while (*Properties != 0) {
    T PropName = *Properties++;
    // Check if the property is valid.
    auto It =
        std::find(ValidProperties.begin(), ValidProperties.end(), PropName);
    if (It == ValidProperties.end()) {
      if (ErrcodeRet)
        *ErrcodeRet = DefaultErrcode;
      return false;
    }
    // Check if the property is duplicated.
    if (ParsedMap.find(PropName) != ParsedMap.end()) {
      if (ErrcodeRet)
        *ErrcodeRet = DefaultErrcode;
      return false;
    }
    T PropValue = *Properties++;

    // Check if PropValue exceeds the highest bit allowed.
    unsigned Idx = It - ValidProperties.begin();
    assert(Idx < HighestBitAllowed.size());
    // HighestBitAllowed[Idx] == 0 means no restriction.
    if (HighestBitAllowed[Idx] != 0) {
      assert(llvm::popcount(HighestBitAllowed[Idx]) == 1 && "Invalid mask");
      T AllowedBits = (HighestBitAllowed[Idx] << 1) - 1;
      if (PropValue & ~AllowedBits) {
        if (ErrcodeRet)
          *ErrcodeRet = DefaultErrcode;
        return false;
      }
    }
    ParsedMap[PropName] = PropValue;
  }
  return true;
}
