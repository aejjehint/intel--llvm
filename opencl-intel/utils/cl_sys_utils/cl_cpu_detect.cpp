...
CPUVendor CPUDetect::GetVendor() {
  uint32_t cpuid_info[4];
  CPUID(cpuid_info, 0);
  // CPU's vendor ID is in register EBX, ECX and EDX
  if (cpuid_info[1] == signature_INTEL_ebx &&
      cpuid_info[2] == signature_INTEL_ecx &&
      cpuid_info[3] == signature_INTEL_edx)
    return GenuineIntel;
  if (cpuid_info[1] == signature_AMD_ebx &&
      cpuid_info[2] == signature_AMD_ecx && cpuid_info[3] == signature_AMD_edx)
    return AuthenticAMD;
  return Generic;
}
...
CPUDetect::CPUDetect(void) : m_is64BitOS(sizeof(void *) == 8) {
  ...
  m_HostCPUFeatures = llvm::sys::getHostCPUFeatures();
  GetHostCPUBrandInfo();
