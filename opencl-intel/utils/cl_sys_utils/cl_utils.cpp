...
bool clIsNumaAvailable() { return false; }
...
bool clTranslateAffinityMask(affinityMask_t *mask, unsigned int *IDs,
                             size_t len) {
#ifdef WINDOWS_ONECORE
  assert(0 && "Affinity is not supported on universal windows platform");
  return false;
...
threadid_t clMyThreadId() { return GetCurrentThreadId(); }
...
void clNUMASetLocalNodeAlloc() { numa_set_localalloc(); }
#else
bool clIsNumaAvailable() { return false; }
...
void clSleep(int milliseconds) { usleep(1000 * milliseconds); }
...
void clResetThreadAffinityMask(threadid_t tid) {
  affinityMask_t mask;
  CPU_ZERO(&mask);
  for (size_t i = 0; i < 4 * sizeof(unsigned long long) * 8;
       i++) // we assume no more than 256 HW threads (writing generic code is
            // too much effort)
  {
    CPU_SET(i, &mask);
  }
...
const char *ClErrTxt(cl_err_code error_code) {
  ...
  case (CL_ERR_LOGGER_FAILED):
    return "CL_ERR_LOGGER_FAILED";
  case (CL_ERR_NOT_IMPLEMENTED):
    return "CL_ERR_NOT_IMPLEMENTED";
  case (CL_ERR_NOT_SUPPORTED):
    return "CL_ERR_NOT_SUPPORTED";
  case (CL_ERR_INITILIZATION_FAILED):
    return "CL_ERR_INITILIZATION_FAILED";
  case (CL_ERR_PLATFORM_FAILED):
    return "CL_ERR_PLATFORM_FAILED";
  case (CL_ERR_CONTEXT_FAILED):
    return "CL_ERR_CONTEXT_FAILED";
  case (CL_ERR_EXECUTION_FAILED):
    return "CL_ERR_EXECUTION_FAILED";
  case (CL_ERR_FILE_NOT_EXISTS):
    return "CL_ERR_FILE_NOT_EXISTS";
  case (CL_ERR_KEY_NOT_FOUND):
    return "CL_ERR_KEY_NOT_FOUND";
  case (CL_ERR_KEY_ALLREADY_EXISTS):
    return "CL_ERR_KEY_ALLREADY_EXISTS";
  case (CL_ERR_LIST_EMPTY):
    return "CL_ERR_LIST_EMPTY";
  case (CL_ERR_DEVICE_INIT_FAIL):
    return "CL_ERR_DEVICE_INIT_FAI";
  case (CL_ERR_FE_COMPILER_INIT_FAIL):
    return "CL_ERR_FE_COMPILER_INIT_FAI";
  default:
    return "Unknown Error Code";
  }
}

// used to make a macro definition into a string simply call DEFINE_TO_STRING
// with you defined macro "s"
#define DEFINE_TO_STRING(s) #s

// just return the DEFINE name as is in a string format
#define CASE_DEFINE_RETURN_STRING(def)                                         \
  case (def):                                                                  \
    return #def
...
#define IF_DEFINE_APPEND_STRING_DEFAULT(bitfield, def, str)                    \
  if (((bitfield) & (def)) != 0)                                               \
  str += string(DEFAULT_PREFIX) + #def + string(DEFAULT_SUFFIX)
...
void clCopyMemoryRegion(SMemCpyParams *pCopyCmd) {
  ...
  if (1 == pCopyCmd->uiDimCount) {
    ...
    MEMCPY_S(pCopyCmd->pDst, pCopyCmd->vRegion[0], pCopyCmd->pSrc,
             pCopyCmd->vRegion[0]);
    return;
  }
  ...
  for (unsigned int i = 0; i < pCopyCmd->vRegion[sRecParam.uiDimCount]; ++i) {
    ...
    sRecParam.pSrc =
        sRecParam.pSrc + pCopyCmd->vSrcPitch[sRecParam.uiDimCount - 1];
    sRecParam.pDst =
        sRecParam.pDst + pCopyCmd->vDstPitch[sRecParam.uiDimCount - 1];
  }
}
...
size_t clGetPixelBytesCount(const cl_image_format *pclImageFormat) {
  ...
  szBytesCount =
      clGetChannelTypeBytesCount(pclImageFormat->image_channel_data_type);
...
size_t clGetPixelElementsCount(const cl_image_format *pclImageFormat) {
  ...
  case CL_ABGR:
    if (CL_UNORM_INT8 != pclImageFormat->image_channel_data_type &&
        CL_SNORM_INT8 != pclImageFormat->image_channel_data_type &&
        CL_SNORM_INT16 != pclImageFormat->image_channel_data_type &&
        CL_SIGNED_INT8 != pclImageFormat->image_channel_data_type &&
  ...
  case CL_INTENSITY:
    return 1;
  case CL_DEPTH:
    if (CL_UNORM_INT16 != pclImageFormat->image_channel_data_type &&
        CL_FLOAT !=
            pclImageFormat->image_channel_data_type) // this isn't allowed
    {
      return 0;
    }
    return 1;
...
// ToNarrow
/////////////////////////////////////////////////////////////////////////
string ToNarrow(const wchar_t *s, char dfault, const locale &loc) {
  ostringstream stm;
...
std::string getModuleDirOrOverrideFromEnvOrFallback(
    [[maybe_unused]] const std::string &LibFileToCheck,
    [[maybe_unused]] const char *FallbackDir,
    [[maybe_unused]] const char *EnvName) {
  // check the current module folder first.
  std::string LibDirPath(MAX_PATH, '\0');
  Intel::OpenCL::Utils::GetModuleDirectory(&LibDirPath[0], MAX_PATH);
  LibDirPath.resize(LibDirPath.find_first_of('\0'));

#ifndef INTEL_PRODUCT_RELEASE
  // Try to get the overridden library dir path from env var.
  // The env var is set in lit.cfg file.
  std::string EnvVal;
  bool Success = Utils::getEnvVar(EnvVal, EnvName);
  if (Success && !EnvVal.empty())
    LibDirPath = EnvVal;

  // OpenCL backend or runtime tests and their dependent libs are not in the
  // same folder. When a test is run from command line, above env var might not
  // be set. In this case, try to search library from fallback directory defined
  // by cmake build.
  llvm::SmallString<MAX_PATH> LibFilePath(LibDirPath);
  llvm::sys::path::append(LibFilePath, LibFileToCheck);
  if (!llvm::sys::fs::exists(LibFilePath)) {
    LibDirPath = FallbackDir;
    LibFilePath.assign(FallbackDir);
    llvm::sys::path::append(LibFilePath, LibFileToCheck);
  }

  if (!llvm::sys::fs::exists(LibFilePath)) {
    llvm::dbgs() << "Didn't find " << LibFileToCheck << " in " << LibDirPath
                 << "\n";
    llvm::report_fatal_error(
        "Specified lib directory does not contain the required file");
  }
