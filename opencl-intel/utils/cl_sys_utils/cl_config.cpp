...
BasicCLConfigWrapper::ParseStringToSize(const std::string &userStr) const {
  ...
  if (units == "GB" || units == "G") {
    assert(integer < (std::numeric_limits<cl_ulong>::max() >> 30) &&
           "integer overflow");
    integer = integer << 30;
...
ConfigFile::ConfigFile(const string &filename, string delimiter, string comment,
                       string sentry)
    : m_sDelimiter(delimiter), m_sComment(comment), m_sSentry(sentry) {
...
cl_err_code ConfigFile::ReadFile(const string &fileName, ConfigFile &cfg) {
  ...
  if (!fsInputStream.is_open()) {
    ...
    std::string modulePath = getModuleDirOrOverrideFromEnvOrFallback(
        "cl.cfg", DEFAULT_OCL_LIBRARY_DIR);
    llvm::SmallString<MAX_PATH> configFilePath(modulePath);
    llvm::sys::path::append(configFilePath, fileName);
    fsInputStream.open(configFilePath.c_str());
    if (!fsInputStream.is_open()) {
