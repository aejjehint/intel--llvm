...
//
// each message logged to the logger is translated into LogMessage structure
// which can be propagated between the different handlers.
class LogMessage {
public:
  ...
  LogMessage(ELogLevel eLevel, ELogConfigField eConfig,
  ...
  // returned message includes all message properties (FileName, LineNumber,
  // FunctionName, ThreadID, Body) in a single string.
  char *GetFormattedMessage() { return m_psFormattedMsg; }

private:
  // CreateFormattedMessage
  // construct formatted message from its fields and stores the result into
  // m_formattedMsg
