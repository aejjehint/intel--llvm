...
static void
RunLoopBody1D(size_t begin, size_t end,
              const Intel::OpenCL::Utils::SharedPtr<ITaskSet> &task) {
  size_t firstWGID[MAX_WORK_DIM] = {begin, 0, 0};
  size_t lastWGID[MAX_WORK_DIM] = {end, 0, 0};

  size_t uiNumberOfWorkGroups = end - begin;
  assert(uiNumberOfWorkGroups <= CL_MAX_INT32);

  TBB_PerActiveThreadData *tls =
      TBB_ThreadManager<TBB_PerActiveThreadData>::GetCurrentThreadDescriptor();
  assert((nullptr != tls) &&
  ...
  void *user_local = task->AttachToThread(tls->user_tls, uiNumberOfWorkGroups,
                                          firstWGID, lastWGID);
  if (!user_local) {
    assert(0 && "Thread local execution environment is NULL");
    return;
  ...
  for (size_t k = begin, f = end; k < f; k++) {
    // OpenCL defines dims as (col, row, page)
...
template <class blocked_range_1d> struct TaskLoopBody1D : public TaskLoopBody {
  ...
  void operator()(const blocked_range_1d &r) const {
    RunLoopBody1D(r.begin(), r.end(), m_task);
  }
};
...
bool TBB_ExecutionSchedulers::parallel_execute(
  ...
  if (dimCount == 1 && dims[0] == 1) {
    RunLoopBody1D(0, 1, task);
