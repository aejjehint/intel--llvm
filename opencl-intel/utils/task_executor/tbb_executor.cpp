...
int TBBTaskExecutor::Init(unsigned int uiNumOfThreads, ocl_gpa_data *pGPAData,
  ...
  m_tbbTaskSchedulerHandle =
      std::move(tbb::task_scheduler_handle{tbb::attach{}});
...
void TBBTaskExecutor::Finalize() {
  ...
  for (auto &device : m_devices)
    device->ShutDown();
  m_devices.clear();
  tbb::finalize(m_tbbTaskSchedulerHandle);
}
...
#ifdef _DEBUG
  std::string tbbPath{TBB_LIBNAME_DEBUG};
#else
  std::string tbbPath{TBB_LIBNAME_RELEASE};
#endif

  tbbPath += ".dll";

  std::string modulePath =
      getModuleDirOrOverrideFromEnvOrFallback(tbbPath, DEFAULT_OCL_LIBRARY_DIR);
  ...
  if (m_err != 0) {
    ...
    if (m_err != 0)
      reportWarning(
          ...
          "HKEY_LOCAL_MACHINE\\SOFTWARE\\Intel\\oneAPI\\TBB\\ location. The "
