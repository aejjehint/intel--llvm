...
changeImageCall(llvm::CallInst *CI,
  ...
  Args.push_back(BitCastInst::CreatePointerCast(CI->getArgOperand(0), ArgTys[0],
                                                "", CI->getIterator()));
  for (unsigned i = 1; i < CI->arg_size(); ++i) {
    ...
    if (auto primitiveType = dyn_cast<reflection::PrimitiveType>(
      ...
      if (primitiveType->getPrimitive() == reflection::PRIMITIVE_SAMPLER_T &&
        ...
        auto IntToPtr = new IntToPtrInst(CI->getArgOperand(i), SamplerTy, "",
                                         CI->getIterator());
  ...
  CallInst *New = CallInst::Create(NewF, Args, "", CI->getIterator());
