...
struct IOCLFEBinaryResultDeleter {
  void operator()(IOCLFEBinaryResult *Result) { Result->Release(); }
};
using IOCLFEBinaryResultPtr =
    std::unique_ptr<IOCLFEBinaryResult, IOCLFEBinaryResultDeleter>;
...
const char *GetOpenCLVersionStr(OPENCL_VERSION ver) {
  ...
  case OPENCL_VERSION_3_0:
    return "300";
  default:
    llvm_unreachable("Invalid OpenCL version string");
  }
}
...
#error "Can't define target triple: unknown architecture."
    ...
    // Workaround for CL1.2 compilation where clang is not aware of
    // cl_khr_extended_bit_ops. See
    // https://github.com/llvm/llvm-project/issues/85049 for detail.
    optionsEx << " -Dcl_khr_extended_bit_ops=1";
  }
  ...
  IOCLFEBinaryResult *BinaryResult = nullptr;
  int res = ::Compile(m_pProgDesc->pProgramSource, InputHeaders.data(),
                      ...
                      &BinaryResult);
  IOCLFEBinaryResultPtr spBinaryResult(BinaryResult);
