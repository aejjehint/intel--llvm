...
public:
  ClangFEDiagnosticHandler(raw_string_ostream &OS) : OS(OS) {}
...
static std::unique_ptr<OCLFEBinaryResult>
LinkInternal(const void **pInputBinaries, unsigned int uiNumBinaries,
  ...
  auto pResult = std::make_unique<OCLFEBinaryResult>();
  ...
  context->setDiagnosticHandler(std::make_unique<ClangFEDiagnosticHandler>(OS));
  ...
  for (unsigned int i = 0; i < uiNumBinaries; ++i) {
    ...
    if (!ModuleOr) {
      std::string Message;
      handleAllErrors(std::move(ModuleOr.takeError()),
    ...
    if (i == 0) {
      ...
      if (Linker::linkModules(*composite, std::move(module))) {
        pResult->setLog("linkModules failed: " + ErrStr);
        pResult->setResult(CL_LINK_PROGRAM_FAILURE);
        return pResult;
      }
    }
  ...
  if (uiNumBinaries > 1 &&
    ...
    pResult->setLog("Error: call of function(s) with different signature:\n" +
                    funcSigErr);
    pResult->setResult(CL_LINK_PROGRAM_FAILURE);
    return pResult;
  }
...
int ClangFECompilerLinkTask::Link(IOCLFEBinaryResult **pBinaryResult) {
  ...
  std::unique_ptr<OCLFEBinaryResult> pResult = LinkInternal(
      m_Binaries.data(), m_pProgDesc->uiNumBinaries, m_BinariesSizes.data(),
      m_pProgDesc->pszOptions, &kernelNames);
  ...
  int resultCode = pResult->getResult();
