...
using unique_array_ptr = std::unique_ptr<char, ArrayDeleter>;

// In this file we use CompileTask, LinkTask and PostBuildTask as building
// blocks to create a general build tree.
//
// for a full BuildProgram we use CompileTask -> LinkTask -> DeviceBuildTask ->
// PostBuildTask for a CompileProgram we use CompileTask -> PostBuildTask for a
// LinkProgram we use LinkTask -> DeviceBuildTask -> PostBuildTask
//
// where CompileTask and LinkTask and DeviceBuildTask are created for each
// device and dependent only on the previouse task for the same device.
// PostBuildTask is created once for all the devices and dependent on the
// previous tasks on all the devices.
//
// Since we need the PostBuildTask to be executed even when a build fail in
// order to clean up all the used resources. All the tasks return
// CL_BUILD_SUCCESS even on error and use program state to notify error.

BuildTask::BuildTask(_cl_context_int *context, const SharedPtr<Program> &pProg,
                     const ConstSharedPtr<FrontEndCompiler> &pFECompiler)
    : BuildEvent(context), m_pProg(pProg), m_pFECompiler(pFECompiler) {}

BuildTask::~BuildTask() {}

long BuildTask::Release() { return 0; }
...
bool BuildTask::Launch() {
  return FrameworkProxy::Instance()->ExecuteImmediate(
      SharedPtr<BuildTask>(this));
}

void BuildTask::SetComplete(cl_int returnCode) {
  // We must assing NULL to shared pointers in order to remove reference count
  // and avoid a raise condition during program release
  m_pProg = NULL;
  m_pFECompiler = NULL;
...
bool CompileTask::Execute() {
  ...
  if (m_pDeviceProgram->GetBinaryTypeInternal() ==
          CL_PROGRAM_BINARY_TYPE_COMPILED_OBJECT ||
      m_pDeviceProgram->GetBinaryTypeInternal() ==
  ...
  char *pBinary = nullptr;
  char *pOutCompileLog = nullptr;
  {
    ...
    if (pIL) {
      ...
      m_pFECompiler->ParseSpirv(
          szSource, binarySize, m_sOptions.c_str(), pIL->GetSpecConstCount(),
          pIL->GetSpecConstIds(), pIL->GetSpecConstValues(), &pBinary,
          &uiOutBinarySize, &pOutCompileLog);
    } else {
      if (IsSPIR) {
        m_pFECompiler->MaterializeSPIR(
            m_pDeviceProgram->GetBinaryInternal(),
            m_pDeviceProgram->GetBinarySizeInternal(), &pBinary,
            &uiOutBinarySize, &pOutCompileLog);
      } else {
        assert(szSource != NULL && "Invalid source code");
        m_pFECompiler->CompileProgram(
            szSource, m_uiNumHeaders, m_pszHeaders, m_pszHeadersNames,
            m_sOptions.c_str(), m_pProg->GetContext()->IsFPGAEmulator(),
            &pBinary, &uiOutBinarySize, &pOutCompileLog);
      }
    }
  }
  unique_array_ptr pOutBinary(pBinary);
  unique_array_ptr szOutCompileLog(pOutCompileLog);
  ...
  // compile succeeded
  ElfWriterPtr pElfWriter(CLElfLib::CElfWriter::Create(
      CLElfLib::EH_TYPE_OPENCL_OBJECTS, CLElfLib::EH_MACHINE_NONE,
  ...
  auto ResolveResult = pElfWriter->ResolveBinary(pBinaryData, uiBinarySize);
  unique_array_ptr spBinary(pBinaryData);
  if (ResolveResult != CLElfLib::SUCCESS) {
    // Build failed
    m_pDeviceProgram->SetBuildLogInternal("Compilation failed\n");
    m_pDeviceProgram->SetStateInternal(DEVICE_PROGRAM_COMPILE_FAILED);
    SetComplete(CL_BUILD_SUCCESS);
  ...
  pBinaryData = new char[uiBinarySize];
  ResolveResult = pElfWriter->ResolveBinary(pBinaryData, uiBinarySize);
  spBinary.reset(pBinaryData);
  if (ResolveResult != CLElfLib::SUCCESS) {
    // Build failed
    m_pDeviceProgram->SetBuildLogInternal("Compilation failed\n");
    m_pDeviceProgram->SetStateInternal(DEVICE_PROGRAM_COMPILE_FAILED);
  ...
  m_pDeviceProgram->SetBuildLogInternal("Compilation done\n");
  m_pDeviceProgram->SetBinaryInternal(uiBinarySize, spBinary.get(),
                                      CL_PROGRAM_BINARY_TYPE_COMPILED_OBJECT);
  SetComplete(CL_BUILD_SUCCESS);
  return true;
}

void CompileTask::Cancel() { SetComplete(CL_BUILD_ERROR); }
...
LinkTask::LinkTask(_cl_context_int *context, const SharedPtr<Program> &pProg,
                   const ConstSharedPtr<FrontEndCompiler> &pFECompiler,
                   DeviceProgram *pDeviceProgram,
                   SharedPtr<Program> *ppPrograms, unsigned int uiNumPrograms,
                   const char *szOptions)
    : BuildTask(context, pProg, pFECompiler), m_pDeviceProgram(pDeviceProgram),
      m_ppPrograms(ppPrograms), m_uiNumPrograms(uiNumPrograms),
...
bool LinkTask::Execute() {
  ...
  if (useInputPrograms) {
    ...
    for (unsigned int i = 0; i < m_uiNumPrograms; ++i) {
      arrBinaries[i] =
          m_ppPrograms[i]->GetBinaryInternal(m_pDeviceProgram->GetDeviceId());
      arrBinariesSizes[i] = m_ppPrograms[i]->GetBinarySizeInternal(
          m_pDeviceProgram->GetDeviceId());
    }
  } else {
  ...
  char *pKernelNames = nullptr;
  {
    ...
    m_pFECompiler->LinkProgram(arrBinaries.data(), m_uiNumPrograms,
                               ...
                               &pBinary, &uiOutBinarySize, linkLog, &bIsLibrary,
                               &pKernelNames);
  }
  unique_array_ptr pOutBinary(pBinary);
  unique_array_ptr pKernelNamesPtr(pKernelNames);

  // filter to get all kernels name which can get ArgsInfo
  if (useInputPrograms)
    getNamesOfAllKernelsWithArgsInfo(m_ppPrograms, pKernelNamesPtr.get(),
                                     m_uiNumPrograms,
  ...
  ElfWriterPtr pElfWriter(CLElfLib::CElfWriter::Create(
      bIsLibrary ? CLElfLib::EH_TYPE_OPENCL_LIBRARY
                 : CLElfLib::EH_TYPE_OPENCL_LINKED_OBJECTS,
      CLElfLib::EH_MACHINE_NONE, CLElfLib::EH_FLAG_NONE));
  CLElfLib::SSectionNode sectionNode;
  sectionNode.Name = ".ocl.ir";
  sectionNode.pData = pOutBinary.get();
  sectionNode.DataSize = uiOutBinarySize;
...
void LinkTask::Cancel() { SetComplete(CL_BUILD_ERROR); }

DeviceBuildTask::DeviceBuildTask(_cl_context_int *context,
                                 const SharedPtr<Program> &pProg,
                                 DeviceProgram *pDeviceProgram,
                                 const char *szOptions)
    : BuildTask(context, pProg, NULL), m_pDeviceProgram(pDeviceProgram),
      m_sOptions(szOptions) {}
...
bool DeviceBuildTask::Execute() {
  ...
  cl_dev_err_code err =
      pDeviceAgent->clDevCheckProgramBinary(uiBinarySize, pBinary);
  if (CL_DEV_SUCCESS != err) {
    // Build failed
    m_pDeviceProgram->SetStateInternal(DEVICE_PROGRAM_BUILD_FAILED);
    m_pDeviceProgram->SetBuildLogInternal(
        "Binary is not supported by the device\n");
    SetComplete(CL_BUILD_SUCCESS);
    return true;
  }

  {
    ...
    if (CL_DEV_SUCCESS != err) {
      ...
      m_pDeviceProgram->SetStateInternal(DEVICE_PROGRAM_BUILD_FAILED);
      m_pDeviceProgram->SetBuildLogInternal(
          "Failed to create device program\n");
      SetComplete(CL_BUILD_SUCCESS);
      return true;
    }
    ...
    std::string OptionsLog = std::string("Options used by backend compiler: ") +
                             m_sOptions + std::string("\n");
    m_pDeviceProgram->SetBuildLogInternal(OptionsLog.c_str());

    err = pDeviceAgent->clDevBuildProgram(programHandle, m_sOptions.c_str(),
                                          &build_status);
    if (CL_DEV_SUCCESS != err) {
      m_pDeviceProgram->SetStateInternal(DEVICE_PROGRAM_BUILD_FAILED);
  ...
  assert((CL_BUILD_ERROR == build_status || CL_BUILD_SUCCESS == build_status) &&
         "Unknown build status returned by the device agent");
...
void DeviceBuildTask::Cancel() { SetComplete(CL_BUILD_ERROR); }
...
PostBuildTask::PostBuildTask(_cl_context_int *context,
    ...
    : BuildTask(context, pProg, NULL), m_num_devices(num_devices),
      m_ppDevicePrograms(ppDevicePrograms), m_uiNumHeaders(uiNumHeaders),
      m_ppHeaders(ppHeaders), m_pszHeadersNames(pszHeadersNames),
      m_uiNumBinaries(uiNumBinaries), m_ppBinaries(ppBinaries),
...
bool PostBuildTask::Execute() {
  ...
  if (m_pfn_notify) {
    if (FrameworkUserLogger::GetInstance()->IsApiLoggingEnabled()) {
      ...
      stream << "BuildProgram callback(" << m_pProg->GetHandle() << ", "
             << m_user_data << ")" << std::endl;
...
void PostBuildTask::Cancel() { SetComplete(CL_BUILD_PROGRAM_FAILURE); }

ProgramService::~ProgramService() {}

cl_err_code ProgramService::CompileProgram(
    const SharedPtr<Program> &program, cl_uint num_devices,
    const cl_device_id *device_list, cl_uint num_input_headers,
    SharedPtr<Program> *input_headers, const char **header_include_names,
    const char *options, pfnNotifyBuildDone pfn_notify, void *user_data) {
  ...
  for (unsigned int i = 0; i < uiNumDevices; ++i) {
    SharedPtr<Device> pDevice =
        ppDevicePrograms[i]->GetDevice()->GetRootDevice();

    arrFeCompilers[i] = pDevice->GetFrontEndCompiler();
    char *szUnrecognizedOptions = new char[buildOptions.size() + 1];
    if (!arrFeCompilers[i]->CheckCompileOptions(buildOptions.c_str(),
                                                szUnrecognizedOptions,
                                                buildOptions.size() + 1)) {
  ...
  for (unsigned int i = 0; i < uiNumDevices; ++i) {
    ...
    case DEVICE_PROGRAM_COMPILED: {
      const char *szLastBuildOptions =
          ppDevicePrograms[i]->GetBuildOptionsInternal();

      // We're already compiled. If compile options changed, compile again.
      // Otherwise, report success immediately.
      if (((NULL == szLastBuildOptions) && (NULL == options)) ||
          ((NULL != szLastBuildOptions) && (NULL != options) &&
           (0 == strcmp(szLastBuildOptions, options)))) {
        // No changes in compile options, so effectively the compilation can
        // succeed vacuously
        break;
      }
    ...
    case DEVICE_PROGRAM_SPIRV: {
      ...
      arrCompileTasks[i] = CompileTask::Allocate(
          ...
          const_cast<const char **>(pszHeadersNames), buildOptions.c_str());
  ...
  SharedPtr<PostBuildTask> pPostBuildTask = PostBuildTask::Allocate(
      context, program, uiNumDevices, ppDevicePrograms, num_input_headers,
      input_headers, pszHeadersNames, 0, NULL, pfn_notify, user_data);
...
cl_err_code ProgramService::LinkProgram(
    const SharedPtr<Program> &program, cl_uint num_devices,
    const cl_device_id *device_list, cl_uint num_input_programs,
    SharedPtr<Program> *input_programs, const char *options,
    pfnNotifyBuildDone pfn_notify, void *user_data) {
  if (program->GetNumKernels() > 0) {
    return CL_INVALID_OPERATION;
  }
  ...
  for (unsigned int devIndex = 0; devIndex < uiNumDevices; ++devIndex) {
    ...
    for (unsigned int libIndex = 0; libIndex < num_input_programs; ++libIndex) {
      ...
      cl_program_binary_type clBinaryType =
          input_programs[libIndex]->GetBinaryTypeInternal(clDeviceID);
    ...
    // according to the its all or nothing, either we have binaries for all the
    // devices or none of them
  ...
  for (unsigned int i = 0; i < uiNumDevices; ++i) {
    if (DEVICE_PROGRAM_CUSTOM_BINARY ==
        ppDevicePrograms[i]->GetStateInternal()) {
      // For custom binaries no need call to FE compiler, no compiler or linker
      // services
      continue;
    }
    SharedPtr<Device> pDevice =
    ...
    arrFeCompilers[i] = pDevice->GetFrontEndCompiler();
    char *szUnrecognizedOptions = new char[buildOptions.size() + 1];
    if (!arrFeCompilers[i]->CheckLinkOptions(buildOptions.c_str(),
                                             szUnrecognizedOptions,
                                             buildOptions.size() + 1)) {
      for (cl_uint j = 0; j < uiNumDevices; ++j) {
  ...
  for (unsigned int i = 0; i < uiNumDevices; ++i) {
    ...
    case DEVICE_PROGRAM_INVALID: {
      // program is created specially for linking purposes so no action should
      // have been done yet
      if (arrBuildForDevice[i]) {
        bNeedToBuild = true;

        ppDevicePrograms[i]->SetStateInternal(DEVICE_PROGRAM_FE_LINKING);
        arrLinkTasks[i] = LinkTask::Allocate(
            context, program, arrFeCompilers[i], ppDevicePrograms[i],
            input_programs, num_input_programs, buildOptions.c_str());
    ...
    case DEVICE_PROGRAM_BE_BUILDING:
    default:
      // Program for link should never be in one of these states when calling
      // LinkProgram
      assert(false);
    }
  }
  ...
  SharedPtr<PostBuildTask> pPostBuildTask = PostBuildTask::Allocate(
      context, program, uiNumDevices, ppDevicePrograms, 0, NULL, NULL,
  ...
  if (isFPGAEmulator) {
    try {
      pCreateAutorunKernelsTask =
          CreateAutorunKernelsTask::Allocate(context, program);
...
cl_err_code ProgramService::BuildProgram(SharedPtr<Program> &program,
  ...
  // Device programs may be null. CL_INVALID_DEVICE will be returned according
  // to spec.
  for (cl_uint i = 0; i < uiNumDevices; ++i) {
  ...
  for (unsigned int i = 0; i < uiNumDevices; ++i) {
    ...
    case DEVICE_PROGRAM_BUILD_DONE: {
      ...
      // We're already built. If build options changed, build again. Otherwise,
      // report success immediately.
      if (((NULL == szLastBuildOptions) && (NULL == options)) ||
          ((NULL != szLastBuildOptions) && (NULL != options) &&
           (0 == strcmp(szLastBuildOptions, options)))) {
        // No changes in build options, so effectively the build can succeed
        // vacuously
        break;
      }
    ...
    case DEVICE_PROGRAM_SPIRV: {
      ...
      ppDevicePrograms[i]->SetStateInternal(DEVICE_PROGRAM_FE_COMPILING);
      arrCompileTasks[i] = CompileTask::Allocate(
          context, program, arrFeCompilers[i], ppDevicePrograms[i], 0, NULL,
          NULL, buildOptions.c_str());
    ...
    case DEVICE_PROGRAM_COMPILED: {
      ...
      bNeedToBuild = true;
      ppDevicePrograms[i]->SetStateInternal(DEVICE_PROGRAM_FE_LINKING);
      arrLinkTasks[i] = LinkTask::Allocate(context, program, arrFeCompilers[i],
                                           ppDevicePrograms[i], NULL, 0,
                                           buildOptions.c_str());
      if (NULL == arrLinkTasks[i].GetPtr()) {
        ppDevicePrograms[i]->SetStateInternal(DEVICE_PROGRAM_BUILD_FAILED);
  ...
  SharedPtr<PostBuildTask> pPostBuildTask =
      PostBuildTask::Allocate(context, program, uiNumDevices, ppDevicePrograms,
                              0, NULL, NULL, 0, NULL, pfn_notify, user_data);
...
void CreateAutorunKernelsTask::Cancel() { SetComplete(CL_BUILD_ERROR); }

CreateAutorunKernelsTask::~CreateAutorunKernelsTask() {}

cl_err_code
ProgramService::SetSpecializationConstant(const SharedPtr<Program> &pProgram,
  ...
  if (!pIL->IsSpecConstInfoCached()) {
    ...
    pFECompiler->GetSpecConstInfo(pIL->GetSourceInternal(), pIL->GetSize(),
