...
static cl_ulong getFormatsKey(cl_mem_object_type clObjType,
                              cl_mem_flags clMemFlags);

///////////////////////////////////////////////////////////////////////////////////////////////////
// Context C'tor
///////////////////////////////////////////////////////////////////////////////////////////////////
Context::Context(const cl_context_properties *clProperties,
                 cl_uint uiNumDevices, cl_uint uiNumRootDevices,
                 SharedPtr<FissionableDevice> *ppDevices, logging_fn pfnNotify,
                 void *pUserData, cl_err_code *pclErr,
  ...
  for (cl_uint ui = 0; ui < uiNumDevices; ++ui) {
    if (m_mapDevices.AddObject(ppDevices[ui], false) ==
        CL_ERR_KEY_ALLREADY_EXISTS) {
      continue; // the spec says that duplicate devices specified in devices are
                // ignored
    }
    ...
    // Create a set of all root devices implicitly/explicitly defined in the
    // context.
    ...
    if (ppDevices[ui]->IsRootLevelDevice()) {
      ...
      // GetRootDevice used just for the purpose of casting to
      // SharedPtr<Device>.
  ...
  if (nullptr == clProperties) {
    ...
    while (0 != clProperties[m_uiContextPropCount]) {
      //            m_mapPropertyMap[clProperties[m_uiContextPropCount]] =
      //            clProperties[m_uiContextPropCount+1];
  ...
  //
  // For each device in this context, "create" it (either really create or
  ...
  for (tSetOfDevices::const_iterator iter = pDevices->begin();
    ...
    const cl_err_code err = (*iter)->GetInfo(
        CL_DEVICE_SVM_CAPABILITIES, sizeof(svmCaps), &svmCaps, nullptr);
  ...
  for (tSetOfDevices::const_iterator iter = pDevices->begin();
    ...
    cl_err_code err = (*iter)->GetInfo(CL_DEVICE_HOST_MEM_CAPABILITIES_INTEL,
                                       sizeof(usmCaps), &usmCaps, nullptr);
    if (CL_SUCCEEDED(err) && (usmCaps & CL_UNIFIED_SHARED_MEMORY_ACCESS_INTEL))
    ...
    err =
        (*iter)->GetInfo(CL_DEVICE_SINGLE_DEVICE_SHARED_MEM_CAPABILITIES_INTEL,
                         sizeof(usmCaps), &usmCaps, nullptr);
    if (CL_SUCCEEDED(err) && (usmCaps & CL_UNIFIED_SHARED_MEMORY_ACCESS_INTEL))
      m_bSupportsUsmSharedSingle = true;

    err = (*iter)->GetInfo(CL_DEVICE_CROSS_DEVICE_SHARED_MEM_CAPABILITIES_INTEL,
                           sizeof(usmCaps), &usmCaps, nullptr);
    if (CL_SUCCEEDED(err) && (usmCaps & CL_UNIFIED_SHARED_MEMORY_ACCESS_INTEL))
      m_bSupportsUsmSharedCross = true;

    err = (*iter)->GetInfo(CL_DEVICE_SHARED_SYSTEM_MEM_CAPABILITIES_INTEL,
                           sizeof(usmCaps), &usmCaps, nullptr);
    if (CL_SUCCEEDED(err) && (usmCaps & CL_UNIFIED_SHARED_MEMORY_ACCESS_INTEL))
...
void Context::Cleanup(bool bTerminate) {
  ...
  // Close all device instances. Device will decide whether to close or just
  // decrease ref count
...
Context::~Context() {
  ...
  for (auto it = m_callbackFuncs.rbegin(); it != m_callbackFuncs.rend(); it++) {
...
cl_err_code Context::SetSpecializationConstant(SharedPtr<Program> pProgram,
  ...
  return m_programService.SetSpecializationConstant(pProgram, uiSpecId,
                                                    szSpecSize, pSpecValue);
}

///////////////////////////////////////////////////////////////////////////////////////////////////
// Context::CreateProgramWithSource
///////////////////////////////////////////////////////////////////////////////////////////////////
cl_err_code Context::CreateProgramWithSource(cl_uint uiCount,
  ...
  cl_err_code clErrRet = CL_SUCCESS;
  // create new program object
  SharedPtr<Program> pProgram = ProgramWithSource::Allocate(
      this, uiCount, ppcStrings, szLengths, &clErrRet);
  if (NULL == pProgram.GetPtr()) {
    if (CL_SUCCESS != clErrRet) {
      return clErrRet;
    }
...
cl_err_code Context::CreateProgramForLink(cl_uint IN uiNumDevices,
  ...
  SharedPtr<Program> pProgram =
      ProgramForLink::Allocate(this, uiNumDevices, ppDevices, &clErrRet);
  delete[] ppDevices;
...
///////////////////////////////////////////////////////////////////////////////////////////////////
cl_err_code Context::CompileProgram(
    cl_program IN clProgram, cl_uint IN uiNumDevices,
    const cl_device_id *IN pclDeviceList, cl_uint IN uiNumHeaders,
    const cl_program *IN pclHeaders, const char **IN pszHeadersNames,
    const char *IN szOptions, pfnNotifyBuildDone IN pfn_notify,
    void *IN user_data) {
  ...
  if (0 < uiNumHeaders) {
    ...
    for (unsigned int i = 0; i < uiNumHeaders; ++i) {
      ppHeaders[i] =
          m_mapPrograms.GetOCLObject((_cl_program_int *)pclHeaders[i])
              .DynamicCast<Program>();
  ...
  cl_int clErr = m_programService.CompileProgram(
      pProg, uiNumDevices, pclDeviceList, uiNumHeaders, ppHeaders,
      pszHeadersNames, szOptions, pfn_notify, user_data);
...
///////////////////////////////////////////////////////////////////////////////////////////////////
cl_err_code
Context::LinkProgram(cl_program IN clProgram, cl_uint IN uiNumDevices,
                     const cl_device_id *IN pclDeviceList,
                     cl_uint IN uiNumBinaries, const cl_program *IN pclBinaries,
                     const char *IN szOptions, pfnNotifyBuildDone IN pfn_notify,
                     void *IN user_data) {
  SharedPtr<Program> pProg =
      m_mapPrograms.GetOCLObject((_cl_program_int *)clProgram)
  ...
  if (0 < uiNumBinaries) {
    ...
    for (unsigned int i = 0; i < uiNumBinaries; ++i) {
      ppBinaries[i] =
          m_mapPrograms.GetOCLObject((_cl_program_int *)pclBinaries[i])
              .DynamicCast<Program>();
  ...
  cl_int clErr = m_programService.LinkProgram(
      pProg, uiNumDevices, pclDeviceList, uiNumBinaries, ppBinaries, szOptions,
...
cl_err_code Context::BuildProgram(cl_program IN clProgram,
  ...
  cl_int clErr = m_programService.BuildProgram(
      pProg, uiNumDevices, pclDeviceList, szOptions, pfn_notify, user_data);
...
SharedPtr<FissionableDevice> Context::GetDeviceByIndex(cl_uint uiDeviceIndex) {
  return m_mapDevices.GetObjectByIndex((cl_int)uiDeviceIndex)
...
// Context::GetGPAData
///////////////////////////////////////////////////////////////////////////////////////////////////
ocl_gpa_data *Context::GetGPAData() const { return m_pGPAData; }

///////////////////////////////////////////////////////////////////////////////////////////////////
// Context::CreateProgramWithBinary
///////////////////////////////////////////////////////////////////////////////////////////////////
cl_err_code Context::CreateProgramWithBinary(cl_uint uiNumDevices,
  ...
  // create program object
  SharedPtr<Program> pProgram =
      ProgramWithBinary::Allocate(this, uiNumDevices, ppDevices, pszLengths,
                                  ppBinaries, piBinaryStatus, &clErrRet);
  delete[] ppDevices;
...
cl_err_code Context::CreateProgramWithBuiltInKernels(
  ...
  // create program object
  SharedPtr<Program> pProgram = ProgramWithBuiltInKernels::Allocate(
      this, uiNumDevices, ppDevices, szKernelNames, &clErrRet);
  delete[] ppDevices;
...
cl_err_code Context::CreateBuffer(cl_mem_flags clFlags, size_t szSize,
  ...
  clErr = MemoryObjectFactory::GetInstance()->CreateMemoryObject(
      m_devTypeMask, CL_MEM_OBJECT_BUFFER, CL_MEMOBJ_GFX_SHARE_NONE, this,
      ppBuffer);
  if (CL_FAILED(clErr)) {
  ...
  clErr = (*ppBuffer)->Initialize(clFlags, nullptr, 1, &szSize, nullptr,
                                  pHostPtr, 0);
  if (CL_FAILED(clErr)) {
...
///////////////////////////////////////////////////////////////////////////////////////////////////
cl_err_code Context::CreateSubBuffer(SharedPtr<MemoryObject> pBuffer,
  ...
  if (!(clFlags & (CL_MEM_READ_ONLY | CL_MEM_WRITE_ONLY | CL_MEM_READ_WRITE))) {
    clFlags |=
        (pflags & (CL_MEM_READ_ONLY | CL_MEM_WRITE_ONLY | CL_MEM_READ_WRITE));
  }
  if (!(clFlags & (CL_MEM_HOST_NO_ACCESS | CL_MEM_HOST_READ_ONLY |
                   CL_MEM_HOST_WRITE_ONLY))) {
    clFlags |= (pflags & (CL_MEM_HOST_NO_ACCESS | CL_MEM_HOST_READ_ONLY |
                          CL_MEM_HOST_WRITE_ONLY));
  }
  // These flags cannot be specified in flags but are inherited from the
  // corresponding memory access qualifiers associated with buffer.
  clFlags |= pflags & (CL_MEM_COPY_HOST_PTR | CL_MEM_USE_HOST_PTR |
                       CL_MEM_ALLOC_HOST_PTR);

  cl_err_code clErr;
  clErr = pBuffer->CreateSubBuffer(clFlags, buffer_create_type,
                                   buffer_create_info, ppBuffer, RequireAlign);
  if (CL_FAILED(clErr)) {
    LOG_ERROR(TEXT("Error initializing sub buffer, returned: %s"),
              ClErrTxt(clErr));
    return clErr;
  }
...
///////////////////////////////////////////////////////////////////////////////////////////////////
cl_err_code Context::CreateImageArray(cl_mem_flags clFlags,
                                      const cl_image_format *pclImageFormat,
                                      void *pHostPtr,
                                      const cl_image_desc *pClImageDesc,
                                      SharedPtr<MemoryObject> *ppImageArr) {
  assert(nullptr != ppImageArr);
  assert(CL_MEM_OBJECT_IMAGE1D_ARRAY == pClImageDesc->image_type ||
         CL_MEM_OBJECT_IMAGE2D_ARRAY == pClImageDesc->image_type);

  if (pClImageDesc->image_array_size < 1 ||
      pClImageDesc->image_array_size > m_szArraySize ||
      pClImageDesc->image_width < 1 ||
      pClImageDesc->image_width > m_sz2dWidth ||
      (CL_MEM_OBJECT_IMAGE2D_ARRAY == pClImageDesc->image_type &&
       (pClImageDesc->image_height < 1 ||
        pClImageDesc->image_height > m_sz2dHeight))) {
    return CL_INVALID_IMAGE_DESCRIPTOR;
  }

  const size_t pixelBytesCnt = clGetPixelBytesCount(pclImageFormat);
  // handle the mess in the spec, where for 1D image array the slice pitch
  // defines the size in bytes of each 1D image
      ...
      CL_MEM_OBJECT_IMAGE1D_ARRAY == pClImageDesc->image_type
          ...
          (0 == pClImageDesc->image_slice_pitch
               ? (0 == pClImageDesc->image_row_pitch
                      ? pClImageDesc->image_width * pixelBytesCnt
                      : pClImageDesc->image_row_pitch)
  ...
  const size_t szPitchDim2 =
      CL_MEM_OBJECT_IMAGE1D_ARRAY == pClImageDesc->image_type ? 0
      : 0 == pClImageDesc->image_slice_pitch
          ? szPitchDim1 * pClImageDesc->image_height
          : pClImageDesc->image_slice_pitch;

  cl_err_code clErr = CheckSupportedImageFormatByMemFlags(
      clFlags, *pclImageFormat, pClImageDesc->image_type);
  if (CL_FAILED(clErr)) {
    return clErr;
  }

  // flags and imageFormat are validated by Image2D and MemoryObject contained
  // inside Image2DArray and hostPtr by MemoryObject::initialize.
  clErr = MemoryObjectFactory::GetInstance()->CreateMemoryObject(
      m_devTypeMask, pClImageDesc->image_type, CL_MEMOBJ_GFX_SHARE_NONE, this,
      ppImageArr);
  if (CL_FAILED(clErr)) {
  ...
  if (CL_MEM_OBJECT_IMAGE1D_ARRAY == pClImageDesc->image_type) {
    ...
    clErr =
        (*ppImageArr)
            ->Initialize(clFlags, pclImageFormat, 2, dim, pitch, pHostPtr, 0);
  } else {
    dim[1] = pClImageDesc->image_height;
    dim[2] = pClImageDesc->image_array_size;
    clErr =
        (*ppImageArr)
            ->Initialize(clFlags, pclImageFormat, 3, dim, pitch, pHostPtr, 0);
  }
  if (CL_FAILED(clErr)) {
    LOG_ERROR(TEXT("Error Initialize new buffer, returned: %s"),
              ClErrTxt(clErr));
...
cl_err_code Context::GetSupportedImageFormats(cl_mem_flags clFlags,
  ...
  if (!(clFlags & (CL_MEM_READ_WRITE | CL_MEM_READ_ONLY | CL_MEM_WRITE_ONLY |
    ...
    if (!(clFlags & (CL_MEM_USE_HOST_PTR | CL_MEM_ALLOC_HOST_PTR |
  ...
  for (cl_uint ui = 0; ui < m_mapDevices.Count(); ++ui) {
    clErr = m_ppAllDevices[ui]->GetDeviceAgent()->clDevGetSupportedImageFormats(
...
///////////////////////////////////////////////////////////////////////////////////////////////////
cl_err_code
Context::GetMaxImageDimensions(size_t &psz2dWidth, size_t &psz2dHeight,
  ...
  size_t sz3dWith = 0, sz3dHeight = 0, szMax3dWith = 0, szMax3dHeight = 0,
         sz3dDepth = 0, szMax3dDepth = 0;
  ...
  for (cl_uint ui = 0; ui < m_mapDevices.Count(); ++ui) {
    ...
    clErr = pDevice->GetInfo(CL_DEVICE_IMAGE2D_MAX_WIDTH, sizeof(size_t),
                             &sz2dWith, nullptr);
    if (CL_SUCCEEDED(clErr)) {
      szMax2dWith =
          ((0 == ui) || (sz2dWith < szMax2dWith)) ? sz2dWith : szMax2dWith;
    }
    clErr = pDevice->GetInfo(CL_DEVICE_IMAGE2D_MAX_HEIGHT, sizeof(size_t),
                             &sz2dHeight, nullptr);
    if (CL_SUCCEEDED(clErr)) {
      szMax2dHeight = ((0 == ui) || (sz2dHeight < szMax2dHeight))
                          ? sz2dHeight
                          : szMax2dHeight;
    }
    clErr = pDevice->GetInfo(CL_DEVICE_IMAGE3D_MAX_WIDTH, sizeof(size_t),
                             &sz3dWith, nullptr);
    if (CL_SUCCEEDED(clErr)) {
      szMax3dWith =
          ((0 == ui) || (sz3dWith < szMax3dWith)) ? sz3dWith : szMax3dWith;
    }
    clErr = pDevice->GetInfo(CL_DEVICE_IMAGE3D_MAX_HEIGHT, sizeof(size_t),
                             &sz3dHeight, nullptr);
    if (CL_SUCCEEDED(clErr)) {
      szMax3dHeight = ((0 == ui) || (sz3dHeight < szMax3dHeight))
                          ? sz3dHeight
                          : szMax3dHeight;
    }
    clErr = pDevice->GetInfo(CL_DEVICE_IMAGE3D_MAX_DEPTH, sizeof(size_t),
                             &sz3dDepth, nullptr);
    if (CL_SUCCEEDED(clErr)) {
      szMax3dDepth =
          ((0 == ui) || (sz3dDepth < szMax3dDepth)) ? sz3dDepth : szMax3dDepth;
    }
    clErr = pDevice->GetInfo(CL_DEVICE_IMAGE_MAX_ARRAY_SIZE, sizeof(size_t),
                             &szArraySize, nullptr);
    if (CL_SUCCEEDED(clErr)) {
      szMaxArraySize = ((0 == ui) || (szArraySize < szMaxArraySize))
                           ? szArraySize
                           : szMaxArraySize;
    }
    clErr = pDevice->GetInfo(CL_DEVICE_IMAGE_MAX_BUFFER_SIZE, sizeof(size_t),
                             &sz1dImgBufSize, nullptr);
    if (CL_SUCCEEDED(clErr)) {
      szMax1dImgBufSize = 0 == ui || sz1dImgBufSize < szMax1dImgBufSize
                              ? sz1dImgBufSize
                              : szMax1dImgBufSize;
    }
  }
...
///////////////////////////////////////////////////////////////////////////////////////////////////
cl_err_code Context::CreateSampler(cl_bool bNormalizedCoords,
  ...
  cl_err_code clErr = pSampler->Initialize(this, bNormalizedCoords,
                                           clAddressingMode, clFilterMode);
  if (CL_FAILED(clErr)) {
    LOG_ERROR(TEXT("Error creating new Sampler, returned: %s"),
              ClErrTxt(clErr));
    pSampler->Release();
    return clErr;
  }
...
SharedPtr<Sampler> Context::GetSampler(cl_sampler clSamplerId) {
  return m_mapSamplers.GetOCLObject((_cl_sampler_int *)clSamplerId)
      .DynamicCast<Sampler>();
}
...
SharedPtr<Device> *
Context::GetExplicitlyAssociatedRootDevices(cl_uint *puiNumDevices) {
  if (nullptr != puiNumDevices) {
...
cl_dev_subdevice_id Context::GetSubdeviceId(cl_device_id id) {
  SharedPtr<FissionableDevice> pDevice =
      m_mapDevices.GetOCLObject((_cl_device_id_int *)id)
          .DynamicCast<FissionableDevice>();
  if (NULL == pDevice.GetPtr()) {
    return 0;
  }
...
///////////////////////////////////////////////////////////////////////////////////////////////////
cl_err_code
Context::CheckSupportedImageFormat(const cl_image_format *pclImageFormat,
                                   cl_mem_flags clMemFlags,
                                   cl_mem_object_type clObjType) {
  // Check for invalid format
  if (nullptr == pclImageFormat ||
      (0 == clGetPixelBytesCount(pclImageFormat))) {
  ...
  { // Critical section
    ...
    if (m_mapSupportedFormats.end() == mapIT) {
      ...
      mapIT = m_mapSupportedFormats.find(key);
      assert(m_mapSupportedFormats.end() != mapIT && "image format not found");
    }
  }
  ...
  while (mapIT->second.end() != listIT) {
    if ((pclImageFormat->image_channel_order == listIT->image_channel_order) &&
        (pclImageFormat->image_channel_data_type ==
         listIT->image_channel_data_type)) {
      return CL_SUCCESS;
    }
    listIT++;
  }
 ...
 */
size_t Context::CalculateSupportedImageFormats(const cl_mem_flags clMemFlags,
                                               cl_mem_object_type clObjType) {
  // Calculate supported format key
  cl_mem_flags key = getFormatsKey(clObjType, clMemFlags);
  ...
  for (rDev = rDevSet->begin(); rDev != rDevSet->end(); ++rDev) {
    cl_uint devSpecificFormatsCount(0);

    // find number of formats to expect.
    clErr = (*rDev)->GetDeviceAgent()->clDevGetSupportedImageFormats(
        clMemFlags, clObjType, 0, nullptr, &devSpecificFormatsCount);
    if (CL_FAILED(clErr)) {
      exitWithErr = true;
      break;
    }
    assert(devSpecificFormatsCount > 0 && "Image format count should not be 0");
    ...
    // get formats
    clErr = (*rDev)->GetDeviceAgent()->clDevGetSupportedImageFormats(
        clMemFlags, clObjType, devSpecificFormatsCount, pFormats, nullptr);
    if (CL_FAILED(clErr)) {
      exitWithErr = true;
      break;
    }
  ...
  if (!exitWithErr) {
    if (rDev == rDevSet->begin() && rDev == rDevSet->end()) {
      assert(0 &&
             "CalculateSupportedImageFormats: No root devices for context.");
      exitWithErr = true;
    }
  }
...
bool compareImageFormats(cl_image_format f1, cl_image_format f2) {
  if (f1.image_channel_data_type < f2.image_channel_data_type)
    return true;
  if (f1.image_channel_data_type > f2.image_channel_data_type)
    return false;

  // here only if image_channel_data_type is equal.
  if (f1.image_channel_order < f2.image_channel_order)
    return true;
...
void Context::RecycleOSEvent(
    Intel::OpenCL::Utils::OclOsDependentEvent *pEvent) {
...
cl_err_code Context::CheckSupportedImageFormatByMemFlags(
    cl_mem_flags clFlags, const cl_image_format &clImageFormat,
    cl_mem_object_type objType) {
  assert(((CL_MEM_WRITE_ONLY | CL_MEM_READ_ONLY) & clFlags) !=
  ...
  if (0 == (CL_MEM_WRITE_ONLY & clFlags)) {
    const cl_err_code clErr =
        CheckSupportedImageFormat(&clImageFormat, CL_MEM_READ_ONLY, objType);
    if (CL_FAILED(clErr)) {
  ...
  if (0 == (CL_MEM_READ_ONLY & clFlags)) {
    const cl_err_code clErr =
        CheckSupportedImageFormat(&clImageFormat, CL_MEM_WRITE_ONLY, objType);
    if (CL_FAILED(clErr)) {
      LOG_ERROR(TEXT("Image format not supported: %S"), ClErrTxt(clErr));
      return clErr;
    }
...
cl_int Context::SetKernelArgSVMPointer(const SharedPtr<Kernel> &pKernel,
                                       cl_uint uiArgIndex,
...
cl_int Context::SetKernelExecInfo(const SharedPtr<Kernel> &pKernel,
  ...
  case CL_KERNEL_EXEC_INFO_SVM_FINE_GRAIN_SYSTEM: {
    ...
    pKernel->SetSvmFineGrainSystem(CL_TRUE == *(const cl_bool *)pParamValue);
    break;
  }
  case CL_KERNEL_EXEC_INFO_USM_PTRS_INTEL: {
    ...
    for (size_t i = 0; i < szParamValueSize / sizeof(void *); i++) {
      ...
      if (nullptr == buf.GetPtr()) {
        ...
        void *usmPtr = USMAlloc(CL_MEM_TYPE_SHARED_INTEL, nullptr, nullptr,
  ...
  case CL_KERNEL_EXEC_INFO_INDIRECT_HOST_ACCESS_INTEL: {
    ...
    if (CL_TRUE == *(const cl_bool *)pParamValue &&
      ...
      return CL_INVALID_OPERATION;
    pKernel->SetUsmIndirectHost(CL_TRUE == *(const cl_bool *)pParamValue);
    break;
  }
  case CL_KERNEL_EXEC_INFO_INDIRECT_DEVICE_ACCESS_INTEL: {
    ...
    if (CL_TRUE == *(const cl_bool *)pParamValue &&
      ...
      return CL_INVALID_OPERATION;
    pKernel->SetUsmIndirectDevice(CL_TRUE == *(const cl_bool *)pParamValue);
    break;
  }
...
void *Context::SVMAlloc(cl_svm_mem_flags flags, size_t size,
                        unsigned int uiAlignment) {
  const cl_svm_mem_flags oldMemFlags =
  ...
  for (tSetOfDevices::const_iterator iter = devices.begin();
    ...
    cl_err_code err =
        (*iter)->GetInfo(CL_DEVICE_SVM_CAPABILITIES, sizeof(devSvmCapabilities),
                         &devSvmCapabilities, nullptr);
    ASSERT_RET_VAL(CL_SUCCEEDED(err),
                   "device can't handle CL_DEVICE_SVM_CAPABILITIES query",
                   nullptr);
  ...
  pSvmBuf->Initialize(flags &
                          ~(CL_MEM_SVM_FINE_GRAIN_BUFFER | CL_MEM_SVM_ATOMICS),
                      nullptr, 1, &size, nullptr, nullptr, 0);

  OclAutoWriter mu(&m_svmBuffersRwlock);
  void *pSvmPtr = pSvmBuf->GetBackingStoreData();
  m_svmBuffers[pSvmPtr] = pSvmBuf;
  ASSERT_RET_VAL(IS_ALIGNED_ON(pSvmPtr, uiAlignment),
                 "pSvmPtr isn't aligned as user requested", nullptr);
  return pSvmPtr;
}
...
SharedPtr<SVMBuffer> Context::GetSVMBufferContainingAddr(void *ptr) {
  ...
  std::map<void *, SharedPtr<SVMBuffer>>::iterator iter =
      m_svmBuffers.upper_bound(ptr);
...
ConstSharedPtr<SVMBuffer>
Context::GetSVMBufferContainingAddr(const void *ptr) const {
  return const_cast<Context *>(this)->GetSVMBufferContainingAddr(
      const_cast<void *>(ptr));
}
...
Context::ParseUSMAllocProperties(const cl_mem_properties_intel *properties,
                                 cl_int *errcode_ret) {
  ...
  if (nullptr != properties) {
    ...
    if (propertyMap.find(CL_MEM_ALLOC_FLAGS_INTEL) != propertyMap.end()) {
      cl_mem_alloc_flags_intel property = propertyMap[CL_MEM_ALLOC_FLAGS_INTEL];
...
void *Context::USMAlloc(cl_unified_shared_memory_type_intel type,
                        ...
                        void *userPtr, unsigned int alignment,
                        cl_int *errcode) {
  OclAutoWriter mu(&m_usmBuffersRwlock);
  ...
  cl_mem_alloc_flags_intel flags = ParseUSMAllocProperties(properties, &err);
  if (CL_SUCCESS != err)
    USM_ALLOC_ERR_RET(err);
  ...
  for (cl_uint i = 0; i < numDevices; i++) {
    ...
    if (CL_SUCCESS != err)
      USM_ALLOC_ERR_RET(err);
    err =
        rootDevice->GetInfo(CL_DEVICE_SHARED_SYSTEM_MEM_CAPABILITIES_INTEL,
  ...
  // these flags aren't needed anymore
  err = usmBuf->Initialize(flags & ~CL_MEM_ALLOC_WRITE_COMBINED_INTEL, nullptr,
                           1, &size, nullptr, userPtr, 0, forceAlignment);
  if (CL_SUCCESS != err)
    USM_ALLOC_ERR_RET(err);
...
void *Context::USMHostAlloc(const cl_mem_properties_intel *properties,
                            size_t size, cl_uint alignment, cl_int *errcode) {
  cl_unified_shared_memory_type_intel type = CL_MEM_TYPE_HOST_INTEL;
  return USMAlloc(type, nullptr, properties, size, nullptr, alignment, errcode);
}
...
void *Context::USMDeviceAlloc(cl_device_id device,
  ...
  return USMAlloc(type, device, properties, size, nullptr, alignment, errcode);
}

cl_int Context::USMDeviceAllocGlobalVariable(cl_device_id device, size_t gvSize,
                                             void *gvPtr) {
  cl_unified_shared_memory_type_intel type = CL_MEM_TYPE_DEVICE_INTEL;
  cl_int errcode;
...
cl_int Context::GetMemAllocInfoINTEL(const void *ptr,
  ...
  case CL_MEM_ALLOC_BASE_PTR_INTEL:
    ...
    basePtr = (type == CL_MEM_TYPE_UNKNOWN_INTEL) ? nullptr : buffer->GetAddr();
    value = &basePtr;
    break;
...
SharedPtr<USMBuffer> Context::GetUSMBufferContainingAddr(void *ptr) {
  ...
  if (((char *)ptr >= (char *)usmBuffer->GetAddr()) &&
    ...
    LOG_INFO(TEXT("GetUSMBufferContainingAddr ptr %p, usmBuffer %p, type %d"),
...
ConstSharedPtr<USMBuffer>
Context::GetUSMBufferContainingAddr(const void *ptr) const {
...
cl_err_code Context::CreatePipe(cl_mem_flags flags, cl_uint uiPipePacketSize,
                                ...
                                void *pHostPtr) {
  cl_err_code err = MemoryObjectFactory::GetInstance()->CreateMemoryObject(
      m_devTypeMask, CL_MEM_OBJECT_PIPE, CL_MEMOBJ_GFX_SHARE_NONE, this,
      &pPipe);
  if (CL_FAILED(err)) {
    return err;
  }
  err = pPipe.StaticCast<Pipe>()->Initialize(flags, uiPipePacketSize,
                                             uiPipeMaxPackets, pHostPtr);
  if (CL_FAILED(err)) {
    return err;
  }
