...
public:
  // Constructor
  DeviceKernel(
      Kernel *pKernel, const SharedPtr<FissionableDevice> &pDevice,
      cl_dev_program devProgramId,
      std::shared_ptr<Intel::OpenCL::Utils::LoggerClient> &pLoggerClient,
      cl_err_code *pErr);
  ...
  // get prefetched info
  const size_t *GetKernelCompileWorkGroupSize() const {
    return m_CL_KERNEL_COMPILE_WORK_GROUP_SIZE;
  }
  cl_ulong GetKernelLocalMemSize() const { return m_CL_KERNEL_LOCAL_MEM_SIZE; }
  cl_bool GetKernelNonUniformWGSizeSupport() const {
    return m_CL_KERNEL_NON_UNIFORM_WG_SIZE_SUPPORT;
  }
  size_t GetKernelArgBufferSize() const {
    return m_sKernelPrototype.m_dispatchBufferProperties.size -
           m_sKernelPrototype.m_dispatchBufferProperties.argumentOffset;
  }
  size_t GetKernelDispatchBufferSize() const {
    return m_sKernelPrototype.m_dispatchBufferProperties.size;
  }
  size_t GetArgumentOffset() const {
    return m_sKernelPrototype.m_dispatchBufferProperties.argumentOffset;
  }
  size_t GetKernelArgBufferAlignment() const {
    return m_sKernelPrototype.m_dispatchBufferProperties.alignment;
  }
...
private:
  ...
  // parent kernel
  // We hold a regular pointer to Kernel rather than a SharedPtr, because Kernel
  // points to DeviceKerel and otherwise we would have a circular reference.
  Kernel *m_pKernel;
...
public:
  ...
  size_t GetLocalBufferSize() const {
    assert(IsLocalPtr() && (NULL != m_pValueLocation) &&
           "Not a local PTR or value location is not set");
    return *(size_t *)m_pValueLocation;
  }
...
public:
   ...
   *
   * Description: returns information about the kernel object that may be
   *              specific to a device
   *
   * Arguments: device [in] identifies a specific device in the list of devices
   *                        associated with
  ...
  cl_err_code GetKernelSuggestedLocalWorkSizeKHR(
      cl_command_queue command_queue, cl_uint work_dim,
  ...
  cl_err_code SetKernelArg(cl_uint uiIndex, size_t szSize, const void *pValue,
                           bool bIsSvmPtr = false, bool bIsUsmPtr = false);

  // returns the number of arguments in the kernel
  size_t GetKernelArgsCount() const {
    return m_sKernelPrototype.m_vArguments.size();
  }

  // returns memory object argument number in the kernel
  size_t GetKernelMemoryArgsCount() const {
    return m_sKernelPrototype.m_MemArgumentsIndx.size();
  }

  // Return true if all kernel arguments were specified
  bool IsValidKernelArgs() const {
    return m_numValidArgs == m_sKernelPrototype.m_vArguments.size();
  }
  ...
  const KernelArg *GetKernelArg(size_t uiIndex) const {
    assert(uiIndex < m_sKernelPrototype.m_vArguments.size() &&
           "Invalide argument index");
    return &(m_vecArgs[uiIndex]);
  }

  const KernelArg *GetKernelMemoryArg(size_t uiIndex) const {
    assert(uiIndex < m_sKernelPrototype.m_MemArgumentsIndx.size() &&
           "Invalide argument index");
  ...
  const char *GetName() const {
    return m_sKernelPrototype.m_szKernelName.c_str();
  }
   ...
   *            pParamValue [out] parameter's value
   *            pszParamValueSizeRet [out] parameter's value return size
   * Return value: CL_INVALID_ARG_INDEX - if arg_indx is not a valid argument
   *                                      index and param_name is not
   *                                      CL_KERNEL_ATTRIBUTES.
   *               CL_INVALID_VALUE - if param_name is not valid, or if size in
   *                                  bytes specified by param_value size is <
   *                                  size of return type as described in table
   *                                  5.17 and param_value is not NULL.
  ...
  /**
   * Set whether this Kernel uses pointers that are fine grain system SVM
   * allocations
   * @param bSvmFineGrainSystem whether this Kernel uses pointers that are fine
   * grain system SVM allocations
   */
  void SetSvmFineGrainSystem(bool bSvmFineGrainSystem) {
    m_bSvmFineGrainSystem = bSvmFineGrainSystem;
  };

  /**
   * @return whether this Kernel uses pointers that are fine grain system SVM
   * allocations
   */
  ...
  /**
   * @param svmBufs a vector of SVMBuffers that are used by Kernel, but not
   ...
   */
  void SetNonArgUsmBuffers(const std::vector<SharedPtr<USMBuffer>> &usmBufs);

  /**
   * @param usmBufs a vector of indirect USMBuffers that are used by
   * Kernel
   */
  void GetNonArgUsmBuffers(std::vector<SharedPtr<USMBuffer>> &usmBufs) const;
...
protected:
  ...
  // Kernel prototype
  cl_err_code SetKernelPrototype(const SKernelPrototype &sKernelPrototype,
