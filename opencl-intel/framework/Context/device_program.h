...
enum EDeviceProgramState {
  ...
  DEVICE_PROGRAM_CREATING_AUTORUN, // Currently creating instances of autorun
                                   // kernels
...
public:
  ...
  // Attempts to attach the given binary to the device associated with this
  // program Creates a copy of the input Returns CL_SUCCESS if nothing
  // unexpected happened -- note that iBinaryStatus can still be
  // CL_INVALID_BINARY
  cl_err_code SetBinary(size_t uiBinarySize, const unsigned char *pBinary,
  ...
  cl_err_code GetBuildInfo(cl_program_build_info clParamName,
                           size_t uiParamValueSize, void *pParamValue,
  ...
  cl_err_code GetKernelNames(char **ppNames, size_t *pszNameSizes,
                             size_t szNumNames);
  ...
  cl_program_binary_type GetBinaryTypeInternal() const {
    return m_clBinaryBitsType;
  };
  ...
  // Clears the current build log, called in the beginning of each build
  // sequence Returns CL_SUCCESS if nothing unexpected happened
  ...
  bool CheckProgramBinary(size_t uiBinSize, const void *pBinary,
                          cl_prog_binary_type *pBinaryType = nullptr);

  cl_int GetFunctionPointer(const char *func_name, cl_ulong *func_pointer_ret);
  ...
  const std::map<std::string, cl_prog_gv> &GetGlobalVariablePointers() const {
    return m_gvPointers;
  }

  // Query arguments info from cached kernel args info map.
  cl_err_code
  queryKernelArgsInfo(const char *KernelName,
                      std::vector<cl_kernel_argument_info> &ArgsInfo);

protected:
  // Initialize args info for all kernels of current program. Args info is
  // retrieved from IR module parsed from bitcode binary bits.
  // 1. If containing binary is already LLVM bitcode, the bitcode will be
  //    parsed.
  // 2. If containing binary is ELF object, the LLVM bitcode will be extracted
  //    from ".ocl.ir" section.
  void initializeAllKernelArgsInfoFromIRData();
...
protected:
  ...
  // Cached kernel argument infos
  std::unordered_map<std::string, std::vector<cl_kernel_argument_info>>
      m_KernelArgsInfo;
  bool m_bKernelArgsInfoCached{false};
  std::mutex m_KernelArgsInfoMutex;
};
} // namespace Framework
} // namespace OpenCL
} // namespace Intel
