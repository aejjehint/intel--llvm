...
ContextModule::CreateContext(const cl_context_properties *clProperties,
  ...
  if (FrameworkProxy::Instance()->GetOCLConfig()->EnableParallelCopy()) {
    if (m_programLibrarys.create(pContext, uiNumDevices, ppDevices) !=
        CL_SUCCESS)
      LOG_ERROR(TEXT("Failed to create program library for context id = %p"),
                pContext->GetHandle());
  }
...
cl_err_code ContextModule::RetainContext(cl_context context) {
  ...
  if (m_programLibrarys.retain(pContext.GetPtr()) != CL_SUCCESS)
    LOG_ERROR(TEXT("Failed to retain program library for context id = %p"),
              context);
...
cl_err_code ContextModule::ReleaseContext(cl_context context) {
  ...
  if (m_programLibrarys.release(pContext.GetPtr()) != CL_SUCCESS)
    LOG_ERROR(TEXT("Failed to release program library for context id = %p"),
              context);
  ...
  cl_int err = m_mapContexts.ReleaseObject((_cl_context_int *)context);
...
cl_int ContextModule::GetKernelSuggestedLocalWorkSizeKHR(
    cl_command_queue command_queue, cl_kernel kernel, cl_uint work_dim,
    const size_t *global_work_offset, const size_t *global_work_size,
    size_t *suggested_local_work_size) {
  LOG_INFO(TEXT("Enter GetKernelSuggestedLocalWorkSizeKHR(command_queue=%p, "
                "kernel=%p, "
                "work_dim=%u, global_work_offset=%zu, global_work_size=%p)"),
           command_queue, kernel, work_dim, global_work_offset,
           global_work_size, suggested_local_work_size);
  SharedPtr<Kernel> pKernel =
      m_mapKernels.GetOCLObject((_cl_kernel_int *)kernel).DynamicCast<Kernel>();
  if (!pKernel.GetPtr()) {
    LOG_ERROR(TEXT("GetOCLObject(%p) returned nullptr"), kernel);
    return CL_INVALID_KERNEL;
  }
  return pKernel->GetKernelSuggestedLocalWorkSizeKHR(
      command_queue, work_dim, global_work_offset, global_work_size,
...
void ContextModule::RemoveAllMemObjects(bool preserve_user_handles) {
  ...
  {
    std::lock_guard<std::mutex> Lock(m_MappedMemMutex);
    for (auto &obj : m_setMappedMemObjects)
...
cl_err_code ContextModule::CheckMemObjectParameters(
  ...
  if (((clMemFlags & CL_MEM_READ_ONLY) && (clMemFlags & CL_MEM_WRITE_ONLY)) ||
      ...
      ((clMemFlags & CL_MEM_USE_HOST_PTR) &&
       (clMemFlags & CL_MEM_COPY_HOST_PTR)) ||
...
SharedPtr<Kernel> ContextModule::GetLibraryKernel(Context *Ctx,
                                                  const std::string &Name) {
  return m_programLibrarys.getLibraryKernel(Ctx, Name);
}
...
void *ContextModule::USMHostAlloc(cl_context context,
  ...
  if (pUsmBuf) {
    std::lock_guard<std::mutex> mu(m_SvmUsmMutex);
    assert(m_mapUSMBuffers.find(pUsmBuf) == m_mapUSMBuffers.end() &&
           "usm ptr already existed");
...
cl_int ContextModule::USMBlockingFree(cl_context context, void *ptr) {
  ...
  {
    std::lock_guard<std::mutex> mu(m_SvmUsmMutex);
    if (m_mapUSMBuffers.find(ptr) == m_mapUSMBuffers.end()) {
...
void ContextModule::RegisterMappedMemoryObject(MemoryObject *pMemObj) {
  std::lock_guard<std::mutex> Lock(m_MappedMemMutex);
  m_setMappedMemObjects.insert(pMemObj);
}

void ContextModule::UnRegisterMappedMemoryObject(MemoryObject *pMemObj) {
  std::lock_guard<std::mutex> Lock(m_MappedMemMutex);
  m_setMappedMemObjects.erase(pMemObj);
}
