...
cl_err_code Program::GetBuildInfo(cl_device_id clDevice,
  ...
  if ((m_deviceToProgram.end() == deviceIdToProgramIter) ||
      (nullptr == deviceIdToProgramIter->second)) {
  ...
  return pDeviceProgram->GetBuildInfo(clParamName, szParamValueSize,
...
bool Program::Finalize() {
  if (!m_isFinalized) {
    ...
    if (m_isFinalized)
      return true;
    bool validProgramForAnyDevice = false;
    for (cl_uint i = 0; i < m_szNumAssociatedDevices; ++i) {
      DeviceProgram *deviceProgram = m_ppDevicePrograms[i].get();
      // Need not to check deviceProgram build status. It is already checked at
      // callsite of current function.
...
cl_err_code Program::GetInfo(cl_int param_name, size_t param_value_size,
  ...
  case CL_PROGRAM_BINARY_SIZES: {
    ...
    if (nullptr != param_value) {
      ...
      tDeviceProgramMap::const_iterator deviceToProgramEnd =
          m_deviceToProgram.end();
      for (size_t i = 0; i < m_szNumAssociatedDevices; ++i) {
        DeviceProgram *pDeviceProgram = m_ppDevicePrograms[i].get();
        deviceToProgramIter =
            m_deviceToProgram.find(pDeviceProgram->GetDeviceId());
        if (deviceToProgramEnd != deviceToProgramIter) {
          pDeviceProgram = deviceToProgramIter->second;
        }
        assert(nullptr != pDeviceProgram);
        cl_int clErrRet =
            pDeviceProgram->GetBinary(0, nullptr, pParamValue + i);
  ...
  case CL_PROGRAM_KERNEL_NAMES: {
    ...
    for (unsigned int i = 0; i < m_szNumAssociatedDevices; ++i) {
      ...
      if (pDevProg->IsBinaryAvailable(CL_PROGRAM_BINARY_TYPE_EXECUTABLE)) {
        ...
        clErrRet = pDevProg->GetKernelNames(nullptr, &puiKernelNameLengths[0],
                                            uiNumKernels);
        if (CL_FAILED(clErrRet)) {
          return clErrRet;
        }
        ...
        clErrRet = pDevProg->GetKernelNames(
            &pszKernelNames[0], &puiKernelNameLengths[0], uiNumKernels);
        if (CL_FAILED(clErrRet)) {
          for (size_t i = 0; i < uiNumKernels; ++i) {
...
///////////////////////////////////////////////////////////////////////////////////////////////////
cl_err_code Program::SetBinaryInternal(cl_device_id clDevice,
                                       size_t uiBinarySize, const void *pBinary,
...
///////////////////////////////////////////////////////////////////////////////////////////////////
cl_err_code Program::SetBuildLogInternal(cl_device_id clDevice,
...
///////////////////////////////////////////////////////////////////////////////////////////////////
cl_err_code Program::SetBuildOptionsInternal(cl_device_id clDevice,
...
///////////////////////////////////////////////////////////////////////////////////////////////////
cl_err_code Program::SetStateInternal(cl_device_id clDevice,
...
// Program::GetStateInternal
///////////////////////////////////////////////////////////////////////////////////////////////////
cl_err_code Program::SetDeviceHandleInternal(cl_device_id clDevice,
...
// CreateKernel
///////////////////////////////////////////////////////////////////////////////////////////////////
cl_err_code Program::CreateKernel(const char *psKernelName,
  ...
  SharedPtr<Kernel> pKernel =
      Kernel::Allocate(this, psKernelName, m_szNumAssociatedDevices);
  ...
  if (CL_FAILED(clErrRet)) {
    LOG_ERROR(TEXT("pKernel->CreateDeviceKernels(ppBinaries) = %s"),
              ClErrTxt(clErrRet));
    pKernel->Release();
    return clErrRet;
  }
...
cl_err_code Program::CreateAllKernels(cl_uint uiNumKernels,
  ...
  //       Also need to see if it's possible for two devices to have different
  //       kernels
  ...
  clErrRet = m_ppDevicePrograms[0]->GetKernelNames(
      ppKernelNames, pszKernelNameLengths, szNumKernels);
  if (CL_FAILED(clErrRet)) {
    for (size_t i = 0; i < szNumKernels; ++i) {
...
cl_err_code Program::CreateAutorunKernels(cl_uint uiNumKernels,
  ...
  try {
    ...
    for (size_t i = 0, end = vsKernelNames.size(); i < end; ++i) {
      ...
      LOG_DEBUG(TEXT("Created autorun kernel: %s"), vsKernelNames[i].c_str());
    }
...
cl_err_code
Program::queryKernelArgsInfo(cl_device_id Device, const char *KernelName,
                             std::vector<cl_kernel_argument_info> &ArgsInfo) {
  DeviceProgram *pDeviceProgram = InternalGetDeviceProgram(Device);
  if (!pDeviceProgram)
    return CL_INVALID_DEVICE;
  return pDeviceProgram->queryKernelArgsInfo(KernelName, ArgsInfo);
}
...
DeviceProgram *Program::InternalGetDeviceProgram(cl_device_id clDeviceId) {
  for (size_t deviceProg = 0; deviceProg < m_szNumAssociatedDevices;
       ++deviceProg) {
...
cl_uint Program::GetNumDevices() { return m_szNumAssociatedDevices; }
...
cl_uint Program::GetNumKernels() { return m_pKernels.Count(); }
...
void Program::SetContextDevicesToProgramMappingInternal() {
  ...
  for (unsigned int i = 0; i < m_szNumAssociatedDevices; i++) {
    if (CL_BUILD_NONE != m_ppDevicePrograms[i]->GetBuildStatus()) {
      realBuiltDeviceToDeviceProgram.insert(pair<cl_device_id, DeviceProgram *>(
          m_ppDevicePrograms[i]->GetDeviceId(), m_ppDevicePrograms[i].get()));
    }
  }
  SharedPtr<FissionableDevice> *ppContextDevices =
      m_pContext->GetDevices(&numDevicesInContext);
  ...
  set<SharedPtr<FissionableDevice>>::const_iterator contextDevicesEnd =
      contextDevicesSet.end();
  tDeviceProgramMap::iterator realBuiltDeviceIter;
  tDeviceProgramMap::const_iterator realBuiltDeviceEnd =
      realBuiltDeviceToDeviceProgram.end();
  // Traverse over all the devices in the context
  for (unsigned int i = 0; i < numDevicesInContext; i++) {
    SharedPtr<FissionableDevice> pCurrentLevelDevice = ppContextDevices[i];
    while (true) {
      realBuiltDeviceIter =
          realBuiltDeviceToDeviceProgram.find(pCurrentLevelDevice->GetHandle());
      // Is this device have built program? and this device in the same context
      if ((realBuiltDeviceEnd != realBuiltDeviceIter) &&
          (contextDevicesEnd != contextDevicesSet.find(pCurrentLevelDevice))) {
        m_deviceToProgram.insert(pair<cl_device_id, DeviceProgram *>(
            ppContextDevices[i]->GetHandle(), realBuiltDeviceIter->second));
        break;
      }
      // If this device is root device than ppContextDevices[i] does not have
      // parent with built program
      if (pCurrentLevelDevice->IsRootLevelDevice()) {
        break;
      }
      pCurrentLevelDevice =
          pCurrentLevelDevice.DynamicCast<SubDevice>()->GetParentDevice();
    }
  }
}
...
bool Program::GetMyRelatedProgramDeviceIDInternal(const cl_device_id devID,
  ...
  // Set the Object ID of the device that built this Program and have
  // DeviceProgram
