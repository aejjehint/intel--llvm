...
public:
  ...
  /*****************************************************************************
   * Function:     GetKernel
   * Description: Gets a pointer to a kernel object accourding to its cl_kernel
   *              value
   ...
   * Description: Returns whether this ContextModule is in the process of
   *              terminating
   * Arguments:    None
   * Return value: Returns whether this ContextModule is in the process of
   *               terminating
   ****************************************************************************/
  ...
  // IContext methods
  //////////////////////////////////////////////////////////////////////////////
  virtual cl_context CreateContext(const cl_context_properties *clProperties,
                                   cl_uint uiNumDevices,
                                   const cl_device_id *pDevices,
                                   logging_fn pfnNotify, void *pUserData,
                                   cl_int *pRrrcodeRet);
  virtual cl_context
  CreateContextFromType(const cl_context_properties *clProperties,
                        cl_device_type clDeviceType, logging_fn pfnNotify,
  ...
  virtual cl_program CreateProgramWithSource(cl_context clContext,
                                             ...
                                             cl_int *pErrcodeRet);
  virtual cl_program CreateProgramWithIL(cl_context clContext,
                                         const unsigned char *pIL,
                                         size_t length, cl_int *pErrcodeRet);
  virtual cl_program CreateProgramWithBinary(cl_context clContext,
                                             cl_uint uiNumDevices,
                                             ...
                                             cl_int *pErrRet);
  virtual cl_program
  CreateProgramWithBuiltInKernels(cl_context clContext, cl_uint uiNumDevices,
                                  const cl_device_id *pclDeviceList,
  ...
  virtual cl_int CompileProgram(
      ...
      void *pUserData);
  virtual cl_program LinkProgram(
      cl_context clContext, cl_uint uiNumDevices,
      const cl_device_id *pclDeviceList, const char *pcOptions,
      cl_uint uiNumInputPrograms, const cl_program *pclInputPrograms,
      void(CL_CALLBACK *pfn_notify)(cl_program program, void *user_data),
      void *pUserData, cl_int *pErrcodeRet);
  virtual cl_int clSetProgramSpecializationConstant(cl_program program,
                                                    cl_uint spec_id,
                                                    size_t spec_size,
                                                    const void *spec_value);
  virtual cl_int BuildProgram(cl_program clProgram, cl_uint uiNumDevices,
                              const cl_device_id *pclDeviceList,
                              const char *pcOptions,
                              void(CL_CALLBACK *pfn_notify)(cl_program program,
                                                            void *user_data),
                              void *pUserData);
  virtual cl_int GetProgramInfo(cl_program clProgram,
                                cl_program_info clParamName,
                                size_t szParamValueSize, void *pParamValue,
                                size_t *pszParamValueSizeRet);
  virtual cl_int GetProgramBuildInfo(cl_program clProgram,
                                     cl_device_id clDevice,
                                     cl_program_info clParamName,
                                     size_t szParamValueSize, void *pParamValue,
                                     size_t *pszParamValueSizeRet);
  // kernel methods
  virtual cl_kernel CreateKernel(cl_program clProgram,
                                 const char *pscKernelName, cl_int *piErr);
  virtual cl_kernel CloneKernel(cl_kernel source_kernel, cl_int *pErr);
  virtual cl_int CreateKernelsInProgram(cl_program clProgram,
  ...
  virtual cl_int SetKernelArg(cl_kernel clKernel, cl_uint uiArgIndex,
                              size_t szArgSize, const void *pszArgValue);
  virtual cl_int GetKernelInfo(cl_kernel clKernel, cl_kernel_info clParamName,
                               size_t szParamValueSize, void *pParamValue,
                               size_t *pszParamValueSizeRet);
  virtual cl_int GetKernelWorkGroupInfo(cl_kernel clKernel,
  ...
  virtual cl_int GetKernelSuggestedLocalWorkSizeKHR(
      cl_command_queue command_queue, cl_kernel kernel, cl_uint work_dim,
  ...
  virtual cl_mem CreateBuffer(cl_context clContext, cl_mem_flags clFlags,
                              ...
                              cl_int *pErrcodeRet);
  virtual cl_mem CreateSubBuffer(cl_mem buffer, cl_mem_flags clFlags,
                                 cl_buffer_create_type buffer_create_type,
                                 const void *buffer_create_info,
                                 cl_int *pErrcodeRet);
  virtual cl_mem CreateImage2D(cl_context clContext, cl_mem_flags clFlags,
                               const cl_image_format *clImageFormat,
                               size_t szImageWidth, size_t szImageHeight,
                               size_t szImageRowPitch, void *pHostPtr,
                               cl_int *pErrcodeRet);
  virtual cl_mem CreateImage3D(cl_context clContext, cl_mem_flags clFlags,
                               const cl_image_format *clImageFormat,
                               size_t szImageWidth, size_t szImageHeight,
                               size_t szImageDepth, size_t szImageRowPitch,
                               size_t szImageSlicePitch, void *pHostPtr,
                               cl_int *pErrcodeRet);
  virtual cl_mem CreateImage(cl_context context, cl_mem_flags flags,
                             const cl_image_format *image_format,
                             const cl_image_desc *image_desc, void *host_ptr,
                             cl_int *errcode_ret);
  virtual cl_mem Create2DImageFromImage(cl_context context, cl_mem_flags flags,
                                        const cl_image_format *pImageFormat,
                                        const cl_image_desc *pImageDesc,
                                        cl_mem otherImgHandle,
                                        cl_int *iErrcodeRet);
  virtual cl_mem CreateImageArray(cl_context clContext, cl_mem_flags clFlags,
                                  const cl_image_format *clImageFormat,
  ...
  virtual cl_int GetMemObjectInfo(cl_mem clMemObj, cl_mem_info clParamName,
                                  size_t szParamValueSize, void *pParamValue,
                                  size_t *pszParamValueSizeRet);
  virtual cl_int GetImageInfo(cl_mem clImage, cl_image_info clParamName,
                              size_t szParamValueSize, void *pParamValue,
                              size_t *pszParamValueSizeRet);
  virtual cl_int SetMemObjectDestructorCallback(cl_mem memObj,
  ...
  virtual cl_sampler CreateSampler(cl_context clContext,
                                   ...
                                   cl_int *pErrcodeRet);
  virtual cl_sampler
  CreateSamplerWithProperties(cl_context clContext,
                              const cl_sampler_properties *pSamplerProperties,
                              cl_int *pErrcodeRet);
  ...
  */
  ...
  virtual cl_int GetKernelArgInfo(cl_kernel clKernel, cl_uint argIndx,
  ...
  void *SVMAlloc(cl_context context, cl_svm_mem_flags flags, size_t size,
                 unsigned int uiAlignment);
  void SVMFree(cl_context context, void *pSvmPtr);
  cl_int SetKernelArgSVMPointer(cl_kernel clKernel, cl_uint uiArgIndex,
                                const void *pArgValue);
  cl_int SetKernelExecInfo(cl_kernel clKernel, cl_kernel_exec_info paramName,
  ...
  cl_int GetKernelSubGroupInfo(cl_kernel kernel, cl_device_id device,
                               ...
                               size_t param_value_size, void *param_value,
                               size_t *param_value_size_ret);
  cl_mem CreatePipe(cl_context context, cl_mem_flags flags,
                    cl_uint uiPipePacketSize, cl_uint uiPipeMaxPackets,
                    const cl_pipe_properties *pProperties, void *pHostPtr,
                    size_t *pSizeRet, cl_int *piErrcodeRet);

  cl_int GetPipeInfo(cl_mem pipe, cl_pipe_info paramName,
                     size_t szParamValueSize, void *pParamValue,
                     size_t *pszParamValueSizeRet);
  ...
  virtual cl_mem CreateBufferWithPropertiesINTEL(
  ...
  /////////////////////////////////////////////////////////////////////
  virtual cl_mem CreateBufferWithProperties(cl_context clContext,
  ...
  /////////////////////////////////////////////////////////////////////
  virtual cl_mem CreateImageWithProperties(cl_context clContext,
                                           const cl_mem_properties *properties,
                                           cl_mem_flags clFlags,
                                           const cl_image_format *image_format,
  ...
  cl_int GetDeviceFunctionPointer(cl_device_id device, cl_program program,
...
private:
  cl_err_code CheckMemObjectParameters(
      ...
      size_t szImageHeight, size_t szImageDepth, size_t szImageRowPitch,
      size_t szImageSlicePitch, size_t szArraySize, void *pHostPtr,
   ...
   */
  cl_err_code CheckContextSpecificParameters(
      ...
      const void *pImgBufferHostPtr = NULL, cl_mem_flags bufFlags = 0);
  ...
  cl_err_code GetRootDevices(cl_uint uiNumDevices,
                             const cl_device_id *pclDeviceIds,
                             SharedPtr<Device> *ppDevices);
  cl_err_code GetDevices(cl_uint uiNumDevices, const cl_device_id *pclDeviceIds,
                         SharedPtr<FissionableDevice> *ppDevices);

  template <size_t DIM, cl_mem_object_type OBJ_TYPE>
  cl_mem CreateScalarImage(cl_context clContext, cl_mem_flags clFlags,
                           const cl_image_format *clImageFormat,
                           size_t szImageWidth, size_t szImageHeight,
                           size_t szImageDepth, size_t szImageRowPitch,
                           size_t szImageSlicePitch, void *pHostPtr,
                           cl_int *pErrcodeRet, bool bIsImageBuffer = false);

  template <size_t DIM, cl_mem_object_type OBJ_TYPE>
  cl_mem CreateImageBuffer(cl_context context, cl_mem_flags clFlags,
                           const cl_image_format *clImageFormat,
                           const cl_image_desc &desc, cl_mem buffer,
                           cl_int *pErrcodeRet);
  ...
  bool
  Check2DImageFromBufferPitch(const ConstSharedPtr<GenericMemObject> &pBuffer,
  ...
  OCLObjectsMap<_cl_sampler_int>
      m_mapSamplers; // map list of all memory objects
  std::map<void *, SharedPtr<Context>>
      m_mapSVMBuffers; // map list of all svm objects
  ...
  std::mutex m_MappedMemMutex;
  std::set<SharedPtr<MemoryObject>>
      m_setMappedMemObjects; // set of all memory objects that were mapped at
                             // least once in a history

  ProgramLibraries m_programLibrarys;
...
template <size_t DIM, cl_mem_object_type OBJ_TYPE>
cl_mem
ContextModule::CreateScalarImage(cl_context clContext, cl_mem_flags clFlags,
                                 const cl_image_format *clImageFormat,
                                 size_t szImageWidth, size_t szImageHeight,
                                 size_t szImageDepth, size_t szImageRowPitch,
                                 size_t szImageSlicePitch, void *pHostPtr,
  ...
  const size_t szDims[] = {szImageWidth, szImageHeight, szImageDepth},
               szPitches[] = {szImageRowPitch, szImageSlicePitch};
  SharedPtr<MemoryObject> pImage = NULL;
  clErr = pContext->CreateImage<DIM, OBJ_TYPE>(clFlags, clImageFormat, pHostPtr,
...
template <size_t DIM, cl_mem_object_type OBJ_TYPE>
cl_mem ContextModule::CreateImageBuffer(cl_context context,
                                        cl_mem_flags clFlags,
                                        const cl_image_format *clImageFormat,
                                        const cl_image_desc &desc,
                                        cl_mem buffer, cl_int *pErrcodeRet) {
  ...
  SharedPtr<GenericMemObject> pBuffer =
  ...
  if (((bufFlags & CL_MEM_WRITE_ONLY) &&
       (clFlags & (CL_MEM_READ_WRITE | CL_MEM_READ_ONLY))) ||
      ((bufFlags & CL_MEM_READ_ONLY) &&
       (clFlags & (CL_MEM_READ_WRITE | CL_MEM_WRITE_ONLY))) ||
      (clFlags &
       (CL_MEM_USE_HOST_PTR | CL_MEM_ALLOC_HOST_PTR | CL_MEM_COPY_HOST_PTR)) ||
      ((bufFlags & CL_MEM_HOST_WRITE_ONLY) &&
       (clFlags & CL_MEM_HOST_READ_ONLY)) ||
      ((bufFlags & CL_MEM_HOST_READ_ONLY) &&
  ...
  clErr = CheckContextSpecificParameters(
      pContext, (cl_mem_object_type)OBJ_TYPE, desc.image_width,
  ...
  if (OBJ_TYPE == CL_MEM_OBJECT_IMAGE2D &&
  ...
  /* if the CL_MEM_READ_WRITE, CL_MEM_READ_ONLY or CL_MEM_WRITE_ONLY values are
     ...
     access qualifers associated with mem_object */
  if (0 ==
      (clFlags & (CL_MEM_READ_WRITE | CL_MEM_READ_ONLY | CL_MEM_WRITE_ONLY))) {
    clFlags |= pBuffer->GetFlags() &
  ...
  /* If the CL_MEM_HOST_WRITE_ONLY, CL_MEM_HOST_READ_ONLY or
     CL_MEM_HOST_NO_ACCESS values are not specified in flags, they are inherited
     from the corresponding memory access qualifiers associated with mem_object.
   */
  if (0 == (clFlags & (CL_MEM_HOST_WRITE_ONLY | CL_MEM_HOST_READ_ONLY |
                       CL_MEM_HOST_NO_ACCESS))) {
    clFlags |=
        pBuffer->GetFlags() & (CL_MEM_HOST_WRITE_ONLY | CL_MEM_HOST_READ_ONLY |
                               CL_MEM_HOST_NO_ACCESS);
  }

  void *const pBufferData = pBuffer->GetBackingStoreData();
  const cl_mem clImgBuf = CreateScalarImage<DIM, OBJ_TYPE>(
      context, clFlags, clImageFormat, desc.image_width, desc.image_height,
      desc.image_depth, desc.image_row_pitch, desc.image_slice_pitch,
      pBufferData, pErrcodeRet, true);
  if (CL_INVALID_HANDLE == clImgBuf) {
    return clImgBuf;
  }

  SharedPtr<MemoryObject> pImgBuf =
      m_mapMemObjects.GetOCLObject((_cl_mem_int *)clImgBuf)
