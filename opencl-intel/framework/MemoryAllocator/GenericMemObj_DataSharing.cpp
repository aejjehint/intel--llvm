...
inline SharedPtr<OclEvent> GenericMemObject::release_data_sharing_lock(
  ...
  if (returned_event && returned_event->completionReq) {
    assert(NULL != returned_event->ev.GetPtr() &&
           "If returned_event->completionReq == true than returned_event->ev "
           "cannot be NULL");
    // no races here because pointer to the event was removed from Sharing
    // Group before, so there is no way to get this pointer in more places
    returned_event->ev->SetEventState(EVENT_STATE_DONE);
    returned_event->ev = NULL;
  }

  return (NULL == returned_event) ? (SharedPtr<OclEvent>)NULL
                                  : (SharedPtr<OclEvent>)(returned_event->ev);
}
...
public:
  ...
  ~DataSharingAutoLock() { m_container.release_data_sharing_lock(NULL); };

  DataSharingAutoLock(DataSharingAutoLock &) = delete;
  DataSharingAutoLock &operator=(DataSharingAutoLock &) = delete;
...
//
void GenericMemObject::drive_copy_between_groups(
    DataCopyState starting_state, unsigned int from_grp_id,
    unsigned int to_grp_id, DataCopyEventWrapper *returned_event) {
  ...
  case DATA_COPY_STATE_TO_BS: {
    ...
    dev_error = from.m_dev_mem_obj->clDevMemObjUpdateBackingStore(
        reinterpret_cast<void *>((uintptr_t)to_grp_id), &update_mode);
    assert(CL_DEV_SUCCEEDED(dev_error) &&
           "clDevMemObjUpdateBackingStore() failed");

    if (CL_DEV_FAILED(dev_error)) {
      LOG_ERROR(TEXT("Device Object returned error 0x%X during updating to "
                     "Backing Store"),
                dev_error);
  ...
  case DATA_COPY_STATE_FROM_BS:
    ...
    dev_error = to.m_dev_mem_obj->clDevMemObjUpdateFromBackingStore(
        reinterpret_cast<void *>((uintptr_t)to_grp_id), &update_mode);
    assert(CL_DEV_SUCCEEDED(dev_error) &&
           "clDevMemObjUpdateFromBackingStore() failed");
    ...
    if (CL_DEV_FAILED(dev_error)) {
      LOG_ERROR(TEXT("Device Object returned error 0x%X during updating from "
                     "Backing Store"),
                dev_error);
      starting_state = DATA_COPY_STATE_INVALID;
    ...
    if (CL_DEV_BS_UPDATE_LAUNCHED == update_mode) {
      ...
      need_event_allocation =
          (NULL == to.m_data_copy_in_process_event.GetPtr());
      break;
    }
    LLVM_FALLTHROUGH;
  ...
  if (need_event_allocation) {
    // allocate event
    assert(NULL == to.m_data_copy_in_process_event.GetPtr());
    to.m_data_copy_in_process_event =
        DataCopyEvent::Allocate(GetParentHandle());

    assert(NULL != to.m_data_copy_in_process_event.GetPtr());
    if (NULL == to.m_data_copy_in_process_event.GetPtr()) {
  ...
  if (returned_event) {
    ...
    if (need_event_completion && (NULL != returned_event->ev.GetPtr())) {
      to.m_data_copy_in_process_event = NULL;

      // complete and release event cannot be done inside lock - need to do
      // this after exit from lock problem - event should be set to NULL
      // inside lock!
      assert(false == returned_event->completionReq &&
             "returned_event.completionReq should set to true only once");
...
//
void GenericMemObject::data_sharing_bring_data_to_sharing_group(
  ...
  *data_transferred = false;
  ...
  if (m_data_valid_state.m_contains_valid_data) {
    ...
    starting_state = (MAX_DEVICE_SHARING_GROUP_ID == copy_from)
                         ? DATA_COPY_STATE_FROM_BS
                         : DATA_COPY_STATE_TO_BS;

    if ((DATA_COPY_STATE_TO_BS == starting_state) &&
        m_pBackingStore->IsDataValid()) {
  ...
  // start directly from Valid if data should be get from BS, that does not
  // contain valid data
  if ((DATA_COPY_STATE_FROM_BS == starting_state) &&
      (!m_pBackingStore->IsDataValid())) {
    starting_state = DATA_COPY_STATE_VALID;
  }

  *data_transferred = true;
  drive_copy_between_groups(starting_state, copy_from, group_id,
                            returned_event);
}
...
//
void GenericMemObject::data_sharing_fsm_process(
    bool acquire, unsigned int group_id, MemObjUsage access,
    DataCopyEventWrapper *returned_event) {
  assert((access >= 0) && (access < MEMOBJ_USAGES_COUNT) &&
         "Wrong MemObjUsage value");
  ...
  if (acquire && is_read) {
    data_sharing_bring_data_to_sharing_group(group_id, &data_transferred,
                                             returned_event);
  }
  ...
  if (acquire) {
    ...
    // In case that the user send in parallel buffers with read / write
    // permission but only read is going to be done, We should invalidate the
    // data of the first request in the group that join to
    // MULTIPLE_VALID_WITH_PARALLEL_WRITERS_HISTORY. if (I'm the first in my
    // sharing group) AND (I didn't get updated data) AND (Joining
    // MULTIPLE_VALID_WITH_PARALLEL_WRITERS_HISTOR)Y AND (not only my sharing
    // group is running now in MULTIPLE_VALID_WITH_PARALLEL_WRITERS_HISTORY
    // state) AND (nobody copies data from me now)
    if ((1 == my_group.m_active_users_count) && (false == data_transferred) &&
        (0 < m_data_valid_state.m_groups_with_active_writers_count) &&
        (m_data_valid_state.m_groups_with_active_writers_count >
         my_group.m_active_writers_count) &&
        (false == my_group.m_data_copy_invalidate_asap)) {
  ...
  // ensure data is set as valid if it is invalid (not valid and not in the
  // transition process) until now this may be the case with WRITE_ENTIRE
  if ((WRITERS_INCREASED == writers_change) &&
      my_group.is_data_copy_invalid()) {
    my_group.m_data_copy_state = DATA_COPY_STATE_VALID;
  }

  assert(m_data_valid_state.m_groups_with_active_writers_count <=
         m_data_valid_state.m_groups_with_active_users_count);
    ...
    if (data_transferred) {
      ...
      if (1 == m_data_valid_state.m_groups_with_active_writers_count) {
        ...
        m_data_valid_state.m_data_sharing_state =
            MULTIPLE_VALID_WITH_SEQUENTIAL_WRITERS_HISTORY;
      } else {
        // data was copied to another group for reading or parallel writing (if
        // writers > 1)
        m_data_valid_state.m_data_sharing_state =
  ...
  case MULTIPLE_VALID_WITH_PARALLEL_WRITERS_HISTORY:
    if ((WRITERS_NOT_CHANGED != writers_change) &&
        (1 == m_data_valid_state.m_groups_with_active_writers_count)) {
      // unique writer added
      m_data_valid_state.m_data_sharing_state =
          MULTIPLE_VALID_WITH_SEQUENTIAL_WRITERS_HISTORY;
    }
  ...
  case MULTIPLE_VALID_WITH_SEQUENTIAL_WRITERS_HISTORY:
    if ((WRITERS_INCREASED == writers_change) &&
      ...
      assert(MAX_DEVICE_SHARING_GROUP_ID !=
             m_data_valid_state.m_last_writer_group);
...
void GenericMemObject::data_sharing_set_init_state(bool valid) {
  ...
  for (unsigned int i = 0; i < MAX_DEVICE_SHARING_GROUP_ID; ++i) {
    ...
    if (valid) {
      ...
      // BUGBUG: COI BUG
      //            cl_dev_err_code dev_error =
      //            group.m_dev_mem_obj->clDevMemObjInvalidateData(); assert(
      //            CL_DEV_SUCCEEDED(dev_error) && "initial
      //            clDevMemObjInvalidateData() failed" );
      //
      //            if (CL_DEV_FAILED(dev_error))
      //            {
      //                LOG_ERROR(TEXT("Device Object returned error 0x%X during
      //                initial invalidation"), dev_error);
      //            }
...
void GenericMemObject::invalidate_data_for_group(SharingGroup &group) {
  ...
  assert(
      (1 >= group.m_active_users_count) &&
      "Try to invalidate data while active users exist (that is not myself)");
  ...
  if (CL_DEV_FAILED(dev_error)) {
    LOG_ERROR(TEXT("Device Object returned error 0x%X during invalidation"),
              dev_error);
  }
...
void GenericMemObject::ensure_single_data_copy(unsigned int group_id) {
  assert((group_id < MAX_DEVICE_SHARING_GROUP_ID) &&
         (!m_sharing_groups[group_id].is_data_copy_invalid()) &&
         "ensure_single_data_copy called with remaining group invalid");
...
bool GenericMemObject::getDeviceSharingGroupId(
    const SharedPtr<FissionableDevice> &dev, unsigned int *pSharingGroupId) {
  assert(NULL != dev.GetPtr() && "LockOnDevice called without device");
...
cl_err_code GenericMemObject::LockOnDevice(
    IN const SharedPtr<FissionableDevice> &dev, IN MemObjUsage usage,
    OUT MemObjUsage *pOutActuallyUsage, OUT SharedPtr<OclEvent> &pOutEvent) {
  pOutEvent = NULL;
  ...
  *pOutActuallyUsage = usage;
  ...
  cl_err_code retCode =
      updateParent(devSharingGroupId, *pOutActuallyUsage, true, pOutEvent);
  if (NULL == pOutEvent.GetPtr()) {
...
cl_err_code
GenericMemObject::UnLockOnDevice(IN const SharedPtr<FissionableDevice> &dev,
...
// non-NULL if data is in the process of copying. Returned event may be added to
// dependency list by the caller
SharedPtr<OclEvent>
GenericMemObject::lockOnDeviceInt(IN unsigned int devSharingGroupId,
                                  IN MemObjUsage usage, bool alreadyLock) {
  DataCopyEventWrapper returned_event;
...
void GenericMemObject::unLockOnDeviceInt(IN unsigned int devSharingGroupId,
                                         IN MemObjUsage usage) {
  DataSharingAutoLock data_sharing_lock(*this);
  data_sharing_fsm_process(false, devSharingGroupId, usage, NULL);
}

// Device Agent should notify when long update to/from backing store operations
// finished.
//      Pass HANDLE value that was provided to Device Agent when update API was
//      called
void GenericMemObject::BackingStoreUpdateFinished(IN void *handle,
                                                  cl_dev_err_code dev_error) {
  assert((m_active_groups_count > 1) &&
         "Asynch GenericMemObject::BackingStoreUpdateFinished is called for "
         "single-shraing group mem object");

  unsigned int to_grp_id = (unsigned int)(size_t)handle;

  assert((to_grp_id < MAX_DEVICE_SHARING_GROUP_ID) &&
         "Device Agent passed wrong handle back to the "
         "GenericMemObject::BackingStoreUpdateFinished");
  ...
  if (CL_DEV_FAILED(dev_error)) {
    LOG_ERROR(TEXT("Device Object returned error 0x%X during asynchronous "
                   "updating %s Backing Store"),
              dev_error,
              (DATA_COPY_STATE_TO_BS == grp.m_data_copy_state) ? TEXT("to")
                                                               : TEXT("from"));
  }

  acquire_data_sharing_lock();
  drive_copy_between_groups(grp.m_data_copy_state, grp.m_data_copy_from_group,
                            to_grp_id, &returned_event);
  release_data_sharing_lock(&returned_event);
}

cl_err_code GenericMemObject::updateParent(unsigned int devSharingGroupId,
                                           MemObjUsage usage, bool isParent,
                                           SharedPtr<OclEvent> &pOutEvent) {
  // Call to updateParentInt with NULL as dataCopyJointEvent and the initial
  // pipeline stage - MOVE_UPDATE_CHILD_LIST_TO_PARENT_DEVICE
  return updateParentInt(
      devSharingGroupId, usage, isParent, NULL,
      PARENT_STAGE_MOVE_UPDATE_CHILD_LIST_AND_ZOMBIES_TO_PARENT_DEVICE,
      pOutEvent);
}

cl_err_code GenericMemObject::updateParentInt(
    unsigned int destDevSharingGroupId, MemObjUsage usage, bool isParent,
    SharedPtr<DataCopyJointEvent> dataCopyJointEvent, update_parent_stage stage,
    SharedPtr<OclEvent> &pOutEvent) {
  unsigned int parentValidSharingGroupId = MAX_DEVICE_SHARING_GROUP_ID;
  if (dataCopyJointEvent.GetPtr() != NULL) {
    acquireBufferSyncLock();
    parentValidSharingGroupId =
        dataCopyJointEvent->getParentValidSharingGroupID();
  } else {
    // If this is sub-buffer and there is no buffer in m_updateParentList than
    // finished.
  ...
  TSubBufferList *pSubBuffersInUpdateProcessList =
      getSubBuffersInUpdateProcessListPtr();
  vector<SharedPtr<OclEvent>> *pEventOfSubBufferInUpdateProcessList =
      getEventsOfSubBuffersInUpdateProcessListPtr();

  // If parentValidSharingGroupId is not valid sharing group ID (only in initial
  // call to updateParentInt)
  if (MAX_DEVICE_SHARING_GROUP_ID == parentValidSharingGroupId) {
    if ((PARENT_STAGE_MOVE_UPDATE_CHILD_LIST_AND_ZOMBIES_TO_PARENT_DEVICE ==
         stage) &&
        (NULL == dataCopyJointEvent.GetPtr())) {
      unsigned int tmp_parent_grp = getParentValidSharingGroupIdDuringUpdate();
      unsigned int preferedSharingGroupId =
          (tmp_parent_grp == MAX_DEVICE_SHARING_GROUP_ID)
              ? destDevSharingGroupId
              : tmp_parent_grp;
      ...
      if ((MAX_DEVICE_SHARING_GROUP_ID != tmp_parent_grp) &&
          (parentValidSharingGroupId != preferedSharingGroupId)) {
        assert(0 &&
               "In case that getParentValidSharingGroupIdDuringUpdate() is "
               "valid the lock of the parent must be on the same device");
      }
#endif
      setParentValidSharingGroupIdDuringUpdate(parentValidSharingGroupId);
      if (NULL != tEvent.GetPtr()) {
        releaseBufferSyncLock();
        if (dataCopyJointEvent.GetPtr() == NULL) {
          dataCopyJointEvent = DataCopyJointEvent::Allocate(
              GetParentHandle(), destDevSharingGroupId, usage, isParent, this,
              parentValidSharingGroupId, pOutEvent);
          assert(NULL != dataCopyJointEvent.GetPtr() &&
                 "Allocation of dataCopyJointEvent failed");
          if (NULL == dataCopyJointEvent.GetPtr()) {
            return CL_OUT_OF_HOST_MEMORY;
          }
        }
        dataCopyJointEvent->setNextStageToExecute(
            PARENT_STAGE_MOVE_UPDATE_CHILD_LIST_AND_ZOMBIES_TO_PARENT_DEVICE);
        // dataCopyJointEvent depends on the event that return from
        // 'lockOnDeviceInt()' call
      ...
      assert(0 && "Only the first stage in update parent pipe should be with "
                  "MAX_DEVICE_SHARING_GROUP_ID == parentValidSharingGroupId");
    }
#endif
  }
  ...
  case PARENT_STAGE_MOVE_UPDATE_CHILD_LIST_AND_ZOMBIES_TO_PARENT_DEVICE: {
    ...
    if ((pEventOfSubBufferInUpdateProcessList->size() > 0) &&
      ...
      for (int i = updateChildEventList.size() - 1; i >= 0; i--) {
        iter = find(pEventOfSubBufferInUpdateProcessList->begin(),
                    pEventOfSubBufferInUpdateProcessList->end(),
                    updateChildEventList[i]);
    ...
    for (unsigned int i = 0; i < pUpdateParentList->size(); i++) {
      // Because we update the previous command, LockOnDevice must be
      // READ_WRITE. (READ_WRITE is a heaviest)
      SharedPtr<OclEvent> tEvent = pUpdateParentList->at(i)->lockOnDeviceInt(
          parentValidSharingGroupId, READ_WRITE);
      // if it is async operation add tEvent to
      // m_eventOfSubBufferInUpdateProcessList
      if (NULL != tEvent.GetPtr()) {
        pEventOfSubBufferInUpdateProcessList->push_back(tEvent);
      }
      // add pUpdateParentList->at(i) to m_updateParentInProcessSubBufferList in
      // order to unlock when all the updates will complete.
      pSubBuffersInUpdateProcessList->push_back(pUpdateParentList->at(i));
    }
    // clear the parent update list
    pUpdateParentList->clear();
    bool hasZombieUpdate = (dataCopyJointEvent.GetPtr() == NULL)
                               ? false
                               : dataCopyJointEvent->getHasZombieUpdate();
    // Update the parent with the zombies
    for (int i = pSubBuffersList->size() - 1; i >= 0; i--) {
      // if the subBuffer m_subBuffersList[i] is not use any more (released and
      // all the related command completed), than update the parent with its
      // content and remove it from m_subBuffersList. Actually the destructor of
      // this subBuffer will call when the update fully complete.
      if (pSubBuffersList->at(i)->isZombie()) {
        // Because we update the previous command, LockOnDevice must be
        // READ_WRITE. (READ_WRITE is a heaviest)
        SharedPtr<OclEvent> tEvent = pSubBuffersList->at(i)->lockOnDeviceInt(
            parentValidSharingGroupId, READ_WRITE);
        // if it is async operation add tEvent to
        // m_eventOfSubBufferInUpdateProcessList
        if (NULL != tEvent.GetPtr()) {
        ...
        // add pSubBuffersList->at(i) to m_updateParentInProcessSubBufferList in
        // order to unlock when all the updates will complete.
        pSubBuffersInUpdateProcessList->push_back(pSubBuffersList->at(i));
        // delete the buffer from pSubBuffersList (the destructor will call when
    ...
    if (pEventOfSubBufferInUpdateProcessList->size() > 0) {
      ...
      vector<SharedPtr<OclEvent>> eventsListSnapshot =
          *pEventOfSubBufferInUpdateProcessList;
      // We need to wait until all the events in
      // pEventOfSubBufferInUpdateProcessList will notify.
      releaseBufferSyncLock();
      if (NULL == dataCopyJointEvent.GetPtr()) {
        dataCopyJointEvent = DataCopyJointEvent::Allocate(
            GetParentHandle(), destDevSharingGroupId, usage, isParent, this,
      ...
      dataCopyJointEvent->setNextStageToExecute(
          PARENT_STAGE_MOVE_UPDATE_CHILD_LIST_AND_ZOMBIES_TO_PARENT_DEVICE);
      // dataCopyJointEvent depends on all the events that return from
      // 'lockOnDeviceInt()' call
      dataCopyJointEvent->AddDependentOnMulti(eventsListSnapshot.size(),
                                              &(eventsListSnapshot[0]));
      pOutEvent = dataCopyJointEvent;
      return CL_SUCCESS;
    } else {
      // The UPDATE process completed
      // UnlockOnDevice all the subBuffers that updated the parent.
      for (unsigned int i = 0; i < pSubBuffersInUpdateProcessList->size();
           i++) {
        pSubBuffersInUpdateProcessList->at(i)->unLockOnDeviceInt(
            parentValidSharingGroupId, READ_WRITE);
      }
      // Update HierarchicalMemoryMode if needed (Do it by one thread only - the
      // first one that reach here [by checking if
      // pSubBuffersInUpdateProcessList->size() > 0])
      bool doUpdateHierarchicalMemoryMode =
          ((hasZombieUpdate) && (pSubBuffersInUpdateProcessList->size() > 0));
      size_t numOfSubBuffers = pSubBuffersList->size();
      pSubBuffersInUpdateProcessList->clear();
      if (doUpdateHierarchicalMemoryMode ||
          (numOfSubBuffers != pSubBuffersList->size())) {
      ...
      if (false == isParent) {
        ...
        // Unlock parent
        getParentMemObj().unLockOnDeviceInt(parentValidSharingGroupId,
                                            READ_WRITE);
        // If dataCopyJointEvent is NULL than return NULL and the caller will
        // call to LockOnDeviceInt of itself.
        if (NULL == dataCopyJointEvent.GetPtr()) {
          return CL_SUCCESS;
        }
        // We trigerred async operation (that already complete) --> we should
        // lock the child buffer in destination device.
        SharedPtr<OclEvent> tEvent =
            lockOnDeviceInt(destDevSharingGroupId, usage);
        releaseBufferSyncLock();
        if (NULL != tEvent.GetPtr()) {
  ...
  case PARENT_STAGE_MOVE_ALL_CHILDS_TO_PARENT_DEVICE: {
    ...
    for (unsigned int i = 0; i < pSubBuffersList->size(); i++) {
      ...
      SharedPtr<OclEvent> tEvent = pSubBuffersList->at(i)->lockOnDeviceInt(
          parentValidSharingGroupId, usage);
      if (NULL != tEvent.GetPtr()) {
    ...
    if (eventsList.size() > 0) {
      ...
      // dataCopyJointEvent depends on all the events that return from
      // 'lockOnDeviceInt()' call
      dataCopyJointEvent->AddDependentOnMulti(eventsList.size(),
                                              &(eventsList[0]));
      pOutEvent = dataCopyJointEvent;
      return CL_SUCCESS;
    }
  ...
  case PARENT_STAGE_LOCK_CHILDS_ON_DESTINATION_DEVICE: {
    ...
    // this == parent memory object.
    if (parentValidSharingGroupId != destDevSharingGroupId) {
      // Unlock all childs from 'parentValidSharingGroupId' device and lock on
      // 'destDevSharingGroupId' (the new location of the parent).
      if ((subBuffersListSnapshot.size() == 0) &&
          (dataCopyJointEvent.GetPtr() != NULL)) {
        subBuffersListSnapshot = dataCopyJointEvent->getParentChildList();
      }
      vector<SharedPtr<OclEvent>> eventsList;
      // for each sub-buffer Unlock from parentValidSharingGroupId and Lock on
      // parent destination sharing group id
      for (unsigned int i = 0; i < subBuffersListSnapshot.size(); i++) {
        subBuffersListSnapshot[i]->unLockOnDeviceInt(parentValidSharingGroupId,
                                                     usage);
        SharedPtr<OclEvent> tEvent = subBuffersListSnapshot[i]->lockOnDeviceInt(
            destDevSharingGroupId, usage);
        if (NULL != tEvent.GetPtr()) {
          eventsList.push_back(tEvent);
        }
      }
  ...
  case PARENT_STAGE_UNLOCK_CHILDS_FROM_DESTINATION_DEVICE: {
    ...
    for (unsigned int i = 0; i < subBuffersListSnapshot.size(); i++) {
      subBuffersListSnapshot[i]->unLockOnDeviceInt(destDevSharingGroupId,
                                                   usage);
    }
  }
    LLVM_FALLTHROUGH;
  case PARENT_STAGE_FINAL: {
    // If one of the stages was async than notify the events that we return to
    // command.
    if (NULL != dataCopyJointEvent.GetPtr()) {
      releaseBufferSyncLock();
      dataCopyJointEvent->SetEventState(EVENT_STATE_DONE);
  ...
  assert(false && "Unrecognized stage");
  return CL_SUCCESS;
}

void GenericMemObject::findValidDeviceAndLock(MemObjUsage usage,
                                              unsigned int preferedDevice,
  ...
  for (unsigned int i = 0; i < MAX_DEVICE_SHARING_GROUP_ID; ++i) {
    ...
    if ((m_sharing_groups[i].is_activated()) &&
        (!m_sharing_groups[i].is_data_copy_invalid())) {
      // if the first device that valid
      if (MAX_DEVICE_SHARING_GROUP_ID == validId) {
        validId = i;
      }
      // if current device is 'preferedDevice' than change validId to be the
      // current device.
      if (i == preferedDevice) {
        validId = i;
        break;
...
void GenericMemObject::unlockOnDeviceAndRemoveFromListInt(
    TSubBufferList *parentList, unsigned int parentValidSharingGroupId,
  ...
  TSubBufferList::iterator it =
      find(parentList->begin(), parentList->end(), tSharedPtrMemObj);
  if (it != parentList->end()) {
    parentList->erase(it);
  }
  assert(find(parentList->begin(), parentList->end(), tSharedPtrMemObj) ==
             parentList->end() &&
         "Error probably more than one instance of the same MemObject exist in "
         "parentList");
}

void GenericMemObject::addToUpdateList(TSubBufferList *parentList,
                                       GenericMemObjectSubBuffer *pMemObj) {
  SharedPtr<GenericMemObjectSubBuffer> tSharedPtrGenericMemObj(pMemObj);
  // If not exist...
  if (find(parentList->begin(), parentList->end(), tSharedPtrGenericMemObj) ==
      parentList->end()) {
...
unsigned int GenericMemObject::getParentValidSharingGroupId() const {
  GenericMemObject &parent =
      const_cast<GenericMemObject *>(this)->getParentMemObj();
...
cl_err_code GenericMemObjectSubBuffer::LockOnDevice(
    IN const SharedPtr<FissionableDevice> &dev, IN MemObjUsage usage,
    OUT MemObjUsage *pOutActuallyUsage, OUT SharedPtr<OclEvent> &pOutEvent) {
  pOutEvent = NULL;
  ...
  *pOutActuallyUsage = usage;
  ...
  // If retEvent is NULL it means that the parent updated without calling to
  // async commands so we can continue with locking this buffer on
  // devSharingGroupId. otherwise the updateParent pipe line will call to
  // lockOnDeviceInt of this buffer.
...
cl_err_code GenericMemObjectSubBuffer::UnLockOnDevice(
    IN const SharedPtr<FissionableDevice> &dev, IN MemObjUsage usage) {
  if (m_active_groups_count <= 1) {
    // single device memory object - nothing to do
    return CL_SUCCESS;
  ...
  if (true == ret) {
    ...
    TSubBufferList *pUpdateParentList = getUpdateParentListPtr();
    // If we are in overlap mode, than we shall add this buffer to
    // m_updateParentList in order to update the parent in next memory request.
    if ((MEMORY_MODE_OVERLAPPING == getHierarchicalMemoryMode()) &&
        (!isInUse())) {
...
void GenericMemObjectSubBuffer::ZombieFlashToParent() {
  ...
  assert((MAX_DEVICE_SHARING_GROUP_ID != devSharingGroupId) &&
         "parent does not have valid devices ???");
