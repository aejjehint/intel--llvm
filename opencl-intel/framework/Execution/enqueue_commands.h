...
public:
  ...
  // This interface is designed for CommandBuffer.
  // Only commands that are allowed to be recorded onto a command-buffer,
  // should implement this interface.
  // Otherwise, this interface needs to be left unimplemented.
  virtual Command *clone() const {
    assert(false && "Command::clone() not implemented");
    return nullptr;
  }
  ...
  void AttachToCommandQueue(const SharedPtr<IOclCommandQueueBase> &CmdQueue);
...
protected:
  Command(const Command &s) = default;
  Command &operator=(const Command &s) = default;
  ...
  std::shared_ptr<ocl_gpa_command> m_pGpaCommand;
...
public:
  ReadHostPipeIntelFPGACommand(void *pDst, void *pipeBS, size_t size,
                               cl_bool blocking);
...
protected:
  ...
  size_t m_size;
  cl_bool m_blocking;
  cl_dev_cmd_param_host_pipe_r m_CmdParams;
};

class WriteHostPipeIntelFPGACommand : public Command {
public:
  WriteHostPipeIntelFPGACommand(void *pipeBS, const void *pSrc, size_t size,
                                cl_bool blocking);
...
protected:
  ...
  size_t m_size;
  cl_bool m_blocking;
  cl_dev_cmd_param_host_pipe_w m_CmdParams;
};

class ReadGVCommand : public Command {
public:
  ReadGVCommand(void *pDst, const void *pSrc, size_t size);
...
protected:
  ...
  const void *m_pSrc;
  size_t m_size;
  cl_dev_cmd_param_global_rw m_CmdParams;
};

class WriteGVCommand : public Command {
public:
  WriteGVCommand(void *pDst, const void *pSrc, size_t size);
...
public:
  ReadMemObjCommand(ocl_entry_points *pOclEntryPoints,
                    const SharedPtr<MemoryObject> &pMemObj,
...
public:
  ReadBufferRectCommand(ocl_entry_points *pOclEntryPoints,
...
public:
  ReadBufferCommand(ocl_entry_points *pOclEntryPoints,
                    const SharedPtr<MemoryObject> &pBuffer,
...
public:
  ReadSvmBufferCommand(ocl_entry_points *pOclEntryPoints,
                       const SharedPtr<MemoryObject> &pBuffer,
                       const size_t pszOffset[MAX_WORK_DIM],
                       const size_t pszCb[MAX_WORK_DIM], void *pDst)
      : ReadBufferCommand(pOclEntryPoints, pBuffer, pszOffset, pszCb, pDst) {
...
public:
  ReadUsmBufferCommand(ocl_entry_points *pOclEntryPoints,
                       const SharedPtr<MemoryObject> &pBuffer,
                       const size_t pszOffset[MAX_WORK_DIM],
                       const size_t pszCb[MAX_WORK_DIM], void *pDst)
      : ReadBufferCommand(pOclEntryPoints, pBuffer, pszOffset, pszCb, pDst) {
...
public:
  ReadImageCommand(ocl_entry_points *pOclEntryPoints,
...
public:
  WriteMemObjCommand(ocl_entry_points *pOclEntryPoints, cl_bool bBlocking,
                     const SharedPtr<MemoryObject> &pMemObj,
                     const size_t *pszOrigin, const size_t *pszRegion,
                     size_t szMemObjRowPitch, size_t szMemObjSlicePitch,
...
public:
   ...
   * @param pattern_size
   */
  FillMemObjCommand(ocl_entry_points *pOclEntryPoints,
                    const SharedPtr<MemoryObject> &pMemObj,
...
public:
  WriteBufferCommand(ocl_entry_points *pOclEntryPoints, cl_bool bBlocking,
                     const SharedPtr<MemoryObject> &pBuffer,
                     const size_t pszOffset[MAX_WORK_DIM],
...
public:
  WriteSvmBufferCommand(ocl_entry_points *pOclEntryPoints, cl_bool bBlocking,
                        const SharedPtr<MemoryObject> &pBuffer,
                        const size_t pszOffset[MAX_WORK_DIM],
                        const size_t pszCb[MAX_WORK_DIM], const void *cpSrc)
      : WriteBufferCommand(pOclEntryPoints, bBlocking, pBuffer, pszOffset,
                           pszCb, cpSrc) {
    m_commandType = CL_COMMAND_SVM_MEMCPY;
  }
};
...
public:
  WriteUsmBufferCommand(ocl_entry_points *pOclEntryPoints, cl_bool bBlocking,
                        const SharedPtr<MemoryObject> &pBuffer,
                        const size_t pszOffset[MAX_WORK_DIM],
                        const size_t pszCb[MAX_WORK_DIM], const void *cpSrc)
      : WriteBufferCommand(pOclEntryPoints, bBlocking, pBuffer, pszOffset,
...
public:
  FillBufferCommand(ocl_entry_points *pOclEntryPoints,
  ...
  virtual Command *clone() const override {
    return new FillBufferCommand(*this);
  }
};

class FillSvmBufferCommand : public FillBufferCommand {
public:
  FillSvmBufferCommand(ocl_entry_points *pOclEntryPoints,
                       const SharedPtr<MemoryObject> &pBuffer,
                       const void *pattern, size_t pattern_size, size_t offset,
                       size_t size)
      : FillBufferCommand(pOclEntryPoints, pBuffer, pattern, pattern_size,
                          offset, size) {
...
public:
  MemFillUsmBufferCommand(ocl_entry_points *pOclEntryPoints,
                          const SharedPtr<MemoryObject> &pBuffer,
...
public:
  WriteBufferRectCommand(ocl_entry_points *pOclEntryPoints, cl_bool bBlocking,
                         const SharedPtr<MemoryObject> &pBuffer,
                         const size_t szBufferOrigin[MAX_WORK_DIM],
...
public:
  WriteImageCommand(ocl_entry_points *pOclEntryPoints, cl_bool bBlocking,
                    const SharedPtr<MemoryObject> &pImage,
                    const size_t *pszOrigin, const size_t *pszRegion,
...
public:
  FillImageCommand(ocl_entry_points *pOclEntryPoints,
  ...
  virtual Command *clone() const override {
    return new FillImageCommand(*this);
  }
};
...
public:
  CopyMemObjCommand(ocl_entry_points *pOclEntryPoints,
  ...
  virtual Command *clone() const override {
    return new CopyMemObjCommand(*this);
  }
...
public:
  CopyBufferCommand(ocl_entry_points *pOclEntryPoints,
...
public:
  CopySvmBufferCommand(ocl_entry_points *pOclEntryPoints,
                       const SharedPtr<MemoryObject> &pSrcBuffer,
                       const SharedPtr<MemoryObject> &pDstBuffer,
                       const size_t szSrcOrigin[MAX_WORK_DIM],
                       const size_t szDstOrigin[MAX_WORK_DIM],
                       const size_t szRegion[MAX_WORK_DIM])
      : CopyBufferCommand(pOclEntryPoints, pSrcBuffer, pDstBuffer, szSrcOrigin,
...
public:
  CopyUsmBufferCommand(ocl_entry_points *pOclEntryPoints,
                       const SharedPtr<MemoryObject> &pSrcBuffer,
                       const SharedPtr<MemoryObject> &pDstBuffer,
                       const size_t szSrcOrigin[MAX_WORK_DIM],
                       const size_t szDstOrigin[MAX_WORK_DIM],
...
public:
  CopyBufferRectCommand(ocl_entry_points *pOclEntryPoints,
                        const SharedPtr<MemoryObject> &pSrcBuffer,
                        const SharedPtr<MemoryObject> &pDstBuffer,
                        const size_t szSrcOrigin[MAX_WORK_DIM],
                        const size_t szDstOrigin[MAX_WORK_DIM],
...
public:
  CopyImageCommand(ocl_entry_points *pOclEntryPoints,
...
public:
  CopyImageToBufferCommand(ocl_entry_points *pOclEntryPoints,
                           const SharedPtr<MemoryObject> &pSrcImage,
                           const SharedPtr<MemoryObject> &pDstBuffer,
...
public:
  CopyBufferToImageCommand(ocl_entry_points *pOclEntryPoints,
                           const SharedPtr<MemoryObject> &pSrcBuffer,
                           const SharedPtr<MemoryObject> &pDstImage,
...
public:
  MapMemObjCommand(ocl_entry_points *pOclEntryPoints,
                   const SharedPtr<MemoryObject> &pMemObj,
...
public:
  MapBufferCommand(ocl_entry_points *pOclEntryPoints,
                   const SharedPtr<MemoryObject> &pBuffer,
...
public:
  MapSvmBufferCommand(ocl_entry_points *pOclEntryPoints,
                      const SharedPtr<MemoryObject> &pBuffer,
                      cl_map_flags clMapFlags, size_t szOffset, size_t szCb)
      : MapBufferCommand(pOclEntryPoints, pBuffer, clMapFlags, szOffset, szCb) {
...
public:
  MapImageCommand(ocl_entry_points *pOclEntryPoints,
                  const SharedPtr<MemoryObject> &pImage,
                  cl_map_flags clMapFlags, const size_t *pOrigin,
                  const size_t *pRegion, size_t *pszImageRowPitch,
                  size_t *pszImageSlicePitch);
...
public:
  UnmapMemObjectCommand(ocl_entry_points *pOclEntryPoints,
...
public:
  UnmapSvmBufferCommand(ocl_entry_points *pOclEntryPoints,
                        const SharedPtr<MemoryObject> &pMemObject,
                        void *pMappedRegion)
      : UnmapMemObjectCommand(pOclEntryPoints, pMemObject, pMappedRegion) {
...
public:
  NDRangeKernelCommand(ocl_entry_points *pOclEntryPoints,
  ...
  virtual Command *clone() const override {
    return new NDRangeKernelCommand(*this);
  }
...
public:
  TaskCommand(ocl_entry_points *pOclEntryPoints,
...
public:
  ...
  NativeKernelCommand(ocl_entry_points *pOclEntryPoints, pUserFnc_t pUserFnc,
...
public:
  MigrateSVMMemCommand(cl_mem_migration_flags clFlags, cl_uint uNumMemObjects,
...
public:
  MigrateMemObjCommand(ocl_entry_points *pOclEntryPoints,
...
public:
  MigrateUSMMemCommand(cl_mem_migration_flags clFlags, const void *ptr,
                       size_t size);
...
public:
  AdviseUSMMemCommand(const void *ptr, size_t size, cl_mem_advice_intel advice);
...
public:
  RuntimeCommand(bool bIsDependentOnEvents = false)
...
public:
  MarkerCommand(bool bIsDependentOnEvents)
      : RuntimeCommand(bIsDependentOnEvents) {
...
public:
  WaitForEventsCommand(bool bIsDependentOnEvents)
      : RuntimeCommand(bIsDependentOnEvents) {
...
public:
  BarrierCommand(bool bIsDependentOnEvents)
      : RuntimeCommand(bIsDependentOnEvents) {
    m_commandType = CL_COMMAND_BARRIER;
  }
  virtual ~BarrierCommand() {}
  virtual Command *clone() const override { return new BarrierCommand(*this); }
};

class CommandBufferStartMarkerCommand : public RuntimeCommand {
public:
  CommandBufferStartMarkerCommand() : RuntimeCommand(false) {
    m_commandType = CL_COMMAND_COMMAND_BUFFER_START;
  }
  virtual ~CommandBufferStartMarkerCommand() {}
};

class CommandBufferEndMarkerCommand : public RuntimeCommand {
public:
  CommandBufferEndMarkerCommand(SharedPtr<CommandBuffer> CmdBuf)
      : RuntimeCommand(false), m_CmdBuf(CmdBuf) {
    // The event attached to this command will be used as the event of the
    // enqueue command-buffer operation.
    // This command naturally reflects the lifecycle of the command-buffer.
    // Therefore, command type is set to CL_COMMAND_COMMAND_BUFFER_KHR.
    m_commandType = CL_COMMAND_COMMAND_BUFFER_KHR;
  }
  virtual cl_err_code CommandDone() override {
    m_CmdBuf->onCommandsCompletion();
    return CL_SUCCESS;
  }
  virtual ~CommandBufferEndMarkerCommand() {}

private:
  SharedPtr<CommandBuffer> m_CmdBuf;
};
...
public:
  SVMMAP_Command_NOOP(bool bIsDependentOnEvents)
      : RuntimeCommand(bIsDependentOnEvents) {
    m_commandType = CL_COMMAND_SVM_MAP;
  }
...
public:
  SVMUNMAP_Command_NOOP(bool bIsDependentOnEvents)
      : RuntimeCommand(bIsDependentOnEvents) {
    m_commandType = CL_COMMAND_SVM_UNMAP;
  }
