...
cl_bool OclCommandQueue::CompatibleWith(
    const SharedPtr<OclCommandQueue> &pQueue) const {
  if (pQueue.GetPtr() == this)
    return CL_TRUE;
  return (pQueue->GetQueueDeviceHandle() == GetQueueDeviceHandle()) &&
         (pQueue->IsOutOfOrderExecModeEnabled() ==
          IsOutOfOrderExecModeEnabled()) &&
         (pQueue->IsProfilingEnabled() == IsProfilingEnabled());
}
