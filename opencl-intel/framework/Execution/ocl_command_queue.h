...
public:
  ...
  // https://registry.khronos.org/OpenCL/specs/3.0-unified/html/OpenCL_API.html#compatible
  // A compatible command-queue is defined as a command-queue with identical
  // properties targeting the same device and in the same OpenCL context.
  cl_bool CompatibleWith(const SharedPtr<OclCommandQueue> &pQueue) const;
