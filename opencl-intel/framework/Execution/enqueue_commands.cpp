 ...
 ******************************************************************/
Command::Command()
    : m_Event(nullptr), m_clDevCmdListId(0), m_pDevice(nullptr),
      m_pCommandQueue(nullptr), m_returnCode(CL_SUCCESS),
      m_bIsBeingDeleted(false), m_bEventDetached(false),
...
void Command::AttachToCommandQueue(
    const SharedPtr<IOclCommandQueueBase> &CmdQueue) {
  assert(CmdQueue.GetPtr());
...
Command::~Command() {
  ...
  if (m_Event.GetPtr()) {
    if (m_bEventDetached) {
      ...
      /* m_Event has already been destroyed in DetachEventSharedPtr() - it's
        deletion triggered the call to ~Command(), so we need to nullify it
        without decreasing its reference counter, otherwise we would decrement a
          reference counter of an object that had already been destroyed. */
      m_Event.NullifyWithoutDecRefCnt();
    } else {
      assert(1 == m_Event.GetRefCnt());

      // In this case Command is being deleted explicitly, not through the
      // event, so we need to free the event ourselves.
      m_Event = nullptr;
    }
  }
  assert((false == m_memory_objects_acquired) &&
         "RelinquishMemoryObjects() was not called!");
  assert((false == m_memory_objects_acquired) &&
         "RelinquishMemoryObjects() was not called!");
}

cl_err_code Command::EnqueueSelf(cl_bool bBlocking,
                                 cl_uint uNumEventsInWaitList,
                                 const cl_event *cpEeventWaitList,
                                 cl_event *pEvent, ApiLogger *apiLogger) {
  if (nullptr != cpEeventWaitList && nullptr != pEvent &&
      pEvent >= cpEeventWaitList &&
      pEvent < &cpEeventWaitList[uNumEventsInWaitList]) {
    // the spec says we should check this, but doesn't actually specify the
    // exact error code
    return CL_INVALID_EVENT;
  }
  // 'this' may disapper during Enqueue if it was successful!
  return GetCommandQueue().StaticCast<IOclCommandQueueBase>()->EnqueueCommand(
      this, bBlocking, uNumEventsInWaitList, cpEeventWaitList, pEvent,
      apiLogger);
}
 ...
 *
 ******************************************************************/
cl_err_code Command::NotifyCmdStatusChanged(cl_int iCmdStatus,
  ...
  if ((nullptr != pGPAData) && (pGPAData->bUseGPA)) {
    ...
    __itt_task_begin(pGPAData->pDeviceDomain, __itt_null, __itt_null,
                     pTaskName);
#endif
  }
#endif
  ...
  case CL_COMPLETE:
    ...
    // Complete command,
    // do that before set event, since side effect of SetEvent(black) may be
    // deleting of this instance. Is error
...
cl_err_code
Command::GetMemObjectDescriptor(const SharedPtr<MemoryObject> &pMemObj,
                                IOCLDevMemoryObject **ppMemObj) {
  if (0 == pMemObj) {
    ppMemObj = nullptr;
  } else {
    SharedPtr<OclEvent> pObjEvent;
    cl_err_code res =
        pMemObj->GetDeviceDescriptor(m_pDevice, ppMemObj, &pObjEvent);
    if (CL_FAILED(res)) {
      return res;
    }
...
// succeeded, other error code in case of error arg Must be reference because it
// content might be change during LockOnDevice.
inline cl_err_code
Command::AcquireSingleMemoryObject(MemoryObjectArg &arg,
                                   const SharedPtr<FissionableDevice> &pDev) {
  assert(0 != arg.pMemObj);
  SharedPtr<OclEvent> mem_event = nullptr;
  // initialize arg.access_rights_realy_used to be arg.access_rights (It can
  // change to READ_WRITE during LockOnDevice() operation)
  arg.access_rights_realy_used = arg.access_rights;
  cl_err_code errCode = arg.pMemObj->LockOnDevice(
      pDev, arg.access_rights, &arg.access_rights_realy_used, mem_event);
...
cl_err_code
Command::AcquireMemoryObjects(MemoryObjectArgList &argList,
                              const SharedPtr<FissionableDevice> &pDev) {
  ...
  const SharedPtr<FissionableDevice> &targetDevice =
      (0 == pDev) ? m_pDevice : pDev;
  ...
  for (; it != it_end; ++it) {
    ...
    if ((CL_SUCCESS != errCode) &&
        ((CL_SUCCESS == retErrCode) || (CL_NOT_READY == retErrCode))) {
      retErrCode = errCode;
    }
  }
...
void Command::RelinquishMemoryObjects(
    MemoryObjectArgList &argList, const SharedPtr<FissionableDevice> &pDev) {
  if (!m_memory_objects_acquired) {
    return;
  }
...
inline void Command::prepare_command_descriptor(cl_dev_cmd_type type,
                                                void *params,
                                                size_t params_size) {
  m_DevCmd.id = reinterpret_cast<cl_dev_cmd_id>(
      (intptr_t)m_Event->GetId()); // event ID is set inside queue, so we cannot
...
void Command::GPA_InitCommand() {
  ...
  if ((nullptr != pGPAData) && (pGPAData->bUseGPA) &&
    ...
    m_pGpaCommand = std::make_shared<ocl_gpa_command>();
...
void MemoryCommand::create_dev_cmd_rw(
    cl_uint uiDimCount, void *pPtr, const size_t *pszMemObjOrigin,
    const size_t *pszPtrOrigin, const size_t *pszRegion, size_t szPtrRowPitch,
    size_t szPtrSlicePitch, size_t szMemObjRowPitch, size_t szMemObjSlicePitch,
  ...
  prepare_command_descriptor(clCmdType, &m_rwParams,
                             sizeof(cl_dev_cmd_param_rw));
}
...
CopyMemObjCommand::CopyMemObjCommand(
    ...
    : m_pSrcMemObj(pSrcMemObj), m_pDstMemObj(pDstMemObj),
      m_szSrcRowPitch(szSrcRowPitch), m_szSrcSlicePitch(szSrcSlicePitch),
...
/******************************************************************
 * Just mark the memory object as use. The actual copy is determined on
 *execution.
 ******************************************************************/
cl_err_code CopyMemObjCommand::Init() {
  ...
  bool override_target = m_pDstMemObj->IsWholeObjectCovered(
      m_uiDstNumDims, m_szDstOrigin, m_szRegion);

  AddToMemoryObjectArgList(m_MemOclObjects, m_pSrcMemObj,
                           MemoryObject::READ_ONLY);
  AddToMemoryObjectArgList(m_MemOclObjects, m_pDstMemObj,
                           override_target ? MemoryObject::WRITE_ENTIRE
 ...
 ******************************************************************/
cl_err_code
CopyMemObjCommand::CopyOnDevice(const SharedPtr<FissionableDevice> &pDevice) {
  cl_dev_cmd_desc *m_pDevCmd = &m_DevCmd;
  cl_dev_cmd_param_copy *pCopyParams = &m_copyParams;

  SharedPtr<OclEvent> pSrcObjEvent;
  cl_err_code clErr = m_pSrcMemObj->GetDeviceDescriptor(
      pDevice, &(pCopyParams->srcMemObj), &pSrcObjEvent);
  if (CL_FAILED(clErr)) {
    return clErr;
  }
  SharedPtr<OclEvent> pDstObjEvent;
  clErr = m_pDstMemObj->GetDeviceDescriptor(pDevice, &(pCopyParams->dstMemObj),
                                            &pDstObjEvent);
  if (CL_FAILED(clErr)) {
    return clErr;
  }
  ...
  prepare_command_descriptor(CL_DEV_CMD_COPY, pCopyParams,
                             sizeof(cl_dev_cmd_param_copy));
  ...
  LOG_DEBUG("Command - EXECUTE: %s (Id: %d)\n", GetCommandName(),
            m_Event->GetId());
  cl_dev_err_code devErr = m_pDevice->GetDeviceAgent()->clDevCommandListExecute(
      m_clDevCmdListId, &m_pDevCmd, 1);
 ...
 *
 ******************************************************************/
CopyBufferCommand::CopyBufferCommand(ocl_entry_points *pOclEntryPoints,
    ...
    : CopyMemObjCommand(pOclEntryPoints, pSrcBuffer, pDstBuffer, pszSrcOrigin,
                        pszDstOrigin, pszRegion) {
...
CopyBufferRectCommand::CopyBufferRectCommand(
    ocl_entry_points *pOclEntryPoints,
    const SharedPtr<MemoryObject> &pSrcBuffer,
    const SharedPtr<MemoryObject> &pDstBuffer, const size_t pszSrcOrigin[3],
    const size_t pszDstOrigin[3], const size_t pszRegion[3],
    const size_t szSrcRowPitch, const size_t szSrcSlicePitch,
    const size_t szDstRowPitch, const size_t szDstSlicePitch)
    : CopyMemObjCommand(pOclEntryPoints, pSrcBuffer, pDstBuffer, pszSrcOrigin,
                        pszDstOrigin, pszRegion, szSrcRowPitch, szSrcSlicePitch,
                        szDstRowPitch, szDstSlicePitch) {
 ...
 *
 ******************************************************************/
CopyImageCommand::CopyImageCommand(ocl_entry_points *pOclEntryPoints,
    ...
    : CopyMemObjCommand(pOclEntryPoints, pSrcImage, pDstImage, pszSrcOrigin,
                        pszDstOrigin, pszRegion) {
...
CopyBufferToImageCommand::CopyBufferToImageCommand(
    ocl_entry_points *pOclEntryPoints,
    const SharedPtr<MemoryObject> &pSrcBuffer,
    const SharedPtr<MemoryObject> &pDstImage, size_t pszSrcOffset[3],
    const size_t *pszDstOrigin, const size_t *pszDstRegion)
    : CopyMemObjCommand(pOclEntryPoints, pSrcBuffer, pDstImage, pszSrcOffset,
                        pszDstOrigin, pszDstRegion) {
...
CopyImageToBufferCommand::CopyImageToBufferCommand(
    ocl_entry_points *pOclEntryPoints, const SharedPtr<MemoryObject> &pSrcImage,
    const SharedPtr<MemoryObject> &pDstBuffer, const size_t *pszSrcOrigin,
    const size_t *pszSrcRegion, size_t pszDstOffset[3])
    : CopyMemObjCommand(pOclEntryPoints, pSrcImage, pDstBuffer, pszSrcOrigin,
                        pszDstOffset, pszSrcRegion) {
 ...
 *
 ******************************************************************/
MapBufferCommand::MapBufferCommand(ocl_entry_points *pOclEntryPoints,
                                   const SharedPtr<MemoryObject> &pBuffer,
                                   cl_map_flags clMapFlags, size_t szOffset,
                                   size_t szCb)
    : MapMemObjCommand(pOclEntryPoints, pBuffer, clMapFlags, nullptr, nullptr,
                       nullptr, nullptr) {
 ...
 *
 ******************************************************************/
MapImageCommand::MapImageCommand(ocl_entry_points *pOclEntryPoints,
    ...
    : MapMemObjCommand(pOclEntryPoints, pImage, clMapFlags, pOrigin, pRegion,
 ...
 *
 ******************************************************************/
MapMemObjCommand::MapMemObjCommand(ocl_entry_points *pOclEntryPoints,
                                   const SharedPtr<MemoryObject> &pMemObj,
                                   cl_map_flags clMapFlags,
                                   const size_t *pOrigin, const size_t *pRegion,
                                   size_t *pszImageRowPitch,
                                   size_t *pszImageSlicePitch)
    : m_clMapFlags(clMapFlags), m_pszImageRowPitch(pszImageRowPitch),
...
MapMemObjCommand::~MapMemObjCommand() {
  if (m_bResourcesAllocated) {
    ...
    m_MemOclObjects[0].pMemObj->ReleaseMappedRegion(m_pMappedRegion,
                                                    m_pHostDataPtr);
  }

  // In case that the map command run on different device that enqueued than
  // change the device to m_pActualMappingDevice in order to release the command
  // object from the device it created.
...
cl_err_code MapMemObjCommand::Init() {
  ...
  cl_err_code err = pMemObj->CreateMappedRegion(
      m_pDevice, m_clMapFlags, m_szOrigin, m_szRegion, m_pszImageRowPitch,
      m_pszImageSlicePitch, &m_pMappedRegion, &m_pHostDataPtr, &actual_dev);
  if (CL_FAILED(err)) {
    assert(0);
    // Case of error
  ...
  if ((0 == (CL_MAP_WRITE_INVALIDATE_REGION &
             m_pMappedRegion
                 ->flags)) && // region was not mapped for overriding by host
      (pMemObj->IsSynchDataWithHostRequired(m_pMappedRegion, m_pHostDataPtr))) {
    m_pPostfixCommand = new PrePostFixRuntimeCommand(
        this, PrePostFixRuntimeCommand::POSTFIX_MODE);
    m_pPostfixCommand->AttachToCommandQueue(GetCommandQueue());
  ...
  if ((CL_MAP_WRITE_INVALIDATE_REGION & m_clMapFlags) &&
      (pMemObj->IsWholeObjectCovered(pMemObj->GetNumDimensions(), m_szOrigin,
                                     m_szRegion))) {
    assert(m_MemOclObjects.size() == 1);
...
cl_err_code MapMemObjCommand::Execute() {
  assert(
      (RUNTIME_EXECUTION_TYPE != m_ExecutionType) &&
      "MapMemObjCommand::Execute() called after it was switch to RUNTIME mode");
  ...
  cl_err_code res =
      AcquireMemoryObjects(m_MemOclObjects, m_pActualMappingDevice);
  if (CL_SUCCESS != res) {
    return res;
  }
  ...
  cl_dev_cmd_desc *m_pDevCmd = &m_DevCmd;
  // Prepare command.
  // Anyhow we send the map command to the device though  we expect that on
  // write there is nothing to do, and on read the device may need to copy from
  // device memory to host memory
  prepare_command_descriptor(CL_DEV_CMD_MAP, m_pMappedRegion,
                             sizeof(cl_dev_cmd_param_map));
  ...
  if (m_pActualMappingDevice != m_pDevice) {
    ...
    // ensure we will exist after device will call for completion so that
    // runtime scheduler will see us
    // m_Event->AddPendency(this);
  }

  cl_dev_err_code errDev =
      m_pActualMappingDevice->GetDeviceAgent()->clDevCommandListExecute(
          device_cmd_list, &m_pDevCmd, 1);
...
cl_err_code MapMemObjCommand::CommandDone() {
  ...
  // Don't remove buffer pendency, the buffer should be alive at least until
  // unmap is done.
 ...
 *
 ******************************************************************/
cl_err_code MapMemObjCommand::EnqueueSelf(cl_bool bBlocking,
                                          cl_uint uNumEventsInWaitList,
                                          const cl_event *cpEeventWaitList,
                                          cl_event *pEvent,
                                          ApiLogger *apiLogger) {
  ...
  if (m_pPostfixCommand) {
    ...
    err = Command::EnqueueSelf(CL_FALSE, uNumEventsInWaitList, cpEeventWaitList,
                               &intermediate_pEvent, apiLogger);
    if (CL_FAILED(err)) {
      LOG_ERROR("Command - Command::EnqueueSelf: %s (Id: %d) failed, Err: %x\n",
    ...
    err = postfix->EnqueueSelf(bBlocking, 1, &intermediate_pEvent, pEvent,
                               apiLogger);
    if (CL_FAILED(err)) {
      LOG_ERROR("Command - ostfix->EnqueueSelf: %s (Id: %d) failed, Err: %x\n",
    ...
    // 'this' may disapper after the self-enqueue is successful!
    return Command::EnqueueSelf(bBlocking, uNumEventsInWaitList,
...
cl_err_code MapMemObjCommand::PostfixExecute() {
  ...
  if ((nullptr != pGPAData) && (pGPAData->bUseGPA)) {
    ...
    size_t regionInBytes[MAX_WORK_DIM] = {m_pMappedRegion->region[0],
                                          m_pMappedRegion->region[1],
                                          m_pMappedRegion->region[2]};

    // Calculate each region size in bytes
    m_pMappedRegion->memObj->clDevMemObjGetDescriptor(
        CL_DEVICE_TYPE_CPU, 0, (cl_dev_memobj_handle *)&pMemObj);
    ...
    __itt_metadata_add(pGPAData->pDeviceDomain, ittID, pGPAData->pSizeHandle,
                       ITT_SIZE_T_METADATA_TYPE, m_pMappedRegion->dim_count,
                       regionInBytes);
  }
#endif // ITT

  assert(m_MemOclObjects.size() == 1);
  err = m_MemOclObjects[0].pMemObj->SynchDataToHost(m_pMappedRegion,
                                                    m_pHostDataPtr);
...
UnmapMemObjectCommand::UnmapMemObjectCommand(
    ...
    : m_pMappedPtr(pMappedPtr), m_pMappedRegion(nullptr),
...
UnmapMemObjectCommand::~UnmapMemObjectCommand() {
  ...
  // In case that the unmap command run on different device that enqueued than
  // change the device to m_pActualMappingDevice in order to release the command
  // object from the device it created.
  if ((m_pActualMappingDevice != m_pDevice) && (0 != m_pActualMappingDevice)) {
    m_pDevice = m_pActualMappingDevice;
...
cl_err_code UnmapMemObjectCommand::Init() {
  ...
  /* First check the the region has been mapped - just get the 1st mapped
     region, the user should handle code with multiple map/unmap commands */
  bool bDiscardPreviousData = false;
  cl_err_code err =
      pMemObj->GetMappedRegionInfo(m_pDevice, m_pMappedPtr, &m_pMappedRegion,
                                   &actual_dev, &bDiscardPreviousData, true);
  ...
  if ((0 !=
       ((CL_MAP_WRITE | CL_MAP_WRITE_INVALIDATE_REGION) &
        m_pMappedRegion->flags)) && // region was mapped for writing on host
      pMemObj->IsSynchDataWithHostRequired(m_pMappedRegion, m_pMappedPtr)) {
    m_pPrefixCommand = new PrePostFixRuntimeCommand(
        this, PrePostFixRuntimeCommand::PREFIX_MODE);
    m_pPrefixCommand->AttachToCommandQueue(GetCommandQueue());
...
cl_err_code UnmapMemObjectCommand::Execute() {
  assert((RUNTIME_EXECUTION_TYPE != m_ExecutionType) &&
         "UnmapMemObjectCommand::Execute() called after it was switch to "
         "RUNTIME mode");
  ...
  prepare_command_descriptor(CL_DEV_CMD_UNMAP, m_pMappedRegion,
                             sizeof(cl_dev_cmd_param_map));
...
cl_err_code UnmapMemObjectCommand::PrefixExecute() {
  ...
  if ((nullptr != pGPAData) && (pGPAData->bUseGPA)) {
    ...
    char pMarkerString[ITT_TASK_NAME_LEN];
    SPRINTF_S(pMarkerString, ITT_TASK_NAME_LEN, "Sync Data Prefix - %s",
              GetCommandName());
    __itt_string_handle *pMarker = __itt_string_handle_create(pMarkerString);
  ...
  assert(m_MemOclObjects.size() == 1);
  err = m_MemOclObjects[0].pMemObj->SynchDataFromHost(m_pMappedRegion,
                                                      m_pMappedPtr);
...
cl_err_code UnmapMemObjectCommand::CommandDone() {
  ...
  assert(m_MemOclObjects.size() == 1);
  errVal = m_MemOclObjects[0].pMemObj->ReleaseMappedRegion(m_pMappedRegion,
                                                           m_pMappedPtr, true);
  ...
  m_pContextModule->UnRegisterMappedMemoryObject(
      m_MemOclObjects[0].pMemObj.GetPtr());
 ...
 *
 ******************************************************************/
cl_err_code UnmapMemObjectCommand::EnqueueSelf(cl_bool bBlocking,
                                               cl_uint uNumEventsInWaitList,
                                               const cl_event *cpEeventWaitList,
                                               cl_event *pEvent,
                                               ApiLogger *apiLogger) {
  ...
  if (m_pPrefixCommand) {
    ...
    err = m_pPrefixCommand->EnqueueSelf(CL_FALSE, uNumEventsInWaitList,
                                        cpEeventWaitList, &intermediate_pEvent,
                                        apiLogger);
    if (CL_FAILED(err)) {
      return err;
    }
    ...
    // 'this' may disapper during Enqueue if it was successful!
    err = Command::EnqueueSelf(bBlocking, uNumEventsInWaitList,
                               cpEeventWaitList, pEvent, apiLogger);
  }
 ...
 *
 ******************************************************************/
NativeKernelCommand::NativeKernelCommand(ocl_entry_points * /*pOclEntryPoints*/,
                                         void(CL_CALLBACK *pUserFnc)(void *),
                                         void *pArgs, size_t szCbArgs,
                                         cl_uint uNumMemObjects,
                                         SharedPtr<MemoryObject> *ppMemObjList,
                                         const void **ppArgsMemLoc)
    : m_pUserFnc(pUserFnc), m_pArgs(pArgs), m_szCbArgs(szCbArgs),
      m_uNumMemObjects(uNumMemObjects), m_ppMemObjList(ppMemObjList),
...
cl_err_code NativeKernelCommand::Init() {
  ...
  for (i = 0; i < m_uNumMemObjects; i++) {
    ...
    cl_dev_memobj_handle *pNewMemObjLocation =
        (cl_dev_memobj_handle *)(pNewArgs + stObjOffset);
    ...
    *((cl_dev_memobj_handle *)pNewMemObjLocation) = clDevMemHndl;
    ...
    AddToMemoryObjectArgList(m_MemOclObjects, pMemObj,
                             MemoryObject::READ_WRITE);
  }
...
cl_err_code NativeKernelCommand::Execute() {
  ...
  cl_dev_cmd_desc *m_pDevCmd = &m_DevCmd;
  prepare_command_descriptor(CL_DEV_CMD_EXEC_NATIVE, &m_nativeParams,
                             sizeof(cl_dev_cmd_param_native));

  return m_pDevice->GetDeviceAgent()->clDevCommandListExecute(m_clDevCmdListId,
                                                              &m_pDevCmd, 1);
}
...
NDRangeKernelCommand::NDRangeKernelCommand(
    ...
    : m_pKernel(pKernel), m_pDeviceKernel(nullptr), m_uiWorkDim(uiWorkDim),
...
cl_err_code NDRangeKernelCommand::Init() {
  ...
  // Thus, we also create and set the device command appropriately as much as we
  // can.
      ...
      if (nonArgSvmBuffersCount > 0) {
        ...
        if (nonArgSvmBufs.size() > 0) {
          ...
          for (size_t i = 0; i < nonArgSvmBufs.size(); i++) {
            AddToMemoryObjectArgList(m_MemOclObjects, nonArgSvmBufs[i],
                                     MemoryObject::READ_WRITE);
            res = GetMemObjectDescriptor(nonArgSvmBufs[i],
                                         &m_nonArgSvmBuffersVec[i]);
            if (CL_FAILED(res)) {
              return res;
            }
          }
  ...
  if (!nonArgUsmBufs.empty()) {
    cl_device_unified_shared_memory_capabilities_intel hostCaps =
        GetDevice()->GetUSMCapabilities(CL_DEVICE_HOST_MEM_CAPABILITIES_INTEL);
    ...
    bool deviceSupported = deviceCaps & CL_UNIFIED_SHARED_MEMORY_ACCESS_INTEL;
    ...
    for (size_t i = 0; i < nonArgUsmBufs.size(); i++) {
      ...
      AddToMemoryObjectArgList(m_MemOclObjects, buf, MemoryObject::READ_WRITE);
  ...
  const size_t *szCompliedWorkGroupSize =
      m_pDeviceKernel->GetKernelCompileWorkGroupSize();

  // If the work-group size is not specified in kernel using the above attribute
  // qualifier (0, 0,0) is returned in szComplieWorkGroupSize.
  if (!((0 == szCompliedWorkGroupSize[0]) &&
        (0 == szCompliedWorkGroupSize[1]) &&
        (0 == szCompliedWorkGroupSize[2]))) {

    // The size of workgroup is specified using
    // __attribute__((reqd_work_group_size(X, Y, Z))) qualifier and
    // local_work_size is NULL.
    if (nullptr == m_cpszLocalWorkSize) {
      return CL_INVALID_WORK_GROUP_SIZE;
    } else {
      // CL_INVALID_WORK_GROUP_SIZE if local_work_size is specified and does not
      // match the work-group size for kernel in the program source given by the
  ...
  // local_work_size
  if (!m_pDeviceKernel->GetKernelNonUniformWGSizeSupport() &&
      m_cpszLocalWorkSize != nullptr) {
    for (unsigned int ui = 0; ui < m_uiWorkDim; ui++) {
  ...
  if (nullptr != m_cpszLocalWorkSize) {
    ...
    if (szWorkGroupSize > szDeviceMaxWorkGroupSize) {
       ...
       * value specified by CL_DEVICE_MAX_WORK_GROUP_SIZE in table 4.3.
       */
      return CL_INVALID_WORK_GROUP_SIZE;
    }
    ...
    if (uiMaxWorkItemDim == 0) {
       ...
       * of local_work_size[0], local_work_size[work_dim - 1] is greater than
  ...
  const size_t uiDispatchAlignment =
      m_pDeviceKernel->GetKernelArgBufferAlignment();
  ...
  if (uiDispatchSize > 0) {
    pDispatchBuffer =
        (char *)ALIGNED_MALLOC(uiDispatchSize, uiDispatchAlignment);
    ...
    if (uiArgSize > 0) {
      MEMCPY_S(pDispatchBuffer + uiArgOffset, uiArgSize,
               m_pKernel->GetArgsBlob(), uiArgSize);
      ...
      for (size_t i = 0; i < szMemArgCount; i++) {
        ...
        if (pArg->IsSvmPtr()) {
          ...
          if (nullptr == clMemId) {
            assert((pArg->IsBuffer() || pArg->IsSvmPtr() || pArg->IsUsmPtr()) &&
                   "nullptr values is allowed "
        ...
        assert(nullptr != pMemObj &&
               "Memory object is not supposed to be NULL on this stage");
        if (nullptr == pMemObj) {
          continue;
        }

        // increment refcount of memory object and save it
        // TODO: Check why we always pass READ_WRITE as usage. Why we need it at
        // all
        AddToMemoryObjectArgList(m_MemOclObjects, pMemObj,
                                 MemoryObject::READ_WRITE);
        ...
        IOCLDevMemoryObject **devObjSrc =
            (IOCLDevMemoryObject **)(pArgValues + pArg->GetOffset());
  ...
  pKernelParam->ppNonArgSvmBuffers =
      m_nonArgSvmBuffersVec.empty() ? nullptr : &m_nonArgSvmBuffersVec[0];
  pKernelParam->uiNonArgSvmBuffersCount = m_nonArgSvmBuffersVec.size();

  pKernelParam->ppNonArgUsmBuffers =
      m_nonArgUsmBuffersVec.empty() ? nullptr : &m_nonArgUsmBuffersVec[0];
  ...
  for (cl_uint i = 0; i < m_uiWorkDim; i++) {
    ...
    if (nullptr == m_cpszLocalWorkSize) {
      ...
      pKernelParam->lcl_wrk_size[NONUNIFORM_WG_SIZE_INDEX][i] =
          0 < tailSize ? tailSize : m_cpszLocalWorkSize[i];
    }
  }
...
cl_err_code NDRangeKernelCommand::Execute() {
  ...
  cl_dev_cmd_desc *m_pDevCmd = &m_DevCmd;
  // Fill command descriptor
  prepare_command_descriptor(CL_DEV_CMD_EXEC_KERNEL, &m_kernelParams,
                             sizeof(cl_dev_cmd_param_kernel));
...
ReadHostPipeIntelFPGACommand::ReadHostPipeIntelFPGACommand(void *pDst,
                                                           void *pipeBS,
                                                           size_t size,
                                                           cl_bool blocking)
    : m_pDst(pDst), m_pipeBS(pipeBS), m_size(size), m_blocking(blocking) {
  m_commandType = CL_COMMAND_READ_HOST_PIPE_INTEL;
}

cl_err_code ReadHostPipeIntelFPGACommand::Init() {
  if (nullptr == m_pDst || nullptr == m_pipeBS)
    return CL_INVALID_VALUE;

  m_CmdParams = {m_pDst, m_pipeBS, m_size, m_blocking};
...
cl_err_code ReadHostPipeIntelFPGACommand::Execute() {
  prepare_command_descriptor(CL_DEV_CMD_READ_HOST_PIPE, &m_CmdParams,
                             sizeof(m_CmdParams));

  LOG_DEBUG("Command - EXECUTE: %s (Id: %d)\n", GetCommandName(),
            m_Event->GetId());

  // Sending command to the target device
  cl_dev_cmd_desc *cmdPList = &m_DevCmd;
  cl_dev_err_code errDev = m_pDevice->GetDeviceAgent()->clDevCommandListExecute(
      m_clDevCmdListId, &cmdPList, 1);
  return CL_DEV_SUCCEEDED(errDev) ? CL_SUCCESS : CL_OUT_OF_RESOURCES;
}

WriteHostPipeIntelFPGACommand::WriteHostPipeIntelFPGACommand(void *pipeBS,
                                                             const void *pSrc,
                                                             size_t size,
                                                             cl_bool blocking)
    : m_pipeBS(pipeBS), m_pSrc(pSrc), m_size(size), m_blocking(blocking) {
  m_commandType = CL_COMMAND_WRITE_HOST_PIPE_INTEL;
}

cl_err_code WriteHostPipeIntelFPGACommand::Init() {
  if (nullptr == m_pipeBS || nullptr == m_pSrc)
    return CL_INVALID_VALUE;

  m_CmdParams = {m_pipeBS, m_pSrc, m_size, m_blocking};
...
cl_err_code WriteHostPipeIntelFPGACommand::Execute() {
  prepare_command_descriptor(CL_DEV_CMD_WRITE_HOST_PIPE, &m_CmdParams,
                             sizeof(m_CmdParams));
...
ReadGVCommand::ReadGVCommand(void *pDst, const void *pSrc, size_t size)
    : m_pDst(pDst), m_pSrc(pSrc), m_size(size) {
...
cl_err_code ReadGVCommand::Init() {
  if (nullptr == m_pDst || nullptr == m_pSrc)
    return CL_INVALID_VALUE;

  m_CmdParams = {m_pDst, m_pSrc, m_size};
...
cl_err_code ReadGVCommand::Execute() {
  prepare_command_descriptor(CL_DEV_CMD_READ_GLOBAL, &m_CmdParams,
                             sizeof(m_CmdParams));
...
WriteGVCommand::WriteGVCommand(void *pDst, const void *pSrc, size_t size)
    : m_pDst(pDst), m_pSrc(pSrc), m_size(size) {
  memset(&m_CmdParams, 0, sizeof(m_CmdParams));
...
cl_err_code WriteGVCommand::Init() {
  if (nullptr == m_pDst || nullptr == m_pSrc)
    return CL_INVALID_VALUE;
...
cl_err_code WriteGVCommand::Execute() {
  prepare_command_descriptor(CL_DEV_CMD_WRITE_GLOBAL, &m_CmdParams,
                             sizeof(m_CmdParams));
 ...
 *
 ******************************************************************/
ReadBufferCommand::ReadBufferCommand(ocl_entry_points *pOclEntryPoints,
                                     const SharedPtr<MemoryObject> &pBuffer,
                                     const size_t pszOffset[3],
                                     const size_t pszCb[3], void *pDst)
    : ReadMemObjCommand(pOclEntryPoints, pBuffer, pszOffset, pszCb, 0, 0,
                        pDst) {
...
ReadBufferRectCommand::ReadBufferRectCommand(
    ocl_entry_points *pOclEntryPoints, const SharedPtr<MemoryObject> &pBuffer,
    const size_t szBufferOrigin[3], const size_t szDstOrigin[3],
    const size_t szRegion[3], const size_t szBufferRowPitch,
    const size_t szBufferSlicePitch, const size_t szDstRowPitch,
    const size_t szDstSlicePitch, void *pDst)
    : ReadMemObjCommand(pOclEntryPoints, pBuffer, szBufferOrigin, szRegion,
                        szBufferRowPitch, szBufferSlicePitch, pDst, szDstOrigin,
                        szDstRowPitch, szDstSlicePitch) {
 ...
 *
 ******************************************************************/
ReadImageCommand::ReadImageCommand(ocl_entry_points *pOclEntryPoints,
                                   const SharedPtr<MemoryObject> &pImage,
                                   const size_t *pszOrigin,
                                   const size_t *pszRegion, size_t szRowPitch,
                                   size_t szSlicePitch, void *pDst)
    : ReadMemObjCommand(pOclEntryPoints, pImage, pszOrigin, pszRegion, 0, 0,
                        pDst, nullptr, szRowPitch, szSlicePitch) {
 ...
 *
 ******************************************************************/
ReadMemObjCommand::ReadMemObjCommand(ocl_entry_points * /*pOclEntryPoints*/,
                                     const SharedPtr<MemoryObject> &pMemObj,
                                     const size_t *pszOrigin,
                                     const size_t *pszRegion, size_t szRowPitch,
                                     size_t szSlicePitch, void *pDst,
    ...
    : m_szMemObjRowPitch(szRowPitch), m_szMemObjSlicePitch(szSlicePitch),
      m_pDst(pDst), m_szDstRowPitch(szDstRowPitch),
...
cl_err_code ReadMemObjCommand::Execute() {
  ...
  if (CL_FAILED(res)) {
    assert(0 && "ReadMemObjCommand::Execute() Failed: on a call to "
                "GetMemObjectDescriptor");
    return res;
  }
  ...
  create_dev_cmd_rw(m_commandType == CL_COMMAND_READ_BUFFER_RECT
                        ? MAX_WORK_DIM
                        : pMemObj->GetNumDimensions(),
                    m_pDst, m_szOrigin, m_szDstOrigin, m_szRegion,
                    m_szDstRowPitch, m_szDstSlicePitch, m_szMemObjRowPitch,
                    m_szMemObjSlicePitch, CL_DEV_CMD_READ);
 ...
 *
 ******************************************************************/
TaskCommand::TaskCommand(ocl_entry_points *pOclEntryPoints,
                         const SharedPtr<Kernel> &pKernel)
    : NDRangeKernelCommand(pOclEntryPoints, pKernel, 1, nullptr,
                           &m_szStaticWorkSize, &m_szStaticWorkSize),
 ...
 *
 ******************************************************************/
WriteBufferCommand::WriteBufferCommand(ocl_entry_points *pOclEntryPoints,
                                       cl_bool bBlocking,
                                       const SharedPtr<MemoryObject> &pBuffer,
                                       const size_t *pszOffset,
                                       const size_t *pszCb, const void *cpSrc)
    : WriteMemObjCommand(pOclEntryPoints, bBlocking, pBuffer, pszOffset, pszCb,
                         0, 0, cpSrc) {
 ...
 *  FillBufferCommand
 ******************************************************************/
FillBufferCommand::FillBufferCommand(ocl_entry_points *pOclEntryPoints,
                                     const SharedPtr<MemoryObject> &pBuffer,
                                     const void *pattern, size_t pattern_size,
                                     size_t offset, size_t size)
    : FillMemObjCommand(pOclEntryPoints, pBuffer, offset, size, pattern,
                        pattern_size) {
 ...
 *  FillImageCommand
 ******************************************************************/
FillImageCommand::FillImageCommand(ocl_entry_points *pOclEntryPoints,
                                   const SharedPtr<MemoryObject> &pImg,
                                   const void *pattern, size_t pattern_size,
                                   const cl_uint num_of_dimms,
                                   const size_t *offset, const size_t *size)
    : FillMemObjCommand(pOclEntryPoints, pImg, offset, size, num_of_dimms,
...
WriteBufferRectCommand::WriteBufferRectCommand(
    ...
    const SharedPtr<MemoryObject> &pBuffer, const size_t szBufferOrigin[3],
    const size_t szSrcOrigin[3], const size_t szRegion[3],
    const size_t szBufferRowPitch, const size_t szBufferSlicePitch,
    const size_t szDstRowPitch, const size_t szDstSlicePitch, const void *pDst)
    : WriteMemObjCommand(pOclEntryPoints, bBlocking, pBuffer, szBufferOrigin,
                         szRegion, szBufferRowPitch, szBufferSlicePitch, pDst,
                         szSrcOrigin, szDstRowPitch, szDstSlicePitch) {
 ...
 *
 ******************************************************************/
WriteImageCommand::WriteImageCommand(ocl_entry_points *pOclEntryPoints,
                                     cl_bool bBlocking,
                                     const SharedPtr<MemoryObject> &pImage,
                                     const size_t *pszOrigin,
                                     const size_t *pszRegion, size_t szRowPitch,
                                     size_t szSlicePitch, const void *cpSrc)
    : WriteMemObjCommand(pOclEntryPoints, bBlocking, pImage, pszOrigin,
                         pszRegion, 0, 0, cpSrc, nullptr, szRowPitch,
                         szSlicePitch) {
...
WriteMemObjCommand::WriteMemObjCommand(
    ...
    : m_bBlocking(bBlocking), m_szMemObjRowPitch(szRowPitch),
      m_szMemObjSlicePitch(szSlicePitch), m_cpSrc(cpSrc),
      m_szSrcRowPitch(szSrcRowPitch), m_szSrcSlicePitch(szSrcSlicePitch),
...
cl_err_code WriteMemObjCommand::Init() {
  ...
  if (m_bBlocking) {
    ...
    sCpyParam.uiDimCount = m_commandType == CL_COMMAND_WRITE_BUFFER_RECT
                               ? MAX_WORK_DIM
                               : pMemObj->GetNumDimensions();
    ...
    // Need to update source origin and pitch, now we will write from temporary
    // buffer
  ...
  if (pMemObj->IsWholeObjectCovered(
          CL_COMMAND_WRITE_BUFFER_RECT
              ? MAX_WORK_DIM
              : m_MemOclObjects[0].pMemObj->GetNumDimensions(),
...
cl_err_code WriteMemObjCommand::Execute() {
  cl_dev_cmd_desc *pDevCmd = &m_DevCmd;
  assert(m_MemOclObjects.size() == 1 &&
         "We expect single memory object in the command");
  const SharedPtr<MemoryObject> &pMemObj = m_MemOclObjects[0].pMemObj;

  cl_err_code res = GetMemObjectDescriptor(pMemObj, &m_rwParams.memObj);
  if (CL_FAILED(res)) {
    assert(0 && "WriteMemObjCommand::Execute() Failed: on a call to "
                "GetMemObjectDescriptor");
    ALIGNED_FREE(m_pTempBuffer);
    m_pTempBuffer = nullptr;
    return res;
  ...
  create_dev_cmd_rw(m_commandType == CL_COMMAND_WRITE_BUFFER_RECT
                    ...
                    m_szSrcSlicePitch, m_szMemObjRowPitch, m_szMemObjSlicePitch,
                    CL_DEV_CMD_WRITE);
 ...
 *
 ******************************************************************/
FillMemObjCommand::FillMemObjCommand(ocl_entry_points * /*pOclEntryPoints*/,
                                     const SharedPtr<MemoryObject> &pMemObj,
                                     const size_t *pszOffset,
                                     const size_t *pszRegion,
...
FillMemObjCommand::FillMemObjCommand(ocl_entry_points * /*pOclEntryPoints*/,
                                     ...
                                     const void *pattern,
                                     const size_t pattern_size)
    : m_numOfDimms(1), m_pattern_size(pattern_size), m_internalErr(CL_SUCCESS) {
...
cl_err_code FillMemObjCommand::Init() {
  ...
  m_MemOclObjects[0].access_rights =
      (pMemObj->IsWholeObjectCovered(m_numOfDimms, m_szOffset, m_szRegion))
...
cl_err_code FillMemObjCommand::Execute() {
  ...
  assert(m_MemOclObjects.size() == 1);
  const SharedPtr<MemoryObject> &pMemObj = m_MemOclObjects[0].pMemObj;
  cl_err_code clErr = pMemObj->GetDeviceDescriptor(
      m_pDevice, &(m_fillCmdParams.memObj), &pObjEvent);
  if (CL_FAILED(clErr)) {
    return clErr;
  }
  ...
  assert(MAX_PATTERN_SIZE >= m_pattern_size &&
         "Trying to use a pattern larger than possible");
  m_fillCmdParams.pattern_size = m_pattern_size;
  MEMCPY_S(m_fillCmdParams.pattern, m_fillCmdParams.pattern_size, m_pattern,
           m_fillCmdParams.pattern_size);

  // FillMemObject::Create(TaskDispatcher* pTD, cl_dev_cmd_desc* pCmd,
  // ITaskBase* *pTask)
  prepare_command_descriptor((m_commandType == CL_DEV_CMD_FILL_BUFFER)
                                 ? CL_DEV_CMD_FILL_BUFFER
                                 : CL_DEV_CMD_FILL_IMAGE,
 ...
 * ******************************************************************/
MigrateSVMMemCommand::MigrateSVMMemCommand(cl_mem_migration_flags clFlags,
                                           cl_uint uNumMemObjects,
                                           const void **pMemObjects,
                                           const size_t *sizes)
    : m_pMemObjects(pMemObjects), m_pSizes(sizes) {
  ...
  MemoryObject::MemObjUsage access =
      (0 !=
       (m_migrateCmdParams.flags & CL_MIGRATE_MEM_OBJECT_CONTENT_UNDEFINED))
          ? MemoryObject::WRITE_ENTIRE
          : MemoryObject::READ_ONLY; // use READ_ONLY for optimization

  SharedPtr<Context> pQueueContext =
      m_pContextModule->GetContext(m_pCommandQueue->GetParentHandle());
  for (cl_uint i = 0; i < m_migrateCmdParams.mem_num; i++) {
    SharedPtr<SVMBuffer> pMemObj = pQueueContext->GetSVMBufferContainingAddr(
        const_cast<void *>(m_pMemObjects[i]));
    if (0 == pMemObj) {
      return CL_INVALID_VALUE;
    }
    if (nullptr != m_pSizes &&
        !pMemObj->IsContainedInBuffer(m_pMemObjects[i], m_pSizes[i])) {
      return CL_INVALID_VALUE;
    }
...
cl_err_code MigrateSVMMemCommand::Execute() {
  ...
  prepare_command_descriptor(CL_DEV_CMD_SVM_MIGRATE, &m_migrateCmdParams,
                             sizeof(cl_dev_cmd_param_migrate));
...
MigrateMemObjCommand::MigrateMemObjCommand(
    ocl_entry_points * /*pOclEntryPoints*/, cl_mem_migration_flags clFlags,
    cl_uint uNumMemObjects, const cl_mem *pMemObjects)
    : m_pMemObjects(pMemObjects) {
  assert(0 != uNumMemObjects);
  assert(nullptr != pMemObjects);
...
cl_err_code MigrateMemObjCommand::Init() {
  ...
  SharedPtr<Context> const pQueueContext =
      m_pContextModule->GetContext(m_pCommandQueue->GetParentHandle());
  for (cl_uint i = 0; i < m_migrateCmdParams.mem_num; i++) {
    const SharedPtr<MemoryObject> &pMemObj =
        m_pContextModule->GetMemoryObject(m_pMemObjects[i]);
    if (0 == pMemObj) {
...
cl_err_code MigrateMemObjCommand::Execute() {
  ...
  prepare_command_descriptor(CL_DEV_CMD_MIGRATE, &m_migrateCmdParams,
                             sizeof(cl_dev_cmd_param_migrate));
 ...
 * ******************************************************************/
MigrateUSMMemCommand::MigrateUSMMemCommand(cl_mem_migration_flags clFlags,
 ...
 * ******************************************************************/
AdviseUSMMemCommand::AdviseUSMMemCommand(const void *ptr, size_t size,
...
cl_err_code ErrorQueueEvent::GetInfo(cl_int iParamName, size_t szParamValueSize,
                                     void *pParamValue,
                                     size_t *pszParamValueSizeRet) const {
  return m_owner->GetEvent()->GetInfo(iParamName, szParamValueSize, pParamValue,
 ...
 *
 ******************************************************************/
PrePostFixRuntimeCommand::PrePostFixRuntimeCommand(Command *relatedUserCommand,
                                                   Mode working_mode)
    : m_relatedUserCommand(relatedUserCommand), m_working_mode(working_mode),
      m_force_error_return(CL_SUCCESS),
      m_error_event(ErrorQueueEvent::Allocate(
          relatedUserCommand->GetCommandQueue()->GetParentHandle())),
      m_task(RuntimeCommandTask::Allocate()) {
 ...
 *
 ******************************************************************/
void PrePostFixRuntimeCommand::ErrorDone() { CommandDone(); }

/******************************************************************
 *
 ******************************************************************/
void PrePostFixRuntimeCommand::ErrorEnqueue(cl_event *intermediate_pEvent,
                                            cl_event *user_pEvent,
                                            cl_err_code err_to_force_return) {
  assert(POSTFIX_MODE == m_working_mode &&
         "Calling PrePostFixRuntimeCommand::ErrorEnqueue on prefix.");
...
cl_err_code PrePostFixRuntimeCommand::Execute() {
  ...
  bool ok = FrameworkProxy::Instance()->Execute(
      SharedPtr<Intel::OpenCL::TaskExecutor::ITaskBase>(m_task));
...
bool RuntimeCommandTask::CompleteAndCheckSyncPoint() { return false; }
