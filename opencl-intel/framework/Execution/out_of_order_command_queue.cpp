...
cl_err_code OutOfOrderCommandQueue::Initialize() {
  ...
  Command *pDepOnAll = new MarkerCommand(false);
  pDepOnAll->AttachToCommandQueue(this);
...
cl_err_code OutOfOrderCommandQueue::AddDependentOnAll(Command *cmd) {
  ...
  Command *pNewDepOnAll = new MarkerCommand(false);
  pNewDepOnAll->AttachToCommandQueue(this);
