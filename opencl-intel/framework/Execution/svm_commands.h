...
/**
 * This class is responsible for freeing the shared virtual memory allocated
 * using clSVMAlloc or a shared system memory pointer
 */
...
public:
   ...
   * @param pSvmPtrs        array of SVM pointers to be
   * freed
   * @param freeFunc        callback function to be called
   * to free the SVM pointers, it may be NULL
   * @param pUserData        optional user data to be passed
   * to freeFunc
   * @param cmdQueue        a pointer to the
   * IOclCommandQueueBase on which this command is enqueued
   * @param bIsDependentOnEvents  whether this command is dependent on
   * some events
   */
  SVMFreeCommand(cl_uint uiNumSvmPtrs, void *pSvmPtrs[], pfnFreeFunc freeFunc,
                 void *pUserData, bool bIsDependentOnEvents)
      : RuntimeCommand(bIsDependentOnEvents),
        m_svmPtrs(pSvmPtrs, pSvmPtrs + uiNumSvmPtrs), m_freeFunc(freeFunc),
  ...
  const char *GetCommandName() const override { return "CL_COMMAND_SVM_FREE"; }
 ...
 * clEnqueueSVMMemcpy in case both destination and source pointers are system
 * pointers
 */
...
public:
   ...
   * some events
   */
  RuntimeSVMMemcpyCommand(void *pDstPtr, const void *pSrcPtr, size_t size,
                          bool bIsDependentOnEvents)
      : RuntimeCommand(bIsDependentOnEvents), m_pDstPtr(pDstPtr),
...
/**
 * This class is responsible for performing the command enqueued by
 * clEnqueueSVMMemFill in case the SVM pointer is a system pointer
 */
class RuntimeSVMMemFillCommand : public RuntimeCommand {
public:
  /**
   * Constructor
   * @param pSvmPtr    a pointer to a memory region that will be filled
   * with the pattern
   ...
   * some events
   */
  RuntimeSVMMemFillCommand(void *pSvmPtr, const void *pPattern,
                           size_t szPatternSize, size_t size,
                           bool bIsDependentOnEvents)
      : RuntimeCommand(bIsDependentOnEvents), m_pSvmPtr(pSvmPtr),
