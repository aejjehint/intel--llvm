...
QueueEvent::QueueEvent(const SharedPtr<IOclCommandQueueBase> &cmdQueue)
    : OclEvent(cmdQueue ? cmdQueue->GetParentHandle() : CL_INVALID_HANDLE),
      ...
      m_pEventQueueId(m_pEventQueue->GetId()),
      m_pEventQueueIsOOO(m_pEventQueue->IsOutOfOrderExecModeEnabled()),
      m_bEverIssuedToDevice(false) {
 ...
 *
 ******************************************************************/
cl_err_code QueueEvent::GetInfo(cl_int paramName, size_t paramValueSize,
                                void *paramValue,
 ...
 *
 ******************************************************************/
cl_err_code QueueEvent::GetProfilingInfo(cl_profiling_info clParamName,
...
void QueueEvent::SetProfilingInfo(cl_profiling_info clParamName,
                                  cl_ulong ulData) {
  switch (clParamName) {
  case CL_PROFILING_COMMAND_QUEUED:
...
void QueueEvent::IncludeProfilingInfo(const SharedPtr<QueueEvent> &other) {
  ...
  if (other->m_bCommandSubmitValid) {
    SetProfilingInfo(CL_PROFILING_COMMAND_SUBMIT,
                     other->m_sProfilingInfo.m_ulCommandSubmit);
  }

  if (other->m_bCommandStartValid) {
    SetProfilingInfo(CL_PROFILING_COMMAND_START,
                     other->m_sProfilingInfo.m_ulCommandStart);
  }

  if (other->m_bCommandEndValid) {
    SetProfilingInfo(CL_PROFILING_COMMAND_END,
                     other->m_sProfilingInfo.m_ulCommandEnd);
  }

  if (other->m_bCommandCompleteValid) {
    SetProfilingInfo(CL_PROFILING_COMMAND_COMPLETE,
                     other->m_sProfilingInfo.m_ulCommandComplete);
  }
}
 ...
 ******************************************************************/
cl_err_code
QueueEvent::ObservedEventStateChanged(const SharedPtr<OclEvent> &pEvent,
                                      cl_int returnCode) {
  if (CL_SUCCESS > returnCode) {
    // Non-blocking read/write commands return CL_PIPE_EMPTY or CL_PIPE_FULL
    // when reading from an empty pipe or writing into a full pipe. The return
    // code is for indicating if data is read/written successfully only, but it
    // does not mean the read/write commands are failed. Instead, the read/write
    // commands should be treated as successful. Otherwise, the marker command
    // of the command queue will be deleted so that subsequent command enqueuing
    // will cause crash.
    // Typical usage of non-blocking read/write:
    //   read: while (!success) read_element = MyPipe::read(q, success);
    //   write: while (!success) MyPipe::write(q, data_element, success);
    if (returnCode == CL_PIPE_EMPTY || returnCode == CL_PIPE_FULL) {
      RemoveFloatingDependence();
      return CL_SUCCESS;
    }
  ...
  if (EVENT_STATE_READY_TO_EXECUTE == newColor) {
    if ((NULL != m_pGPAData) && (m_pGPAData->bUseGPA)) {
      if (m_pGPAData->cStatusMarkerFlags & ITT_SHOW_SUBMITTED_MARKER) {
        ...
        __itt_string_handle *pMarker =
            __itt_string_handle_create(strMarkerString);
        ...
        __itt_task_begin(m_pGPAData->pDeviceDomain, m_ittID, __itt_null,
                         m_pGPAData->pMarkerHandle);

        __itt_marker(m_pGPAData->pDeviceDomain, m_ittID, pMarker,
...
void QueueEvent::DoneWithDependencies(const SharedPtr<OclEvent> &pEvent) {
  if (EVENT_STATE_HAS_DEPENDENCIES == GetEventState()) {
    ...
    if (bSameQueue && !m_pEventQueueIsOOO) {
      // If we're both on the same in-order command queue, the other event will
      // flush it so I don't need to
      SetEventState(EVENT_STATE_READY_TO_EXECUTE);
    } else {
      // Else, I need to notify my queue that I'm ready to execute
      // I need to cache my own event queue since once I set my state to "ready
      // to execute" I may complete and NULLify/deref my queue
      //  no need to lock here as we are still EVENT_STATE_HAS_DEPENDENCIES and
      //  queue may be reset only after execution
      SharedPtr<IOclCommandQueueBase> pMyEventQueue = m_pEventQueue;
      SetEventState(EVENT_STATE_READY_TO_EXECUTE);
      pMyEventQueue->NotifyStateChange(this, EVENT_STATE_HAS_DEPENDENCIES,
                                       EVENT_STATE_READY_TO_EXECUTE);
    }
  }
}
...
void QueueEvent::NotifyComplete(cl_int returnCode /* = CL_SUCCESS */) {
  ...
  m_pEventQueue->NotifyStateChange(this, EVENT_STATE_EXECUTING_ON_DEVICE,
                                   EVENT_STATE_DONE);
...
cl_int QueueEvent::GetReturnCode() const { return m_pCommand->GetReturnCode(); }
...
void QueueEvent::Wait() {
  if ((NULL != m_pGPAData) && (m_pGPAData->bUseGPA)) {
    ...
    __itt_task_begin(m_pGPAData->pDeviceDomain, __itt_null, __itt_null,
                     pTaskName);
  }
...
void QueueEvent::operator delete(void *p) {
  ...
  if (!self->m_pCommand->IsBeingDeleted()) {
    delete self->m_pCommand; // this will delete myself as well, since
                             // m_pCommand aggregates me
  }
...
void QueueEvent::AddProfilerMarker(const char *szMarkerName,
  ...
  if ((NULL != m_pGPAData) && (m_pGPAData->bUseGPA)) {
    ...
    if (m_pGPAData->cStatusMarkerFlags & ITT_SHOW_QUEUED_MARKER) {
      ...
      SPRINTF_S(pMarkerString, ITT_TASK_NAME_LEN, "%s - %s", szMarkerName,
                m_pCommand->GetCommandName());
      ...
      __itt_marker(m_pGPAData->pDeviceDomain, ittID, pMarker,
                   __itt_marker_scope_global);
