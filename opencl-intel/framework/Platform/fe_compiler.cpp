...
cl_err_code FrontEndCompiler::Initialize(const void *pDeviceInfo,
  ...
  try {
    err = CreateFrontEndInstance(pDeviceInfo, stDevInfoSize, &m_pFECompiler);
  } catch (std::bad_alloc &) {
    LOG_ERROR(TEXT("CreateFrontEndInstance failed with CL_OUT_OF_HOST_MEMORY"));
...
cl_err_code FrontEndCompiler::LinkProgram(
  ...
  IOCLFEBinaryResult *Result = nullptr;
  FELinkProgramsDescriptor linkDesc;
  IOCLFELinkKernelNames *LinkKernelName = nullptr;
  ...
  try {
    ...
    if (CL_SUCCESS != Error) {
      LOG_ERROR(TEXT("LinkPrograms failed = %x"), Error);
    }

    if ((*linkDesc.pKernelNames)) {
