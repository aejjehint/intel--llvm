...
Device::Device(_cl_platform_id_int *platform)
    : FissionableDevice(platform), m_iNextClientId(1), m_devId(0),
      m_pDevice(nullptr) {
  // initialize logger client
  INIT_LOGGER_CLIENT(TEXT("Device"), LL_DEBUG);
  m_mapDeviceLoggerClients[0] = GET_LOGGER_CLIENT;
...
cl_err_code Device::CreateAndInitAllDevicesOfDeviceType(
  ...
  unsigned int *deviceIdsList = (unsigned int *)STACK_ALLOC(
      sizeof(unsigned int) * numDevicesInDeviceType);
...
cl_err_code Device::InitDevice(unsigned int devId) {
  ...
  // We should wait for CreateContext or Device Fission to create Device Agent
  // instance, potentially saves memory footprint on Atom machines
...
cl_err_code Device::CreateInstance() {
  ...
  if (0 == m_pDeviceRefCount) {
    ...
    LOG_DEBUG(TEXT("%s"),
              TEXT("Device::fn_clDevCreateDeviceInstance exit. (CL_SUCCESS)"));
    return CL_SUCCESS;
  }
  m_pDeviceRefCount++;
  LOG_DEBUG(TEXT("%s"),
            TEXT("Device::CreateInstance exit without doing anything"));
  return CL_SUCCESS;
}
...
cl_int Device::clLogCreateClient(cl_int /*device_id*/, const char *client_name,
  ...
  *client_id = m_iNextClientId++;
  m_mapDeviceLoggerClients[*client_id] =
      std::make_shared<LoggerClient>(client_name, LL_DEBUG);
  return CL_SUCCESS;
}

cl_int Device::clLogReleaseClient(cl_int client_id) {
  m_mapDeviceLoggerClients.erase(client_id);
  return CL_SUCCESS;
}
...
cl_int Device::clLogAddLine(cl_int client_id, cl_int log_level,
                            ...
                            ...) {
  auto it = m_mapDeviceLoggerClients.find(client_id);
  if (it == m_mapDeviceLoggerClients.end()) {
    return CL_ERR_KEY_NOT_FOUND;
  }
  const auto &pLoggerClient = it->second;
  if (pLoggerClient) {
    va_list va;
...
void Device::clDevCmdStatusChanged(cl_dev_cmd_id cmd_id, void *pData,
  ...
  pObserver->NotifyCmdStatusChanged(
      cmd_status,
      (cl_int(CL_DEV_COMMAND_CANCELLED) == status_result)
...
Device::GetUSMCapabilities(cl_device_info param_name) const {
  switch (param_name) {
...
void Device::InitFECompiler() const {
  const IOCLDeviceFECompilerDescription *pFEConfig =
      m_pDevice->clDevGetFECompilerDecription();
...
const SharedPtr<FrontEndCompiler> &Device::GetFrontEndCompiler() const {
  std::call_once(m_FECompilerInitFlag, [&] { InitFECompiler(); });
...
cl_err_code FissionableDevice::FissionDevice(
  ...
  if (CL_DEV_PARTITION_BY_COUNTS == partitionMode) {
    ...
    if (0 == partitionSizes.size() ||
        GetInfo(CL_DEVICE_PARTITION_MAX_SUB_DEVICES, sizeof(maxSubDevices),
                &maxSubDevices, nullptr) != CL_SUCCESS ||
    ...
    // If the user doesn't actually want fission, no reason to send it to the
    // device, just return the size
    if (nullptr == out_devices) {
      *num_devices = (cl_uint)partitionSizes.size();
      return CL_SUCCESS;
    }
    dev_ret = GetDeviceAgent()->clDevPartition(partitionMode, num_entries,
                                               GetSubdeviceId(), num_devices,
                                               &partitionSizes, out_devices);
    ...
    size_t partitionIndex = 1;
    while ((cl_device_partition_property)CL_PARTITION_BY_NAMES_LIST_END_INTEL !=
           props[partitionIndex]) {
    ...
    for (auto &nextPartitionMode : nextPartitionableList) {
      dev_ret = GetDeviceAgent()->clDevPartition(nextPartitionMode, num_entries,
                                                 GetSubdeviceId(), num_devices,
                                                 sizes, out_devices);
      if (CL_SUCCESS == dev_ret)
        break;
    }
...
bool FissionableDevice::IsImageFormatSupported(
    const cl_image_format &clImgFormat, cl_mem_flags clMemFlags,
    cl_mem_object_type clMemObjType) const {
  cl_uint uiNumEntries;
  bool bSupported = false;
  cl_dev_err_code clErr = GetDeviceAgent()->clDevGetSupportedImageFormats(
      clMemFlags, clMemObjType, 0, nullptr, &uiNumEntries);
  (void)clErr;
  ...
  for (cl_uint i = 0; i < uiNumEntries; i++) {
    if (pFormats[i].image_channel_data_type ==
            clImgFormat.image_channel_data_type &&
...
SubDevice::SubDevice(SharedPtr<FissionableDevice> pParent,
                     size_t numComputeUnits, cl_dev_subdevice_id id,
                     const cl_device_partition_property *props)
    : FissionableDevice((_cl_platform_id_int *)pParent->GetHandle()),
      m_pParentDevice(pParent), m_deviceId(id),
      m_numComputeUnits(numComputeUnits), m_cachedFissionMode(nullptr),
...
cl_err_code SubDevice::GetInfo(cl_int param_name, size_t param_value_size,
                               void *param_value,
                               size_t *param_value_size_ret) const {
  size_t szParamValueSize = 0;
  cl_uint uValue = 0;
  ...
  case CL_DEVICE_PARENT_DEVICE:
    ...
    // CL_DEVICE_PARTITION_TYPES_EXT and CL_DEVICE_AFFINITY_DOMAINS_EXT handled
    // on root-level device
  ...
  case CL_DEVICE_UUID_KHR:
    return clDevGetDeviceInfo(GetDeviceFissionId(), param_name,
  ...
  case CL_DEVICE_NUM_SLICES_INTEL:
    ...
    if (param_name == CL_DEVICE_NUM_SLICES_INTEL &&
        m_cachedFissionLength >= 2 &&
    ...
    LLVM_FALLTHROUGH;
  default:
    return m_pRootDevice->GetInfo(param_name, param_value_size, param_value,
                                  param_value_size_ret);
  }
...
void SubDevice::CacheFissionProperties(
    const cl_device_partition_property *props) {
  m_cachedFissionLength = 0;
  // Todo: don't copy the partition properties for every sub-device, keep it in
  // the parent
  if (props) {
