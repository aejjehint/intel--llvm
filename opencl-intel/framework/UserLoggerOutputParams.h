...
public:
  ...
  OutputParamsValueProvider(OutputParamsValueProvider &) = delete;
  OutputParamsValueProvider &operator=(OutputParamsValueProvider &) = delete;
...
inline OutputParamsValueProvider::~OutputParamsValueProvider() {
  if (Intel::OpenCL::Utils::FrameworkUserLogger::GetInstance()
          ->IsApiLoggingEnabled()) {
    Print2Logger();
  }
}
