...
TEST_F(GetDeviceInfoTest, basic) {
  ...
  if (0 < szDeviceILSVersionSize) {
    std::vector<cl_name_version> deviceILSVersionVec(szDeviceILSVersionSize /
                                                     sizeof(cl_name_version));
    err = clGetDeviceInfo(m_device, CL_DEVICE_ILS_WITH_VERSION,
                          szDeviceILSVersionSize, deviceILSVersionVec.data(),
                          NULL);
    ASSERT_OCL_SUCCESS(err, "clGetDeviceInfo")
        << "CL_DEVICE_ILS_WITH_VERSION - get cl_name_version";
    for (unsigned int i = 0; i < deviceILSVersionVec.size(); ++i)
      ASSERT_TRUE(CL_VERSION_MAJOR(deviceILSVersionVec[i].version) == 1 &&
                  CL_VERSION_MINOR(deviceILSVersionVec[i].version) == i &&
                  CL_VERSION_PATCH(deviceILSVersionVec[i].version) == 0)
          << "Unexpected CL_DEVICE_ILS_WITH_VERSION!";
  }
  ...
  err = clGetDeviceInfo(m_device, CL_DEVICE_PREFERRED_VECTOR_WIDTH_FLOAT,
                        sizeof(cl_uint), &uiPreferredVecWidth, NULL);
  ASSERT_OCL_SUCCESS(err, "clGetDeviceInfo")
      << "CL_DEVICE_PREFERRED_VECTOR_WIDTH_FLOAT - query size";
  ASSERT_EQ(1, uiPreferredVecWidth);

  uiPreferredVecWidth = 0;
  err = clGetDeviceInfo(m_device, CL_DEVICE_PREFERRED_VECTOR_WIDTH_HALF,
                        sizeof(cl_uint), &uiPreferredVecWidth, NULL);
  ASSERT_OCL_SUCCESS(err, "clGetDeviceInfo")
      << "CL_DEVICE_PREFERRED_VECTOR_WIDTH_HALF - query size";
  ASSERT_EQ(1, uiPreferredVecWidth);
  ...
  uiNativeVecWidth = 0;
  err = clGetDeviceInfo(m_device, CL_DEVICE_NATIVE_VECTOR_WIDTH_HALF,
                        sizeof(cl_uint), &uiNativeVecWidth, NULL);
  ASSERT_OCL_SUCCESS(err, "clGetDeviceInfo")
      << "CL_DEVICE_NATIVE_VECTOR_WIDTH_HALF - query size";
  if (CPUID->IsFeatureSupported(CFS_AVX512F))
    ASSERT_EQ(32, uiNativeVecWidth) << "check value AVX512";
  else if (CPUID->IsFeatureSupported(CFS_AVX20))
    ASSERT_EQ(8, uiNativeVecWidth) << "check value AVX2";
  else if (CPUID->IsFeatureSupported(CFS_AVX10))
    ASSERT_EQ(8, uiNativeVecWidth) << "check value AVX";
  ...
  err = clGetDeviceInfo(m_device, CL_DEVICE_EXTENSIONS, 0, nullptr, &size_ret);
  ASSERT_OCL_SUCCESS(err, "clGetDeviceInfo")
      << "CL_DEVICE_EXTENSIONS: return size only args";

  std::string extensions(size_ret, '\0');
  err = clGetDeviceInfo(m_device, CL_DEVICE_EXTENSIONS, extensions.size(),
                        &extensions[0], nullptr);
  ASSERT_OCL_SUCCESS(err, "clGetDeviceInfo")
      << "CL_DEVICE_EXTENSIONS: return error code";

  bool hasHalf = extensions.find("cl_khr_fp16") != std::string::npos;
  bool hasDouble = extensions.find("cl_khr_fp64") != std::string::npos;

  cl_device_fp_atomic_capabilities_ext expectedFpAtomicCap =
  ...
  auto checkFpAtomicCap =
      [&](cl_device_fp_atomic_capabilities_ext expectedFpAtomicCap,
          cl_device_info cap, const std::string &capStr) {
        cl_device_fp_atomic_capabilities_ext quiriedFpAtomicCap;
        // return size only args
        err = clGetDeviceInfo(m_device, cap, 0, NULL, &size_ret);
        ASSERT_OCL_SUCCESS(err, "clGetDeviceInfo")
            << capStr + ": return size only args";
        ASSERT_EQ(sizeof(quiriedFpAtomicCap), size_ret)
            << capStr + ": check size of capabilities";
        // size < actual size
        err = clGetDeviceInfo(m_device, cap, 1, &quiriedFpAtomicCap, NULL);
        ASSERT_EQ(CL_INVALID_VALUE, err) << capStr + ": size < actual size";

        err =
            clGetDeviceInfo(m_device, cap, size_ret, &quiriedFpAtomicCap, NULL);
        ASSERT_OCL_SUCCESS(err, "clGetDeviceInfo")
            << capStr + ": return error code";
        ASSERT_EQ(expectedFpAtomicCap, quiriedFpAtomicCap)
            << capStr + ": get unexpected capabilities";
      };
  checkFpAtomicCap(expectedFpAtomicCap,
                   CL_DEVICE_SINGLE_FP_ATOMIC_CAPABILITIES_EXT,
                   "CL_DEVICE_SINGLE_FP_ATOMIC_CAPABILITIES_EXT");
  checkFpAtomicCap(hasDouble ? expectedFpAtomicCap : 0,
                   CL_DEVICE_DOUBLE_FP_ATOMIC_CAPABILITIES_EXT,
                   "CL_DEVICE_DOUBLE_FP_ATOMIC_CAPABILITIES_EXT");
  checkFpAtomicCap(hasHalf ? expectedFpAtomicCap : 0,
