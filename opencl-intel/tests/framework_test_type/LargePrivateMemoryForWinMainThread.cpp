...
class LargePrivateMemoryForWinMainThreadTest : public ::testing::Test {
protected:
  ...
  // stack size 1MB. And there should be no stack overflow issue.
...
void LargePrivateMemoryForWinMainThreadTest::setUp() {
...
#ifdef _WIN32
TEST_F(LargePrivateMemoryForWinMainThreadTest, testWithSource) {
  printf("testWithSource\n");

  setUp();

  const char *ps = programSources.c_str();
  std::string options = "-DPRI_MEM_SIZE=" + std::to_string(1024 * 1024 * 2);
...
void LargePrivateMemoryForWinMainThreadTest::buildFromBinary(
...
#ifdef _WIN32
TEST_F(LargePrivateMemoryForWinMainThreadTest, testWithBinary) {
...
void LargePrivateMemoryForWinMainThreadTest::cleanupPrivate() {
  if (buffer_private)
...
void LargePrivateMemoryForWinMainThreadTest::clDevicePrivateMemSizeTestBody() {
  cl_int iRet = CL_SUCCESS;
