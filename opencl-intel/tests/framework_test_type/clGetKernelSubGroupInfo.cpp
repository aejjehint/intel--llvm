...
TEST_F(CL21, GetKernelSubGroupInfo_MAX_SG_SIZE) {
  ...
  {
    std::vector<size_t> local_work_sizes = {128, 1, 1};
    size_t max_SG_size = 0;
    size_t returned_size = 0;
    iRet = clGetKernelSubGroupInfo(
        kern, m_device, CL_KERNEL_MAX_SUB_GROUP_SIZE_FOR_NDRANGE,
    ...
    ASSERT_EQ(sizeof(size_t), returned_size)
        << " clGetKernelSubGroupInfo failed. Expected and returned sizes "
           "differ. ";
    ASSERT_EQ(max_SG_size, CPU_DEV_MAX_SG_SIZE)
        << " clGetKernelSubGroupInfo failed. Expected and returned max "
           "subgroup sizes differ. ";

    for (size_t reqd_sg_size : CPU_DEV_SUB_GROUP_SIZES) {
      cl_kernel kern_requiring_sg_size = nullptr;
      GetDummyKernel(kern_requiring_sg_size, reqd_sg_size);
      max_SG_size = 0;
      returned_size = 0;
      iRet = clGetKernelSubGroupInfo(kern_requiring_sg_size, m_device,
                                     CL_KERNEL_MAX_SUB_GROUP_SIZE_FOR_NDRANGE,
                                     local_work_sizes.size() *
                                         sizeof(local_work_sizes[0]),
                                     &local_work_sizes[0], sizeof(max_SG_size),
                                     &max_SG_size, &returned_size);
      ASSERT_EQ(CL_SUCCESS, iRet) << " clGetKernelSubGroupInfo failed. ";
      ASSERT_EQ(sizeof(size_t), returned_size)
          << " clGetKernelSubGroupInfo failed. Expected and returned sizes "
             "differ. ";
      ASSERT_EQ(max_SG_size, reqd_sg_size)
          << " clGetKernelSubGroupInfo failed. Required and returned max "
             "subgroup sizes differ. ";
    }
  }
}
