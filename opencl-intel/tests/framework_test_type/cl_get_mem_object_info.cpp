...
TEST_F(GetMemObjectInfoTest, CL_MEM_HOST_PTR) {
  ...
  // Test CL_MEM_USE_HOST_PTR not present. Queried host ptr should be nullptr,
  // even when user provides a non-null host ptr for one-time init
  // (CL_MEM_COPY_HOST_PTR).
  queriedHostPtr = (void *)&v[0];
  cl_mem bufferCopyHostPtr =
      clCreateBuffer(m_context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
                     bufferSize, queriedHostPtr, &err);
  err = clGetMemObjectInfo(bufferCopyHostPtr, CL_MEM_HOST_PTR,
