...
TEST_F(CreateBufferTest, basic) {
  ...
  bufferForErr = clCreateBuffer(m_context,
                                CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR |
                                    CL_MEM_ALLOC_HOST_PTR,
                                100, nullptr, &iRet);
  EXPECT_EQ(oclErr(CL_INVALID_VALUE), oclErr(iRet))
      << "clCreateBuffer with flags (CL_MEM_USE_HOST_PTR | "
         "CL_MEM_ALLOC_HOST_PTR) should fail.";
  EXPECT_EQ(bufferForErr, nullptr);

  void *hostptr = malloc(100);
  bufferForErr = clCreateBuffer(
      m_context, CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR | CL_MEM_COPY_HOST_PTR,
      100, hostptr, &iRet);
  EXPECT_EQ(oclErr(CL_INVALID_VALUE), oclErr(iRet))
      << "clCreateBuffer with flags (CL_MEM_USE_HOST_PTR | "
         "CL_MEM_COPY_HOST_PTR) should fail.";
  EXPECT_EQ(bufferForErr, nullptr);
  free(hostptr);
