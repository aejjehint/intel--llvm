...
TEST_P(GVPointerExtensionTest, singleProgramGlobalArraySizeCorrentlyReturn) {
  // Build program
  const char *source = R"(
      typedef struct {
        int x;
  ...
  size_t gvSize;
  void *gvPtr;
  err = m_clGetDeviceGlobalVariablePointerINTEL(m_device, program, "s", &gvSize,
                                                &gvPtr);
  ASSERT_OCL_SUCCESS(err, "m_clGetDeviceGlobalVariablePointerINTEL");
  ASSERT_EQ(2048, gvSize);
  err = clReleaseProgram(program);
  EXPECT_OCL_SUCCESS(err, "clReleaseProgram");
}
