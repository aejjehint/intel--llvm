...
TEST(FrameworkTestType, Test_clBuildProgramWithBinaryTest) {
  std::map<cl_device_type, openBcFunc>::iterator iter =
      gBcfuncMap.find(gDeviceType);
...
TEST(FrameworkTestType, Test_memset_test) { EXPECT_TRUE(memset_test()); }
...
TEST(FrameworkTestType, Test_TBB) { EXPECT_TRUE(TBBTest()); }
...
TEST(FrameworkTestType, Test_opencl_printf_percent_sign_test) {
  EXPECT_TRUE(opencl_printf_percent_sign_test());
}
...
// Because we don't copy misaligned buffers with CL_MEM_USE_HOST_PTR anymore,
// the kernel code crashes. This can be fixed by changes in LLVM to handle
// misaligned access, if it is decided to be done.
TEST(FrameworkTestType, DISABLED_Test_MisalignedHostPtr) {
  EXPECT_TRUE(MisalignedUseHostPtrTest());
}

TEST(FrameworkTestType, Test_userDefinedStruct) { EXPECT_TRUE(clStructTest()); }

TEST(FrameworkTestType, Test_clapiTest) { EXPECT_TRUE(api_test()); }
...
// Disable the test as it is very compute-intensive and should be moved to
// Nightly.
TEST(FrameworkTestType, DISABLED_Test_clMultipleExecutionTest) {
  EXPECT_TRUE(clMultipleExecutionTest());
}

TEST(FrameworkTestType, Test_clSvmTest) { EXPECT_TRUE(clSvmTest()); }

TEST(FrameworkTestType,
     DISABLED_Test_clFlexibleNdrange) // disabled until BE finish their
                                      // implementation
{
  EXPECT_TRUE(clFlexibleNdrange());
}

TEST(FrameworkTestType, Test_clPipes) { EXPECT_TRUE(clPipes()); }

TEST(FrameworkTestType, Test_clSampler) { EXPECT_TRUE(clSampler()); }
...
TEST(FrameworkTestType, Test_clBuildOptions) { clBuildOptionsTest(); }

TEST(FrameworkTestType, Test_clShutdownSVMTest) { clShutdownSVMTest(); }
...
TEST(FrameworkTestType, Test_clGetCommandQueueInfo) { clGetCommandQueueInfo(); }
