...
protected:
  ...
  void testBody(const std::vector<std::string> &suffix,
    ...
    cl_int err = clBuildProgram(m_program, 1, &m_device, build_options, nullptr,
                                nullptr);
    ASSERT_OCL_SUCCESS(err, "clBuildProgram");

    ASSERT_NO_FATAL_FAILURE(
        validateDumpedFiles(suffix, patterns, ProgramIndex));
  }
...
TEST_F(DumpEnvTest, Asm) {
  ...
  ASSERT_NO_FATAL_FAILURE(testBody(suffix, patterns, "-g", 2));
  err = clReleaseProgram(m_program);
  EXPECT_OCL_SUCCESS(err, "clReleaseProgram");
  m_program = nullptr;

  ASSERT_TRUE(SETENV("CL_CONFIG_DUMP_ASM", "2"));
  patterns[0].push_back("# -- Begin function " + m_kernelName);
  ASSERT_NO_FATAL_FAILURE(testBody(suffix, patterns, "", 3));
  err = clReleaseProgram(m_program);
  EXPECT_OCL_SUCCESS(err, "clReleaseProgram");
  m_program = nullptr;
  ...
  ASSERT_EQ(m_dumpFilenames.size(), 3);
...
TEST_F(DumpEnvTest, Disassembly) {
  ...
  std::vector<std::vector<std::string>> patterns = {
      {m_kernelName, "Disassembly of section",
       /*leading address*/ "0:", /*at&t assembly style*/ "pushq\t%rbp"}};
  ASSERT_NO_FATAL_FAILURE(testBody(suffix, patterns));
}
