...
// The binary is built with source code which has one device global with legacy
// spirv host access id.
static const char *binaryFileLegacyId = "device_global_legacy_id.bc";
...
TEST_F(GVAccessExtentionTest, readGlobalVariable) {
  ...
  int gv;
  err = m_clEnqueueReadGlobalVariableINTEL(m_queue, m_program, "gv", CL_TRUE,
...
TEST_F(GVAccessExtentionTest, readAfterWriteGlobalVariable) {
  ...
  err = m_clEnqueueWriteGlobalVariableINTEL(m_queue, m_program, "gv", CL_FALSE,
...
TEST_F(GVAccessExtentionTest, readGlobalVariableArrary) {
  ...
  err = m_clEnqueueReadGlobalVariableINTEL(m_queue, m_program, "gvArr", CL_TRUE,
...
TEST_F(GVAccessExtentionTest, readAfterWriteGlobalVariableArrary) {
  ...
  err = m_clEnqueueWriteGlobalVariableINTEL(m_queue, m_program, "gvArr",
                                            CL_FALSE, sizeof(gvArr), 0,
                                            gvArrToWrite, 0, nullptr, nullptr);
  ASSERT_OCL_SUCCESS(err, "clEnqueueWriteGlobalVariableINTEL");
...
TEST_F(GVAccessExtentionTest, LegacyId) {
  cl_int err;
  std::vector<unsigned char> binary;
  ASSERT_NO_FATAL_FAILURE(
      readBinary((get_exe_dir() + binaryFileLegacyId).c_str(), binary));
  ...
  const size_t globalSize = 1, localSize = 1;
  err = clEnqueueNDRangeKernel(m_queue, kernel, 1, nullptr, &globalSize,
                               &localSize, 0, nullptr, nullptr);
  ASSERT_OCL_SUCCESS(err, "clEnqueueNDRangeKernel");

  int val = 0;
  err = m_clEnqueueReadGlobalVariableINTEL(m_queue, m_program, "dev_global",
                                           CL_TRUE, sizeof(int), 0, &val, 0,
                                           nullptr, nullptr);
  ASSERT_OCL_SUCCESS(err, "clEnqueueReadGlobalVariableINTEL");
  ASSERT_EQ(val, 42) << "device global value is not correct";
}
