...
static const char *BinaryFileLargeSize = "device_global_large_size.bc";
...
TEST_F(DeviceGlobalTest, LargeSize) {
  cl_int Err;
  clProgramWrapper Program;
  ASSERT_NO_FATAL_FAILURE(
      BuildProgram(BinaryFileLargeSize, m_context, m_device, &Program));

  std::random_device Rd;
  std::mt19937 Rand(Rd());

  // The IR device_global_large_size.bc has a device global with type
  // int[512][2048].
  constexpr int Width = 512, Depth = 2048, Val = 1;
  const int Idx = Rand() % (Width * Depth);

  Err = m_clEnqueueWriteGlobalVariableINTEL(
      m_queue, Program, "device_global", CL_TRUE, sizeof(int),
      sizeof(int) * Idx, &Val, 0, nullptr, nullptr);
  ASSERT_OCL_SUCCESS(Err, "clEnqueueWriteGlobalVariableINTEL");

  clKernelWrapper Kernel = clCreateKernel(Program, "test", &Err);
  ASSERT_OCL_SUCCESS(Err, "clCreateKernel");

  int Output = 0;
  clMemWrapper Buffer =
      clCreateBuffer(m_context, CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR,
                     sizeof(int), &Output, &Err);
  ASSERT_OCL_SUCCESS(Err, "clCreateBuffer");

  Err = clSetKernelArg(Kernel, 0, sizeof(cl_mem), &Buffer);
  ASSERT_OCL_SUCCESS(Err, "clSetKernelArg");

  Err = clSetKernelArg(Kernel, 1, sizeof(int), &Idx);
  ASSERT_OCL_SUCCESS(Err, "clSetKernelArg");

  constexpr size_t global_size = 1, local_size = 1;
  ...
  ASSERT_EQ(Output, Width * Depth) << "The output is not correct";
}
