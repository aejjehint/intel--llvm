 ...
 *    a.  The task creation order is unknown. Task execution will be handled by
 * list. Lists are of two types, ordered - wherein the tasks shall be executed
 * by the given order un-ordered - wherein tasks are executed without decencies
 ...
 *        I.  Simple - single function task, single instance of function will be
 * executed II. Complex (Task Set) - the main function will be executed multiple
 * times This complex function will have initialization and finalization stages
 *            single function that should be called before and after the main
 * loop.
 *
 */
...
typedef enum {
  ...
  TE_WAIT_MASTER_THREAD_BLOCKING, // WaitForCompletion was blocked by another
                                  // master thread
...
typedef enum {
  ...
  TE_CMD_LIST_IMMEDIATE // Process each task immediately using the caller thread
                        // also
} TE_CMD_LIST_TYPE;
...
typedef enum {
  TE_CMD_LIST_PREFERRED_SCHEDULING_DYNAMIC =
      0, // in TBB case - use auto_partitioner for TaskSets
  TE_CMD_LIST_PREFERRED_SCHEDULING_PRESERVE_TASK_AFFINITY, // try to preserve
                                                           // task affinities to
                                                           // threads in TaskSet
  TE_CMD_LIST_PREFERRED_SCHEDULING_STATIC,                 // in TBB case - use
                                           // static_partitioner for TaskSets
  TE_CMD_LIST_PREFERRED_SCHEDULING_UNEVEN_OPENCL, // Use OpenCL specific
                                                  // partitioner
...
typedef enum {
  CPU_CU_AFFINITY_CLOSE = 0, // keep task close to the master task in contiguous
                             // threads/logical processors
  CPU_CU_AFFINITY_SPREAD,    // distribute tasks evenly across threads/logical
                             // processors
...
typedef enum {
  TASK_SET_OPTIMIZE_DEFAULT =
      0, // in TBB case - use TBB internal by-tile approach in 2D/3D cases
...
typedef enum { TE_NO = 0, TE_YES = 1, TE_USE_DEFAULT = 2 } TE_BOOLEAN_ANSWER;
...
struct CommandListCreationParam {
  ...
  CommandListCreationParam(TE_CMD_LIST_TYPE type,
                           TE_CMD_LIST_PREFERRED_SCHEDULING sched =
                               TE_CMD_LIST_PREFERRED_SCHEDULING_DYNAMIC,
                           bool bIsProfilingEnabled = false,
                           bool bIsDefaultQueue = false)
      : cmdListType(type), preferredScheduling(sched),
        isProfilingEnabled(bIsProfilingEnabled),
        isQueueDefault(bIsDefaultQueue) {}
...
// if uiNumThreads == AUTO_THREADS, number of threads will be defined by
// implementation
...
enum TE_MASTERS_JOINING {
  TE_ENABLE_MASTERS_JOIN, // enable  master threads join execution in specific
                          // Root Device
  TE_DISABLE_MASTERS_JOIN // disable master threads join execution in specific
                          // Root Device
};
...
struct RootDeviceCreationParam {
  ...
  unsigned int
      uiNumOfExecPlacesForMasters; // if TE_ENABLE_MASTERS_JOIN, how many cores
                                   // in device should be reserved for masters
                                   // only. Only 0 or 1 is supported now and
                                   // cannot exceed number of threads on the top
                                   // level

  /**
   * Create Root Device in hierarchical mode - threads are splitted into levels.
   * Each thread belongs to all levels where the top level is 0. Use this to
   * express HW structure. TE_AUTO_THREADS cannot be used if uiNumOfLevels > 1.
   * @param levels            - number of levels in Root Device. Must be >0 and
   * <TE_MAX_LEVELS_COUNT
   * @param threadsPerLevel   - pointer to array with number of threads per each
   * level. Must be > 0 and overall number is limited to
   * GetMaxNumOfConcurrentThreads().
   * @param joining           - can user threads join the execution in this
   * device?
   * @param reservedForMasters- if joining is enabled, how many places should be
   * reserved in the top level device for masters. Extra joining masters will
   * either replace workers or will not allow joining
   */
  RootDeviceCreationParam(unsigned int levels,
                          const unsigned int threadsPerLevel[],
                          TE_MASTERS_JOINING joining = TE_ENABLE_MASTERS_JOIN,
                          unsigned int reservedForMasters = 0)
      : uiNumOfLevels(levels), mastersJoining(joining),
        uiNumOfExecPlacesForMasters(reservedForMasters) {
    if (nullptr != threadsPerLevel) {
      MEMCPY_S(uiThreadsPerLevel, sizeof(uiThreadsPerLevel), threadsPerLevel,
               sizeof(unsigned int) * ((levels < TE_MAX_LEVELS_COUNT)
                                           ? levels
                                           : TE_MAX_LEVELS_COUNT));
    }
  }
  ...
  /**
   * Create Root Device in flat mode - threads are not splitted into levels.
   * This means that all threads are at level 0 only.
   * @param overallThreads    - number of threads in the root device or
   * TE_AUTO_THREADS.
   * @param joining           - can user threads join the execution in this
   * device?
   * @param reservedForMasters- if joining is enabled, how many places should be
  ...
  RootDeviceCreationParam(unsigned int overallThreads = TE_AUTO_THREADS,
                          TE_MASTERS_JOINING joining = TE_ENABLE_MASTERS_JOIN,
                          unsigned int reservedForMasters = 0)
      : uiNumOfLevels(1), mastersJoining(joining),
        uiNumOfExecPlacesForMasters(reservedForMasters) {
...
/**
 * This class represents a task group that is implemented using a native task
 * group class of the threading library (in TBB, it is tbb::task_group). It is
 * required because in TBB our implementation of class ITaskGroup is required
 * for some scenarios, but not appropriate for others.
 */
class IThreadLibTaskGroup
    : public Intel::OpenCL::Utils::ReferenceCountedObject {
public:
   ...
   * @return the status of this IThreadLibTaskGroup after the wait has been
   * completed
   */
...
public:
  ...
  // Return task priority, currently the implementation shall return
  // TASK_PRIORITY_MEDIUM
  ...
  // Task execution routine called instead of actual execution if CommandList is
  // canceled
  ...
  // For NDRange commands return the ITaskGroup used to group its children; for
...
public:
  ...
  // Task execution routine, will be called by task executor instead of
  // Execute() if CommandList is canceled virtual void    Cancel() = 0;
};
...
public:
  ...
  virtual int Init(size_t region[], unsigned int &regCount,
                   size_t numberOfThreads = 1) = 0;

  virtual void *GetConcreteClass() { return this; }

  // Is called when the task is going to be called for the first time within
  // specific thread.
  // @param currentThreadData - data returned by OnThreadEntry()
  // Returns data to be passed to ExecuteIteration methods, if attach process
  // succeeded, otherwise NULL to abort
  virtual void *AttachToThread(void *currentThreadData,
                               size_t uiNumberOfWorkGroups, size_t firstWGID[],
                               size_t lastWGID[]) = 0;
  ...
  // "Main loop"
  // The function is called with different 'inx' parameters for each iteration
  // number Return false to abort
  virtual bool ExecuteIteration(size_t x, size_t y, size_t z,
                                void *data_from_AttachToThread) = 0;

  // Final stage, free execution resources
  // Return false when command execution fails
  virtual bool Finish(FINISH_REASON reason) = 0;

  // Task execution routine, will be called by task executor instead of Init()
  // if CommandList is canceled. If Init() was already called, Cancel() is not
  // called - normal processing is continued virtual void    Cancel() = 0;
...
public:
  ...
  // Enqueue a given task for execution, the function is asynchronous and exits
  // immediately in in-order and out-of-order lists. In immediate list the task
  // is enqueued, flushed and executes immediately. Functions returns only after
  // the task completes. Returns actual number of enqueued tasks Task execution
  // may be started immediately or postponed till Flush() command
  virtual unsigned int
  Enqueue(const Intel::OpenCL::Utils::SharedPtr<ITaskBase>
              &pTask) = 0; // Dynamically detect Task or TaskSet
  ...
  // Immediately spawn a task without enqueuing it first in order to save the
  // lock on the queue.
  virtual void Spawn(const Intel::OpenCL::Utils::SharedPtr<ITaskBase> &pTask,
                     IThreadLibTaskGroup &taskGroup) = 0;
  ...
  virtual te_wait_result WaitForCompletion(
      const Intel::OpenCL::Utils::SharedPtr<ITaskBase> &pTaskToWait) = 0;
  ...
  // Returns an ITaskGroup for NDRange commands to use to wait for their
  // children
  virtual Intel::OpenCL::Utils::SharedPtr<IThreadLibTaskGroup>
  GetNDRangeChildrenTaskGroup() = 0;
};

class ITaskExecutorObserver;

/////////////////////////////////////////////////////////////////////////////
// ITEDevice interface - defines a function set for TaskExecutor Device and
// SubDevice paradigm
...
public:
   ...
   * sub-device. In the hierarchical mode it must be a subset of the level 0
   * units.
  ...
  CreateTaskList(const CommandListCreationParam &param) = 0;
...
// Note: ITaskExecutorObserver methods are called inside observer lock, used
// also by ITEDevice::ResetObserver()
class ITaskExecutorObserver {
public:
  /**
   *
   ...
   *       IsMaster()              - to determine is current thread a master or
   * worker GetPosition()           - to determine current thread position
   * inside a TE Device
   */

  /**
   * Notify about caller thread entry into either ITaskExecutor Root Device or
   * some SubDevice
  ...
  /**
   * Notify about caller thread exit from either ITaskExecutor Root Device or
   * some SubDevice
   * @param  currentThreadData - per-thread data currently associated with
   * caller thread
   */
  virtual void OnThreadExit(void *currentThreadData) = 0;

  /**
   * Can thread leave the device? The question is asked only when there is no
   * hard demand. If there is high demand thread leavs the device
   * unconditionally.
   * @param  currentThreadData - per-thread data currently associated with
   * caller thread
   * @return TE_YES - may leave, TE_NO - prefer not to leave, TE_USE_DEFAULT -
   * decide yourself
   */
...
public:
  ...
  /**
   * Return number of threads that may participate in execution at the same
   * time. Initialized at the first Init(). It is the same value Init() returns
   * on success.
   * @return real number of threads that may concurrently participate in
   * execution overall
   */
  ...
  /**
   * Create Root Device
   * @param device_desc       - device description. Specifies device structure
   * and behavior
   * @param user_data         - some user data to associate with RootDevice -
   * will be returned back by GetCurrentDevice()
   * @return Root Device, if succeeded, else NULL
   */
  virtual Intel::OpenCL::Utils::SharedPtr<ITEDevice>
  CreateRootDevice(const RootDeviceCreationParam &device_desc,
                   void *user_data = nullptr,
                   ITaskExecutorObserver *my_observer = nullptr) = 0;
   ...
   * @return array of 2 handles - root or sub-device pointer + user handle to
   * root or sub-device If thread is outside of any TE Device or sub-device
   * teDevice is set to NULL
   */
  struct DeviceHandleStruct {
    ...
    DeviceHandleStruct(ITEDevice *dev = nullptr, void *user = nullptr)
        : teDevice(dev), user_handle(user){};
  };
  virtual DeviceHandleStruct GetCurrentDevice() const = 0;

  /**
   * Is current thread a user created thread (master) or internall created
   * thread (worker)
   ...
   * @param  level - request position at the given hierachy level. 0 - top
   * level. Must be 0 in the flat mode.
   * @return 0-based position inside sub-device at given level or TE_UNKNOWN if
   * level is above maximum or thread is outside of any TE Device or sub-device
   */
   ...
   */
  virtual Intel::OpenCL::Utils::SharedPtr<IThreadLibTaskGroup>
  CreateTaskGroup(const Intel::OpenCL::Utils::SharedPtr<ITEDevice> &device) = 0;
...
OCL_APIEXPORT ITaskExecutor *GetTaskExecutor();
