...
// Default CPU local memory size
#define CPU_DEV_LCL_MEM_SIZE (256 * 1024)

// Default FPGA Emulator local memory size
// FPGA in general requires a large local memory size limit as the hardware
// doesn't define the limit. 64MB should be sufficient for most FPGA designs.
#define FPGA_DEV_LCL_MEM_SIZE (64 * 1024 * 1024)
...
// Lower bound and upper bound of CL_CONFIG_CPU_FORCE_MAX_WORK_GROUP_SIZE
#define CPU_MAX_WORK_GROUP_SIZE_LOWER_BOUND (8 * 1024)
...
// TBB worker thread stack size is the sum of CPU_DEV_BASE_STACK_SIZE,
// CPU_DEV_LCL_MEM_SIZE and CPU_DEV_MAX_WG_PRIVATE_SIZE.
// Refer to TBB worker thread stack size calculation in TBBTaskExecutor::Init.
