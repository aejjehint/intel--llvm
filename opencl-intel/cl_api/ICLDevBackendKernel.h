...
public:
   ...
   * execution. Used by clEnqueueNDRangeKernel and clGetKernelWorkGroupInfo
   */
  ...
  /** @returns the number of SGs will be generated for specified local work
   * sizes.
   */
  virtual size_t GetNumberOfSubGroups(size_t size,
                                      const size_t *WGSizes) const = 0;

  /** @returns the maximum number of sub-groups that may make up each work-group
   * to execute kernel.
  ...
  GetLocalSizeForSubGroupCount(size_t const desiredSGCount,
                               ...
                               size_t *pValue, size_t const dim) const = 0;

  /**
   * @returns the required number of sub-groups that was declared during kernel
   * compilation. 0 when this attribute is not present.
   */
  virtual size_t GetRequiredNumSubGroups() const = 0;

  /** @returns the max. possible WG size with respect to the specified limits.
   */
  virtual size_t GetMaxWorkGroupSize(size_t const maxWGSize,
                                     size_t const maxWGPrivateSize) const = 0;
  ...
  /**
   * @returns true if the specified kernel is compiled with support of
   * non-unifrom WG size false otherwise
   */
  ...
  /**
   * @returns true if the specified kernel uses root group barrier, false
   * otherwise
   */
  virtual bool HasRootGroupBarrier() const = 0;
...
public:
   ...
   */
  virtual cl_dev_err_code
  PrepareThreadState(ICLDevExecutionState &state) const = 0;
   ...
   */
  virtual cl_dev_err_code
  RestoreThreadState(ICLDevExecutionState &state) const = 0;
};
...
public:
   ...
   * unique per kernel - ; in case of failure 0 will be returned
   */
