...
bool NDRange::ExecuteIteration(size_t x, size_t y, size_t z, void *pWgCtx) {
  ...
  if (m_pRunner->RunGroup(m_pKernelArgs, groupId, &childKernelsForWG) !=
      CL_DEV_SUCCESS)
    return false;
...
/////////////////////////////////////////////////////////////////////////
// OCL read/write device global command

cl_dev_err_code ReadWriteGlobal::Create(TaskDispatcher *pTD,
                                        cl_dev_cmd_desc *pCmd,
                                        SharedPtr<ITaskBase> *pTask,
                                        const SharedPtr<ITaskList> &pList) {
  ReadWriteGlobal *pCommand = new ReadWriteGlobal(pTD, pCmd);
...
ReadWriteGlobal::ReadWriteGlobal(TaskDispatcher *pTD, cl_dev_cmd_desc *pCmd)
    : CommandBaseClass<ITask>(pTD, pCmd) {}

cl_dev_err_code ReadWriteGlobal::CheckCommandParams(cl_dev_cmd_desc *pCmd) {
  if ((CL_DEV_CMD_READ_GLOBAL != pCmd->type) &&
      (CL_DEV_CMD_WRITE_GLOBAL != pCmd->type))
    return CL_DEV_INVALID_COMMAND_TYPE;

  if (sizeof(cl_dev_cmd_param_global_rw) != pCmd->param_size)
    return CL_DEV_INVALID_COMMAND_PARAM;

  cl_dev_cmd_param_global_rw *cmdParams =
      (cl_dev_cmd_param_global_rw *)pCmd->params;

  if (!cmdParams->dst || !cmdParams->src)
    return CL_DEV_INVALID_VALUE;

  return CL_DEV_SUCCESS;
}

bool ReadWriteGlobal::Execute() {
  cl_dev_cmd_param_global_rw *cmdParams =
      (cl_dev_cmd_param_global_rw *)m_pCmd->params;

  NotifyCommandStatusChanged(m_pCmd, CL_RUNNING, CL_DEV_SUCCESS);

  memcpy(cmdParams->dst, cmdParams->src, cmdParams->size);
...
/////////////////////////////////////////////////////////////////////////
// OCL read/write host pipe command

cl_dev_err_code ReadHostPipe::Create(TaskDispatcher *pTD, cl_dev_cmd_desc *pCmd,
                                     SharedPtr<ITaskBase> *pTask,
                                     const SharedPtr<ITaskList> &pList) {
  ReadHostPipe *pCommand = new ReadHostPipe(pTD, pCmd);
...
ReadHostPipe::ReadHostPipe(TaskDispatcher *pTD, cl_dev_cmd_desc *pCmd)
    : CommandBaseClass<ITask>(pTD, pCmd) {}

cl_dev_err_code ReadHostPipe::CheckCommandParams(cl_dev_cmd_desc *pCmd) {
  if (CL_DEV_CMD_READ_HOST_PIPE != pCmd->type)
    return CL_DEV_INVALID_COMMAND_TYPE;

  if (sizeof(cl_dev_cmd_param_host_pipe_r) != pCmd->param_size)
    return CL_DEV_INVALID_COMMAND_PARAM;

  cl_dev_cmd_param_host_pipe_r *cmdParams =
      (cl_dev_cmd_param_host_pipe_r *)pCmd->params;

  if (!cmdParams->dst || !cmdParams->pipeBS)
    return CL_DEV_INVALID_VALUE;

  return CL_DEV_SUCCESS;
}

bool ReadHostPipe::Execute() {
  cl_dev_cmd_param_host_pipe_r *cmdParams =
      (cl_dev_cmd_param_host_pipe_r *)m_pCmd->params;

  NotifyCommandStatusChanged(m_pCmd, CL_RUNNING, CL_DEV_SUCCESS);

  cl_err_code ret = CL_SUCCESS;
  if (cmdParams->blocking) {
    // Under blocking mode, it will try to read until success.
    while (__read_pipe_2_fpga(cmdParams->pipeBS, cmdParams->dst,
                              cmdParams->size, cmdParams->size))
      hw_pause();
  } else {
    // Under non-blocking mode, it will try to read for one time.
    if (__read_pipe_2_fpga(cmdParams->pipeBS, cmdParams->dst, cmdParams->size,
                           cmdParams->size))
      ret = CL_PIPE_EMPTY;
  }
...
cl_dev_err_code WriteHostPipe::Create(TaskDispatcher *pTD,
                                      cl_dev_cmd_desc *pCmd,
                                      SharedPtr<ITaskBase> *pTask,
                                      const SharedPtr<ITaskList> &pList) {
  WriteHostPipe *pCommand = new WriteHostPipe(pTD, pCmd);
...
WriteHostPipe::WriteHostPipe(TaskDispatcher *pTD, cl_dev_cmd_desc *pCmd)
    : CommandBaseClass<ITask>(pTD, pCmd) {}

cl_dev_err_code WriteHostPipe::CheckCommandParams(cl_dev_cmd_desc *pCmd) {
  if (CL_DEV_CMD_WRITE_HOST_PIPE != pCmd->type)
    return CL_DEV_INVALID_COMMAND_TYPE;

  if (sizeof(cl_dev_cmd_param_host_pipe_w) != pCmd->param_size)
    return CL_DEV_INVALID_COMMAND_PARAM;

  cl_dev_cmd_param_host_pipe_w *cmdParams =
      (cl_dev_cmd_param_host_pipe_w *)pCmd->params;

  if (!cmdParams->src || !cmdParams->pipeBS)
    return CL_DEV_INVALID_VALUE;

  return CL_DEV_SUCCESS;
}

bool WriteHostPipe::Execute() {
  cl_dev_cmd_param_host_pipe_w *cmdParams =
      (cl_dev_cmd_param_host_pipe_w *)m_pCmd->params;
  ...
  if (cmdParams->blocking) {
    ...
    while (__write_pipe_2_fpga(cmdParams->pipeBS, cmdParams->src,
                               cmdParams->size, cmdParams->size))
      hw_pause();
  } else {
    // Under non-blocking mode, it will try to write for one time.
    if (__write_pipe_2_fpga(cmdParams->pipeBS, cmdParams->src, cmdParams->size,
                            cmdParams->size))
      ret = CL_PIPE_FULL;
  }
