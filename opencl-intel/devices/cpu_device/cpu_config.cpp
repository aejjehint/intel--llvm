...
CPUDeviceConfig::GetExtensionsWithVersion() {
  ...
  if (CPU_DEVICE == GetDeviceMode())
    GetExtVer(OCL_EXT_FLOAT_ATOMICS, 1, 0, 0);
  ...
  GetExtVer(OCL_EXT_KHR_EXTENDED_BIT_OPS, 1, 0, 0);
  GetExtVer(OCL_EXT_KHR_ICD, 1, 0, 0);
  GetExtVer(OCL_EXT_KHR_IL_PROGRAM, 1, 0, 0);
  GetExtVer(OCL_EXT_KHR_SUGGESTED_LOCAL_WORK_SIZE, 1, 0, 0);
  ...
  GetExtVer(OCL_EXT_KHR_SUBGROUP_NON_UNIFORM_VOTE, 1, 0, 0);
  GetExtVer(OCL_EXT_KHR_SUBGROUP_CLUSTERED_REDUCE, 1, 0, 0);
  ...
  // CPU device UUID extension
  if (CPU_DEVICE == GetDeviceMode())
    GetExtVer(OCL_EXT_KHR_DEVICE_UUID, 1, 0, 0);

  // integer dot product extension
  if (OPENCL_VERSION_3_0 <= GetOpenCLVersion()) {
    GetExtVer(OCL_EXT_KHR_INTEGER_DOT_PRODUCT, 2, 0, 0);
  }

  // https://registry.khronos.org/OpenCL/specs/3.0-unified/html/OpenCL_API.html#cl_khr_command_buffer
  // TODO: Remove experimental flag when extension is officially supported.
  if (m_pConfigFile->Read<bool>("CL_CONFIG_CPU_EXPERIMENTAL_COMMAND_BUFFER",
                                false))
    GetExtVer(OCL_EXT_KHR_COMMAND_BUFFER, 0, 9, 5);
...
CPUDeviceConfig::GetOpenCLCFeaturesWithVersion() {
  ...
  GetFeature(OPENCL_C_INTEGER_DOT_PRODUCT_INPUT_4x8BIT, 3, 0, 0);
  GetFeature(OPENCL_C_INTEGER_DOT_PRODUCT_INPUT_4x8BIT_PACKED, 3, 0, 0);
