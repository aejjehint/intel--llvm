...
cl_ulong GetLocalMemorySize(const CPUDeviceConfig &config) {
  ...
  if (0 == localMemSize) {
    ...
    // fallback to default local memory size
      ...
      localMemSize = (FPGA_EMU_DEVICE == config.GetDeviceMode())
...
cl_dev_err_code CPUDevice::clDevGetDeviceInfo(unsigned int IN dev_id,
  ...
  case (CL_DEVICE_PREFERRED_VECTOR_WIDTH_HALF): // Keeping half separate to
                                                ...
                                                // config
  {
    ...
    *pinternalRetunedValueSize = sizeof(cl_uint);
    ...
    // if OUT paramVal is NULL it should be ignored
    if (nullptr != paramVal) {
      if (m_CPUDeviceConfig.IsHalfSupported()) {
        *(cl_uint *)paramVal = 1;
      } else {
        *(cl_uint *)paramVal = 0;
      }
  ...
  case (CL_DEVICE_NATIVE_VECTOR_WIDTH_HALF): // Keeping half separate to allow
                                             // control via the CPU config
  {
    *pinternalRetunedValueSize = sizeof(cl_uint);
    ...
    if (nullptr != paramVal) {
      if (m_CPUDeviceConfig.IsHalfSupported()) {
        *(cl_uint *)paramVal = GetNativeVectorWidth(CPU_DEVICE_DATA_TYPE_HALF);
      } else {
        *(cl_uint *)paramVal = 0;
      }
    }
...
// CL_DEVICE_GLOBAL_MEM_SIZE.
  ...
  case CL_DEVICE_IMAGE_MAX_BUFFER_SIZE:
    *pinternalRetunedValueSize = sizeof(size_t);
    ...
    if (nullptr != paramVal) {
      // We can support the maximum buffer when the pixel size is maximal,
      // i.e. GetMaxMemAllocSize(m_CPUDeviceConfig)/sizeof(CL_RGBA(INT32)).
      // However, this causes OpenCL CTS_cl_copy_images/1Dbuffer running longer
      // than 3 hours. Therefore, a smaller size is chosen.
      // Note the limit in CUDA 9.0 is 1^28.
      *(size_t *)paramVal = ((size_t)1 << 30);
    }
    return CL_DEV_SUCCESS;
  ...
  case CL_DEVICE_UUID_KHR: {
    if (isFPGAEmuDeviceMode)
      return CL_DEV_INVALID_VALUE;
    *pinternalRetunedValueSize = CL_UUID_SIZE_KHR * sizeof(cl_uchar);
    if (nullptr == paramVal)
      return CL_DEV_SUCCESS;
    if (valSize < *pinternalRetunedValueSize)
      return CL_DEV_INVALID_VALUE;
      // CPU Device UUID is generated based on vendor, version (e.g., family,
      // model, stepping), feature flag and subdevice index to guarantee
      // uniqueness. Note subdevice ID is always 0 for root device.
#pragma pack(push, 2)
    struct {
      uint16_t vendor;
      uint32_t version;
...
#pragma pack(pop)
    static_assert(sizeof(DeviceUUID) == CL_UUID_SIZE_KHR,
                  "Incorrect length of CPU device UUID");

    auto Vendor = CPUDetect::GetVendor();
    DeviceUUID.vendor = (Vendor == GenuineIntel)
                            ? 0x8086
                            : (Vendor == AuthenticAMD ? 0x1022 : 0);
    // Get processor info
    CPUID(viCPUInfo, 1);
    DeviceUUID.version = viCPUInfo[0]; // EAX, stepping, model and family info
    DeviceUUID.features[0] = viCPUInfo[2]; // ECX, feature info flag #1
    DeviceUUID.features[1] = viCPUInfo[3]; // EDX, feature info flag #2
    DeviceUUID.subDeviceId = static_cast<unsigned short>(dev_id);
    MEMCPY_S(paramVal, valSize, &DeviceUUID, *pinternalRetunedValueSize);
    return CL_DEV_SUCCESS;
  }
  case CL_DRIVER_UUID_KHR: {
    ...
    *pinternalRetunedValueSize = CL_UUID_SIZE_KHR * sizeof(cl_uchar);
    if (nullptr != paramVal && valSize < *pinternalRetunedValueSize)
      return CL_DEV_INVALID_VALUE;
    if (nullptr != paramVal) {
      const char *prodVer = GetModuleProductVersion();
      cl_uchar uuid[CL_UUID_SIZE_KHR] = {0};
      // Use product version string (at most the first 16 chars) as driver UUID
      MEMCPY_S(&uuid[0], valSize, prodVer,
               std::min(*pinternalRetunedValueSize, strlen(prodVer)));
      MEMCPY_S(paramVal, valSize, &uuid[0], *pinternalRetunedValueSize);
    }
    return CL_DEV_SUCCESS;
  }
  case CL_DEVICE_LUID_VALID_KHR:
    ...
    *pinternalRetunedValueSize = sizeof(cl_bool);
    if (nullptr != paramVal && valSize < *pinternalRetunedValueSize)
      return CL_DEV_INVALID_VALUE;
    // This currently is for GPU feature query only. Returns CL_FALSE for CPU.
    if (nullptr != paramVal)
      *(cl_bool *)paramVal = CL_FALSE;
    return CL_DEV_SUCCESS;
  case CL_DEVICE_LUID_KHR:
    if (isFPGAEmuDeviceMode)
      return CL_DEV_INVALID_VALUE;
    *pinternalRetunedValueSize = CL_LUID_SIZE_KHR * sizeof(cl_uchar);
    if (nullptr != paramVal && valSize < *pinternalRetunedValueSize)
      return CL_DEV_INVALID_VALUE;
    // This currently is for GPU feature query only. Returns zero-initialized
    // LUID for CPU.
    if (nullptr != paramVal)
      memset(paramVal, 0, CL_LUID_SIZE_KHR);
    return CL_DEV_SUCCESS;
  ...
  case CL_DEVICE_COMMAND_BUFFER_REQUIRED_QUEUE_PROPERTIES_KHR:
    *pinternalRetunedValueSize = sizeof(cl_command_queue_properties);
    if (paramVal && valSize < *pinternalRetunedValueSize)
      return CL_DEV_INVALID_VALUE;
    // Our implementation doesn't have any requirement on queue properties to
    // support command buffer. Return 0.
    if (paramVal)
      *(cl_command_queue_properties *)paramVal = 0;
    return CL_DEV_SUCCESS;
  case CL_DEVICE_COMMAND_BUFFER_CAPABILITIES_KHR:
    *pinternalRetunedValueSize =
    ...
    if (paramVal && valSize < *pinternalRetunedValueSize)
      return CL_DEV_INVALID_VALUE;
    if (paramVal) {
      // Our implementation supports all command buffer capabilities.
      *(cl_device_command_buffer_capabilities_khr *)paramVal =
  ...
  case CL_DEVICE_INTEGER_DOT_PRODUCT_CAPABILITIES_KHR:
    if (ver >= OPENCL_VERSION_3_0) {
      ...
      *pinternalRetunedValueSize = sizeof(dot_product_capabilities);
      if (paramVal && valSize < *pinternalRetunedValueSize)
        return CL_DEV_INVALID_VALUE;
      if (paramVal)
        *reinterpret_cast<dot_product_capabilities *>(paramVal) =
  ...
  case CL_DEVICE_INTEGER_DOT_PRODUCT_ACCELERATION_PROPERTIES_4x8BIT_PACKED_KHR:
    if (ver >= OPENCL_VERSION_3_0) {
      ...
      *pinternalRetunedValueSize = sizeof(dot_product_acceleration_props);
      if (paramVal && valSize < *pinternalRetunedValueSize)
        return CL_DEV_INVALID_VALUE;
      if (paramVal)
        *reinterpret_cast<dot_product_acceleration_props *>(paramVal) = {
            CL_TRUE, CL_TRUE, CL_TRUE, CL_TRUE, CL_TRUE, CL_TRUE};
      return CL_DEV_SUCCESS;
    }
    return CL_DEV_INVALID_VALUE;
  case CL_DEVICE_SINGLE_FP_ATOMIC_CAPABILITIES_EXT:
    *pinternalRetunedValueSize = sizeof(cl_device_fp_atomic_capabilities_ext);
    if (paramVal && valSize < *pinternalRetunedValueSize)
      return CL_DEV_INVALID_VALUE;
    if (paramVal)
      *reinterpret_cast<cl_device_fp_atomic_capabilities_ext *>(paramVal) =
  ...
  case CL_DEVICE_DOUBLE_FP_ATOMIC_CAPABILITIES_EXT:
    *pinternalRetunedValueSize = sizeof(cl_device_fp_atomic_capabilities_ext);
    ...
    if (paramVal)
      *reinterpret_cast<cl_device_fp_atomic_capabilities_ext *>(paramVal) =
          m_CPUDeviceConfig.IsDoubleSupported()
              ? CL_DEVICE_GLOBAL_FP_ATOMIC_LOAD_STORE_EXT |
                    CL_DEVICE_GLOBAL_FP_ATOMIC_ADD_EXT |
                    CL_DEVICE_GLOBAL_FP_ATOMIC_MIN_MAX_EXT |
                    CL_DEVICE_LOCAL_FP_ATOMIC_LOAD_STORE_EXT |
  ...
  case CL_DEVICE_HALF_FP_ATOMIC_CAPABILITIES_EXT:
    *pinternalRetunedValueSize = sizeof(cl_device_fp_atomic_capabilities_ext);
    ...
    if (paramVal)
      *reinterpret_cast<cl_device_fp_atomic_capabilities_ext *>(paramVal) =
          m_CPUDeviceConfig.IsHalfSupported()
              ? CL_DEVICE_GLOBAL_FP_ATOMIC_LOAD_STORE_EXT |
                    CL_DEVICE_GLOBAL_FP_ATOMIC_ADD_EXT |
                    CL_DEVICE_GLOBAL_FP_ATOMIC_MIN_MAX_EXT |
                    CL_DEVICE_LOCAL_FP_ATOMIC_LOAD_STORE_EXT |
...
cl_dev_err_code CPUDevice::clDevGetKernelSuggestedLocalWorkSizeKHR(
    cl_dev_kernel IN kernel, cl_uint IN work_dim,
    const size_t *IN global_work_offset, const size_t *IN global_work_size,
    size_t *OUT suggested_local_work_size) {
  CpuInfoLog(m_pLogDescriptor, m_iLogHandle, TEXT("%s"),
             TEXT("clDevGetKernelSuggestedLocalWorkSizeKHR Function enter"));
  return m_pProgramService->GetKernelSuggestedLocalWorkSizeKHR(
