...
ProgramService::ProgramService(cl_int devId,
    ...
    : m_iDevId(devId), m_pLogDescriptor(logDesc), m_iLogHandle(0),
      m_pCallBacks(devCallbacks), m_pBackendFactory(pBackendFactory),
      m_pBackendCompiler(nullptr), m_pBackendExecutor(nullptr),
      m_pBackendImageService(nullptr), m_pCPUConfig(config) {
  assert(m_pBackendFactory && "getting backend factory assumed to allways "
                              "succeed if initialization has succeeded");

  if (nullptr != logDesc) {
    cl_int ret = m_pLogDescriptor->clLogCreateClient(
        m_iDevId, "CPU Device: Program Service", &m_iLogHandle);
  ...
  CpuInfoLog(m_pLogDescriptor, m_iLogHandle, TEXT("%s"),
             TEXT("CPUDevice: Program Service - Created"));
}

ProgramService::~ProgramService() {
  CpuInfoLog(m_pLogDescriptor, m_iLogHandle, TEXT("%s"),
             TEXT("CPUDevice: Program Service - Destructed"));
...
/*******************************************************************************
 Init
    Description
        Initializes the program service. Initialize all the components that
        could fail - this is an only way to return the initialization error
        information when the exceptions are not used
    Input
        NONE
    Output
        NONE
...
cl_dev_err_code ProgramService::Init() {
  ...
  cl_dev_err_code ret =
      m_pBackendFactory->GetCompilationService(&programConfig, &pCompiler);
...
/*******************************************************************************
 CheckProgramBinary
    Description
        Performs syntax validation of the intermediate or binary to be built by
        the device during later stages. Call backend compiler to do the check
    Input
        bin_size                Size of the binary buffer
        bin                     A pointer to binary buffer that holds program
                                container defined by cl_prog_container.
    Output
        NONE
    Returns
        CL_DEV_SUCCESS          The function is executed successfully.
        CL_DEV_INVALID_VALUE    If bin_size is 0 or bin is NULL.
        CL_DEV_INVALID_BINARY   If the binary is not supported by the device
                                or program container content is invalid.
*******************************************************************************/
cl_dev_err_code ProgramService::CheckProgramBinary(size_t IN binSize,
                                                   const void *IN bin) {
  CpuInfoLog(m_pLogDescriptor, m_iLogHandle, TEXT("%s"),
             TEXT("CheckProgramBinary enter"));
...
clDevCreateProgram
    ...
    Input
        bin_size  Size of the binary buffer
        bin       A pointer to binary buffer that holds program container
                  defined by cl_prog_container. prop Specifies the origin of the
                  input binary. The values is defined by cl_dev_binary_prop.
    Output
        prog A handle to created program object.
    Returns
        CL_DEV_SUCCESS         The function is executed successfully.
        CL_DEV_INVALID_BINARY  If the back-end compiler failed to process
                               binary.
...
cl_dev_err_code ProgramService::CreateProgram(size_t IN binSize,
  ...
  CpuInfoLog(m_pLogDescriptor, m_iLogHandle, TEXT("%s"),
             TEXT("CreateProgram enter"));

  // Input parameters validation
  if (0 == binSize || nullptr == bin) {
    CpuInfoLog(m_pLogDescriptor, m_iLogHandle, TEXT("%s"),
               TEXT("Invalid binSize or bin parameters"));
    return CL_DEV_INVALID_VALUE;
  }

  if (nullptr == prog) {
    CpuInfoLog(m_pLogDescriptor, m_iLogHandle, TEXT("%s"),
               TEXT("Invalid prog parameter"));
    return CL_DEV_INVALID_VALUE;
  }
  ...
  if (prop == CL_DEV_BINARY_USER) {
    ...
    if (CL_DEV_FAILED(rc)) {
      CpuInfoLog(m_pLogDescriptor, m_iLogHandle, TEXT("%s"),
                 TEXT("Check program binary failed"));
      return rc;
    }
  }
  ...
  if (CL_DEV_FAILED(ret)) {
    CpuErrLog(m_pLogDescriptor, m_iLogHandle,
              TEXT("Failed to create program from given container<%0X>"), ret);
    delete pEntry;
...
ProgramService::CreateBuiltInKernelProgram(const char *IN szBuiltInNames,
  ...
  cl_dev_err_code err =
      BuiltInKernelRegistry::GetInstance()->CreateBuiltInProgram(szBuiltInNames,
                                                                 &pProg);
  if (CL_DEV_FAILED(err)) {
    CpuErrLog(m_pLogDescriptor, m_iLogHandle,
              TEXT("CreateBuiltInProgram failed with %x"), err);
    return err;
  }
...
clDevBuildProgram
    ...
    Input
      bin_size  Size of the binary buffer
      bin       A pointer to binary buffer that holds program container defined
                by cl_prog_container.
      options   A pointer to a string that describes the build options to be
                used for building the program executable. The list of supported
                options is described in section 5.4.3 in OCL spec. document.
      user_data This value will be passed as an argument when clDevBuildFinished
                is called. Can be NULL.
      prop      Specifies the origin of the input binary. The values is defined
                by cl_dev_binary_prop.
    Output
         prog A handle to created program object.
    Returns
        CL_DEV_SUCCESS                The function is executed successfully.
        CL_DEV_INVALID_BUILD_OPTIONS  If build options for back-end compiler
                                      specified by options are invalid.
        CL_DEV_INVALID_BINARY         If the back-end compiler failed to process
                                      binary.
        CL_DEV_OUT_OF_MEMORY          If the device failed to allocate memory
                                      for the program
*******************************************************************************/
...
cl_dev_err_code ProgramService::BuildProgram(cl_dev_program OUT prog,
  ...
  if (CL_BUILD_IN_PROGRESS == pEntry->clBuildStatus) {
    CpuErrLog(m_pLogDescriptor, m_iLogHandle,
              TEXT("Invalid build status(%d), should be CL_BUILD_NONE(%d)"),
              pEntry->clBuildStatus, CL_BUILD_NONE);
  ...
  cl_dev_err_code ret =
      m_pBackendCompiler->BuildProgram(pEntry->pProgram, nullptr, options);
...
cl_dev_err_code
ProgramService::GetFunctionPointerFor(cl_dev_program IN prog,
...
compiler. This is a hint from the framework and does not guarantee that the
compiler will not be used in the future or that the compiler will actually be
unloaded by the device. Input NONE Output NONE Returns CL_DEV_SUCCESS  The
function is executed successfully.
*******************************************************************************/
cl_dev_err_code ProgramService::UnloadCompiler() {
  CpuInfoLog(m_pLogDescriptor, m_iLogHandle, TEXT("%s"),
             TEXT("UnloadCompiler enter"));
  return CL_DEV_SUCCESS;
}
...
container as defined cl_prog_binary_desc.
    ...
    Output
        ...
        size_ret  The actual size in bytes of the returned buffer. When size is
                  equal to 0 and binary is NULL, returns size in bytes of a
                  program binary. If NULL the parameter is ignored.
    Returns
        CL_DEV_SUCCESS          The function is executed successfully.
        CL_DEV_INVALID_PROGRAM  If program is not valid program object.
        CL_DEV_INVALID_VALUE    If size is not enough to store the binary or
                                binary is NULL and size is not 0.
*******************************************************************************/
...
/*******************************************************************************
    ...
    Input
        count  Size of the buffer passed to the function in terms of
               cl_prog_binary_desc.
    Output
        types      A pointer to buffer wherein binary types will be stored.
        count_ret  The actual size ofthe buffer returned by the function in
                   terms of cl_prog_binary_desc. When count is equal to 0 and
                   types is NULL, function returns a size of the list. If NULL
                   the parameter is ignored.
    Returns
        CL_DEV_SUCCESS          The function is executed successfully.
        CL_DEV_INVALID_PROGRAM  If program is not valid program object.
        CL_DEV_INVALID_VALUE    If count is not enough to store the binary or
                                types is NULL and count is not 0.
*******************************************************************************/
cl_dev_err_code ProgramService::GetSupportedBinaries(
    size_t IN size, cl_prog_binary_desc *OUT types, size_t *OUT sizeRet) {
  CpuInfoLog(m_pLogDescriptor, m_iLogHandle, TEXT("%s"),
             TEXT("GetSupportedBinaries enter"));
  if (nullptr != sizeRet) {
...
cl_dev_err_code ProgramService::GetKernelId(cl_dev_program IN prog,
                                            const char *IN name,
                                            cl_dev_kernel *OUT kernelId) {
  CpuInfoLog(m_pLogDescriptor, m_iLogHandle, TEXT("%s"),
             TEXT("GetKernelId enter"));
...
cl_dev_err_code ProgramService::GetProgramKernels(cl_dev_program IN prog,
                                                  cl_uint IN num_kernels,
                                                  cl_dev_kernel *OUT kernels,
                                                  cl_uint *OUT numKernelsRet) {
  CpuInfoLog(m_pLogDescriptor, m_iLogHandle, TEXT("%s"),
             TEXT("GetProgramKernels enter"));
  ...
  unsigned int const uiNonBlockProgKernels =
      pEntry->pProgram->GetNonBlockKernelsCount();
  cl_dev_err_code iRet;
  ...
  for (unsigned int i = 0, ret_kernel_idx = 0; i < uiAllProgKernels; ++i) {
    ...
    if (CL_DEV_FAILED(iRet)) {
      CpuErrLog(m_pLogDescriptor, m_iLogHandle,
                TEXT("Failed to retrieve kernels<%X>"), iRet);
      return iRet;
    }
    ...
    kernels[ret_kernel_idx++] =
        (cl_dev_kernel) & (pEntry->mapKernels[szKernelName]);
  }
...
cl_dev_err_code ProgramService::GetKernelInfo(
    cl_dev_kernel IN kernel, cl_dev_kernel_info IN param,
    size_t IN input_value_size, const void *IN input_value,
    size_t IN value_size, void *OUT value, size_t *OUT valueSizeRet) const {
  CpuInfoLog(m_pLogDescriptor, m_iLogHandle, TEXT("%s"),
             TEXT("GetKernelInfo enter"));
  ...
  case CL_DEV_KERNEL_WG_SIZE:
    // TODO: Current implementation uses constants and it's OK with allocated on
    ...
    {
      size_t maxPrivateMemSize = IsFPGAEmulator ? FPGA_DEV_MAX_WG_PRIVATE_SIZE
      ...
      if (param == CL_DEV_KERNEL_WG_SIZE) {
        ullValue = std::min(
            ullValue, (unsigned long long)pKernelProps->GetKernelPackCount());
      }
  ...
  case CL_DEV_KERNEL_ARG_INFO:
    stValSize =
        pKernel->GetKernelParamsCount() * sizeof(cl_kernel_argument_info);
  ...
  case CL_DEV_KERNEL_LOCAL_SIZE_FOR_SUB_GROUP_COUNT: {
    ...
    if (nullptr != input_value && nullptr != value && dim > 0) {
...
cl_dev_err_code ProgramService::GetKernelSuggestedLocalWorkSizeKHR(
    cl_dev_kernel IN kernel, cl_uint IN work_dim,
    const size_t *IN global_work_offset, const size_t *IN global_work_size,
    size_t *OUT suggested_local_work_size) const {
  const ICLDevBackendKernel_ *pKernel =
      static_cast<const KernelMapEntry *>(kernel)->pBEKernel;
  size_t numberOfThreads = Intel::OpenCL::TaskExecutor::GetTaskExecutor()
                               ->GetMaxNumOfConcurrentThreads();
  if (m_pCPUConfig->GetDeviceMode() == FPGA_EMU_DEVICE)
    numberOfThreads = std::max(1, (int)numberOfThreads - 1);
  cl_dev_dispatch_buffer_prop buffProp;
  auto err = GetKernelInfo(kernel, CL_DEV_KERNEL_DISPATCH_BUFFER_PROPERTIES, 0,
                           nullptr, sizeof(buffProp), &buffProp, nullptr);
  if (CL_DEV_FAILED(err))
    return CL_DEV_OUT_OF_MEMORY;

  char *argBuf = (char *)ALIGNED_MALLOC(buffProp.size, buffProp.alignment);
  if (!argBuf)
    return CL_DEV_OUT_OF_MEMORY;
  llvm::UniformKernelArgs &kernelArgs = *reinterpret_cast<UniformKernelArgs *>(
      argBuf + pKernel->GetExplicitArgumentBufferSize());
  kernelArgs.WorkDim = work_dim;
  kernelArgs.MinWorkGroupNum = numberOfThreads;
  unsigned int i = 0;
  for (; i < work_dim; ++i) {
    if (global_work_offset)
      kernelArgs.GlobalOffset[i] = global_work_offset[i];
    kernelArgs.UserGlobalSize[i] = global_work_size[i];
    kernelArgs.UserLocalSize[NONUNIFORM_WG_SIZE_INDEX][i] = 0;
    kernelArgs.UserLocalSize[UNIFORM_WG_SIZE_INDEX][i] = 0;
  }
  for (; i < MAX_WORK_DIM; ++i) {
    kernelArgs.GlobalOffset[i] = 0;
    kernelArgs.UserGlobalSize[i] = 1;
    kernelArgs.UserLocalSize[NONUNIFORM_WG_SIZE_INDEX][i] = 1;
    kernelArgs.UserLocalSize[UNIFORM_WG_SIZE_INDEX][i] = 1;
  }
  pKernel->GetKernelRunner()->PrepareKernelArguments(
      argBuf, nullptr, 0, numberOfThreads, true /*bool calculateWGSize*/);
  for (unsigned int i = 0; i < work_dim; ++i)
    suggested_local_work_size[i] =
        kernelArgs.LocalSize[UNIFORM_WG_SIZE_INDEX][i];
  ALIGNED_FREE(argBuf);
  return CL_DEV_SUCCESS;
}

cl_dev_err_code
ProgramService::GetGlobalVariableTotalSize(cl_dev_program IN prog,
                                           size_t *OUT size) const {
  CpuInfoLog(m_pLogDescriptor, m_iLogHandle, TEXT("%s"),
             TEXT("GetGlobalVariableTotalSize enter"));
...
cl_dev_err_code ProgramService::GetSupportedImageFormats(
  ...
  const cl_image_format *supportedImageFormats =
      m_pBackendImageService->GetSupportedImageFormats(&uiNumEntries, imageType,
                                                       flags);
...
void ProgramService::DeleteProgramEntry(TProgramEntry *pEntry) {
  ...
  if (PTBuiltInProgram == pEntry->programType) {
    BuiltInProgram *pBIProgram =
        static_cast<BuiltInProgram *>(pEntry->pProgram);
    delete pBIProgram;
  } else
#endif
  {
