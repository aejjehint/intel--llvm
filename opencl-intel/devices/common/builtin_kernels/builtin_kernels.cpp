...
cl_dev_err_code
BuiltInProgram::ParseFunctionList(const char *szBuiltInKernelList) {
  ...
  while (pCurrFuncName < pNameListEnd) {
    ...
    if (m_mapKernels.find(funcName) == m_mapKernels.end()) {
      ...
      cl_dev_err_code err =
          BuiltInKernelRegistry::GetInstance()->CreateBuiltInKernel(
              funcName.c_str(), &pBIKernel);
    ...
    // Nothing to do on else. If kernel is not supported by the device, just
    // don't add it to the list. Other devices might support it
...
cl_dev_err_code BuiltInProgram::GetKernelByName(
...
cl_dev_err_code BuiltInProgram::GetKernel(
    int kernelIndex,
    const Intel::OpenCL::DeviceBackend::ICLDevBackendKernel_ **ppKernel) const {
...
void BuiltInKernelRegistry::RegisterBuiltInKernel(
    const char *szBIKernelName, fn_BuiltInFunctionCreate *pCreator) {
  assert(m_mapKernelCreators.find(szBIKernelName) ==
             m_mapKernelCreators.end() &&
         "The function can't be registered twice");

  m_stKernelNameStrLength += (strlen(szBIKernelName) + 1); // Count also for ';'
  m_mapKernelCreators[szBIKernelName] = pCreator;
}

void BuiltInKernelRegistry::GetBuiltInKernelList(char *szBIKernelList,
                                                 size_t stSize) const {
  ...
  szBIKernelList[0] = '\0';
  for (it = m_mapKernelCreators.begin(); it != m_mapKernelCreators.end();
       ++it) {
    ...
    STRNCPY_S(&szBIKernelList[stTotalSize], stSize - stTotalSize,
              it->first.c_str(), it->first.length());
    stTotalSize += it->first.length();
  }
}

cl_dev_err_code BuiltInKernelRegistry::CreateBuiltInKernel(
    const char *szMKLFuncName, IBuiltInKernel **pMKLExecutor) const {
  KernelCreatorMap_t::const_iterator it =
      m_mapKernelCreators.find(szMKLFuncName);
...
cl_dev_err_code BuiltInKernelRegistry::CreateBuiltInProgram(
...
cl_dev_err_code OMPExecutorThread::Execute(
...
Intel::OpenCL::BuiltInKernels::OMPExecutorThread *
Intel::OpenCL::BuiltInKernels::OMPExecutorThread::Create(
    unsigned int uiNumOfWorkers) {
  Intel::OpenCL::BuiltInKernels::OMPExecutorThread *pNewThread =
      new OMPExecutorThread(uiNumOfWorkers);
  return pNewThread;
}
#endif
